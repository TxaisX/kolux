...
function buildAtomFeed(tags: string[]): string {
  const entries = tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/TxaisX/kolux/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/TxaisX/kolux/releases.atom') {
  ...
  it.each([
    ...
    async (platform, manifestName) => {
      ...
      expect(manifestUrls).toEqual([
        `https://github.com/TxaisX/kolux/releases/download/v1.4.1/${manifestName}`
      ])
      expect(assetUrls).toEqual([
        'https://github.com/TxaisX/kolux/releases/download/v1.4.1/Kolux-1.4.1-arm64-mac.zip'
      ])
