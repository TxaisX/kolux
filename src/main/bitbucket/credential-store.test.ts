...
beforeEach(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'kolux-bitbucket-store-'))
...
describe('Bitbucket credential store', () => {
  ...
  it('rejects non-string fields from hand-edited metadata and secret files', async () => {
    ...
    writeFileSync(
      join(tempHome, '.kolux', 'bitbucket-credential.json'),
      JSON.stringify({ version: 1, authMode: 'basic', email: { evil: true }, account: 42 })
    )
    writeFileSync(
      join(tempHome, '.kolux', 'bitbucket-credential.enc'),
  ...
  it('keeps the previous credential intact when the secret write fails (STA-3941)', async () => {
    ...
    const before = readFileSync(join(tempHome, '.kolux', 'bitbucket-credential.enc'))
    ...
    expect(readFileSync(join(tempHome, '.kolux', 'bitbucket-credential.enc'))).toEqual(before)
    expect(existsSync(join(tempHome, '.kolux', 'bitbucket-credential.enc.tmp'))).toBe(false)
  })
  ...
  it('clears both files and in-memory state on disconnect', async () => {
    ...
    expect(existsSync(join(tempHome, '.kolux', 'bitbucket-credential.enc'))).toBe(false)
    expect(existsSync(join(tempHome, '.kolux', 'bitbucket-credential.json'))).toBe(false)
  })
  ...
  it('surfaces a non-ENOENT delete failure instead of silently keeping the files', async () => {
    ...
    expect(existsSync(join(tempHome, '.kolux', 'bitbucket-credential.enc'))).toBe(true)
  })
