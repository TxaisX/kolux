...
describe('fetchGitLabMergeRequestHeadRef', () => {
  ...
  it('fetches a GitLab MR head into its remote-scoped Kolux ref for local repos', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--no-tags',
        'origin',
        `+refs/merge-requests/42/head:refs/kolux/merge-requests/${ORIGIN_COMPONENT}/42`
      ],
      { cwd: '/repo', timeout: REVIEW_HEAD_FETCH_TIMEOUT_MS }
    )
    expect(localRef).toBe(gitlabMergeRequestHeadLocalRef(ORIGIN_COMPONENT, 42))
    expect(localRef).toBe(`refs/kolux/merge-requests/${ORIGIN_COMPONENT}/42`)
  })
  ...
  it('uses the SSH GitLab MR-head RPC and never runs git directly', async () => {
    const expectedRef = `refs/kolux/merge-requests/${ORIGIN_COMPONENT}/77`
