...
set "kolux_real=%KOLUX_REAL___KOLUX_UPPER_COMMAND__%"
set "kolux_wrapper_dir=%~dp0"
set "kolux_legacy_wrapper_dir=%KOLUX_ATTRIBUTION_SHIM_DIR%"
set "kolux_clean_path="
...
set "kolux_legacy_norm="
if defined kolux_legacy_wrapper_dir call :kolux_normalize_legacy_dir
...
if not defined PATH goto :kolux_path_walked
...
rem below ever saw it, and the cwd would then be searched for __KOLUX_COMMAND__. Proven on
...
for %%P in ("%PATH:;=" "%") do (
  set "kolux_entry=%%~P"
  call :kolux_append_path
)
:kolux_path_walked
set "PATH=%kolux_clean_path%"
set "KOLUX_ENABLE_GIT_ATTRIBUTION="
set "KOLUX_GIT_COMMIT_TRAILER="
set "KOLUX_GH_PR_FOOTER="
set "KOLUX_GH_ISSUE_FOOTER="
set "KOLUX_ATTRIBUTION_SHIM_DIR="
set "KOLUX_ATTRIBUTION_BYPASS="
set "KOLUX_REAL_GIT="
set "KOLUX_REAL_GH="
if defined kolux_real for %%G in ("%kolux_real%") do if /I "%%~dpG"=="%~dp0" set "kolux_real="
rem Why: a captured path may be relative, and both "if exist" and the invocation resolve it
rem against the current directory, so an inherited .\__KOLUX_COMMAND__.exe would run from the repo.
set "kolux_probe=%kolux_real%"
if defined kolux_real call :kolux_check_rooted
if defined kolux_real if not defined kolux_rooted set "kolux_real="
rem Why: clear a captured path that no longer exists, or the PATH walk below is skipped.
if defined kolux_real if not exist "%kolux_real%" set "kolux_real="
if defined kolux_real goto run
rem Why: an unqualified Windows command lookup searches the current directory before PATH, so a
rem repository-local __KOLUX_COMMAND__.exe would win. Walk the cleaned PATH ourselves instead.
if not defined kolux_clean_path goto :kolux_candidates_walked
for %%P in ("%kolux_clean_path:;=" "%") do (
  set "kolux_entry=%%~P"
  call :kolux_try_candidate
)
:kolux_candidates_walked
if not defined kolux_real (
  echo Kolux compatibility wrapper could not locate __KOLUX_COMMAND__ on PATH. 1>&2
  exit /b 127
)
:run
"%kolux_real%" %*
...
:kolux_normalize_legacy_dir
rem Why the rooted test first: full-path expansion resolves a relative value against the current
rem directory, so a
rem relative KOLUX_ATTRIBUTION_SHIM_DIR would let the cwd decide which PATH entry counts as the
...
set "kolux_probe=%kolux_legacy_wrapper_dir%"
call :kolux_check_rooted
if not defined kolux_rooted exit /b
for %%G in ("%kolux_legacy_wrapper_dir%") do set "kolux_legacy_norm=%%~fG"
rem Why: full-path expansion preserves a trailing separator; normalize before comparing.
if "%kolux_legacy_norm:~-1%."=="%kolux_sep%." set "kolux_legacy_norm=%kolux_legacy_norm:~0,-1%"
exit /b
...
rem Why kolux_probe rather than an argument: see the CALL re-expansion note above.
set "kolux_rooted="
rem Why: an unset probe would leave the substring syntax below unexpanded and mangle the line.
if not defined kolux_probe exit /b
if "%kolux_probe:~0,2%"=="\\" set "kolux_rooted=1"
if "%kolux_probe:~1,2%"==":\" set "kolux_rooted=1"
if "%kolux_probe:~1,2%"==":/" set "kolux_rooted=1"
exit /b

:kolux_try_candidate
if defined kolux_real exit /b
if not defined kolux_entry exit /b
rem Why: a relative entry resolves against the current directory, same exposure as an empty one.
set "kolux_probe=%kolux_entry%"
call :kolux_check_rooted
if not defined kolux_rooted exit /b
for %%G in ("%kolux_entry%") do set "kolux_candidate_dir=%%~fG"
...
if "%kolux_candidate_dir:~-1%."=="%kolux_sep%." set "kolux_candidate_dir=%kolux_candidate_dir:~0,-1%"
...
if /I "%kolux_candidate_dir%\"=="%kolux_wrapper_dir%" exit /b
if exist "%kolux_candidate_dir%\__KOLUX_COMMAND__.exe" set "kolux_real=%kolux_candidate_dir%\__KOLUX_COMMAND__.exe"
if not defined kolux_real if exist "%kolux_candidate_dir%\__KOLUX_COMMAND__.cmd" set "kolux_real=%kolux_candidate_dir%\__KOLUX_COMMAND__.cmd"
if not defined kolux_real if exist "%kolux_candidate_dir%\__KOLUX_COMMAND__.bat" set "kolux_real=%kolux_candidate_dir%\__KOLUX_COMMAND__.bat"
exit /b
...
call :kolux_check_rooted
if not defined kolux_rooted exit /b
for %%G in ("%kolux_entry%") do set "kolux_path_entry_dir=%%~fG"
rem Why: full-path expansion preserves a trailing separator; normalize before comparing.
if "%kolux_path_entry_dir:~-1%."=="%kolux_sep%." set "kolux_path_entry_dir=%kolux_path_entry_dir:~0,-1%"
set "kolux_path_entry_dir=%kolux_path_entry_dir%\"
if /I "%kolux_path_entry_dir%"=="%kolux_wrapper_dir%" exit /b
set "kolux_skip_entry="
if defined kolux_legacy_wrapper_dir call :kolux_reject_legacy_dir
if defined kolux_skip_entry exit /b
if defined kolux_clean_path (set "kolux_clean_path=%kolux_clean_path%;%kolux_entry%") else set "kolux_clean_path=%kolux_entry%"
exit /b

:kolux_reject_legacy_dir
rem Why: an unrooted legacy dir leaves this unset, and comparing against a bare separator could
rem only ever misfire.
if not defined kolux_legacy_norm exit /b
if /I "%kolux_path_entry_dir%"=="%kolux_legacy_norm%\" set "kolux_skip_entry=1"
exit /b
`

const POWERSHELL_PASSTHROUGH_WRAPPER = String.raw`$ErrorActionPreference = 'Stop'
$commandName = '__KOLUX_COMMAND__'
$realCommand = [Environment]::GetEnvironmentVariable('KOLUX_REAL___KOLUX_UPPER_COMMAND__')
...
'KOLUX_ENABLE_GIT_ATTRIBUTION', 'KOLUX_GIT_COMMIT_TRAILER', 'KOLUX_GH_PR_FOOTER', 'KOLUX_GH_ISSUE_FOOTER', 'KOLUX_ATTRIBUTION_SHIM_DIR', 'KOLUX_REAL_GIT', 'KOLUX_REAL_GH', 'KOLUX_ATTRIBUTION_BYPASS' | ForEach-Object { Remove-Item "Env:$_" -ErrorAction SilentlyContinue }
...
if (-not $realCommand) {
  [Console]::Error.WriteLine("Kolux compatibility wrapper could not locate $commandName on PATH.")
  exit 127
}
& $realCommand @args
exit $LASTEXITCODE
...
function renderWindowsWrapper(template: string, command: string, upperCommand: string): string {
  return template
    .replaceAll('__KOLUX_UPPER_COMMAND__', upperCommand)
    .replaceAll('__KOLUX_COMMAND__', command)
}
