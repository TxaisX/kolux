...
describe('safeRemoveOverlay', () => {
  it('removes valid overlay children whose names start with dot-dot', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-overlay-root-'))
    tempRoots.push(root)
  ...
  it('refuses parent traversal outside the overlay root', async () => {
    const parent = await mkdtemp(join(tmpdir(), 'kolux-overlay-parent-'))
  ...
  it('refuses Windows overlay targets on a different drive than the root', async () => {
    ...
    safeRemoveOverlayWithWinPath(String.raw`D:\users\me\config`, String.raw`C:\kolux\overlays`)

    expect(warnSpy).toHaveBeenCalledWith(
      '[overlay-mirror] refusing to remove overlay outside root: target=D:\\users\\me\\config root=C:\\kolux\\overlays'
    )
