...
describe('readShellStartupEnvVar', () => {
  ...
  // Issue #1534 / PR description: GUI-launched Kolux does not inherit
  // MIMOCODE_HOME; the user's .zshrc exports it. The fallback must
  ...
  it('mirrors the user scenario: GUI-launched Kolux discovers .zshrc-only export', () => {
    mockStartupFiles({
      '.zshrc': 'export MIMOCODE_HOME="$HOME/.config/mimocode"\n'
    })

    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe(
      '/home/alice/.config/mimocode'
    )
  })
  ...
  it('returns undefined when HOME is unset', () => {
    ...
    try {
      expect(readShellStartupEnvVar('MIMOCODE_HOME')).toBeUndefined()
  ...
  it('returns undefined on Windows', () => {
    ...
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=/win\n' })
    expect(isShellStartupEnvProbeSupported()).toBe(false)
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBeUndefined()
  })
  ...
  it('returns the LAST assignment when multiple files re-export', () => {
    mockStartupFiles({
      '.zshenv': 'export MIMOCODE_HOME="/old/zshenv"\n',
      '.zprofile': 'export MIMOCODE_HOME="/middle/zprofile"\n',
      '.zshrc': 'export MIMOCODE_HOME="/new/zshrc"\n',
      '.zlogin': 'export MIMOCODE_HOME="/newest/zlogin"\n'
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/newest/zlogin')
  })

  it('uses ZDOTDIR exported from .zshenv for later zsh startup files', () => {
    mockStartupFiles({
      '/home/alice/.zshenv': 'export ZDOTDIR="$HOME/.config/zsh"\n',
      '/home/alice/.config/zsh/.zshrc': 'export MIMOCODE_HOME="$HOME/company/mimocode"\n'
    })

    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice', '/bin/zsh')).toBe(
      '/home/alice/company/mimocode'
    )
  })

  it('handles double-quoted values', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME="/quoted/path"\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/quoted/path')
  })
  ...
  it('handles unquoted values', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=/unquoted/path\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/unquoted/path')
  })

  it('expands $HOME in values', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME="$HOME/.mimocode"\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/home/alice/.mimocode')
  })
  ...
  it('expands leading ~ in values', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=~/.mimocode\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/home/alice/.mimocode')
  })
  ...
  it('ignores bare assignments without the export keyword', () => {
    ...
    // construction.
    mockStartupFiles({ '.zshrc': 'MIMOCODE_HOME=/no/export\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBeUndefined()
  })
  ...
  it('preserves a # inside a double-quoted value', () => {
    ...
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME="/path/with#hash"\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/path/with#hash')
  })
  ...
  it('strips a trailing # comment from an unquoted value', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=/bare/path # trailing comment\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/bare/path')
  })

  it('strips a trailing # comment after a double-quoted value', () => {
    mockStartupFiles({
      '.zshrc': 'export MIMOCODE_HOME="$HOME/.mimocode" # trailing comment\n'
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/home/alice/.mimocode')
  })
  ...
  it('strips a trailing # comment after a single-quoted value', () => {
    mockStartupFiles({
      '.zshrc': "export MIMOCODE_HOME='/literal/path' # trailing comment\n"
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/literal/path')
  })

  it('does not expand $HOME inside single quotes', () => {
    // Why: POSIX shells do not perform parameter expansion in single quotes.
    mockStartupFiles({ '.zshrc': "export MIMOCODE_HOME='$HOME/.mimocode'\n" })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('$HOME/.mimocode')
  })

  it('does not partially expand $HOMER / $HOMEPATH', () => {
    // Why: real shells require a word boundary; $HOMER is the var HOMER,
    // not $HOME + R.
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME="$HOMER/agent"\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('$HOMER/agent')
  })
  ...
  it('only scans zsh startup files when SHELL is zsh', () => {
    ...
    process.env.SHELL = '/bin/zsh'
    mockStartupFiles({
      '.zshrc': 'export MIMOCODE_HOME=/from/zshrc\n',
      '.bash_profile': 'export MIMOCODE_HOME=/from/bash_profile\n'
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/from/zshrc')
  })
  ...
  it('only scans bash startup files when SHELL is bash', () => {
    ...
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/from/bash_profile')
  })
  ...
  it('does not scan .bashrc for bash shells', () => {
    // Why: Kolux launches bash as a login shell and the shell-ready wrappers
    // intentionally do NOT source .bashrc, so a value present only in .bashrc
    // would never be set in the live Kolux bash shell.
    process.env.SHELL = '/bin/bash'
    mockStartupFiles({ '.bashrc': 'export MIMOCODE_HOME=/from/bashrc\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBeUndefined()
  })
  ...
  it('does not scan zsh or bash files for an explicit unsupported shell', () => {
    ...
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice', '/opt/bin/fish')).toBeUndefined()
  })
  ...
  it('honors an explicit shell argument over process.env.SHELL', () => {
    ...
    // differs from the Kolux process's own $SHELL.
    process.env.SHELL = '/bin/zsh'
    mockStartupFiles({
      '.zshrc': 'export MIMOCODE_HOME=/from/zshrc\n',
      '.bash_profile': 'export MIMOCODE_HOME=/from/bash_profile\n'
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice', '/bin/bash')).toBe(
      '/from/bash_profile'
    )
  })

  it('memoizes results across calls within the same process', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=/cached\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/cached')
  ...
  it('does not match other variable names', () => {
    mockStartupFiles({
      '.zshrc': 'export MIMOCODE_HOME_BACKUP=/backup\nexport NOT_MIMOCODE_HOME=/x\n'
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBeUndefined()
  })
  ...
  it('survives a readFileSync error on one file and continues', () => {
    ...
    readFileSyncMock.mockImplementation((p: string) => {
      ...
      if (p.endsWith('.zshrc')) {
        return 'export MIMOCODE_HOME=/found\n'
      }
      return ''
    })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/found')
  })

  it('handles CRLF line endings', () => {
    mockStartupFiles({ '.zshrc': 'export MIMOCODE_HOME=/crlf\r\nexport OTHER=x\r\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBe('/crlf')
  })

  it('does not match an MIMOCODE_HOME mention in a comment', () => {
    mockStartupFiles({ '.zshrc': '# export MIMOCODE_HOME=/from-comment\n' })
    expect(readShellStartupEnvVar('MIMOCODE_HOME', '/home/alice')).toBeUndefined()
  })
