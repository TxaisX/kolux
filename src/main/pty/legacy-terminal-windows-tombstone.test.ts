...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'kolux-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expect(cmd).toContain('if defined kolux_real if not exist "%kolux_real%" set "kolux_real="')
    expectOrdered(cmd, 'if not exist "%kolux_real%"', ':kolux_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%kolux_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%kolux_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "kolux_sep=\\"')
      ...
      // Why: a captured KOLUX_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined kolux_real call :kolux_check_rooted')
      expect(cmd).toContain('if defined kolux_real if not defined kolux_rooted set "kolux_real="')
      ...
      // Why the exact block, not an ordering pin: the first ':kolux_append_path' in the file is
      // the top-level CALL, and the first 'if not defined kolux_rooted' belongs to a different
      // subroutine, so an ordering pin stayed green with this subroutine's guard deleted.
      expect(cmd).toContain(':kolux_append_path\r\nif not defined kolux_entry exit /b\r\n')
      expect(cmd).toContain(
      ...
      expect(cmd).not.toContain('call :kolux_check_rooted "')
      expect(cmd).not.toContain('call :kolux_try_candidate "')
      expect(cmd).not.toContain('call :kolux_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain('set "kolux_probe=%kolux_entry%"\r\ncall :kolux_check_rooted\r\n')
      // Why: kolux_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "kolux_sep=', 'if "%kolux_legacy_norm:~-1%."')
      // Why: %kolux_sep% expands before the line is parsed, so `=="%kolux_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%kolux_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :kolux_check_rooted\r\nif not defined kolux_rooted exit /b',
        'for %%G in ("%kolux_entry%") do set "kolux_candidate_dir='
      )
      expect(cmd).toContain('if not defined kolux_rooted exit /b')
      expect(cmd).toContain('if "%kolux_candidate_dir:~-1%."=="%kolux_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if /I "%kolux_candidate_dir%\\"=="%kolux_wrapper_dir%" exit /b')
      ...
      expect(cmd).toContain('if "%kolux_probe:~1,2%"==":\\" set "kolux_rooted=1"')
      expect(cmd).toContain('if "%kolux_probe:~0,2%"=="\\\\" set "kolux_rooted=1"')
      ...
      expect(cmd).toContain('if not defined kolux_entry exit /b')
      ...
      expect(cmd).toContain('if "%kolux_legacy_norm:~-1%."=="%kolux_sep%."')
      ...
      expect(cmd).toContain('if defined kolux_legacy_wrapper_dir call :kolux_normalize_legacy_dir')
      expect(cmd).not.toContain('if defined kolux_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :kolux_path_walked')
      expect(cmd).toContain('if not defined kolux_clean_path goto :kolux_candidates_walked')
      for (const [guard, loop] of [
        ['if not defined PATH goto :kolux_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined kolux_clean_path goto :kolux_candidates_walked',
          'for %%P in ("%kolux_clean_path:;='
        ]
      ] as const) {
