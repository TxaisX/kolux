...
function worktreeSetupWslenvEntries(env: Record<string, string | undefined>): string[] {
  return [
    ...
    ...['KOLUX_ROOT_PATH', 'KOLUX_WORKTREE_PATH', 'CONDUCTOR_ROOT_PATH', 'GHOSTX_ROOT_PATH'].map(
      (name) => `${name}/${env[name]?.startsWith('/') ? 'u' : 'p'}`
    ),
    // A display name, never a path.
    'KOLUX_WORKSPACE_NAME/u'
  ]
}

export function addKoluxWslInteropEnv(env: Record<string, string>): void {
  ...
  env.KOLUX_SHELL_READY_ROOT = getShellReadyWrapperRoot()
  ...
  const passthroughEntries = [
    ...
    // and it cannot derive the hash segment from KOLUX_USER_DATA_PATH alone.
    'KOLUX_SHELL_READY_ROOT/p',
    'KOLUX_CLI_COMMAND/u',
    'KOLUX_CODEX_LAUNCH_PREFLIGHT/p',
    'KOLUX_PANE_KEY/u',
    'KOLUX_TAB_ID/u',
    'KOLUX_WORKTREE_ID/u',
    'KOLUX_AGENT_LAUNCH_TOKEN/u',
    `${SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV}/u`,
    `${SETUP_AGENT_SEQUENCE_STARTUP_SCRIPT_ENV}/u`,
    'KOLUX_ORCHESTRATION_COMPATIBILITY_HOST_KIND/u',
    'KOLUX_ORCHESTRATION_COMPATIBILITY_HOST_ID/u',
    'KOLUX_ORCHESTRATION_COMPATIBILITY_HOST_INCARNATION/u',
    'KOLUX_AGENT_HOOK_PORT/u',
    'KOLUX_AGENT_HOOK_TOKEN/u',
    'KOLUX_AGENT_HOOK_ENV/u',
