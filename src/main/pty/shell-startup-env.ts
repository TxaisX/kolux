...
// Why: Kolux launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Kolux bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Kolux's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Kolux's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Kolux does not inherit interactive shell exports, but the
