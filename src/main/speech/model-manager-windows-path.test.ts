...
describe('ModelManager Windows model path handling', () => {
  beforeEach(() => {
    ...
    appGetPathMock.mockImplementation(() => join(tmpdir(), 'kolux-speech-models-test'))
  })
  ...
  it('uses an ASCII cache path when the Windows default user data path has non-ASCII characters', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-model-manager-'))
    try {
      setPlatform('win32')
      const programDataDir = join(dir, 'ProgramData')
      const userDataDir = join(dir, '用户', 'Kolux')
      ...
      expect(manager.getModelsDir()).toContain(join(programDataDir, 'Kolux', 'speech-models'))
