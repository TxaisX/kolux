 ...
 * Kolux's history tree. That is why deletion needs its own path here instead of
 ...
 */
const SESSION_PREFIX = 'kolux_'
const RELAY_SESSION_PREFIX = 'kolux_relay_'
const SAFE_SESSION_NAME = /^kolux_(?:relay_)?[0-9a-f]{1,64}$/
/** Deliberately does NOT match the relay prefix. A relay host keyed by its
 *  CLIENT's worktree ids shares the one fish data dir with any desktop Kolux on
...
const KOLUX_HISTORY_FILE = /^kolux_([0-9a-f]{1,64})_history$/
...
/** Drop a `fish_history` this process inherited from an outer Kolux.
 *  fish EXPORTS the variable, so a Kolux launched from a fish pane hands the
 *  LAUNCHING worktree's session name to every pane of the nested app, which then
 *  writes into that worktree's history file (STA-4682). Only Kolux-minted names
...
export function dropInheritedKoluxFishHistory(env: Record<string, string | undefined>): void {
 ...
 *  Resolve this from the PTY's SPAWN env: Kolux can be launched with a different
 ...
 *  the two disagree whenever Kolux's environment differs from the PTY's. */
...
/**
 * Delete every Kolux-minted fish history file whose worktree is gone.
 *
 ...
 * scan of the data dir can find those. Matching on Kolux's own `kolux_<hex>_`
...
export function sweepOrphanedFishHistoryFiles(
  ...
  for (const dir of new Set(dirs)) {
    ...
    for (const entry of entries) {
      const hash = KOLUX_HISTORY_FILE.exec(entry)?.[1]
