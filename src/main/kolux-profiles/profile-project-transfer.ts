...
function assertKnownProfiles(args: TransferKoluxProfileProjectArgs, userDataPath: string): void {
  const profiles = getKoluxProfileListState(userDataPath).profiles
  const ids = new Set(profiles.map((profile) => profile.id))
  if (!ids.has(args.sourceProfileId)) {
    throw new Error('unknown_source_kolux_profile')
  }
  if (!ids.has(args.targetProfileId)) {
    throw new Error('unknown_target_kolux_profile')
  }
  if (args.sourceProfileId === args.targetProfileId) {
    throw new Error('matching_kolux_profile_transfer')
  }
}

export function transferKoluxProfileProject(
