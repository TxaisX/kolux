...
export type KoluxCloudAuthConfig = {
  apiBaseUrl: string
...
const PRODUCTION_API_BASE_URL = 'https://login.kolux.invalid'
const PRODUCTION_CLIENT_ID = 'kolux-desktop'
const PRODUCTION_RELAY_DIRECTOR_URL = 'https://relay.kolux.invalid'
...
function isPackagedKoluxBuild(): boolean {
  try {
...
export function getKoluxCloudAuthConfig(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedKoluxBuild()
):
  | { configured: true; config: KoluxCloudAuthConfig }
  ...
  const configuredApiBaseUrl = env.KOLUX_CLOUD_API_URL?.trim()
  ...
  const apiBaseUrl = configuredApiBaseUrl
    ...
    : packaged
      ...
      : null
  const clientId =
    env.KOLUX_CLOUD_CLIENT_ID?.trim() || (packaged ? PRODUCTION_CLIENT_ID : undefined)
  ...
  const authBaseUrl = cleanEndpointUrl(env.KOLUX_CLOUD_AUTH_URL) ?? apiBaseUrl
  return {
    configured: true,
    config: {
      apiBaseUrl,
      authorizeEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_AUTHORIZE_URL) ??
        endpoint(authBaseUrl, '/v1/desktop/auth/authorize'),
      sessionEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_SESSION_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/session'),
      refreshEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_REFRESH_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/refresh'),
      capabilitiesEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_CAPABILITIES_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/capabilities'),
      profileEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_PROFILE_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/profile'),
      orgEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_ORG_URL) ?? endpoint(apiBaseUrl, '/v1/desktop/auth/org'),
      logoutEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_LOGOUT_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/logout'),
      relayTokenEndpoint:
        cleanEndpointUrl(env.KOLUX_CLOUD_RELAY_TOKEN_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/relay-token'),
      relayDirectorUrl:
        cleanOrigin(env.KOLUX_RELAY_URL, allowLoopbackHttp) ?? PRODUCTION_RELAY_DIRECTOR_URL,
      clientId,
      scope: env.KOLUX_CLOUD_AUTH_SCOPE?.trim() || DEFAULT_SCOPE
    }
  }
}

export function allowsPlaintextKoluxCloudSession(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedKoluxBuild()
): boolean {
  return (
    env.KOLUX_CLOUD_ALLOW_PLAINTEXT_SESSION === '1' && env.NODE_ENV !== 'production' && !packaged
  )
}

export function isKoluxCloudDevAuthEnabled(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedKoluxBuild()
): boolean {
  return env.KOLUX_CLOUD_DEV_AUTH === '1' && env.NODE_ENV !== 'production' && !packaged
}
