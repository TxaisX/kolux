...
describe('Kolux cloud client', () => {
  ...
  it('normalizes session exchange organizations', async () => {
    ...
    await expect(
      exchangeKoluxCloudAuthCode(config, {
        code: 'code',
  ...
  it('normalizes organization selection response metadata', async () => {
    ...
    await expect(selectKoluxCloudOrg(config, session, 'org-2')).resolves.toEqual({
  ...
  it('creates cloud profiles with a profile-scoped session response', async () => {
    ...
    await expect(
      createKoluxCloudProfile(config, session, { orgId: 'org-1', name: 'Acme' })
    ).resolves.toMatchObject({
      accessToken: 'new-access-token',
      refreshToken: 'new-refresh-token',
  ...
  it('refreshes session material without exposing refresh tokens in URLs', async () => {
    ...
    await expect(refreshKoluxCloudSession(config, session)).resolves.toMatchObject({
      accessToken: 'rotated-access-token',
      refreshToken: 'rotated-refresh-token'
    })
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('refreshes capability flags and optional org metadata with the current access token', async () => {
    ...
    await expect(refreshKoluxCloudCapabilities(config, session)).resolves.toEqual({
  ...
  it('trims cloud metadata and drops blank active org fields', async () => {
    ...
    await expect(refreshKoluxCloudCapabilities(config, session)).resolves.toMatchObject({
      cloud: {
        cloudProfileId: 'cloud-profile-1',
        userId: 'user-1',
        email: 'nina@example.com',
  ...
  it('cancels the unread error-response body so bundled undici cannot crash on socket close', async () => {
    ...
    await expect(refreshKoluxCloudCapabilities(config, session)).rejects.toThrow()
