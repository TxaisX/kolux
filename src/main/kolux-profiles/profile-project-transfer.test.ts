...
function profile(id: string, name: string): KoluxProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'kolux-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kolux-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/kolux-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:/workspace/kolux-nautilus': { exhaustedTiers: 0, names: ['seahorse'] }
        },
        worktreeMeta: {
    ...
    const { transferKoluxProfileProject } = await loadTransferModule()
    const result = transferKoluxProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/kolux-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/kolux' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/kolux-feature'
    ...
    writeProfileState(
      ...
      makeState({
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:posix:/srv/kolux-kolux-retirement-probe': {
            exhaustedTiers: 0,
            names: ['seahorse']
          }
        },
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:kolux-profile-personal-deadbeef-browser-default',
    ...
    expect(target.retiredWorktreeNamesByNamespace).toEqual({
      'ssh:builder.example.com|22|dev:posix:/srv/kolux-kolux-retirement-probe': {
        exhaustedTiers: 0,
        names: ['seahorse']
      }
    })
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Kolux\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/kolux' })]
      })
    )
