...
import type { KoluxProfileCloudSummary, KoluxProfileSummary } from '../../shared/kolux-profiles'
...
function cleanupMarkerPath(profileId: string, userDataPath: string): string {
  return join(getKoluxProfileDirectory(profileId, userDataPath), 'artifact-cloud-cleanup.json')
}

export function artifactCloudIdentity(cloud: KoluxProfileCloudSummary | undefined): string {
  return cloud
...
export function prepareArtifactCloudCleanup(
  profileId: string,
  userDataPath: string,
  targetCloud: KoluxProfileCloudSummary | undefined
): void {
...
export function completeArtifactCloudCleanupIfCommitted(
  ...
  bestEffortFsyncDirectorySync(getKoluxProfileDirectory(profileId, userDataPath))
}
...
function assertArtifactCloudCleanupReady(
  ...
  if (
    ...
    throw new Error('The Kolux profile transition must be retried before publishing artifacts.')
  }
}

export function prepareArtifactCloudUse(
  profile: Pick<KoluxProfileSummary, 'id' | 'cloud'>,
  userDataPath: string
): void {
