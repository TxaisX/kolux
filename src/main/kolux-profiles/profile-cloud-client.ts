...
export class KoluxCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_kolux_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): KoluxCloudCapabilities {
...
function normalizeOrganizations(value: unknown): KoluxCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): KoluxProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_kolux_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): KoluxCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_kolux_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
export function isAmbiguousCloudRequestFailure(error: unknown): boolean {
  return !(error instanceof KoluxCloudRequestError)
}
...
async function postJson<T>(url: string, body: unknown, options?: PostJsonOptions): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new KoluxCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeKoluxCloudAuthCode(
  config: KoluxCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<KoluxCloudSessionExchangeResponse> {
...
export async function refreshKoluxCloudCapabilities(
  config: KoluxCloudAuthConfig,
...
export async function refreshKoluxCloudSession(
  config: KoluxCloudAuthConfig,
  session: KoluxCloudSession
): Promise<KoluxCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createKoluxCloudProfile(
  config: KoluxCloudAuthConfig,
  session: KoluxCloudSession,
...
export async function selectKoluxCloudOrg(
  config: KoluxCloudAuthConfig,
  session: KoluxCloudSession,
  orgId: string
...
export async function revokeKoluxCloudSession(
  config: KoluxCloudAuthConfig,
  session: KoluxCloudSession
): Promise<void> {
  await postJson(
