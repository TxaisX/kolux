...
import {
  ...
  DEFAULT_LOCAL_KOLUX_PROFILE_NAME,
  KOLUX_PROFILE_INDEX_SCHEMA_VERSION,
  type CreateLocalKoluxProfileArgs,
  type CreateLocalKoluxProfileResult,
  type KoluxProfileIndex,
  type KoluxProfileListState,
  type KoluxProfileSummary
} from '../../shared/kolux-profiles'
import {
  getKoluxProfileBrowserSessionMetaFile,
  getKoluxProfileDataFile,
  getKoluxProfileDirectory,
  getKoluxProfileIndexPath,
...
export {
  ...
  getKoluxProfileDirectory,
  getKoluxProfileIndexPath,
  getKoluxProfilesDirectory,
  initKoluxProfilePaths
} from './profile-storage-paths'

export type ActiveKoluxProfileState = {
  index: KoluxProfileIndex
...
function isProfileSummary(value: unknown): value is KoluxProfileSummary {
  if (!isObject(value)) {
    return false
  }
...
function normalizeProfileIndex(raw: unknown): KoluxProfileIndex | null {
...
function readProfileIndexFile(indexPath: string): KoluxProfileIndex | null {
  try {
...
export function readProfileIndex(indexPath: string): KoluxProfileIndex | null {
...
export function writeProfileIndex(indexPath: string, index: KoluxProfileIndex): void {
...
function copyLegacyStateToProfile(userDataPath: string, profileId: string): void {
  const profileDataFile = getKoluxProfileDataFile(profileId, userDataPath)
  copyIfPresent(legacyDataFilePath(userDataPath), profileDataFile)
  copyIfPresent(
    legacyBrowserSessionMetaPath(userDataPath),
    getKoluxProfileBrowserSessionMetaFile(profileId, userDataPath)
  )
...
export function seedNewKoluxProfileTelemetryConsent(
  profileId: string,
  ...
  const dataFile = getKoluxProfileDataFile(profileId, userDataPath)
...
function createInitialProfileIndex(now = Date.now()): KoluxProfileIndex {
  const profile = createDefaultLocalKoluxProfile(now)
  return {
    schemaVersion: KOLUX_PROFILE_INDEX_SCHEMA_VERSION,
...
export function loadOrCreateProfileIndex(userDataPath: string): KoluxProfileIndex {
  const indexPath = getKoluxProfileIndexPath(userDataPath)
...
function getActiveProfile(index: KoluxProfileIndex): KoluxProfileSummary {
  return (
    index.profiles.find((profile) => profile.id === index.activeProfileId) ??
    index.profiles[0] ??
    createDefaultLocalKoluxProfile(Date.now())
  )
}

export function ensureActiveKoluxProfile(
  userDataPath = getProfileUserDataPath()
): ActiveKoluxProfileState {
  const indexPath = getKoluxProfileIndexPath(userDataPath)
  ...
  const profileDirectory = getKoluxProfileDirectory(activeProfile.id, userDataPath)
  mkdirSync(profileDirectory, { recursive: true })
  if (activeProfile.id === DEFAULT_LOCAL_KOLUX_PROFILE_ID) {
  ...
  return {
    ...
    dataFile: getKoluxProfileDataFile(activeProfile.id, userDataPath),
    profileDirectory
  }
}

export function isDefaultLocalKoluxProfileId(profileId: string): boolean {
  return profileId === DEFAULT_LOCAL_KOLUX_PROFILE_ID
}

export function getKoluxProfileListState(
  userDataPath = getProfileUserDataPath()
): KoluxProfileListState {
  const { index } = ensureActiveKoluxProfile(userDataPath)
  return {
...
export function createLocalKoluxProfile(
  args: CreateLocalKoluxProfileArgs = {},
  userDataPath = getProfileUserDataPath()
): CreateLocalKoluxProfileResult {
  const index = loadOrCreateProfileIndex(userDataPath)
  const now = Date.now()
  const name = sanitizeProfileName(args.name)
  const profile: KoluxProfileSummary = {
    id: `local-${randomUUID()}`,
    name,
    avatar: {
      kind: 'initials',
      initials: (
        name.match(/[A-Za-z0-9]/)?.[0] ?? DEFAULT_LOCAL_KOLUX_PROFILE_NAME[0]
      ).toUpperCase(),
      color: 'neutral'
    },
    kind: 'local',
  ...
  const nextIndex: KoluxProfileIndex = {
    ...index,
    profiles: [...index.profiles, profile]
  }
  mkdirSync(getKoluxProfileDirectory(profile.id, userDataPath), { recursive: true })
  writeProfileIndex(getKoluxProfileIndexPath(userDataPath), nextIndex)
  return {
...
export function setActiveKoluxProfile(
  profileId: string,
  userDataPath = getProfileUserDataPath()
): KoluxProfileListState {
  const index = loadOrCreateProfileIndex(userDataPath)
  ...
  if (!found) {
    throw new Error('unknown_kolux_profile')
  }
  const nextIndex: KoluxProfileIndex = {
    ...index,
  ...
  mkdirSync(getKoluxProfileDirectory(profileId, userDataPath), { recursive: true })
  writeProfileIndex(getKoluxProfileIndexPath(userDataPath), nextIndex)
  return {
    activeProfileId: nextIndex.activeProfileId,
    profiles: nextIndex.profiles
