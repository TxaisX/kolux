...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kolux-profile-test-'))
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'kolux-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'kolux-data.json.bak.0'),
    ...
    const { ensureActiveKoluxProfile, getKoluxProfileIndexPath } = await loadProfileIndexStore()
    const activeProfile = ensureActiveKoluxProfile()

    expect(activeProfile.profile.id).toBe(DEFAULT_LOCAL_KOLUX_PROFILE_ID)
    expect(activeProfile.profile.name).toBe(DEFAULT_LOCAL_KOLUX_PROFILE_NAME)
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_KOLUX_PROFILE_ID, 'kolux-data.json')
    )
    ...
    expect(
      readJson(
        join(testState.dir, 'profiles', DEFAULT_LOCAL_KOLUX_PROFILE_ID, 'browser-session-meta.json')
      )
    ).toEqual(legacyBrowserSessionMeta)
    expect(existsSync(join(testState.dir, 'kolux-data.json'))).toBe(true)

    expect(readJson(getKoluxProfileIndexPath())).toMatchObject({
      schemaVersion: KOLUX_PROFILE_INDEX_SCHEMA_VERSION,
      activeProfileId: DEFAULT_LOCAL_KOLUX_PROFILE_ID,
      profiles: [expect.objectContaining({ id: DEFAULT_LOCAL_KOLUX_PROFILE_ID, kind: 'local' })]
    })
  })
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'kolux-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
    ...
    const { ensureActiveKoluxProfile } = await loadProfileIndexStore()
    const activeProfile = ensureActiveKoluxProfile()

    expect(activeProfile.profile.id).toBe(profileId)
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'kolux-data.json'))
  ...
  it('creates an empty local profile without copying legacy state into it', async () => {
    ...
    const { createLocalKoluxProfile, getKoluxProfileDataFile, getKoluxProfileListState } =
      await loadProfileIndexStore()
    const created = createLocalKoluxProfile({ name: ' Work ' })

    expect(created.profile.name).toBe('Work')
    expect(created.profile.id).toMatch(/^local-/)
    expect(created.activeProfileId).toBe(DEFAULT_LOCAL_KOLUX_PROFILE_ID)
    expect(created.profiles.map((profile) => profile.id)).toContain(created.profile.id)
    expect(existsSync(getKoluxProfileDataFile(created.profile.id))).toBe(false)
    expect(getKoluxProfileListState().profiles.map((profile) => profile.id)).toContain(
      created.profile.id
    )
  })

  it('switches the active profile and updates last-opened metadata', async () => {
    const { createLocalKoluxProfile, setActiveKoluxProfile } = await loadProfileIndexStore()
    const created = createLocalKoluxProfile({ name: 'Work' })

    const switched = setActiveKoluxProfile(created.profile.id)
  ...
  it('rejects switching to an unknown profile', async () => {
    const { setActiveKoluxProfile } = await loadProfileIndexStore()

    expect(() => setActiveKoluxProfile('missing-profile')).toThrow('unknown_kolux_profile')
  })
  ...
  posixIt('writes a fresh profile index when umask removes owner-write permission', async () => {
    ...
    const indexPath = store.getKoluxProfileIndexPath()
    const profile = createDefaultLocalKoluxProfile(1)
    const index: KoluxProfileIndex = {
      schemaVersion: KOLUX_PROFILE_INDEX_SCHEMA_VERSION,
  ...
  it('recovers a corrupted profile index from the backup copy', async () => {
    const store = await loadProfileIndexStore()
    store.ensureActiveKoluxProfile()
    const created = store.createLocalKoluxProfile({ name: 'Work' })
    // Trigger one more write so the backup captures the two-profile index.
    store.setActiveKoluxProfile(created.profile.id)

    const indexPath = store.getKoluxProfileIndexPath()
    expect(existsSync(`${indexPath}.bak`)).toBe(true)
    writeFileSync(indexPath, '{ not json', 'utf-8')

    const recovered = store.getKoluxProfileListState()
  ...
  it('rejects profile ids that are not safe path segments', async () => {
    ...
    const state = store.ensureActiveKoluxProfile()
    expect(state.profile.id).toBe(DEFAULT_LOCAL_KOLUX_PROFILE_ID)
  })
})
