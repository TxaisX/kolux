...
function devSelf(): KoluxOrgMember {
  return {
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@kolux.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to Kolux yet" disabled row.
      {
        userId: null,
        email: 'invited-member@kolux.local',
        displayName: undefined,
        role: 'member'
      }
    ],
    pendingInvites: [{ email: 'pending@kolux.local', role: 'member', createdAt: Date.now() }]
  }
}
...
export function listDevKoluxCloudOrgMembers(orgId: string): KoluxOrgMembersRoster {
...
export function inviteDevKoluxCloudOrgMember(
...
export function revokeDevKoluxCloudOrgInvite(
...
export function changeDevKoluxCloudOrgMemberRole(
...
export function removeDevKoluxCloudOrgMember(
