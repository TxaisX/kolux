...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginKoluxCloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginKoluxCloudPkceFlow(config, 'local-default')
...
describe('Kolux cloud PKCE flow', () => {
  beforeEach(() => {
  ...
  it('keeps the loopback listener alive after an invalid callback', async () => {
    ...
    expect(validResponse.body).toContain('<h1>Signed in to Kolux</h1>')
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    await expect(observedFlow).resolves.toMatchObject({ message: 'kolux_cloud_auth_denied' })
  })
