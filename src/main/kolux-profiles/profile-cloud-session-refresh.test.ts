...
vi.mock('./profile-cloud-session-store', () => ({
  readKoluxCloudSession: readMock,
  saveKoluxCloudSessionIfCurrent: saveIfCurrentMock,
  clearKoluxCloudSession: clearMock
}))
...
vi.mock('./profile-cloud-client', async (importOriginal) => {
  ...
  return { ...original, refreshKoluxCloudSession: refreshMock }
})

vi.mock('./profile-cloud-index', () => ({ linkKoluxProfileToCloud: linkMock }))

import { readFreshKoluxCloudSession } from './profile-cloud-session-refresh'
import { KoluxCloudRequestError } from './profile-cloud-client'
import { onKoluxCloudSessionInvalidated } from './profile-cloud-session-invalidation'
import { forgetAmbiguousRefreshAttempt } from './profile-cloud-refresh-replay-guard'

const config = {} as KoluxCloudAuthConfig
const active = {
  profile: {
    id: 'profile-1',
    cloud: {
...
describe('profile cloud session refresh', () => {
  ...
  it('does not publish a refresh whose persistent mutation snapshot became stale', async () => {
    ...
    const refreshing = readFreshKoluxCloudSession(config, active, '/data')
  ...
  it('single-flights concurrent rotating refresh-token use per profile and store', async () => {
    ...
    const first = readFreshKoluxCloudSession(config, active, '/data')
    const second = readFreshKoluxCloudSession(config, active, '/data')
  ...
  it('notifies subscribers when an auth failure clears the stored session', async () => {
    ...
    const unsubscribe = onKoluxCloudSessionInvalidated(invalidated)
    refreshMock.mockRejectedValue(new KoluxCloudRequestError(401))

    await expect(readFreshKoluxCloudSession(config, active, '/data')).resolves.toEqual({
...
describe('refresh-token replay after an ambiguous attempt', () => {
  ...
  it('never resends a refresh token whose attempt timed out', async () => {
    ...
    await expect(readFreshKoluxCloudSession(config, active, '/data')).rejects.toThrow(
    ...
    await expect(readFreshKoluxCloudSession(config, active, '/data')).rejects.toThrow(
      'kolux_cloud_refresh_replay_blocked'
    )
    expect(refreshMock).toHaveBeenCalledTimes(1)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('retries once after a definitive 5xx, which cannot have rotated the token', async () => {
    refreshMock
      .mockRejectedValueOnce(new KoluxCloudRequestError(503))
      .mockResolvedValueOnce(rotatedResponse)

    const result = await readFreshKoluxCloudSession(config, active, '/data')
  ...
  it('gives a definitive 5xx exactly one retry', async () => {
    refreshMock.mockRejectedValue(new KoluxCloudRequestError(503))

    await expect(readFreshKoluxCloudSession(config, active, '/data')).rejects.toThrow(
      'kolux_cloud_request_failed_503'
    )
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('marks a 401 that follows an ambiguous attempt as a possible self-replay', async () => {
    ...
    refreshMock.mockRejectedValueOnce(new KoluxCloudRequestError(401))
    ...
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(warn.mock.calls.flat().join(' ')).toContain('kolux_cloud_refresh_possible_replay')
    expect(clearMock).toHaveBeenCalledTimes(1)
    expect(invalidated).toHaveBeenCalledTimes(1)
    unsubscribe()
  })
  ...
  it('does not mark a 401 that follows no ambiguous attempt', async () => {
    ...
    expect(warn.mock.calls.flat().join(' ')).not.toContain('kolux_cloud_refresh_possible_replay')
    expect(clearMock).toHaveBeenCalledTimes(1)
  })
})
