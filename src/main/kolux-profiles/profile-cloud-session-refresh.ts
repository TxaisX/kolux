...
import { emitKoluxCloudSessionInvalidated } from './profile-cloud-session-invalidation'

const CLOUD_SESSION_REFRESH_SKEW_MS = 60_000

export type FreshCloudSessionResult =
  | { status: 'found'; session: KoluxCloudSession }
...
function shouldRefreshCloudSession(session: KoluxCloudSession, now = Date.now()): boolean {
  return session.expiresAt <= now + CLOUD_SESSION_REFRESH_SKEW_MS
}

export function isKoluxCloudAuthFailure(error: unknown): boolean {
  return (
...
const inflightCloudSessionRefreshes = new Map<string, Promise<KoluxCloudSession>>()
...
function clearCloudSessionIfUnchanged(
  profileId: string,
  userDataPath: string,
  failed: KoluxCloudSession,
  active: ActiveKoluxProfileState
): void {
  const current = readKoluxCloudSession(profileId, userDataPath)
  ...
  // not even be the one that failed, and `clearKoluxCloudSession` unlinks the file outright.
  ...
  clearKoluxCloudSession(profileId, userDataPath)
  ...
  emitKoluxCloudSessionInvalidated()
}
...
function warnIfPossibleRefreshReplay(
  ...
  console.warn(
    '[kolux-cloud] kolux_cloud_refresh_possible_replay: refresh rejected 401 for a token whose earlier attempt never answered'
  )
}

type CloudSessionRefreshAttempt =
  | { status: 'refreshed'; response: KoluxCloudSessionExchangeResponse }
  | { status: 'rotated-elsewhere'; session: KoluxCloudSession }

function isRetryableCloudRefreshRejection(error: unknown): boolean {
  return error instanceof KoluxCloudRequestError && error.statusCode >= 500
}
...
async function attemptCloudSessionRefresh(
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      const response = await refreshKoluxCloudSession(config, session)
      ...
      const current = readKoluxCloudSession(active.profile.id, userDataPath)
...
async function refreshStoredCloudSession(
  ...
  const task = (async () => {
    ...
    if (
      saveKoluxCloudSessionIfCurrent(active.profile.id, userDataPath, nextSession, snapshot) ===
      null
    ) {
      throw new StaleCloudSessionMutationError()
    }
    linkKoluxProfileToCloud(active.profile.id, refreshed.cloud, userDataPath)
    return nextSession
  })()
...
export async function readFreshKoluxCloudSession(
  config: KoluxCloudAuthConfig,
  active: ActiveKoluxProfileState,
  userDataPath: string
  ...
  try {
    ...
    if (isKoluxCloudAuthFailure(error)) {
...
export async function forceRefreshKoluxCloudSession(
  config: KoluxCloudAuthConfig,
  active: ActiveKoluxProfileState,
  userDataPath: string,
  session: KoluxCloudSession
...
export async function runWithFreshKoluxCloudSession<T>(
  config: KoluxCloudAuthConfig,
  active: ActiveKoluxProfileState,
  userDataPath: string,
  operation: (session: KoluxCloudSession) => Promise<T>
): Promise<CloudSessionOperationResult<T>> {
  const session = await readFreshKoluxCloudSession(config, active, userDataPath)
  if (session.status !== 'found') {
    return { status: 'reconnect-required' }
  }
  try {
    ...
    const refreshed = await forceRefreshKoluxCloudSession(
      config,
      active,
      userDataPath,
      session.session
    ...
    try {
      ...
      if (retryError instanceof KoluxCloudRequestError && retryError.statusCode === 401) {
