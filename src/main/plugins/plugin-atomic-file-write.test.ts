...
describe('writePluginFileAtomically', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'kolux-plugin-atomic-'))
  })
...
describe('renamePluginFileWithWindowsRetry', () => {
  it('renames when the source exists', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'kolux-plugin-rename-'))
    try {
  ...
  describe('on Windows', () => {
    ...
    beforeEach(async () => {
      dir = await mkdtemp(join(tmpdir(), 'kolux-plugin-rename-win-'))
  ...
  it('does not retry off Windows', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'kolux-plugin-rename-posix-'))
