...
describe('PluginCommandRegistry', () => {
  ...
  it('uses saved effective bindings to recover conflicting plugins', () => {
    ...
    registry.reconcile(
      [first, second],
      () => true,
      { 'plugin:kolux-samples.first/tasks': ['Mod+Shift+T'] },
      'linux'
    )
  ...
  it('rejects conflicting saved bindings within one plugin', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:kolux-samples.aliases/tasks': ['Mod+Alt+T'],
        'plugin:kolux-samples.aliases/sidebar': ['Mod+Alt+T']
      },
      'linux'
    )
