...
async function pluginWithCatalog(catalog: unknown): Promise<ValidDiscoveredPlugin> {
  const rootDir = await mkdtemp(join(tmpdir(), 'kolux-plugin-language-registry-'))
  roots.push(rootDir)
...
describe('PluginLanguagePackRegistry', () => {
  it('loads approved catalogs under an isolated plugin language id', async () => {
    ...
    expect(registry.list()).toEqual([
      {
        id: 'plugin:kolux-samples.portuguese/pt-BR',
        resourceLanguage:
          'plugin0070006c007500670069006e003a006b006f006c00750078002d00730061006d0070006c00650073002e0070006f00720074007500670075006500730065002f00700074002d00420052',
        pluginKey: 'kolux-samples.portuguese',
        locale: 'pt-BR',
