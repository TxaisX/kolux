...
export abstract class UpdaterScheduling extends UpdaterCheckFailure {
  ...
  protected runBackgroundUpdateCheck(
    ...
    if (
      ...
      // Why: found releases auto-download, so a fallback build sits in 'downloaded' while its retry waits for the newest tag.
      ((this.currentStatus.state === 'available' || this.currentStatus.state === 'downloaded') &&
        !allowWhileAvailable)
    ) {
      return false
    }
