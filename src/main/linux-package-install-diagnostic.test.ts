...
describe('redactLinuxPackageInstallText', () => {
  ...
  it('replaces every cached package-path occurrence before the home directory', () => {
    ...
    const packagePath = `${home}/.cache/kolux-updater/Kolux-1.2.3.deb`
    const text = `${packagePath} failed; retry ${packagePath}; config ${home}/.config/kolux`
    expect(diagnostic.redactLinuxPackageInstallText(text, packagePath)).toBe(
      '<package> failed; retry <package>; config <home>/.config/kolux'
    )
  })
