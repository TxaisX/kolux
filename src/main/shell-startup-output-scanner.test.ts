...
const READY_MARKER = '\x1b]777;kolux-shell-ready\x07'
const IDENTITY_MARKER = '\x1b]777;kolux-shell-start:12345\x07'
...
describe('shell startup output scanner', () => {
  ...
  it('drains every incomplete scanner prefix in byte order', () => {
    const state = createShellStartupOutputScanState()
    expect(scanShellStartupOutput(state, '\x1b]777;kolux-shell-st').output).toBe('')

    expect(drainShellStartupOutputScanState(state)).toBe('\x1b]777;kolux-shell-st')
  })

  it('drains simultaneous identity and readiness prefixes in input order', () => {
    const state = createShellStartupOutputScanState()
    expect(scanShellStartupOutput(state, '\x1b]777;kolux-shell-st').output).toBe('')
    expect(scanShellStartupOutput(state, '\x1b]777;kolux-shell-rea').output).toBe('')

    expect(drainShellStartupOutputScanState(state)).toBe(
      '\x1b]777;kolux-shell-st\x1b]777;kolux-shell-rea'
    )
  })
})
