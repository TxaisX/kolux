...
export function installMainThreadHangWatchdog(options: {
  ...
  if (!app.isPackaged && process.env.KOLUX_HANG_WATCHDOG_FORCE !== '1') {
    return null
  }
  ...
  const workerData: HangWatchdogWorkerData = {
    ...
    timeoutMs: positiveTiming(process.env.KOLUX_HANG_WATCHDOG_TIMEOUT_MS, HANG_WATCHDOG_TIMEOUT_MS),
    checkIntervalMs: positiveTiming(
      process.env.KOLUX_HANG_WATCHDOG_CHECK_INTERVAL_MS,
      HANG_WATCHDOG_CHECK_INTERVAL_MS
    )
  }
  let worker: Worker
  try {
    ...
    worker = new Worker(workerPath, {
      name: 'kolux-main-thread-hang-watchdog',
      workerData
    })
  } catch (error) {
