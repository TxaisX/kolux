  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.KOLUX_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.KOLUX_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.KOLUX_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.KOLUX_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/kolux-agent-status.ts'
        )
        expect(env.KOLUX_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.KOLUX_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.KOLUX_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
