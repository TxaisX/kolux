...
describe('registerPtyHandlers', () => {
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    const originalKoluxOrigZdotdir = process.env.KOLUX_ORIG_ZDOTDIR
    ...
    try {
      ...
      // config dir to hand back and Kolux must not invent one — the wrapper
      // leaves ZDOTDIR unset, exactly as an unwrapped login zsh would.
      expect(options.env.KOLUX_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
      ...
      if (originalKoluxOrigZdotdir === undefined) {
        delete process.env.KOLUX_ORIG_ZDOTDIR
      } else {
        process.env.KOLUX_ORIG_ZDOTDIR = originalKoluxOrigZdotdir
      }
