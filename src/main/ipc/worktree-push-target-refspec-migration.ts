...
// Kolux-provenance `pr-*` remote's refspec to only the branches Kolux actually tracked
...
// alone isn't proof of Kolux provenance the way a metadata entry is.
//
...
/** `pushTarget`-derived branches to keep per remote, gated on at least one entry proving Kolux created it. */
...
export async function migrateForkRemoteRefspecsWithExec(
  ...
  for (const remoteName of candidateNames) {
    ...
    if (branches.size === 0) {
      ...
      // pre-#17828 Kolux's), not a `pr`-prefixed remote a user configured by hand. Clears
