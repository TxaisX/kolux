...
describe('registerNotificationHandlers', () => {
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    const originalBundleId = process.env.KOLUX_DEV_MACOS_BUNDLE_ID
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true })
    process.env.KOLUX_DEV_MACOS_BUNDLE_ID = 'com.txais.kolux.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.txais.kolux.dev.fb5a47066f08'
      )
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
      if (originalBundleId === undefined) {
        delete process.env.KOLUX_DEV_MACOS_BUNDLE_ID
      } else {
        process.env.KOLUX_DEV_MACOS_BUNDLE_ID = originalBundleId
      }
    }
  })
  ...
  it('suppresses active-worktree notifications while Kolux is focused', async () => {
