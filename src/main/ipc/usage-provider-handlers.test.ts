...
describe('usage provider IPC handlers', () => {
  it('registers every provider route and forwards query arguments', () => {
    ...
    call('claudeUsage', 'getSnapshot', { scope: 'kolux', range: '30d', limit: 7 })
    call('claudeUsage', 'getSummary', { scope: 'all', range: '7d' })
    call('claudeUsage', 'getDaily', { scope: 'kolux', range: '90d' })
    call('claudeUsage', 'getBreakdown', { scope: 'all', range: 'all', kind: 'model' })
    call('claudeUsage', 'getRecentSessions', { scope: 'kolux', range: '30d', limit: 4 })
    ...
    expect(claudeUsage.getDaily).toHaveBeenCalledWith('kolux', '90d')
    expect(claudeUsage.getBreakdown).toHaveBeenCalledWith('all', 'all', 'model')
    expect(claudeUsage.getRecentSessions).toHaveBeenCalledWith('kolux', '30d', 4)
  })
})
