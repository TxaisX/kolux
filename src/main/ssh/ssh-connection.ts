...
function isGitHubRestrictedShellProbeSuccess(
  ...
  if (firstLine !== 'Invalid command: echo KOLUX-SYSTEM-SSH-OK') {
    return false
  }
...
export class SshConnection {
  ...
  canRunConcurrentExecCommands(): boolean {
    ...
    return (
      getKoluxControlSocketPath(this.target, {
  ...
  private async attemptConnect(connectGeneration = ++this.connectGeneration): Promise<void> {
    ...
    try {
      ...
      if (isSystemSshFallbackError(err)) {
        ...
        this.proxyProcess = null
        try {
          // Why: on macOS, per-app network policy can block Kolux's direct TCP socket while the system OpenSSH binary is still allowed.
          await this.doSystemSshProbeWithControlMasterRetry(connectGeneration, resolved)
          return
        } catch {
          this.systemSshResolvedConfig = null
  ...
  private async doSystemSshProbe(connectGeneration: number): Promise<void> {
    ...
    const channel = this.spawnTrackedSystemSshCommand('echo KOLUX-SYSTEM-SSH-OK', {
      wrapCommand: false
    })
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const onClose = (code: number | null): void => {
          settle(() => {
            ...
            if (
              (code === 0 && stdout.includes('KOLUX-SYSTEM-SSH-OK')) ||
  ...
  private async doSystemSshProbeWithControlMasterRetry(
    ...
    const controlPath = getKoluxControlSocketPath(this.target, {
