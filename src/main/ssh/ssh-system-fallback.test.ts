...
function expectNoKoluxControlMasterArgs(args: string[]): void {
...
function expectKoluxControlMasterArgs(args: string[]): void {
  if (process.platform === 'win32') {
    expectNoKoluxControlMasterArgs(args)
    return
  }
...
describe('spawnSystemSsh', () => {
  ...
  it('passes an explicit main-owned OpenSSH config as one argument', () => {
    const args = buildSshArgs(createTarget({ configHost: 'isolated-host', source: 'ssh-config' }), {
      configFile: '/tmp/kolux isolated/ssh_config'
    })

    expect(args.slice(0, 2)).toEqual(['-F', '/tmp/kolux isolated/ssh_config'])
  ...
  it('passes explicit options for manual targets with implicit configHost', () => {
    const args = buildSshArgs(
      createTarget({
        ...
        host: '127.0.0.1',
        port: 2222,
        identityFile: '/tmp/kolux-docker-key',
        identitiesOnly: true
      })
    )

    expect(args).toEqual(expect.arrayContaining(['-p', '2222', '-i', '/tmp/kolux-docker-key']))
    expect(args).toContain('IdentitiesOnly=yes')
  ...
  it('does not inject Kolux ControlMaster flags when ssh config already owns muxing', () => {
  ...
  it('injects Kolux ControlMaster flags when ssh config only sets ControlPersist', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPersist: '10m'
    ...
    expectKoluxControlMasterArgs(args)
    expect(args).not.toContain('-S')
  })

  it('injects Kolux ControlMaster flags when ssh config only sets ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPath: '/Users/me/.ssh/cm/%r@%h:%p'
  ...
  it('injects Kolux ControlMaster flags when ssh config omits ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'auto'
      })
  ...
  it('does not inject Kolux ControlMaster flags for unresolved ssh-config targets', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }))
  ...
  it('does not inject Kolux ControlMaster flags for unresolved legacy config aliases', () => {
  ...
  it('can inject Kolux ControlMaster flags for ssh-config targets with resolved config', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig()
    })
  ...
  it('adds keepalive options to Kolux-owned ControlMaster connections', () => {
  ...
  it('streams a local file through one POSIX system SSH command', async () => {
    ...
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'kolux-system-ssh-upload-'))
  ...
  it('downloads files from POSIX system SSH targets', async () => {
    const proc = createEventedProcess()
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'kolux-system-ssh-download-'))
  ...
  it('sends Windows file writes over sftp, not through a remote PowerShell stdin', async () => {
    ...
    const promise = writeFileViaSystemSsh(
      createTarget(),
      'C:/Users/me/.kolux-remote/relay/.version',
      '0.1.0',
      { hostPlatform }
    )
    ...
    expect(batch).toContain('/C:/Users/me/.kolux-remote/relay/.version.kolux-partial-')
  ...
  it('uploads a Windows directory as a mkdir batch plus per-file writes, never one blob', async () => {
    const localDir = mkdtempSync(join(tmpdir(), 'kolux-system-ssh-upload-'))
    ...
    expect(mkdirBatch).toBe('-mkdir "/C:/Users/me/.kolux-remote/relay"\n')
    const putBatch = String(spawned[1]!.stdin.end.mock.calls[0]?.[0] ?? '')
    expect(putBatch).toContain('put ')
    expect(putBatch).toContain('/C:/Users/me/.kolux-remote/relay/relay.js.kolux-partial-')
