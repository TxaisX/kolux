...
describe('SshConnection', () => {
  ...
  it('passes the detected host platform to system SSH file operations', async () => {
    ...
    await conn.connect()
    await conn.uploadDirectory('/tmp/local-relay', 'C:/Users/me/.kolux-remote/relay', {
      hostPlatform
    })
    await conn.writeFile('C:/Users/me/.kolux-remote/relay/.version', '0.1.0', {
      hostPlatform
    })
    await conn.writeBuffer('C:/Users/me/.kolux-remote/relay/logo.png', Buffer.from('png'), {
      hostPlatform,
      exclusive: true
    })
    await conn.downloadFile('C:/Users/me/.kolux-remote/relay/logo.png', '/tmp/logo.png', {
      hostPlatform
    })
    ...
    expect(writeBufferViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      'C:/Users/me/.kolux-remote/relay/logo.png',
      Buffer.from('png'),
      expect.objectContaining({
        hostPlatform,
        exclusive: true,
