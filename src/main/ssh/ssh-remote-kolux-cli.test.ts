...
describe('runRemoteKoluxCli', () => {
  ...
  it('uses the remote KOLUX_TERMINAL_HANDLE as orchestration sender identity', async () => {
    ...
    const result = await runRemoteKoluxCli(
      runtime,
      {
        ...
        env: {
          ...
          KOLUX_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
    )
    ...
    try {
      const result = await runRemoteKoluxCli(
        runtime,
        {
          ...
          cwd: '/home/alice/repo',
          env: { KOLUX_PANE_KEY: 'tab_foreign:leaf_foreign' }
        },
        LEGACY_FALLBACK_OPTIONS
      )
    ...
    try {
      const result = await runRemoteKoluxCli(
        runtime,
        {
          ...
          env: {
            KOLUX_TERMINAL_HANDLE: 'term_ssh',
            KOLUX_PANE_KEY: 'tab_owner:leaf_owner'
          }
        },
        LEGACY_FALLBACK_OPTIONS
      )
  ...
  it('uses the remote KOLUX_TERMINAL_HANDLE as orchestration check identity', async () => {
    const { runtime, db } = createRuntime()
    db.insertMessage({
      from: 'term_windows',
      to: 'term_ssh',
  ...
  it('carries the remote pane key for an implicit orchestration check', async () => {
    ...
    const result = await runRemoteKoluxCli(
      runtime,
      {
        ...
        cwd: '/home/alice/repo',
        env: {
          KOLUX_TERMINAL_HANDLE: 'term_stale_ssh',
          KOLUX_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
  ...
  it('routes previously-unsupported commands through the full host CLI', async () => {
    ...
    const resultPromise = runRemoteKoluxCli(
      runtime,
      {
        argv: ['worktree', 'create', '--repo', 'kolux', '--branch', 'fix/x', '--json'],
        cwd: '/home/alice/repo',
        env: { KOLUX_TERMINAL_HANDLE: 'term_ssh' }
      },
      {
  ...
  it('rejects host-interactive commands with a targeted error instead of bridging them', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('kolux serve')
  ...
  it('rejects interactive account add but still bridges account list', async () => {
    ...
    const addResult = await runRemoteKoluxCli(
      runtime,
    ...
    const listPromise = runRemoteKoluxCli(
      runtime,
  ...
  it('bridges account add help because it does not start an interactive login', async () => {
    ...
    await Promise.resolve()
    child.stdout.emit('data', Buffer.from('Usage: kolux account add\n'))
    child.emit('close', 0)

    await expect(resultPromise).resolves.toEqual({
      stdout: 'Usage: kolux account add\n',
      stderr: '',
      exitCode: 0
    })
    expect(spawn).toHaveBeenCalledOnce()
  ...
  it('explains the root cause when falling back and the command is not in the legacy switch', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('Unsupported SSH Kolux CLI command: worktree list')
    expect(result.stderr).toContain('full Kolux CLI bridge unavailable')
  })
  ...
  it('does not parse Android --activity values as Linear boolean flags', async () => {
    ...
    expect(result.stderr).toContain(
      'Unsupported SSH Kolux CLI command: emulator launch com.acme.app'
    )
