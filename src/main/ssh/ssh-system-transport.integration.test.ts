...
const sentinel = 'KOLUX-RELAY v0.1.0 READY\\n';
...
function createRelayTree(root: string, remoteHome: string): void {
  ...
  const remoteDir = join(remoteHome, '.kolux-remote', `relay-${RELAY_VERSION}`)
...
describe('system SSH transport integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join('/tmp', 'kolux-ssh-'))
    oldHome = process.env.HOME
    oldRelayPath = process.env.KOLUX_RELAY_PATH
    oldSystemSshPath = process.env.KOLUX_SYSTEM_SSH_PATH
    oldForceSystemTransport = process.env.KOLUX_SSH_FORCE_SYSTEM_TRANSPORT
    ...
    process.env.KOLUX_RELAY_PATH = relayRoot
    process.env.KOLUX_SYSTEM_SSH_PATH = writeFakeSsh(tempDir)
  ...
  afterEach(() => {
    ...
    if (oldRelayPath === undefined) {
      delete process.env.KOLUX_RELAY_PATH
    } else {
      process.env.KOLUX_RELAY_PATH = oldRelayPath
    }
    if (oldSystemSshPath === undefined) {
      delete process.env.KOLUX_SYSTEM_SSH_PATH
    } else {
      process.env.KOLUX_SYSTEM_SSH_PATH = oldSystemSshPath
    }
    if (oldForceSystemTransport === undefined) {
      delete process.env.KOLUX_SSH_FORCE_SYSTEM_TRANSPORT
    } else {
      process.env.KOLUX_SSH_FORCE_SYSTEM_TRANSPORT = oldForceSystemTransport
    }
