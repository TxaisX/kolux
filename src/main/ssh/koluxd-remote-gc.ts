/**
 * koluxd's garbage collection, and the half of §06 falsifier 1 that says who owns it.
 *
 * **Each model GCs only its own namespace, permanently.** koluxd removes `koluxd-<v>/`
 ...
 * to keep intact.
 *
 * On top of the ownership rule, koluxd pins three directories that are idle-looking but
...
import { koluxdGcPinnedDirNames, type KoluxdActivationRecord } from './koluxd-activation-record'
import {
...
export async function gcOldKoluxdVersions(options: KoluxdGcOptions): Promise<void> {
  await gcOldRemoteInstallVersions(
    ...
    options.host,
    {
      pinnedDirNames: koluxdGcPinnedDirNames(options.record, options.liveDaemonVersion),
      isDirLive: async (dir) => {
        try {
          const probe = await execCommand(
            options.conn,
            koluxdLivenessProbeCommand(options.host, dir),
            {
              wrapCommand: options.host.commandDialect !== 'powershell',
              signal: options.signal
            }
          )
          return koluxdLivenessBlocksGc(parseKoluxdLiveness(probe))
        } catch {
