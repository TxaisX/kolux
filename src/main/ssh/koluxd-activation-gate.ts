/**
 * Whether a freshly launched koluxd has earned the right to become the active one.
 *
 * The failure this exists to prevent is the one `docs/design/shipping-koluxd.html` names
 * throughout: a deployment that reports success because a port opened. koluxd answers RPC
...
export type KoluxdActivationRejectCode =
  | 'koluxd_activation_no_readiness'
  | 'koluxd_activation_no_health'
  | 'koluxd_activation_build_mismatch'
  | 'koluxd_activation_not_listening'
  | 'koluxd_activation_daemon_absent'
  | 'koluxd_activation_daemon_degraded'
  | 'koluxd_activation_pty_self_test_failed'
  | 'koluxd_activation_no_persistent_terminals'

export type KoluxdActivationVerdict =
  | {
      decision: 'activate'
      /**
  ...
  | { decision: 'reject'; code: KoluxdActivationRejectCode; reason: string }

export type KoluxdActivationExpectation = {
  /** sha256(koluxd.js).slice(0,16) computed from the bytes this client just uploaded. */
  buildHash: string
 ...
 */
export function evaluateKoluxdActivation(
  readiness: ServeReadiness | null,
  expected: KoluxdActivationExpectation
): KoluxdActivationVerdict {
  if (!readiness) {
    return {
      decision: 'reject',
      code: 'koluxd_activation_no_readiness',
      reason:
        'The candidate koluxd never published an `kolux_server_ready` line. It may have exited, ' +
  ...
  if (!health) {
    return {
      decision: 'reject',
      code: 'koluxd_activation_no_health',
      reason:
  ...
  // Why identity first: a stale koluxd already holding the port would answer readiness and
  ...
  if (daemon.state === 'absent') {
    return {
      ...
      reason:
        ...
        'koluxd process and die with it, which is the exact regression the daemon exists to ' +
  ...
  if (health.buildVersion !== expected.fullVersion) {
    // Not a rejection: the hash already proved identity, and KOLUX_VERSION is whatever the
    ...
    warnings.push(
      `The candidate reports version ${health.buildVersion} but was installed as ` +
        `${expected.fullVersion}; check KOLUX_VERSION in the launch command.`
    )
  }
