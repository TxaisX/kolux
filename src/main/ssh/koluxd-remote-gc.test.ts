...
import { gcOldKoluxdVersions } from './koluxd-remote-gc'
import { emptyKoluxdActivationRecord } from './koluxd-activation-record'
...
function scriptHost(options: {
  ...
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (command.includes('.koluxd-pid')) {
...
describe('koluxd GC', () => {
  ...
  it('scopes its remote listing to the koluxd namespace', async () => {
    const removed: string[] = []
    scriptHost({ listing: [], removed })
    await gcOldKoluxdVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.kolux-remote/koluxd-0.2.0+bb',
      record: emptyKoluxdActivationRecord()
    })
    const listCommand = mockExec.mock.calls
      .map((call) => String(call[1]))
      .find((c) => c.includes('find'))
    expect(listCommand).toContain("-name 'koluxd-*'")
  ...
  it('leaves relay directories alone even when the host hands them to it', async () => {
    ...
    const removed: string[] = []
    scriptHost({
      listing: ['relay-0.1.0+aa', 'relay-0.0.9+ff', 'koluxd-0.1.0+aa'],
      removed
    })
    await gcOldKoluxdVersions({
      conn,
    ...
    expect(removed).toEqual(['koluxd-0.1.0+aa'])
  })

  it('never removes the active or the previous version', async () => {
    const removed: string[] = []
    scriptHost({
      listing: ['koluxd-0.1.0+01d', 'koluxd-0.2.0+9ee0', 'koluxd-0.3.0+cc0'],
      removed
    })
    await gcOldKoluxdVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.kolux-remote/koluxd-0.3.0+cc0',
      record: {
    ...
    expect(removed).toEqual(['koluxd-0.1.0+01d'])
  })

  it('never removes the version a live daemon was forked from', async () => {
    const removed: string[] = []
    scriptHost({ listing: ['koluxd-0.1.0+01d', 'koluxd-0.0.9+01de'], removed })
    await gcOldKoluxdVersions({
      conn,
      host,
      remoteHome: '/home/u',
      currentDirAbsPath: '/home/u/.kolux-remote/koluxd-0.3.0+cc0',
      record: { ...emptyKoluxdActivationRecord(), active: '0.3.0+cc0' },
      liveDaemonVersion: '0.1.0+01d'
    })
    expect(removed).toEqual(['koluxd-0.0.9+01de'])
  })

  it('treats an unanswerable liveness probe as in use', async () => {
    const removed: string[] = []
    scriptHost({
      listing: ['koluxd-0.1.0+bb0', 'koluxd-0.0.9+dead'],
      liveness: { 'koluxd-0.1.0+bb0': 'UNKNOWN', 'koluxd-0.0.9+dead': 'DEAD' },
      removed
    })
    await gcOldKoluxdVersions({
      conn,
    ...
    expect(removed).toEqual(['koluxd-0.0.9+dead'])
  })
})
