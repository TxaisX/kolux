/**
 * Starting a candidate koluxd on the host and reading back what it says about itself.
 *
 * This is the piece `docs/design/shipping-koluxd.html` §02 marks **fork**, not reuse: the
 * relay launches detached and proves itself by printing an `KOLUX-RELAY` sentinel, and koluxd
 * has no such interface. It publishes a single `kolux_server_ready` JSON line on stdout,
...
import {
  assertPosixKoluxdHost as assertPosixHost,
  KOLUXD_PID_FILENAME,
  posixProcessAliveShellFunction
} from './koluxd-remote-host-support'
import type { ServeReadiness } from '../server/serve-readiness'

/** Stdout of the launched candidate: exactly one `kolux_server_ready` line, then nothing. */
export const KOLUXD_READINESS_FILENAME = '.koluxd-readiness'
/** Stderr, including the bind-exposure line and every supervision message. */
export const KOLUXD_LOG_FILENAME = 'koluxd.log'
export {
  KOLUXD_PID_FILENAME,
  KoluxdRemoteLaunchUnsupportedError
} from './koluxd-remote-host-support'

export type KoluxdLaunchSpec = {
 ...
 *
 * Why `--bind` is always passed explicitly: koluxd defaults to loopback, but a default is a
 ...
 */
export function koluxdLaunchCommand(host: RemoteHostPlatform, spec: KoluxdLaunchSpec): string {
  assertPosixHost(host)
  const dir = shellEscape(spec.remoteInstallDir)
  const readiness = shellEscape(
    joinRemotePath(host, spec.remoteInstallDir, KOLUXD_READINESS_FILENAME)
  )
  const log = shellEscape(joinRemotePath(host, spec.remoteInstallDir, KOLUXD_LOG_FILENAME))
  const pidFile = shellEscape(joinRemotePath(host, spec.remoteInstallDir, KOLUXD_PID_FILENAME))
  const entry = shellEscape(joinRemotePath(host, spec.remoteInstallDir, 'koluxd.js'))
  return [
    `cd ${dir} &&`,
    ...
    `: > ${readiness} &&`,
    'umask 077 &&',
    `KOLUX_VERSION=${shellEscape(spec.fullVersion)}`,
    `KOLUX_USER_DATA=${shellEscape(spec.userDataDir)}`,
...
export function readKoluxdReadinessCommand(
  host: RemoteHostPlatform,
  remoteInstallDir: string
): string {
  assertPosixHost(host)
  const readiness = shellEscape(joinRemotePath(host, remoteInstallDir, KOLUXD_READINESS_FILENAME))
 ...
 */
export function koluxdLivenessProbeCommand(
  host: RemoteHostPlatform,
  remoteInstallDir: string
): string {
  assertPosixHost(host)
  const pidFile = shellEscape(joinRemotePath(host, remoteInstallDir, KOLUXD_PID_FILENAME))
  return [
    posixProcessAliveShellFunction(),
    ...
    '* ) if koluxd_alive "$pid"; then echo LIVE;',
...
export type KoluxdLiveness = 'LIVE' | 'DEAD' | 'UNKNOWN'

export function parseKoluxdLiveness(output: string): KoluxdLiveness {
...
export function koluxdLivenessBlocksGc(liveness: KoluxdLiveness): boolean {
  return liveness !== 'DEAD'
}

export type KoluxdReadinessParse =
  | { state: 'ready'; readiness: ServeReadiness }
  | { state: 'pending' }
  | { state: 'malformed'; reason: string }

/**
 * Pull the `kolux_server_ready` payload out of whatever the candidate has written so far.
 *
 ...
 */
export function parseKoluxdReadinessOutput(raw: string): KoluxdReadinessParse {
  ...
  for (const line of lines) {
    ...
    if (payload.type !== 'kolux_server_ready') {
      return {
        state: 'malformed',
        reason: `expected a kolux_server_ready line, got type=${JSON.stringify(payload.type)}`
      }
    }
