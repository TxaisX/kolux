...
vi.mock('./koluxd-local-build-hash', () => ({
  computeLocalKoluxdBuildHash: () => 'abc123def4567890'
}))

import { execCommand } from './ssh-relay-deploy-helpers'
import { acquireInstallLock } from './ssh-relay-install-lock'
import { uploadRelayDirectory, writeRelayFile } from './ssh-relay-install-transfers'
import { deployKoluxd, type KoluxdDeployOptions } from './koluxd-remote-deploy'
import { emptyKoluxdActivationRecord, withActivatedVersion } from './koluxd-activation-record'
...
function scriptHost(script: HostScript): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('koluxd-active.json')) {
      return script.activationRecord
    }
    if (text.includes('.koluxd-readiness') && text.startsWith('cat ')) {
...
function options(overrides: Partial<KoluxdDeployOptions> = {}): KoluxdDeployOptions {
  return {
    conn: {} as SshConnection,
    host: getRemoteHostPlatform('linux-x64'),
    remoteHome: '/home/u',
    localKoluxdDir: '/local/out/koluxd',
...
const ACTIVE_OLD = JSON.stringify(
  withActivatedVersion(emptyKoluxdActivationRecord(), OLD_VERSION, null, new Date(0))
)

describe('deployKoluxd', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('installs under the koluxd namespace, not the relay one', async () => {
    ...
    scriptHost(script)
    await deployKoluxd(options())
    expect(vi.mocked(acquireInstallLock).mock.calls[0][1]).toBe(
      `/home/u/.kolux-remote/koluxd-${NEW_VERSION}`
    )
    expect(vi.mocked(uploadRelayDirectory).mock.calls[0][2]).toContain(`koluxd-${NEW_VERSION}`)
  })
  ...
  it('installs but does not activate when terminals are running', async () => {
    ...
    scriptHost(script)
    const result = await deployKoluxd(
  ...
  it('does not write the activation record when the candidate fails its health gate', async () => {
    ...
    scriptHost(script)
    const result = await deployKoluxd(options())
    expect(result).toMatchObject({ code: 'koluxd_activation_daemon_degraded' })
    expect(
      vi
        .mocked(writeRelayFile)
        .mock.calls.some((call) => String(call[2]).endsWith('koluxd-active.json'))
    ).toBe(false)
  })
  ...
  it('puts the previous version back after a rejected candidate, rather than leaving the host down', async () => {
    ...
    expect(result.outcome === 'installed-not-activated' && result.reason).toContain(
      `koluxd ${OLD_VERSION} was restarted and is serving again`
    )
  })
  ...
  it('refuses to activate when a different build answered the port', async () => {
    ...
    scriptHost(script)
    const result = await deployKoluxd(options())
    expect(result).toMatchObject({ code: 'koluxd_activation_build_mismatch' })
  })
  ...
  it('refuses to treat an unreadable activation record as an empty one', async () => {
    ...
    scriptHost(script)
    await expect(deployKoluxd(options())).rejects.toThrow('activation record')
