...
export type KnownHostsEvidence = {
   ...
   * state (ssh creates known_hosts on its own first connect, and most Kolux profiles start without
 ...
 * ssh2 dials. Never the Kolux label — bastions tunnelled through `localhost:port` depend on the
...
export function resolveKnownHostsLookupHost(
  ...
  // no Host block matches, which for a manual target is the Kolux label. Keying on that consults a
