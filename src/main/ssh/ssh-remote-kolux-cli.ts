...
import {
  ...
  type RemoteKoluxCliRequest,
  type RemoteKoluxCliResult
...
export type { RemoteKoluxCliRequest, RemoteKoluxCliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'kolux serve starts a foreground headless Kolux server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Kolux.',
  'claude-teams':
    'kolux claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Kolux host machine.',
  'agent-teams-tmux':
    'kolux agent-teams-tmux is a tmux pane shim for the Kolux host machine and cannot run through the SSH relay bridge.',
  'account add':
    'kolux account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Kolux host machine.'
}

export async function runRemoteKoluxCli(
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteKoluxCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostKoluxCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteKoluxCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteKoluxCli(
  runtime: KoluxRuntimeService,
  request: RemoteKoluxCliRequest,
...
async function dispatchRemoteCli(
  ...
  runtimeAuthority: RemoteKoluxCliRequest['runtimeAuthority']
): Promise<RpcResponse> {
  const command = parsed.commandPath.join(' ')
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the Kolux host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.KOLUX_PANE_KEY || undefined
        },
        {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Kolux CLI command: ${command} (full Kolux CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
