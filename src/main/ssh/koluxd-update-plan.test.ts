...
import { assessKoluxdRollback, planKoluxdUpdate } from './koluxd-update-plan'
import {
...
describe('planKoluxdUpdate', () => {
  ...
  it('defers rather than restarting a host with live terminals', () => {
    ...
    expect(plan).toMatchObject({ action: 'defer', code: 'koluxd_update_terminals_running' })
...
describe('assessKoluxdRollback', () => {
  ...
  it('refuses when there is no previous version to go back to', () => {
    ...
    expect(safety).toMatchObject({ safety: 'unsafe', code: 'koluxd_rollback_no_target' })
  })
})
