/**
 * When an update may restart koluxd, and when going back is still sound.
 *
 * Two constraints shape everything here.
 *
 * **The daemon must outlive the restart.** koluxd forks the terminal daemon and deliberately
 * does not kill it on stop (`koluxd-daemon-supervision.ts` uses `disconnectDaemon`, never
 ...
 *
 * **The state root is shared across versions.** `~/.kolux/` (or `$KOLUX_USER_DATA`) is outside
 * every version dir, and Kolux's persisted state carries no schema version — migrations run
 ...
 */
import type { KoluxdActivationRecord } from './koluxd-activation-record'

export type KoluxdTerminalCensus = {
  /**
...
export type KoluxdUpdateDecision =
  ...
  | { action: 'defer'; code: KoluxdUpdateDeferCode; reason: string }

export type KoluxdUpdateDeferCode =
  | 'koluxd_update_terminals_running'
  | 'koluxd_update_terminal_census_unavailable'

/**
 * Decide whether to restart koluxd onto `candidateVersion`.
 *
 * Deferring on live terminals is a deliberate choice, not caution. The restart itself is
 * non-destructive, but it leaves the host running a NEW koluxd against an OLD daemon until
...
export function planKoluxdUpdate(input: {
  ...
  if (liveSessions === null) {
    if (!input.force) {
      return {
        action: 'defer',
        code: 'koluxd_update_terminal_census_unavailable',
        reason:
  ...
  if (liveSessions > 0 && !input.force) {
    return {
      action: 'defer',
      code: 'koluxd_update_terminals_running',
      reason:
        ...
        `koluxd ${input.candidateVersion} against a daemon forked from ` +
...
export type KoluxdRollbackSafety =
  | { safety: 'clean'; target: string; notes: string[] }
  | { safety: 'lossy'; target: string; discards: string[] }
  | { safety: 'unsafe'; code: KoluxdRollbackUnsafeCode; reason: string }

export type KoluxdRollbackUnsafeCode =
  | 'koluxd_rollback_no_target'
  | 'koluxd_rollback_snapshot_missing'
  | 'koluxd_rollback_orphans_live_terminals'
  | 'koluxd_rollback_census_unavailable'
 ...
 */
export function assessKoluxdRollback(input: {
  record: KoluxdActivationRecord
...
}): KoluxdRollbackSafety {
  const target = input.record.previous
  if (!target) {
    return {
      safety: 'unsafe',
      code: 'koluxd_rollback_no_target',
      reason:
        'This host has no previous koluxd version recorded, so there is nothing to roll back ' +
  ...
  if (!input.record.snapshot || !input.snapshotPresent) {
    return {
      safety: 'unsafe',
      code: 'koluxd_rollback_snapshot_missing',
      reason:
        `The pre-activation state snapshot for ${input.record.active ?? 'the active version'} ` +
        'is gone, and Kolux state carries no schema version that could prove the older build ' +
  ...
  if (startedSinceActivation === null) {
    return {
      safety: 'unsafe',
      code: 'koluxd_rollback_census_unavailable',
      reason:
  ...
  if (startedSinceActivation > 0) {
    return {
      safety: 'unsafe',
      code: 'koluxd_rollback_orphans_live_terminals',
      reason:
