...
export function readLocalFullVersion(localRelayDir: string): string {
  ...
  if (!existsSync(versionFile)) {
    throw new Error(
      `Kolux's local relay build is missing its version marker at ${versionFile}. ` +
        `This usually indicates a packaging or build problem; reinstall Kolux.`
    )
  }
  const v = readFileSync(versionFile, 'utf-8').trim()
  if (!v) {
    throw new Error(
      `Kolux's local relay version marker at ${versionFile} is empty. ` +
        `This usually indicates a packaging or build problem; reinstall Kolux.`
    )
  }
  return v
