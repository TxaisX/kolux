...
const powerShellExecutable = [
  process.env.KOLUX_POWERSHELL_EXECUTABLE,
...
describe('ssh remote command builders', () => {
  it('keeps POSIX deploy commands POSIX-native', () => {
    ...
    expect(makeRemoteDirectoryCommand(posix, '/home/me/.kolux-remote')).toContain('mkdir -p')
  ...
  function stageRelayInstall(isWindows: boolean): string {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-relay-probe-'))
  ...
  it('uses encoded PowerShell for Windows deploy commands', () => {
    ...
    expect(makeRemoteDirectoryCommand(windows, 'C:/Users/me/.kolux-remote')).toContain(
      '-EncodedCommand'
    )
  ...
  it('uses a legacy-visible Windows lock directory with an exclusive owner file', () => {
    const mkdirScript = decodePowerShellCommand(
      makeRemoteDirectoryCommand(windows, 'C:/Users/me/.kolux-remote')
    )
    const lockScript = decodePowerShellCommand(
      tryCreateInstallLockCommand(windows, 'C:/Users/me/.kolux-remote/relay/.install-lock')
    )
  ...
  it('emits an explicit POSIX liveness result so GC can fail closed', () => {
    const command = relayLivenessProbeCommand(posix, '/home/u/.kolux-remote/relay-0.1.0')
  ...
  it('uses named pipe try-connect liveness for Windows GC', () => {
    const command = relayLivenessProbeCommand(windows, 'C:/Users/me/.kolux-remote/relay-0.1.0', {
      nodePath: 'C:/Program Files/nodejs/node.exe',
      pipePaths: ['\\\\.\\pipe\\kolux-relay-1234567890abcdef1234']
    })
    ...
    expect(listRelayBaseDirsCommand(windows, 'C:/Users/me/.kolux-remote')).toContain(
      '-EncodedCommand'
    )
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-relay-gc-scale-'))
      try {
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-relay-gc-failure-'))
      try {
  ...
  it('escapes double quotes before passing JavaScript to native Windows commands', () => {
    const script = decodePowerShellCommand(
      relayLivenessProbeCommand(windows, 'C:/Users/me/.kolux-remote/relay-0.1.0', {
        nodePath: 'C:/Program Files/nodejs/node.exe',
        pipePaths: ['\\\\.\\pipe\\kolux-relay-1234567890abcdef1234']
      })
    )
  ...
  it('prepends the Windows node bin directory to PATH with native separators', () => {
    const script = decodePowerShellCommand(
      commandWithNodePath(
        ...
        'C:/Users/me/.kolux-remote/relay-0.1.0',
        "'READY'"
      )
    )
  ...
  it('computes install-lock age on the remote host clock', () => {
    const posixCommand = lockAgeSecondsCommand(posix, '/home/me/.kolux-remote/relay/.install-lock')
    const windowsScript = decodePowerShellCommand(
      lockAgeSecondsCommand(windows, 'C:/Users/me/.kolux-remote/relay/.install-lock')
    )
  ...
  it('serializes stale recovery with unbounded numbered sibling claims', () => {
    const posixCommand = tryStealInstallLockCommand(
      posix,
      '/home/me/.kolux-remote/relay/.install-lock',
      20 * 60
    )
    const windowsScript = decodePowerShellCommand(
      tryStealInstallLockCommand(windows, 'C:/Users/me/.kolux-remote/relay/.install-lock', 20 * 60)
    )
  ...
  it.runIf(powerShellExecutable)(
    ...
    () => {
      const script = decodePowerShellCommand(
        tryStealInstallLockCommand(
          windows,
          'C:/Users/kolux-missing/.kolux-remote/relay/.install-lock',
          20 * 60
        )
      )
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-windows-race-'))
      try {
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-windows-reboot-'))
      try {
        const lockPath = join(root, '.install-lock')
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-windows-stale-'))
      try {
        const lockPath = join(root, '.install-lock')
        mkdirSync(lockPath)
  ...
  it.runIf(powerShell51Executable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-windows-steal-race-'))
      try {
        const lockPath = join(root, '.install-lock')
        mkdirSync(lockPath)
        const staleDate = new Date(Date.now() - 60 * 60_000)
  ...
  it.runIf(powerShellExecutable)(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-windows-compat-'))
      try {
        const lockPath = join(root, '.install-lock')
        const acquire = decodePowerShellCommand(tryCreateInstallLockCommand(windows, lockPath))
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-'))
      try {
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-reboot-'))
      try {
        const lockDir = join(root, '.install-lock')
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-legacy-'))
      try {
        const lockDir = join(root, '.install-lock')
        mkdirSync(lockDir)
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), 'kolux-install-lock-race-'))
      try {
        const lockDir = join(root, '.install-lock')
        mkdirSync(lockDir)
        const staleDate = new Date(Date.now() - 60 * 60_000)
  ...
  it('makes Windows remote directory changes fail before running scoped commands', () => {
    const scopedCommand = decodePowerShellCommand(
      commandInRemoteDirectory(windows, 'C:/Users/me/.kolux-remote/relay-0.1.0', "'READY'")
    )
    ...
    expect(scopedCommand).toContain(
      "Set-Location -ErrorAction Stop -LiteralPath 'C:/Users/me/.kolux-remote/relay-0.1.0'"
    )
