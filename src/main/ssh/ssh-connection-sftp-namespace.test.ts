...
vi.mock('./ssh-system-fallback', () => ({
  getKoluxControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.kolux-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'kolux-relay-')))
