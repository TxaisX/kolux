...
describe('deployAndLaunchRelay', () => {
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Linux x86_64') // tagged POSIX platform probe
    mockExecCommand.mockResolvedValueOnce('/home/user') // echo $HOME
    mockExecCommand.mockResolvedValueOnce('KOLUX-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(mockExecCommand).toHaveBeenCalledWith(
      conn,
      "printf '\\n%s ' '__KOLUX_REMOTE_PLATFORM__'; uname -sm",
  ...
  it('reports progress via callback', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    mockExecCommand.mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('KOLUX-NATIVE-DEPS-OK')
    queueLaunchNamespaceAndDeadSocketProbe()
  ...
  it('uses a content-hashed versioned remote install directory', async () => {
    ...
    const sawVersionedDir = allCmds.some((cmd) =>
      cmd.includes('/.kolux-remote/relay-0.1.0+abcdef012345')
    )
    ...
    try {
      ...
      vi.mocked(execCommand).mockImplementation((_conn, command) => {
        ...
        if (command.includes('__KOLUX_UPLOAD_STAGE_SLOT__') && marker) {
          return Promise.resolve(`__KOLUX_UPLOAD_STAGE_SLOT__${marker}:slot-0`)
        }
    ...
    try {
      ...
      mockExecCommand
        ...
        .mockImplementationOnce(
          () =>
            new Promise<string>((resolve) =>
              setTimeout(() => resolve('KOLUX-NATIVE-DEPS-OK'), 899_900)
            )
        )
        .mockResolvedValueOnce('') // launch namespace marker
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
      .mockResolvedValueOnce('C:\\Users\\me user') // remote home
      .mockResolvedValueOnce('KOLUX-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\kolux-relay-[0-9a-f]{20}$/)
    ...
    expect(launchScript).toContain(
      '"C:/Users/me user/.kolux-remote/relay-0.1.0+abcdef012345/relay.js"'
    )
    expect(launchScript).toContain(
      '"C:/Users/me user/.kolux-remote/relay-0.1.0+abcdef012345/agent-hooks/kolux-relay-'
    )
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\kolux-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\kolux-relay-[0-9a-f]{20}$/)
  ...
  it('prefers a persisted Windows fallback pipe on later reconnects', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    const persistedPipe = '\\\\.\\pipe\\kolux-relay-1234567890abcdef1234'
    vi.mocked(resolveRemoteNodePath).mockResolvedValue('C:/Program Files/nodejs/node.exe')
    mockExecCommand
      .mockRejectedValueOnce(new Error('uname not found')) // tagged POSIX platform probe
      .mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
  ...
  it('scopes persisted Windows active pipe markers by relay target', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__KOLUX_REMOTE_PLATFORM__ Windows X64')
