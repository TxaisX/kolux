 ...
 *
 * `docs/design/shipping-koluxd.html` §04's state-schema row asks for "backward-readable
 * migrations or a pre-activation snapshot". Only the second is available here, and not as a
 * preference: Kolux's persisted state carries **no schema version**. Migrations are cohort
 ...
 * daemon's socket, PID record and auth token, and that daemon outlives every koluxd restart
...
import { assertPosixKoluxdHost as assertPosixHost } from './koluxd-remote-host-support'
 ...
 */
export const KOLUXD_SNAPSHOT_MEMBERS = [
  'kolux-profile-index.json',
...
export const KOLUXD_SNAPSHOT_EXCLUDED = ['daemon', 'logs'] as const

/**
 * The member names go into the command unquoted (see `captureKoluxdStateSnapshotCommand`), so
...
function assertPlainMemberName(member: string): string {
  if (!/^[A-Za-z0-9._-]+$/.test(member)) {
    throw new Error(`Unsafe koluxd snapshot member name: ${JSON.stringify(member)}`)
  }
  return member
}

export function koluxdSnapshotDirName(fullVersion: string, takenAtMs: number): string {
 ...
 */
export function captureKoluxdStateSnapshotCommand(
  ...
  const memberTests = KOLUXD_SNAPSHOT_MEMBERS.map(
...
export type KoluxdSnapshotCapture = 'captured' | 'empty' | 'failed'

export function parseKoluxdSnapshotCapture(output: string): KoluxdSnapshotCapture {
...
export function probeKoluxdStateSnapshotCommand(
  host: RemoteHostPlatform,
  snapshotDir: string
): string {
  assertPosixHost(host)
 ...
 *
 * The caller must have stopped koluxd first. This does not check — it cannot, from a shell —
 * so `koluxd-remote-deploy.ts` owns that ordering.
 */
export function restoreKoluxdStateSnapshotCommand(
  host: RemoteHostPlatform,
  userDataDir: string,
  snapshotDir: string
): string {
  ...
  const root = shellEscape(userDataDir)
  const archive = shellEscape(joinRemotePath(host, snapshotDir, 'state.tar'))
  const removals = KOLUXD_SNAPSHOT_MEMBERS.map(
...
export type KoluxdSnapshotRestore = 'restored' | 'missing' | 'failed'

export function parseKoluxdSnapshotRestore(output: string): KoluxdSnapshotRestore {
  const value = output.trim().split('\n').pop()?.trim()
  if (value === 'RESTORED') {
    return 'restored'
  }
 ...
 * caller compares; an `UNKNOWN` becomes `null`, which `assessKoluxdRollback` treats as "yes,
 * assume writes".
 */
export function newestStateMtimeCommand(host: RemoteHostPlatform, userDataDir: string): string {
  assertPosixHost(host)
  const root = shellEscape(userDataDir)
  const paths = KOLUXD_SNAPSHOT_MEMBERS.map((member) => `${root}/${shellEscape(member)}`).join(' ')
  return [
