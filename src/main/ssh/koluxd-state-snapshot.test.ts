...
import {
  KOLUXD_SNAPSHOT_EXCLUDED,
  KOLUXD_SNAPSHOT_MEMBERS,
...
const ROOT = '/home/u/.kolux'
const SNAP = '/home/u/.kolux-remote/koluxd-state-snapshots/pre-0.2.0+bb01-1000'

describe('capturing the pre-activation snapshot', () => {
  it('captures the profile state a rollback needs', () => {
    const command = captureKoluxdStateSnapshotCommand(posix, ROOT, SNAP)
    for (const member of KOLUXD_SNAPSHOT_MEMBERS) {
  ...
  it.each(KOLUXD_SNAPSHOT_EXCLUDED)('never captures %s', (excluded) => {
    expect(captureKoluxdStateSnapshotCommand(posix, ROOT, SNAP)).not.toContain(`'${excluded}'`)
  })

  it.each(KOLUXD_SNAPSHOT_EXCLUDED)('never removes or restores over %s', (excluded) => {
    expect(restoreKoluxdStateSnapshotCommand(posix, ROOT, SNAP)).not.toContain(`'${excluded}'`)
  })
  ...
  it.each([
    ...
    expect(parseKoluxdSnapshotCapture(output)).toBe(expected)
  })

  it('keys the snapshot dir on both version and time, so a retry cannot overwrite one', () => {
    expect(koluxdSnapshotDirName('0.2.0+bb01', 1000)).not.toBe(
      koluxdSnapshotDirName('0.2.0+bb01', 2000)
    )
  })
})
...
describe('restoring the snapshot', () => {
  it('clears the members before extracting, so files the new build added do not survive', () => {
    const command = restoreKoluxdStateSnapshotCommand(posix, ROOT, SNAP)
  ...
  it('reports a missing archive instead of extracting nothing and claiming success', () => {
    expect(restoreKoluxdStateSnapshotCommand(posix, ROOT, SNAP)).toContain('echo MISSING')
    expect(parseKoluxdSnapshotRestore('MISSING')).toBe('missing')
    expect(parseKoluxdSnapshotRestore('RESTORED')).toBe('restored')
    expect(parseKoluxdSnapshotRestore('FAILED')).toBe('failed')
  })
})
...
describe('Windows hosts', () => {
  it.each([
    ['capture', () => captureKoluxdStateSnapshotCommand(windows, ROOT, SNAP)],
    ['restore', () => restoreKoluxdStateSnapshotCommand(windows, ROOT, SNAP)],
    ['mtime', () => newestStateMtimeCommand(windows, ROOT)]
  ])('refuses %s rather than emitting a POSIX command', (_label, build) => {
    expect(build).toThrow('koluxd to a Windows host is not implemented')
  })
})
