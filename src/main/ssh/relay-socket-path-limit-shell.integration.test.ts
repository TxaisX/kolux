...
// Why skipIf: no /bin/sh on Windows, and POSIX mode bits (chmod 0o700) aren't meaningful there.
describe.skipIf(process.platform === 'win32')(
  'short relay socket dir guard, against a real shell',
  () => {
    let root: string
    ...
    beforeEach(async () => {
      root = await mkdtemp(join(tmpdir(), 'kolux-relay-dir-guard-'))
    })
