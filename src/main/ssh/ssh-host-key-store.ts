/**
 * Kolux's own record of accepted SSH host keys, consulted alongside the user's `known_hosts`.
 *
...
export type TrustedHostKeyRecord = {
  /** Lower-cased `HostKeyAlias` or resolved hostname — never the Kolux target label (D2 lookup key). */
  host: string
  port: number
  keyType: string
...
async function readStore(storeFile: string): Promise<StoreSnapshot> {
  ...
  // Why the version is consulted: it was written and never read, so a store from a future Kolux would
