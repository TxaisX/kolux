...
import { rollbackKoluxd, type KoluxdRollbackOptions } from './koluxd-remote-rollback'
import {
...
function record(overrides: Partial<KoluxdActivationRecord> = {}): KoluxdActivationRecord {
  return {
    ...emptyKoluxdActivationRecord(),
    active: ACTIVE,
    previous: TARGET,
...
function scriptHost(log: string[], overrides: { restore?: string } = {}): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('.koluxd-readiness')) {
...
function options(overrides: Partial<KoluxdRollbackOptions> = {}): KoluxdRollbackOptions {
  return {
...
describe('rollbackKoluxd', () => {
  ...
  it('stops, restores state, then starts the target — in that order', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackKoluxd(options())
    expect(result).toMatchObject({ outcome: 'rolled-back', target: TARGET })
    // Restoring under a running koluxd would replace the store beneath a process holding it;
  ...
  it('refuses before touching anything when terminals started after activation', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackKoluxd(
  ...
  it('does not start the old build when the restore failed', async () => {
    ...
    const result = await rollbackKoluxd(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'koluxd_rollback_restore_failed' })
  ...
  it('leaves the record naming the newer version when the target fails to come up', async () => {
    ...
    const result = await rollbackKoluxd(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'koluxd_activation_no_readiness' })
