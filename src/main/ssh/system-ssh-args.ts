...
export type SystemSshBuildArgsOptions = {
  ...
  suppressKoluxControlMaster?: boolean
  gssapiOnly?: boolean
...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to Kolux's existing ssh2 prompt path.
  ...
  const controlPath = getKoluxControlSocketPath(target, options)
  ...
  // as the wildcard's user, at the wildcard's host, discarding the endpoint Kolux stored. Keep the
...
export function getKoluxControlSocketPath(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): string | null {
  if (shouldDisableKoluxControlMaster(target, options)) {
    return null
  }
...
export function getSystemSshBuildArgsFromOperationOptions(
  ...
  if (options?.suppressKoluxControlMaster === true) {
    buildArgsOptions.suppressKoluxControlMaster = true
  }
...
function shouldDisableKoluxControlMaster(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): boolean {
  // Why: unresolved ssh_config aliases could otherwise share one Kolux socket
  ...
  return (
    options?.disableControlMaster === true ||
    options?.suppressKoluxControlMaster === true ||
    target.systemSshConnectionReuse === false ||
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master Kolux needs.
  return (
