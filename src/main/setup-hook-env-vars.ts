...
// Why: kept in sync with pty/wsl-kolux-env.ts, which path-translates the same keys for WSL.
// KOLUX_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
...
export function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    KOLUX_ROOT_PATH: repo.path,
    KOLUX_WORKTREE_PATH: worktreePath,
    KOLUX_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
