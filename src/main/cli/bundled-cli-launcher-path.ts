...
// Why `kolux-ide` on Linux: GNOME Orca ships /usr/bin/orca, so the CLI never claims that name.
export const LINUX_CLI_COMMAND_NAME = 'kolux-ide'
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin') {
    return join(resourcesPath, 'bin', 'kolux')
  }
  if (platform === 'linux') {
    return join(resourcesPath, 'bin', LINUX_CLI_COMMAND_NAME)
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'kolux.exe')
  }
  return null
}
