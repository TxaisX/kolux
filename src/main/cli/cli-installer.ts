...
export class CliInstaller extends CliPathRegistration {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (!launcherPath) {
      const detail = this.hasUnverifiedAppImageRuntime
        ? 'Kolux could not verify the inherited AppImage runtime identity, so CLI registration is unavailable.'
    ...
    const baseStatus =
      ...
      !isAppImageInstalledLauncherCurrent(extractionOptions)
        ? {
            ...
            detail: `${spec.commandPath} does not point to the current Kolux AppImage payload.`
          }
        : inspectedStatus
  ...
  private async installUnlocked(): Promise<CliInstallStatus> {
    ...
    if (initialStatus.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kolux command at ${initialStatus.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kolux command at ${status.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.installMethod === 'symlink') {
      ...
      // Why: packaged Windows already ships resources/bin/kolux.exe; registration only owns the PATH entry.
    } else {
  ...
  private async removeUnlocked(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Kolux command at ${status.commandPath}.`)
    }
    if (status.state === 'stale' && status.installMethod !== 'symlink') {
      throw new Error(`Refusing to remove a command not owned by Kolux at ${status.commandPath}.`)
    }
