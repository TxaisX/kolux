...
import { buildBareKoluxCliScript } from './linux-bare-kolux-dispatcher'
import { quoteShell } from './cli-install-path-format'

const SHIM_DIR_NAME = 'linux-kolux-cli-shim'

export type LinuxTerminalKoluxCliShimOptions = {
  userDataPath: string
...
// Why: on Linux the CLI installs as `kolux-ide` so it never shadows the GNOME
...
export function ensureLinuxTerminalKoluxCliShimDir(
  options: LinuxTerminalKoluxCliShimOptions
): string | null {
  const resourcesPath =
...
fail() {
  printf 'Kolux CLI is unavailable; reopen Kolux or register the CLI again.\\n' >&2
  exit 1
}
...
function ensureShimForLauncher(userDataPath: string, launcherPath: string): string | null {
  const script = buildBareKoluxCliScript(launcherPath)
