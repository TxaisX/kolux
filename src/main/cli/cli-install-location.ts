...
export abstract class CliInstallLocation {
  ...
  protected get commandName(): string {
    ...
    // Why: packaged Linux uses `kolux-ide` to avoid shadowing GNOME Orca's /usr/bin/orca.
    return this.platform === 'linux' ? LINUX_CLI_COMMAND_NAME : 'kolux'
  }
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.KOLUX_CLI_INSTALL_PATH ?? null
    ...
    this.macCommandPath = existsSync(dirname(candidateMacPath))
      ? candidateMacPath
      : join(this.homePath, '.local', 'bin', 'kolux')
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-Kolux command after an empty default slot can be shadowed by installing there; no user file replaced.
          continue
        }
  ...
  protected resolveCommandPath(): string | null {
    ...
    if (this.platform === 'linux') {
      ...
      // Why `kolux-ide`: GNOME Orca ships /usr/bin/orca, so avoid shadowing that screen reader.
