...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'kolux-terminal-cli-shim-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled kolux-ide launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'kolux-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalKoluxCliShimDir', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const shimDir = ensureLinuxTerminalKoluxCliShimDir({ userDataPath, resourcesPath })

    expect(shimDir).toBe(join(userDataPath, 'linux-kolux-cli-shim'))
    expect(readFileSync(join(shimDir!, 'kolux'), 'utf8')).toContain(
      `exec '${join(resourcesPath, 'bin', 'kolux-ide')}' "$@"`
    )
  })
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      const { userDataPath, resourcesPath } = await makeFixture()
      const appImagePath = join(userDataPath, 'Kolux.AppImage')
      ...
      const shimPath = join(shimDir!, 'kolux')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const firstLauncher = join(resourcesPath, 'bin', 'kolux-ide')
      ...
      const shimDir = ensureLinuxTerminalKoluxCliShimDir(options)
      const shimPath = join(shimDir!, 'kolux')
      const originalShim = readFileSync(shimPath, 'utf8')

      const nextResourcesPath = join(userDataPath, 'next-mount', 'resources')
      const nextLauncher = join(nextResourcesPath, 'bin', 'kolux-ide')
      ...
      expect(
        ensureLinuxTerminalKoluxCliShimDir({ ...options, resourcesPath: nextResourcesPath })
      ).toBe(shimDir)
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { mode: 0o755 })
      const liveLauncher = join(resourcesPath, 'bin', 'kolux-ide')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-terminal-cli-shim-missing-'))
    created.push(root)
    const userDataPath = join(root, 'user-data')

    const missing = ensureLinuxTerminalKoluxCliShimDir({
      userDataPath,
    ...
    mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
    writeFileSync(join(resourcesPath, 'bin', 'kolux-ide'), '#!/usr/bin/env bash\n', 'utf8')
    const recovered = ensureLinuxTerminalKoluxCliShimDir({
      userDataPath,
      resourcesPath,
      appImagePath: null
    })
    expect(recovered).toBe(join(userDataPath, 'linux-kolux-cli-shim'))
  })
})
