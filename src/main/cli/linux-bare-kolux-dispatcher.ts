...
// our own file idempotently but never clobber a user's own ~/.local/bin/kolux.
const DISPATCHER_MARKER = '# kolux-serve-bare-kolux-dispatcher'

export type LinuxBareKoluxDispatcherOptions = {
  /** Packaged app resources root; the bundled `kolux-ide` launcher lives under it. */
  ...
  /** Test seam — defaults to $XDG_CACHE_HOME/kolux/appimage. */
...
export type LinuxBareKoluxDispatcherState =
  | 'installed'
  | 'skipped-foreign'
  | 'skipped-launcher-missing'

export type LinuxBareKoluxDispatcherResult = {
  state: LinuxBareKoluxDispatcherState
  dispatcherPath: string
  /** The bundled `kolux-ide` launcher the dispatcher execs. */
  target: string | null
}

// Why: on Linux the CLI installs as `kolux-ide`, not bare `kolux`, to avoid
// shadowing GNOME Orca's /usr/bin/orca. But the Claude Team launcher typed into
// the initial managed terminal invokes the literal `kolux claude-teams`, so a
// headless serve box needs a bare-`kolux` dispatcher on the managed-terminal PATH
...
// never reclaims it.
export async function installLinuxBareKoluxDispatcher(
  options: LinuxBareKoluxDispatcherOptions
): Promise<LinuxBareKoluxDispatcherResult> {
  const dispatcherPath = join(options.homePath ?? homedir(), '.local', 'bin', 'kolux')
  ...
  const installed = await publishDispatcher(
    dispatcherPath,
    insertDispatcherMarker(buildBareKoluxCliScript(launcher))
  )
  return installed
...
/** Bare-`kolux` script that execs the one Linux CLI launcher. */
export function buildBareKoluxCliScript(launcherPath: string): string {
...
async function publishDispatcher(dispatcherPath: string, content: string): Promise<boolean> {
  ...
  const temporaryPath = join(directoryPath, `.kolux-dispatcher-${process.pid}-${randomUUID()}`)
  ...
  try {
    ...
    const displacedPath = join(
      directoryPath,
      `.kolux-preserved-dispatcher-${process.pid}-${randomUUID()}`
    )
    try {
