...
const detected: DetectedAutomationEvent = {
  event: { kind: 'review_opened', key: 'review_opened:github:local:/repo:3', summary: 'PR #3 opened' },
  repoId: 'r1'
}

function fakeStore(automations: Automation[], runsByAutomation: Record<string, AutomationRun[]> = {}) {
  return {
...
describe('automationMatchesEvent', () => {
  ...
  it('does not match a different repo', () => {
    expect(automationMatchesEvent(makeAutomation({ projectId: 'other-repo' }), detected)).toBe(false)
  })

  it('does not match a scheduled automation (no eventTrigger)', () => {
    expect(automationMatchesEvent(makeAutomation({ eventTrigger: undefined }), detected)).toBe(false)
  })
})
...
describe('automationEventInCooldown', () => {
  it('is true within the cooldown window of the latest event run', () => {
    ...
    expect(automationEventInCooldown(store, 'a1', 1000 + AUTOMATION_EVENT_COOLDOWN_MS - 1)).toBe(true)
  })
