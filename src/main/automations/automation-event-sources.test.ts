...
describe('agentDoneEvent', () => {
  function makePayload(overrides: Partial<EnrichedAgentHookEventPayload> = {}): EnrichedAgentHookEventPayload {
    return {
  ...
  it('fires for a plain done state with a worktree', () => {
    expect(agentDoneEvent(makePayload())).toEqual(
      expect.objectContaining({ kind: 'agent_done', key: 'agent_done:tab-1:11111111-1111-4111-8111-111111111111:1' })
    )
  })
  ...
  it('ignores an interrupted done', () => {
    expect(agentDoneEvent(makePayload({ payload: { state: 'done', interrupted: true } } as never))).toBeNull()
  })
...
describe('wireAutomationEventSources', () => {
  ...
  it('fires once a later poll transitions null to open', () => {
    ...
    expect(handleAutomationEvent).toHaveBeenCalledWith(
      expect.objectContaining({ repoId: 'r1', event: expect.objectContaining({ kind: 'review_opened' }) })
    )
  })
  ...
  it('routes agent_done through getRepoIdFromWorktreeId', () => {
    ...
    expect(handleAutomationEvent).toHaveBeenCalledWith(
      expect.objectContaining({ repoId: 'r1', event: expect.objectContaining({ kind: 'agent_done' }) })
    )
  })
})
