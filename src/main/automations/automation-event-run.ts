...
import { automationSchedulableOnThisHost, type AutomationRunTargetResult } from './run-target-resolution'
...
export function automationMatchesEvent(automation: Automation, detected: DetectedAutomationEvent): boolean {
  return (
    automation.enabled &&
...
export function automationEventAlreadyRun(store: Store, automationId: string, key: string): boolean {
...
export async function runAutomationEvent(ctx: AutomationEventRunContext): Promise<void> {
  for (const automation of ctx.store.listAutomations()) {
    ...
    if (!automationSchedulableOnThisHost(automation, { allowRemoteHostScheduling: ctx.allowRemoteHostScheduling })) {
      continue
    }
