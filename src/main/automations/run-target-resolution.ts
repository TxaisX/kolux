...
const CAPTURED_HOST_REFUSALS: Record<AutomationCapturedHostIssue, string> = {
  ...
  [AUTOMATION_ORPHAN_ISSUES.workspaceHostAmbiguous]:
    'The automation workspace spans more than one host, so Kolux cannot tell which one to run it on.'
}
...
export function automationSchedulableOnThisHost(
  ...
  return (
    ...
    (options.allowRemoteHostScheduling === true && automation.schedulerOwner === 'remote_host_service')
  )
}
