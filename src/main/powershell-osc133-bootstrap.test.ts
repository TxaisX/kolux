...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('KOLUX_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:KOLUX_OMP_STATUS_EXTENSION')
    expect(script).toContain('KOLUX_CODEX_HOME')
    expect(script).toContain('KOLUX_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__KoluxCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:KOLUX_CODEX_HOME)')
