export function getStatusPluginFactorySource(options: { emitSessionStart: boolean }): string[] {
  return [
    ...
    '// of destructuring (`async ({ client }) => …`). MiMo Code can invoke the',
    '// plugin factory with undefined during startup, which makes the',
    '// destructuring form throw synchronously and crash MiMo Code with an opaque',
    '// UnknownError before any event is ever dispatched.',
    'export const KoluxMimoCodeStatusPlugin = async (_ctx) => {',
    ...
    '    // MiMo Code fires message.updated (user) and message.part.updated (text)',
    ...
    '      // Why: MiMo Code injects a finished background task back into the',
    '      // parent turn as a synthetic `<task id=…>` text part. It is machinery,',
    '      // not what the human typed or the agent replied — MiMo Code hides it',
    ...
    '};',
    '',
    '// Why: MiMo Code also resolves plugins through the module default export, and that',
    ...
    'export default {',
    '  id: "kolux-mimo-code-status",',
    '  server: KoluxMimoCodeStatusPlugin,',
    '};',
    ''
  ]
}
