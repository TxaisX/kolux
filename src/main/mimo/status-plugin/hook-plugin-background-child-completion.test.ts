/**
 * Executes the generated MiMo Code plugin artifact to verify that a running
...
describe('MiMo Code plugin background child completion', () => {
  let tempDir: string
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kolux-mimo-code-background-child-'))
    posts = []
    savedEnv = {}
  ...
  async function createHooks(
    ...
    if (!pluginFactory) {
      ...
      const module = (await import(pathToFileURL(pluginPath).href)) as {
        KoluxMimoCodeStatusPlugin: PluginFactory
      }
      pluginFactory = module.KoluxMimoCodeStatusPlugin
    }
  ...
  it('ignores MiMo Code synthetic task-result parts injected into the parent turn', async () => {
    const hooks = await createHooks(TASK_TREE)
