export function getStatusPluginPostSource(hookPathname: string): string[] {
  return [
    ...
    '  // rewritten by a newer Kolux since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the MiMo Code process), not per-Kolux-instance.',
    ...
    '    return response.ok;',
    '  } catch {',
    '    // Why: MiMo Code session events must never fail the agent run just',
    '    // because Kolux is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
