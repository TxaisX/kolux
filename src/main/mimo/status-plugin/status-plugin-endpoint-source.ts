export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// endpoint file does not spam MiMo Code's stderr once per hook post.",
    '// This guard lives inside the plugin source because the plugin runs in',
    "// MiMo Code's Node process (not Kolux's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Kolux restart',
    ...
    '      // writeEndpointFile on the Kolux side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future KOLUX_AGENT_HOOK_*',
    ...
    '      console.warn("[kolux-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when MiMo Code was fork()ed — stale after a Kolux restart. The',
    '  // file is rewritten on every Kolux start(), so sourcing it per post lets',
    '  // a long-running MiMo Code session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Kolux',
