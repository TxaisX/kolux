...
/**
 * MiMo Code loads a plugin file either through a named factory export or through the
 * module default export. The default-export loader rejects the module outright unless
 * the default is an object exposing `server()` — verified against MiMo Code's plugin
 * loader, which logs `failed to load plugin … must default export an object with server()` for
 ...
 */
describe('MiMo Code status plugin module contract', () => {
  type PluginHooks = {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kolux-mimo-code-plugin-contract-'))
  ...
  async function loadPluginModule(): Promise<PluginModule> {
    ...
    const pluginPath = join(
      tempDir,
      `kolux-mimo-code-status-${Math.random().toString(36).slice(2)}.mjs`
    )
    writeFileSync(
      pluginPath,
  ...
  it('exposes a default export carrying a string id and a callable server()', async () => {
    ...
    expect(module.default?.id).toBe('kolux-mimo-code-status')
    expect(module.default?.server).toBeTypeOf('function')
  })

  it('rejects the shape MiMo Code refuses: a default export without server()', async () => {
    const module = await loadPluginModule()
  ...
  it('keeps the named factory export so the factory-based loader still resolves', async () => {
    ...
    expect(module.KoluxMimoCodeStatusPlugin).toBeTypeOf('function')
  })
  ...
  it('returns an event handler from the default export server(), like the named factory', async () => {
    ...
    const fromNamed = await module.KoluxMimoCodeStatusPlugin?.({})
  ...
  it('reports a session lifecycle event through the hook endpoint when driven via the default export', async () => {
    ...
    const hookPosts = posts.filter((post) => post.url.includes('/hook/mimo-code'))
