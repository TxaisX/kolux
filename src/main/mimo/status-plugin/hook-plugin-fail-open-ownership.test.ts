/**
 * Executes the generated MiMo Code plugin source because fail-open ownership
...
describe('MiMo Code plugin fail-open ownership', () => {
  let tempDir: string
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kolux-mimo-code-fail-open-plugin-'))
    posts = []
    savedEnv = {}
  ...
  async function loadHooksWithContext(context: unknown): Promise<PluginHooks> {
    ...
    return module.KoluxMimoCodeStatusPlugin(context)
  }
