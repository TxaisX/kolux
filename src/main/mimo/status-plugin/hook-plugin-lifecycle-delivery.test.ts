/**
 * Executes the generated MiMo Code plugin source because this delivery state
 * lives inside MiMo Code's process, not in Kolux's TypeScript runtime.
 */
...
import { _internals } from './mimo-code-plugin-source'
...
describe('MiMo Code plugin lifecycle delivery', () => {
  let tempDir: string
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kolux-mimo-code-lifecycle-plugin-'))
    posts = []
    savedEnv = {}
  ...
  async function loadHooksWithSession(session: object): Promise<PluginHooks> {
    const pluginPath = join(tempDir, 'kolux-mimo-code-status.mjs')
    writeFileSync(
      pluginPath,
      _internals.getMimoCodePluginSource('/hook/mimo-code', { emitSessionStart: false })
    )
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      KoluxMimoCodeStatusPlugin: (ctx: unknown) => Promise<PluginHooks>
    }
    return module.KoluxMimoCodeStatusPlugin({ client: { session } })
  }
  ...
  it('maps only root session.created to SessionStart', async () => {
    // Why: session-start emission is a shared-factory option MiMo Code's own hook
    // service always passes as false; exercise it directly against the generator.
    const pluginPath = join(tempDir, 'kolux-mimo-code-status-session-start.mjs')
    writeFileSync(
      pluginPath,
      _internals.getMimoCodePluginSource('/hook/mimo-code', { emitSessionStart: true })
    )
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      KoluxMimoCodeStatusPlugin: (ctx: unknown) => Promise<PluginHooks>
    }
    const hooks = await module.KoluxMimoCodeStatusPlugin({
  ...
  it('falls back per coordinate when the endpoint file is partial or malformed', async () => {
    ...
    expect(String(url)).toBe('http://127.0.0.1:45678/hook/mimo-code')
  ...
  it('preserves FIFO lifecycle order while the first session lookup is delayed', async () => {
    ...
    try {
      // Why: MiMo Code does not await event hooks, so the plugin must prevent
  ...
  it('lets Idle supersede a pending Busy retry', async () => {
    ...
    // Why: a stale retry must not resurrect Working after MiMo Code is done.
    await vi.advanceTimersByTimeAsync(60_000)
    expect(names()).toEqual(['SessionBusy', 'SessionIdle'])
  })
  ...
  it('uses a current nonempty text delta to retry a dirty Busy transition immediately', async () => {
    ...
    // Why: current MiMo Code streams deltas after an empty text-start part; the
  ...
  it('keeps backoff across MiMo Code canonical and deprecated Idle duplicates', async () => {
    vi.useFakeTimers()
    let idleAttempts = 0
    globalThis.fetch = vi.fn(async (_url: RequestInfo | URL, init?: RequestInit) => {
      const payload = readPayload(init)
  ...
  it('cancels lifecycle retries when MiMo Code disposes the plugin', async () => {
    vi.useFakeTimers()
    globalThis.fetch = vi.fn(async (_url: RequestInfo | URL, init?: RequestInit) => {
      posts.push(readPayload(init))
      return new Response(null, { status: 503 })
  ...
  it('lets MiMo Code duplicate Idle supersede a later-delivered MessagePart', async () => {
    const hooks = await loadHooks()
    await hooks.event({ event: status('busy') })
    await hooks.event({ event: status('idle') })
    await hooks.event({
  ...
  it('reasserts Busy through the refreshed endpoint after a live text delta', async () => {
    ...
    expect(deliveries).toEqual([
      { url: 'http://127.0.0.1:45678/hook/mimo-code', token: 'test-token' },
      { url: 'http://127.0.0.1:56789/hook/mimo-code', token: 'refreshed-token' }
    ])
  })
  ...
  it('clears unanswered question attention on authoritative root Idle', async () => {
    ...
    // MiMo Code can finish the question tool without question.replied.
    await handler({ event: status('idle') })
    expect(names().at(-1)).toBe('SessionIdle')
  })
})
