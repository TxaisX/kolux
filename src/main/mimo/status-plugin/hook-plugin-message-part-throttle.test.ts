/**
 * Executes the generated MiMo Code plugin source (the artifact that runs inside
 * MiMo Code's process) to verify streamed message.part.updated events are
...
describe('MiMo Code plugin MessagePart throttling', () => {
  let tempDir: string
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kolux-mimo-code-plugin-test-'))
    posts = []
    savedEnv = {}
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    ...
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      KoluxMimoCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.KoluxMimoCodeStatusPlugin({ client })
    return hooks.event
  }
  ...
  it('coalesces a streamed reply into leading + trailing posts with capped text', async () => {
    ...
    // accumulated text so far (how MiMo Code actually publishes parts).
    let text = ''
    for (let i = 0; i < 50; i++) {
