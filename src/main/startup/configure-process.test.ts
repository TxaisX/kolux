...
describe('configureDevUserDataPath', () => {
  it('forces Electron home into the disposable E2E profile', async () => {
    ...
    const originalE2EUserDataDir = process.env.KOLUX_E2E_USER_DATA_DIR
    const originalE2EHomeDir = process.env.KOLUX_E2E_HOME_DIR
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const tempRoot = mkdtempSync(join(tmpdir(), 'kolux-configure-e2e-home-'))
    const e2eRoot = join(tempRoot, 'user-data')
    const e2eHome = join(tempRoot, 'home')
    process.env.KOLUX_E2E_USER_DATA_DIR = e2eRoot
    process.env.KOLUX_E2E_HOME_DIR = e2eHome
    ...
    try {
      ...
      if (originalE2EUserDataDir === undefined) {
        delete process.env.KOLUX_E2E_USER_DATA_DIR
      } else {
        process.env.KOLUX_E2E_USER_DATA_DIR = originalE2EUserDataDir
      }
      if (originalE2EHomeDir === undefined) {
        delete process.env.KOLUX_E2E_HOME_DIR
      } else {
        process.env.KOLUX_E2E_HOME_DIR = originalE2EHomeDir
      }
      if (originalHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('rejects an E2E launch whose Node home escaped the disposable profile', async () => {
    ...
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const e2eRoot = mkdtempSync(join(tmpdir(), 'kolux-configure-e2e-escape-'))
    process.env.KOLUX_E2E_USER_DATA_DIR = e2eRoot
    process.env.KOLUX_E2E_HOME_DIR = join(e2eRoot, 'home')
    ...
    try {
      ...
      restoreEnv('KOLUX_E2E_USER_DATA_DIR', originalE2EUserDataDir)
      restoreEnv('KOLUX_E2E_HOME_DIR', originalE2EHomeDir)
  ...
  it('uses an explicit dev userData override when provided', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    const originalOverride = process.env.KOLUX_DEV_USER_DATA_PATH
    process.env.KOLUX_DEV_USER_DATA_PATH = '/tmp/kolux-dev-repro'

    try {
      configureDevUserDataPath(true)
    } finally {
      if (originalOverride === undefined) {
        delete process.env.KOLUX_DEV_USER_DATA_PATH
      } else {
        process.env.KOLUX_DEV_USER_DATA_PATH = originalOverride
      }
    }

    expect(app.setPath).toHaveBeenCalledWith('userData', '/tmp/kolux-dev-repro')
  })

  it('moves dev runs onto a kolux-dev userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'kolux-dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'kolux-dev'))
  })
...
describe('configureKoluxUserDataPathEnv', () => {
  it('overwrites stale inherited KOLUX_USER_DATA_PATH with Electron userData', async () => {
    const { app } = await import('electron')
    const { configureKoluxUserDataPathEnv } = await import('./configure-process')
    const originalUserDataPath = process.env.KOLUX_USER_DATA_PATH
    process.env.KOLUX_USER_DATA_PATH = '/tmp/stale-kolux-user-data'
    app.setPath('userData', '/tmp/current-kolux-user-data')
    let configuredUserDataPath: string | undefined

    try {
      configureKoluxUserDataPathEnv()
      configuredUserDataPath = process.env.KOLUX_USER_DATA_PATH
    } finally {
      if (originalUserDataPath === undefined) {
        delete process.env.KOLUX_USER_DATA_PATH
      } else {
        process.env.KOLUX_USER_DATA_PATH = originalUserDataPath
      }
    }

    expect(configuredUserDataPath).toBe('/tmp/current-kolux-user-data')
  })
})
...
describe('configureElectronNetworkCompatibility', () => {
  ...
  const originalEnvValue = process.env.KOLUX_DISABLE_HTTP2

  function createUserDataDir(settings: Record<string, unknown>): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-http1-compat-'))
    tempDirs.push(userDataPath)
    writeFileSync(join(userDataPath, 'kolux-data.json'), JSON.stringify({ settings }), 'utf-8')
    return userDataPath
  }
  ...
  afterEach(() => {
    ...
    if (originalEnvValue === undefined) {
      delete process.env.KOLUX_DISABLE_HTTP2
    } else {
      process.env.KOLUX_DISABLE_HTTP2 = originalEnvValue
    }
  })
  ...
  it('answers from the marker without reading the settings file', async () => {
    ...
    rmSync(join(userDataPath, 'kolux-data.json'), { force: true })
...
describe('enableMainProcessGpuFeatures', () => {
  ...
  it('disables the GPU process for Linux E2E runs', async () => {
    ...
    setPlatform('linux')
    process.env.KOLUX_E2E_USER_DATA_DIR = '/tmp/kolux-e2e'
    vi.mocked(app.disableHardwareAcceleration).mockClear()
    vi.mocked(app.commandLine.appendSwitch).mockClear()
