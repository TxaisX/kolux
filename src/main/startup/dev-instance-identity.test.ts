...
describe('dev-instance-identity', () => {
  it('keeps packaged identity stable', () => {
    expect(getDevInstanceIdentity(false, {})).toMatchObject({
      ...
      devLabel: null,
      dockBadgeLabel: null,
      appUserModelId: 'com.txais.kolux'
    })
  })

  it('pins a stable dev appName across branches so the safeStorage key does not churn', () => {
    const a = getDevInstanceIdentity(true, { KOLUX_DEV_BRANCH: 'feature/a' })
    const b = getDevInstanceIdentity(true, { KOLUX_DEV_BRANCH: 'feature/b' })

    // Per-branch label differs (window title / app menu)...
    expect(a.name).not.toBe(b.name)
    // ...but the Keychain-driving appName is identical and distinct from prod.
    expect(a.appName).toBe('Kolux Dev')
    expect(b.appName).toBe('Kolux Dev')
    expect(a.appName).not.toBe('Kolux')
  })
  ...
  it('never renames a packaged build before ready', () => {
    ...
    // a pre-ready rename would repoint forks ("Kolux ALab Edition") at Kolux's key.
  ...
  it('derives a readable dev label from worktree and branch env', () => {
    const identity = getDevInstanceIdentity(true, {
      KOLUX_DEV_REPO_ROOT: '/repo/worktrees/dev-indicator',
      KOLUX_DEV_WORKTREE_NAME: 'dev-indicator',
      KOLUX_DEV_BRANCH: 'nwparker/dev-indicator'
    })
    ...
    expect(identity.name).toBe('Kolux: nwparker/dev-indicator')
    expect(identity.dockBadgeLabel).toBeNull()
    expect(identity.appUserModelId).toMatch(/^com\.txais\.kolux\.dev\.[a-f0-9]{10}$/)
  })

  it('includes the branch when it differs from the worktree basename', () => {
    const identity = getDevInstanceIdentity(true, {
      KOLUX_DEV_REPO_ROOT: '/repo/worktrees/payment-ui',
      KOLUX_DEV_WORKTREE_NAME: 'payment-ui',
      KOLUX_DEV_BRANCH: 'feature/billing-shell'
    })

    expect(identity.devLabel).toBe('payment-ui @ feature/billing-shell')
    expect(identity.name).toBe('Kolux: feature/billing-shell')
    expect(identity.dockBadgeLabel).toBeNull()
  })

  it('allows an explicit label override', () => {
    const identity = getDevInstanceIdentity(true, {
      KOLUX_DEV_INSTANCE_LABEL: 'manual label',
      KOLUX_DEV_WORKTREE_NAME: 'dev-indicator',
      KOLUX_DEV_BRANCH: 'feature/other'
    })

    expect(identity.devLabel).toBe('manual label')
    expect(identity.name).toBe('Kolux: feature/other')
    expect(identity.dockBadgeLabel).toBeNull()
  })
})
