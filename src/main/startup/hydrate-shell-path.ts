...
// Tools installed into ~/.cargo/bin, pyenv/volta/fnm
// shims, and countless other user-local locations end up invisible to our
// `which` probe even though they work fine from Terminal (see TxaisX/kolux#829).
//
// Probe the profile-loading shell once instead of hard-coding every tool's install path.

const DELIMITER = '__KOLUX_SHELL_PATH__'
...
const PROBE_MARKER_ENV_VAR = 'KOLUX_SHELL_PATH_PROBE'
...
/** Run a synchronous launcher without passing Kolux's seeded PATH to its child. */
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, probe.args, {
      ...
      // and we don't want that in Kolux's console.
      env: shellProbeEnv(),
