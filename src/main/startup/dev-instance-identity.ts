...
const BASE_APP_NAME = 'Kolux'
const BASE_APP_USER_MODEL_ID = 'com.txais.kolux'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Kolux'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Kolux ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.KOLUX_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.KOLUX_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.KOLUX_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.KOLUX_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.KOLUX_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Kolux Dev Safe Storage') for all dev
