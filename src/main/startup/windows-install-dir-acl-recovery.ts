  ...
  const started = repairWindowsInstallDirPackageAcl({
    ...
    onDone: (result) => {
      ...
      if (result.mode === 'repaired' && !claimDisproved) {
        ...
        // Never a user-confirmed marker: "keep safe graphics" is a choice, not Kolux's latch.
...
const CAUSE =
  "Windows permissions on Kolux's install folder are blocking its own sandboxed processes from reading the files it shipped with."
...
export function describeInstallDirAclPoison(): InstallDirAclPoisonDiagnosis | null {
  ...
  if (poison.stage === 'repaired') {
    return {
      detail: `${CAUSE}\n\nKolux repaired the permissions. Reload to use them.`,
      commands
    }
  }
  const status =
    poison.stage === 'pending'
      ? 'Kolux is repairing the permissions now.'
      : 'Kolux could not repair them, which usually means the folder needs an administrator.'
  return {
    detail: `${CAUSE} ${status}\n\nRun these in an Administrator Command Prompt, then relaunch Kolux:\n\n${commands.join('\n')}`,
    commands
  }
}
