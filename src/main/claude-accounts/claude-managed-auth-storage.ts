...
export class ClaudeManagedAuthStorage {
  async create(
    ...
    writeFileSync(join(managedAuthPath, '.kolux-managed-claude-auth'), `${accountId}\n`, {
      encoding: 'utf-8',
      mode: 0o600
    })
    return {
  ...
  async assertOwned(candidatePath: string, expectedAccountId?: string): Promise<string> {
    ...
    if (!trustedPath) {
      throw new Error('Managed Claude auth storage is not owned by Kolux.')
    }
    return trustedPath
  }
  ...
  private async tryCreateWsl(
    ...
    const linuxPath = `${home.replace(/\/$/, '')}/.local/share/kolux/claude-accounts/${accountId}/auth`
    const created = await runWslProcess({
      distro,
      loginPath: 'none',
      shell: 'bash',
      script: 'umask 077; mkdir -p "$1" && printf \'%s\\n\' "$2" > "$1/.kolux-managed-claude-auth"',
  ...
  private async assertOwnedWsl(
    ...
    if (
      !wslInfo.linuxPath.includes('/.local/share/kolux/claude-accounts/') ||
      !wslInfo.linuxPath.endsWith('/auth')
    ) {
      throw new Error('Managed WSL Claude auth storage is outside Kolux account storage.')
    }
    if (process.platform !== 'win32') {
      if (
        !existsSync(candidatePath) ||
        !existsSync(join(candidatePath, '.kolux-managed-claude-auth'))
      ) {
        throw new Error('Managed Claude auth storage is not owned by Kolux.')
      }
    ...
    try {
      const expected = expectedAccountId
        ? `test "$(cat "$candidate_real/.kolux-managed-claude-auth")" = ${shellQuote(expectedAccountId)}`
        : 'test -n "$(cat "$candidate_real/.kolux-managed-claude-auth")"'
      const owned = await runWslProcess({
        ...
        script: [
          ...
          'managed_root="${HOME%/}/.local/share/kolux/claude-accounts"',
          'candidate_real=$(readlink -f -- "$candidate")',
          'managed_root_real=$(readlink -f -- "$managed_root")',
          'test -f "$candidate_real/.kolux-managed-claude-auth"',
          expected,
