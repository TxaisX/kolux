...
const repository = { owner: 'acme', repo: 'kolux', host: 'github.com' }
const parentReview = { number: 41, url: 'https://github.com/acme/kolux/pull/41' }
const currentReview = { number: 42, url: 'https://github.com/acme/kolux/pull/42' }

function pullRequest(number: number, head: string, base: string) {
  return {
    number,
    html_url: `https://github.com/acme/kolux/pull/${number}`,
    head: { ref: head },
    base: { ref: base }
  }
}
...
describe('prepareGitHubStackedPullRequest', () => {
  it('resolves an open parent PR and an existing current PR', async () => {
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual([
      'api',
      'repos/acme/kolux/pulls?head=acme%3Astack%2Fparent&state=open&per_page=2'
    ])
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual([
      'api',
      'repos/acme/kolux/pulls?head=acme%3Astack%2Fchild&base=stack%2Fparent&state=open&per_page=2'
    ])
  })
...
describe('registerGitHubStackedPullRequest', () => {
  ...
  it('appends the current PR when the parent is the existing top', async () => {
    ...
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual([
      ...
      '-X',
      'POST',
      'repos/acme/kolux/stacks/50/add',
      '-F',
      'pull_requests[]=42'
    ])
