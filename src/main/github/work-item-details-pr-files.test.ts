...
describe('getWorkItemDetails PR file listing', () => {
  ...
  it('backfills the Enterprise origin host before a host-less PR detail fan-out', async () => {
    ...
    getWorkItemMock.mockResolvedValueOnce({
      ...
      author: '',
      prRepo: { owner: 'team', repo: 'kolux' }
    })
    getOwnerRepoMock.mockResolvedValue(null)
    getEnterpriseGitHubRepoSlugMock.mockResolvedValue(enterprise)
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/')) ?? ''
      if (endpoint === 'repos/team/kolux/pulls/8') {
        return { stdout: JSON.stringify({ body: 'Enterprise body' }) }
      }
      if (endpoint === 'repos/team/kolux/pulls/8/files?per_page=100') {
        return { stdout: '[]' }
      }
      return auxiliaryPRResponse(args)
    })
  ...
  it('uses the selected upstream GitHub Enterprise repo for both PR file sides', async () => {
    ...
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/')) ?? ''
      if (endpoint === 'repos/team/kolux/contents/src/path%23with%3Fchars.ts?ref=base-sha') {
        return { stdout: 'base content' }
      }
      if (endpoint === 'repos/team/kolux/contents/src/path%23with%3Fchars.ts?ref=head-sha') {
  ...
  it('skips an added file base read and preserves a head max-buffer overflow', async () => {
    ...
    const contents = await getPRFileContents({
      repoPath: '/repo-root',
      prRepo: { owner: 'team', repo: 'kolux', host: 'github.acme-corp.com' },
      prNumber: 7,
      path: 'src/large.ts',
      status: 'added',
      headSha: 'head-sha',
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toContain(
      'repos/team/kolux/contents/src/large.ts?ref=head-sha'
    )
