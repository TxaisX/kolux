...
describe('GitHub issue source split', () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            title: 'Fork PR',
            state: 'open',
            html_url: 'https://github.com/fork/kolux/pull/42',
            labels: [],
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    ...
    await listWorkItems('/home/jinwoo/kolux', 10, undefined, undefined, 'auto', 'openclaw-2')
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/kolux']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/kolux is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('pins typed PR metadata to explicit origin when upstream has the same number', async () => {
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some((call) =>
        (call[0] as string[]).some((arg) => arg.includes('upstream/kolux'))
      )
    ).toBe(false)
  ...
  it('does not probe a second PR repository after a non-not-found failure', async () => {
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some(([args]) =>
        (args as string[]).some((arg) => arg.includes('fork/kolux'))
      )
    ).toBe(false)
  })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream → queries origin", async () => {
      ...
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'kolux' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('solo/kolux'), {
        cwd: '/repo-root'
      })
    })
    ...
    it("preference='upstream' + no upstream → falls back to origin with fellBack=true", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:solo/kolux is:issue is:open')
      expect(result.issueSourceFellBack).toBe(true)
    })
    ...
    it("preference='origin' + upstream exists → queries origin (not upstream)", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:fork/kolux is:issue is:open')
    })
