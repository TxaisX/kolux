...
/**
 * Check if the authenticated user has starred the Kolux repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export function checkKoluxStarred(): Promise<boolean | null> {
  ...
  inFlightStarCheck ??= runKoluxStarredCheck().finally(() => {
...
export function __resetKoluxStarCheckForTests(): void {
  inFlightStarCheck = null
}

async function runKoluxStarredCheck(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await ghExecFileAsync(
      ['api', '--include', `user/starred/${KOLUX_REPO}`],
...
/**
 * Star the Kolux repo for the authenticated user.
 */
export async function starKolux(): Promise<boolean> {
  await acquire()
  try {
    await ghExecFileAsync(['api', '-X', 'PUT', `user/starred/${KOLUX_REPO}`], {
      encoding: 'utf-8',
