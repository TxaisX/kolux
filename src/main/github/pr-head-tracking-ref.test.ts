...
describe('fetchPrHeadTrackingRef', () => {
  ...
  it('fetches a GitHub pull head into its remote-scoped Kolux ref for local repos', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['fetch', '--no-tags', 'origin', `+refs/pull/42/head:refs/kolux/pull/${ORIGIN_COMPONENT}/42`],
      { cwd: '/repo', timeout: REVIEW_HEAD_FETCH_TIMEOUT_MS }
    )
    expect(localRef).toBe(githubPullRequestHeadLocalRef(ORIGIN_COMPONENT, 42))
    expect(localRef).toBe(`refs/kolux/pull/${ORIGIN_COMPONENT}/42`)
  })
  ...
  it('uses the SSH GitHub pull-head RPC and never runs git directly', async () => {
    const expectedRef = `refs/kolux/pull/${ORIGIN_COMPONENT}/42`
