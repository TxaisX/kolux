/**
 * Seam a usage source implements to be scanned by Kolux, including plugin-contributed ones.
 *
 * Deliberately generic over each provider's record types: Claude bills per turn while
 * Codex bills per event, and `cachedInput` is a subset of `input` for the latter but a
