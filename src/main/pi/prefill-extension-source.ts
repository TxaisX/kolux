...
export const KOLUX_PI_PREFILL_EXTENSION_FILE = 'kolux-prefill.ts'

// Why: prefill-without-submit needs an env-var the bundled `kolux-prefill.ts`
...
export function getPiPrefillExtensionSource(kind: PrefillAgentKind): string {
  ...
  return [
    ...
    '    if (!process.env.KOLUX_PANE_KEY) return',
