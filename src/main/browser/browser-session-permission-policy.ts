const AUTO_GRANTED_BROWSER_PERMISSIONS = new Set([
  ...
  'persistent-storage',
  // Chromium still requires user activation, so this only removes Kolux's
  // otherwise unactionable denial for immersive browser apps.
  'pointerLock',
  // Kolux allows unpartitioned third-party cookies, so cross-site frames already send them; denying
  ...
  // the gesture path. Revisit if Kolux ever gains a cookie or storage-partitioning control: the
...
// Related Website Sets rather than cookie policy, and Kolux has no such source, so the rationale
