...
describe('browserManager', () => {
  ...
  it('allows registered safe popup URLs as Electron child windows', () => {
    ...
    // for Kolux's origin-bar window without losing the popup contents.
  ...
  it('routes unnamed featureless window.open safely, including after renderer destruction', () => {
    ...
    expect(rendererSendMock).toHaveBeenCalledWith('browser:open-link-in-kolux-tab', {
      browserPageId: 'browser-1',
      url: 'https://docs.example.com/guide'
    })
    expect(rendererSendMock).toHaveBeenCalledWith('browser:open-link-in-kolux-tab', {
  ...
  it('shares the page-initiated tab budget across the whole opener popup tree', () => {
    ...
    const routedTabs = rendererSendMock.mock.calls.filter(
      ([channel]) => channel === 'browser:open-link-in-kolux-tab'
    )
  ...
  it('keeps plain links current and routes explicit new-tab gestures to Kolux tabs', async () => {
    const rendererSendMock = vi.fn()
    ...
    const clickedLinkFrameNames = {
      foreground: script.match(/__kolux_clicked_link_foreground_[0-9a-f-]+/)?.[0],
      background: script.match(/__kolux_clicked_link_background_[0-9a-f-]+/)?.[0]
    }
    ...
    expect(clickedLinkFrameNames.foreground).toMatch(/^__kolux_clicked_link_foreground_/)
  ...
  it('routes child-frame gestures with one-use tokens', async () => {
    ...
    const foregroundFrameName = firstScript.match(
      /__kolux_clicked_link_iframe_foreground_[0-9a-f-]+/
    )?.[0]
    const backgroundFrameName = firstScript.match(
      /__kolux_clicked_link_iframe_background_[0-9a-f-]+/
    )?.[0]
  ...
  it('blocks unsafe popup URLs for registered guests', () => {
    ...
    expect(rendererSendMock).not.toHaveBeenCalledWith(
      'browser:open-link-in-kolux-tab',
      expect.anything()
    )
    expect(rendererSendMock).toHaveBeenCalledWith('browser:popup', {
      browserPageId: 'browser-1',
  ...
  it('offers opening a link in another Kolux browser tab from the guest context menu', () => {
    const rendererSendMock = vi.fn()
    const guest = {
      id: 104,
      isDestroyed: vi.fn(() => false),
