  ...
  async resolveRef(guest: WebContents, sender: CdpCommandSender, ref: string): Promise<RefEntry> {
    ...
    if (!state.snapshotResult) {
      throw new BrowserError(
        'browser_stale_ref',
        "No snapshot exists for this tab. Run 'kolux snapshot' first."
      )
    }
    ...
    if (!entry) {
      throw new BrowserError(
        ...
        `Element ref ${ref} was not found. Run 'kolux snapshot' to see available refs.`
      )
    }
    ...
    if (!entry.sessionId) {
      ...
      if (state.navigationId && currentNavId !== state.navigationId) {
        ...
        throw new BrowserError(
          'browser_stale_ref',
          "The page has navigated since the last snapshot. Run 'kolux snapshot' to get fresh refs."
        )
      }
    }
    ...
    try {
      ...
      throw new BrowserError(
        'browser_stale_ref',
        `Element ${ref} no longer exists in the DOM. Run 'kolux snapshot' to get fresh refs.`
      )
    }
  }
