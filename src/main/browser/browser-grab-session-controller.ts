...
function isGuestCancellationPayload(rawPayload: unknown): boolean {
  ...
  if (payload.__koluxCancelled === true) {
    return true
  }
...
export class BrowserGrabSessionController {
  ...
  awaitGrabSelection(
    ...
    return new Promise<BrowserGrabResult>((resolve) => {
      ...
      // __koluxGrabResolve() which is wired by the 'awaitClick' script to
      ...
      const awaitGuestClick = async (): Promise<void> => {
        try {
          ...
          // Why: the guest wraps right-click results in { __koluxContextMenu, payload }
          // so the renderer can show the full action dropdown instead of auto-copying.
          const isContextMenu =
            '__koluxContextMenu' in (rawPayload as Record<string, unknown>) &&
            (rawPayload as Record<string, unknown>).__koluxContextMenu === true
