...
describe('agent-browser process environment', () => {
  it('bounds Unix socket paths independently of a long profile path', () => {
    ...
    expect(socketDirectory).toMatch(/^\/tmp\/kolux-ab-[0-9a-f]{16}$/)
    expect(
      `${socketDirectory}/kolux-tab-00000000-0000-4000-8000-000000000000.sock`.length
    ).toBeLessThan(104)
  })

  // Why ownsSocketDirectory is false for both: a directory Kolux did not derive can be shared with
  // another Kolux profile, so `session list` under it is no proof of ownership.
  ...
  // Why: the only daemon bound that survives a SIGKILL'd Kolux, so it must be set on every platform.
