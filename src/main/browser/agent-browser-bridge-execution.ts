...
export abstract class AgentBrowserBridgeExecution extends AgentBrowserBridgeTabs {
  ...
  protected requireTargetWebContents(target: ResolvedBrowserCommandTarget): WebContents {
    ...
    if (!wc || wc.isDestroyed()) {
      throw this.createPageUnavailableError(`${KOLUX_TAB_SESSION_PREFIX}${target.browserPageId}`)
    }
    return wc
  }
  ...
  protected async execAgentBrowser(
    ...
    // Why: a restarted named daemon auto-launches Chrome unless every invocation reasserts Kolux's CDP owner.
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        child = execFile(
          ...
          // Why windowsHide: agent-browser is console-subsystem and Kolux's main
