...
function readInstalledConfig(home: string): InstalledConfig {
  return JSON.parse(
    readFileSync(join(home, '.grok', 'hooks', 'kolux-status.json'), 'utf8')
  ) as InstalledConfig
}
...
describe('Windows Grok managed hook launch shape', () => {
  ...
  describe('registered command, from a synthetic Windows path', () => {
    const scriptPath = 'C:\\Users\\dev\\.kolux\\agent-hooks\\grok-hook.cmd'
    ...
    it('still wraps a path cmd.exe would split or expand (#6078)', () => {
      const spaced = 'C:\\Users\\Jane Doe\\.kolux\\agent-hooks\\grok-hook.cmd'
  ...
  describe('installed config', () => {
    ...
    beforeEach(() => {
      home = mkdtempSync(join(tmpdir(), 'kolux-grok-launcher-'))
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        const scriptPath = join(home, '.kolux', 'agent-hooks', 'grok-hook.cmd')
    ...
    it.skipIf(process.platform !== 'win32')(
      ...
      () => {
        ...
        const configPath = join(home, '.grok', 'hooks', 'kolux-status.json')
        ...
        // Why: sweeping stale Kolux entries must not touch hooks the user wrote.
    ...
    it('falls back to the encoded launcher when the profile path is not cmd-safe (#6078)', () => {
      const spaceHome = mkdtempSync(join(tmpdir(), 'kolux grok spaced '))
