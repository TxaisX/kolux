...
beforeEach(async () => {
  scratchDir = await mkdtemp(join(tmpdir(), 'kolux-worktree-trash-'))
})
...
describe('sweepStaleWorktreeTrash', () => {
  it('removes leftover entries from both workspace layouts and nothing else', async () => {
    ...
    // Not generated by Kolux: a stray directory and file inside the trash root must survive.
  ...
  it('also sweeps a pre-rename ".nightshift-worktree-trash" leftover', async () => {
    const legacyTrashRoot = join(scratchDir, 'repo', '.nightshift-worktree-trash')
    await mkdir(join(legacyTrashRoot, 'wt-1700000000004-abcdef05'), { recursive: true })
    const liveWorktree = join(scratchDir, 'repo', 'feature')
    await createWorktreeDirectory(liveWorktree)
    ...
    expect(existsSync(join(legacyTrashRoot, 'wt-1700000000004-abcdef05'))).toBe(false)
    expect(existsSync(join(liveWorktree, 'node_modules', 'pkg', 'index.js'))).toBe(true)
  })
...
describe('collectWorktreeTrashSweepRoots', () => {
  const settings = { workspaceDir: '/home/dev/kolux/workspaces', nestWorkspaces: true }

  function repo(overrides: Partial<Repo>): Repo {
    return { id: 'repo-1', path: '/home/dev/code/kolux', ...overrides } as unknown as Repo
  }
  ...
  it('collects one root per local git repo', () => {
    ...
    ).toEqual(['/home/dev/kolux/workspaces'])
  })
  ...
  it('skips WSL repos that cannot create host trash', () => {
    ...
    try {
      expect(
        collectWorktreeTrashSweepRoots(
          [
            repo({ path: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\kolux' }),
            repo({
              id: 'repo-2',
              path: 'C:\\code\\kolux',
