...
describe('scanForShellReady', () => {
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;kolux-shell-readyx')).toEqual({
  ...
  it('reports post-marker bytes only when bytes follow the BEL terminator in the matching call', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;kolux-shell-ready\x07')).toEqual({
      output: 'before ',
      matched: true,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, 'before \x1b]777;kolux-shell-ready')).toEqual({
      output: 'before ',
      matched: false,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, '\x1b]777;kolux-shell-ready')).toEqual({
      output: '',
      matched: false,
      postMarkerBytesObserved: false
    })
