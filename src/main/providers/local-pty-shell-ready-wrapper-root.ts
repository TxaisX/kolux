...
function getShellReadyWrapperBaseDir(): string {
  // Why: bundled into the daemon fork (no electron), so read KOLUX_USER_DATA_PATH rather than electron's userData; main and the fork both set it to the same path.
  // Why a truthiness test rather than `??`: a set-but-empty KOLUX_USER_DATA_PATH
  // would leave a relative base dir, so wrapper trees would be written under
  // whatever the process cwd happens to be.
  const userDataPath = process.env.KOLUX_USER_DATA_PATH
  ...
  // reaches it -- KOLUX_USER_DATA_PATH is seeded before anything spawns and the
  ...
  return userDataPath
    ...
    : join(tmpdir(), 'kolux-shell-wrappers')
}
...
// self-invalidates if KOLUX_USER_DATA_PATH is ever re-pointed mid-process.
