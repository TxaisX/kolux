/**
 * Decides which inherited ZDOTDIR Kolux may treat as the user's real zsh config
 * dir, on the Node side of the launch.
 *
 * Why: Kolux used to recognise only its OWN wrapper path shape (a path ending in
 * shell-ready/zsh). Launched from any other terminal that had already
 * hijacked ZDOTDIR, Kolux captured that dir as "the user's config" and re-sourced
 * it. Ownership is now established positively — a stamped marker file, or Kolux's
...
export function isKoluxOwnedZshWrapperDir(dir: string): boolean {
...
function usableInheritedZdotdir(value: string | undefined): string | null {
  ...
  if (!normalized || isKoluxOwnedZshWrapperDir(normalized)) {
    return null
  }
 ...
 * one Kolux invented. `ZDOTDIR=$HOME` and an unset ZDOTDIR look identical to zsh
...
export function resolveInheritedZdotdir(env: EnvLike): string | null {
  return (
    usableInheritedZdotdir(env.ZDOTDIR) ?? usableInheritedZdotdir(env.KOLUX_ORIG_ZDOTDIR) ?? null
  )
}
...
export function inheritedZdotdirEnv(inherited: string | null): Record<string, string> {
  return inherited ? { KOLUX_ORIG_ZDOTDIR: inherited } : {}
}
