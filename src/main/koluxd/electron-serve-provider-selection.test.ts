...
let provider: KoluxdBrowserProvider | null
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'koluxd-provider-selection-'))
...
describe('resolveKoluxdBrowserProvider Electron preference', () => {
  it('uses the installed Electron app even when a Chromium executable is configured', async () => {
    provider = await resolveKoluxdBrowserProvider({
      userDataPath: join(harnessRoot, 'state'),
      environment: { KOLUX_BROWSER_EXECUTABLE: chromiumExecutable },
  ...
  it('falls back to the operator Chromium when the installed Electron app cannot start', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      '[koluxd] Installed Electron browser provider unavailable:',
      expect.anything()
    )
  })
