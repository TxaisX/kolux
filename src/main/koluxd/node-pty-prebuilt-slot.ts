 ...
 * by config/scripts/build-koluxd-prebuilds.mjs. An upstream tarball would not do: the patch
 ...
 * `koluxd.js` itself, so this stays correct wherever the install directory ends up.
 */
export function resolveKoluxdPrebuildsDir(entryScript = process.argv[1]): string | null {
  const override = process.env.KOLUX_KOLUXD_PREBUILDS_DIR
  if (override) {
    return override
  }
...
export function installPrebuiltSlot(options: {
  ...
  const prebuildsDir = options.prebuildsDir ?? resolveKoluxdPrebuildsDir()
