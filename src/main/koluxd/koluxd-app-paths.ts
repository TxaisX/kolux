/**
 * The path half of koluxd's `AppEnvironment`: every `AppPathName` gets a real answer for
...
/** XDG-ish data root. `$KOLUX_USER_DATA` wins so a smoke test can isolate state. */
export function resolveUserDataPath(): string {
  const explicit = env('KOLUX_USER_DATA')
  if (explicit) {
    return explicit
  }
  const xdg = env('XDG_DATA_HOME')
  return xdg ? join(xdg, 'Kolux') : join(homedir(), '.kolux')
}
...
/**
 * The directory koluxd was launched from — the bundle root that `koluxd.js` and the
 ...
 */
export function resolveKoluxdInstallRoot(scriptPath = process.argv[1]): string {
  if (!scriptPath) {
    throw new Error(
      'koluxd_install_root_unavailable — process.argv[1] is unset, so this process has no ' +
...
export function resolveKoluxdPath(name: AppPathName): string {
  switch (name) {
    case 'userData':
