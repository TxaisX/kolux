...
const reported = (message: string) =>
  classifyNodePtyProbeResult({
    code: 4,
    signal: null,
    stdout: `KOLUX_NODE_PTY_LOAD_ERROR ${JSON.stringify(message)}\n`,
    stderr: '',
    timedOut: false
  })
...
describe('classifyNodePtyProbeResult', () => {
  it('accepts only a clean exit that printed the token', () => {
    expect(probe({ code: 0, stdout: 'KOLUX_NODE_PTY_LOAD_OK /x/build/Release' })).toBeNull()
  ...
  it('distinguishes no binary anywhere from a binary the loader refused', () => {
    ...
    expect(probe({ code: 3, stdout: 'KOLUX_NODE_PTY_NO_BINARY\n' })).toEqual({
      status: 'blocked',
  ...
  it('ignores its own token strings echoed back inside the child stderr', () => {
    ...
    const echoedSource =
      '[eval]:1\nif(!f){console.log("KOLUX_NODE_PTY_NO_BINARY");process.exit(3)}\n' +
    ...
    expect(
      classifyNodePtyProbeResult({
        code: 4,
        signal: null,
        stdout: `KOLUX_NODE_PTY_LOAD_ERROR ${JSON.stringify('dlopen(/app/pty.node): slice is not valid mach-o file')}`,
        stderr: echoedSource,
        timedOut: false
      })
...
describe('checkNodePtyPrecondition', () => {
  ...
  const stageNodePty = (): string => {
    const root = mkdtempSync(join(tmpdir(), 'koluxd-node-pty-'))
  ...
  // koluxd degraded while its terminals worked (#17844). Gated on a loadable binding for
  ...
  it('places the matching slot even when the payload is not loadable', () => {
    ...
    const prebuildsDir = mkdtempSync(join(tmpdir(), 'koluxd-prebuilds-'))
