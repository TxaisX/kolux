...
import { resolveKoluxdAgentBrowserBinary } from './koluxd-agent-browser-binary'
import { ElectronServeBrowserProcess } from './electron-serve-browser-process'

export type KoluxdBrowserProvider = {
...
export type KoluxdBrowserProviderOptions = {
  userDataPath: string
...
export function installedElectronCandidates(
  ...
  if (platform === 'darwin') {
    return [
      ...
      joinPath(homePath, 'Applications', 'Kolux.app', 'Contents', 'MacOS', 'Kolux')
    ]
  }
  if (platform === 'win32') {
    return [
      ...(environment.LOCALAPPDATA
        ? [joinPath(environment.LOCALAPPDATA, 'Programs', 'Kolux', 'Kolux.exe')]
        : []),
      ...(environment.ProgramFiles
        ? [joinPath(environment.ProgramFiles, 'Kolux', 'Kolux.exe')]
        : [])
    ]
  }
  return [
    joinPath(homePath, '.local', 'bin', 'kolux-ide'),
    '/usr/local/bin/kolux-ide',
    '/usr/bin/orca-ide',
    '/opt/Kolux/kolux-ide'
  ]
}
...
async function startElectronServeProvider(executablePath: string): Promise<KoluxdBrowserProvider> {
...
export async function resolveKoluxdBrowserProvider(
  options: KoluxdBrowserProviderOptions
): Promise<KoluxdBrowserProvider | null> {
  ...
  if (installedElectronExecutable) {
    try {
      ...
      console.warn('[koluxd] Installed Electron browser provider unavailable:', error)
  ...
  // problem, not an unconfigured host. Telling someone to set KOLUX_BROWSER_EXECUTABLE when
  // they already did is the misdirection this ordering exists to prevent.
  const chromiumExecutable = environment.KOLUX_BROWSER_EXECUTABLE?.trim()
  if (!chromiumExecutable) {
    return declined(electronFailure ?? { reason: 'unconfigured' })
  }

  const agentBrowserPath = (options.resolveAgentBrowserBinary ?? resolveKoluxdAgentBrowserBinary)()
  if (!agentBrowserPath) {
  ...
  try {
    ...
    console.warn('[koluxd] External Chromium browser provider unavailable:', error)
