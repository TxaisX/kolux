...
const dirs: string[] = []
const temp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'koluxd-slot-'))
  dirs.push(dir)
  return dir
}
afterEach(() => {
  ...
  delete process.env.KOLUX_KOLUXD_PREBUILDS_DIR
})
...
describe('resolveKoluxdPrebuildsDir', () => {
  it('looks beside the running bundle', () => {
    expect(resolveKoluxdPrebuildsDir('/opt/koluxd/koluxd.js')).toBe(
      join('/opt/koluxd', 'prebuilds')
    )
  })

  it('honours an explicit override', () => {
    process.env.KOLUX_KOLUXD_PREBUILDS_DIR = '/custom/prebuilds'
    expect(resolveKoluxdPrebuildsDir('/opt/koluxd/koluxd.js')).toBe('/custom/prebuilds')
  })
})
...
describe('installPrebuiltSlot', () => {
  ...
  it('installs a Linux slot without claiming a spawn-helper it never execs', () => {
    ...
    // macOS is what made every Linux koluxd boot degraded on spawn_helper_missing (#17844).
    const prebuilds = temp()
    const nodePtyDir = temp()
