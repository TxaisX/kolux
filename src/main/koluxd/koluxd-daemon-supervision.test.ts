...
vi.mock('../daemon/daemon-init', () => ({
  ...
  // koluxd shutdown is what would make a koluxd restart destructive again.
...
describe('startKoluxdDaemon', () => {
  it('reports live with the daemon pid once the provider is installed', async () => {
    await expect(startKoluxdDaemon()).resolves.toEqual({ state: 'live', pid: 4242 })
  })

  it('does not arm the macOS login-session death watch', async () => {
    await startKoluxdDaemon()
    // That watch retires the daemon when the spawning GUI login session dies. A koluxd
  ...
  it('reports degraded when fresh terminals would fall back to the local provider', async () => {
    ...
    const result = await startKoluxdDaemon()
  ...
  it('fails open when the daemon cannot start at all', async () => {
    ...
    await expect(startKoluxdDaemon()).resolves.toEqual({
      state: 'unavailable',
...
describe('stopKoluxdDaemon', () => {
  it('disconnects and never shuts the daemon down', async () => {
    ...
    // The whole point of item 4: a koluxd restart is non-destructive only if the daemon
