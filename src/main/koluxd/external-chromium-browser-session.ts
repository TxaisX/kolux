...
// The stable session name plus the `close` in start() is what reclaims a killed koluxd's tree (#16367).
...
export class ExternalChromiumBrowserSession {
  ...
  constructor(
    ...
    const identity = createHash('sha256')
      ...
      .digest('hex')
      .slice(0, 16)
    this.sessionName = `kolux-koluxd-${identity}`
    this.profilePath = join(statePath, `browser-${launch.provider}`)
  }

  async start(): Promise<string> {
    await mkdir(this.profilePath, { recursive: true })
    // Why: the session name is stable across runs, so a daemon an earlier koluxd left behind is
