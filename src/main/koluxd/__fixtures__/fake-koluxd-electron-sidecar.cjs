/**
 * Stands in for an installed Kolux app running `--serve`, so the koluxd Electron
 ...
 * listen on a local transport, then publish `kolux-runtime.json` there.
 *
 * Test-only channels (injected by the spawn wrapper, never by production code):
 *   KOLUX_FAKE_SIDECAR_LOG      JSONL of every request received.
 *   KOLUX_FAKE_SIDECAR_CONTROL  JSON re-read per request: { capabilities, errors }.
 *   KOLUX_FAKE_SIDECAR_MODE     'exit-before-ready' to die without publishing.
 */
'use strict'
...
const logPath = process.env.KOLUX_FAKE_SIDECAR_LOG
const controlPath = process.env.KOLUX_FAKE_SIDECAR_CONTROL

if (!userDataDir) {
  process.stderr.write('fake sidecar: missing --user-data-dir\n')
  process.exit(2)
}
if (process.env.KOLUX_FAKE_SIDECAR_MODE === 'exit-before-ready') {
  process.exit(3)
}
...
const endpoint =
  process.platform === 'win32'
    ? `\\\\.\\pipe\\koluxd-fake-sidecar-${process.pid}`
...
server.listen(endpoint, () => {
  const metadataPath = join(userDataDir, 'kolux-runtime.json')
