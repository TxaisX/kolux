...
import {
  ...
  resolveKoluxdBrowserProvider
} from './koluxd-browser-provider'
import { koluxdAgentBrowserNativeName } from './koluxd-agent-browser-binary'
import {
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'koluxd-browser-provider-'))
...
describe('ExternalChromiumBrowserProcess', () => {
  it('navigates, evaluates, and screenshots through the operator browser', async () => {
    ...
    const commands = processHandle.createCommands({
      ...
      // KoluxRuntimeService, which owns the client-hosted registries.
...
describe('cross-platform browser provider paths', () => {
  it('resolves installed Electron launchers on macOS, Linux, and Windows', () => {
    expect(installedElectronCandidates('darwin', '/Users/test', {})).toContain(
      '/Users/test/Applications/Kolux.app/Contents/MacOS/Kolux'
    )
    expect(installedElectronCandidates('linux', '/home/test', {})).toContain(
      '/home/test/.local/bin/kolux-ide'
    )
    expect(
      installedElectronCandidates('win32', 'C:\\Users\\test', {
        LOCALAPPDATA: 'C:\\Users\\test\\AppData\\Local'
      })
    ).toContain('C:\\Users\\test\\AppData\\Local\\Programs\\Kolux\\Kolux.exe')
  })

  it('uses platform-specific bundled agent-browser names', () => {
    expect(koluxdAgentBrowserNativeName('darwin', 'arm64')).toBe('agent-browser-darwin-arm64')
    expect(koluxdAgentBrowserNativeName('linux', 'x64')).toBe('agent-browser-linux-x64')
    expect(koluxdAgentBrowserNativeName('win32', 'x64')).toBe('agent-browser-win32-x64.exe')
  })
})

describe('resolveKoluxdBrowserProvider', () => {
  it('uses KOLUX_BROWSER_EXECUTABLE when Electron is absent', async () => {
    ...
    const provider = await resolveKoluxdBrowserProvider({
      userDataPath: root,
      environment: { KOLUX_BROWSER_EXECUTABLE: executable },
  ...
  it('reports an KOLUX_BROWSER_EXECUTABLE path that does not exist', async () => {
    const missing = join(root, 'absent-chromium')

    await expect(
      resolveKoluxdBrowserProvider({
        userDataPath: root,
        environment: { KOLUX_BROWSER_EXECUTABLE: missing },
        resolveInstalledElectronExecutable: async () => null,
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
  ...
  it.skipIf(process.platform === 'win32')(
    'separates a non-executable KOLUX_BROWSER_EXECUTABLE from a missing one',
    async () => {
  ...
  it('reports a failed Electron start instead of staying invisible to the client', async () => {
    ...
    await expect(
      resolveKoluxdBrowserProvider({
        userDataPath: root,
        environment: {},
        resolveInstalledElectronExecutable: async () => '/opt/Kolux/kolux-ide',
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
    expect(runtimeBrowserUnavailableCause()).toEqual({
  ...
  it('prefers the configured Chromium fault over an earlier Electron failure', async () => {
    ...
    await expect(
      resolveKoluxdBrowserProvider({
        userDataPath: root,
        environment: { KOLUX_BROWSER_EXECUTABLE: join(root, 'absent-chromium') },
        resolveInstalledElectronExecutable: async () => '/opt/Kolux/kolux-ide',
        resolveAgentBrowserBinary: () => null
      })
    ).resolves.toBeNull()
