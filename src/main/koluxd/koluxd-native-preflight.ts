/**
 * The native precondition koluxd runs before its runtime loads anything native.
 *
 ...
 */
export const KOLUXD_NATIVE_PRECONDITION_EXIT_CODE = 78
 ...
 */
export function runKoluxdNativePreflight(hooks: NativePreflightHooks = {}): boolean {
  ...
  if (verdict.status === 'blocked') {
    ...
    fail(`koluxd: ${formatNodePtyPreconditionReport(verdict, message, hints)}`)
    exit(KOLUXD_NATIVE_PRECONDITION_EXIT_CODE)
    return false
  }
  ...
  warn(`koluxd: ${formatNodePtyPreconditionReport(verdict, message)}`)
  return true
}
