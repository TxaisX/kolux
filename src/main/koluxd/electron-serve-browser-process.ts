...
} from '../runtime/kolux-runtime-browser'
...
import { sendKoluxdSidecarRequest } from './koluxd-sidecar-runtime-client'
import {
...
export class ElectronServeBrowserProcess {
  ...
  async start(): Promise<void> {
    ...
    const userDataPath = await mkdtemp(join(temporaryRoot, 'koluxd-browser-'))
      ...
      if (metadata) {
        try {
          const status = RuntimeStatusResult.parse(
            await sendKoluxdSidecarRequest(metadata, 'status.get', undefined, 5_000)
          )
  ...
  private async invoke(
    ...
    if (method === 'browserScreencast') {
      throw new BrowserError(
        ...
        'The koluxd Electron provider does not proxy screencast.'
      )
    }
    ...
    let result: unknown
    try {
      result = await sendKoluxdSidecarRequest(metadata, rpcMethod, params)
    } catch (error) {
      if (
        targetPage &&
