/**
 * The address `koluxd` binds its RPC listener to.
 *
 * Why loopback by default: the desktop stays on loopback until the user pairs, and the
 * shipping design reaches a remote koluxd over an SSH local port-forward — so the wide
 * bind koluxd had was both a departure from the desktop's posture and unnecessary for the
 * deploy model. Exposure is now something an operator asks for by name.
 */
import { isIP } from 'node:net'

export const KOLUXD_LOOPBACK_BIND_HOST = '127.0.0.1'
const ALL_INTERFACES_V4 = '0.0.0.0'
const ALL_INTERFACES_V6 = '::'

export class KoluxdBindAddressError extends Error {
  readonly code = 'koluxd_invalid_bind_address'
}
 ...
 */
export function resolveKoluxdBindHost(raw?: string): string {
  if (raw === undefined) {
    return KOLUXD_LOOPBACK_BIND_HOST
  }
  const value = raw.trim()
  if (value === '') {
    throw new KoluxdBindAddressError('--bind expects an address')
  }
  if (value === 'localhost') {
    return KOLUXD_LOOPBACK_BIND_HOST
  }
  if (isIP(value) === 0) {
    throw new KoluxdBindAddressError(
...
export function describeKoluxdBindExposure(host: string): string {
  return bindHostIsNetworkExposed(host)
    ? `koluxd is bound to ${host} and is reachable from the network. Anything that can reach ` +
        'this port can attempt pairing.'
    : `koluxd is bound to ${host} (local only). Reach it from another machine with an SSH ` +
