...
import {
  ...
  KOLUXD_LOCK_FILE_NAME,
  KoluxdInstanceLockError,
  type KoluxdInstanceLockHooks
} from './koluxd-instance-lock'

const roots: string[] = []

function makeRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'koluxd-lock-'))
  roots.push(root)
  return root
}

/** Deterministic identity/liveness so the assertions do not depend on this machine's pids. */
function hooks(overrides: KoluxdInstanceLockHooks = {}): KoluxdInstanceLockHooks {
  return {
    identity: () => 'uid-1000',
    version: () => '1.0.0-test',
    startedAtMs: () => 1_000,
...
describe('acquireKoluxdInstanceLock', () => {
  it('publishes a record naming the holder and removes it on release', () => {
    const root = makeRoot()
    const lock = acquireKoluxdInstanceLock(root, hooks())
    const record = JSON.parse(readFileSync(join(root, KOLUXD_LOCK_FILE_NAME), 'utf8'))
    expect(record.pid).toBe(process.pid)
    expect(record.identity).toBe('uid-1000')
    expect(record.version).toBe('1.0.0-test')
    lock.release()
    expect(() => readFileSync(join(root, KOLUXD_LOCK_FILE_NAME), 'utf8')).toThrow()
  })

  it('refuses a second instance while the holder is alive', () => {
    const root = makeRoot()
    acquireKoluxdInstanceLock(root, hooks())
    expect(() => acquireKoluxdInstanceLock(root, hooks({ processIsAlive: () => true }))).toThrow(
      KoluxdInstanceLockError
    )
    expect(() => acquireKoluxdInstanceLock(root, hooks({ processIsAlive: () => true }))).toThrow(
      expect.objectContaining({ code: 'koluxd_instance_lock_held' })
    )
  })
  ...
  it('reclaims the record of a holder that is gone', () => {
    const root = makeRoot()
    writeFileSync(
      join(root, KOLUXD_LOCK_FILE_NAME),
      JSON.stringify({ pid: 424242, identity: 'uid-1000', startedAtMs: 1, nonce: 'stale' })
    )
    const lock = acquireKoluxdInstanceLock(root, hooks({ processIsAlive: () => false }))
  ...
  it('treats a live pid whose start time does not match as a recycled pid, not a holder', () => {
    ...
    const lock = acquireKoluxdInstanceLock(
      root,
  ...
  it('never reclaims a lock held by a different identity, even a dead one', () => {
    ...
    expect(() => acquireKoluxdInstanceLock(root, hooks({ processIsAlive: () => false }))).toThrow(
      expect.objectContaining({ code: 'koluxd_instance_lock_foreign_identity' })
    )
  })
  ...
  it.runIf(process.platform !== 'win32')('refuses a data root owned by another uid', () => {
    ...
    expect(() => acquireKoluxdInstanceLock('/tmp', hooks())).toThrow(
      expect.objectContaining({ code: 'koluxd_data_root_wrong_owner' })
    )
    // And the private root this test made is still acceptable, so the refusal is about
    // ownership rather than a blanket rejection.
    expect(acquireKoluxdInstanceLock(root, hooks()).record.identity).toBe('uid-1000')
  })
  ...
  it('leaves the terminal daemon alone: the lock covers only the runtime role', () => {
    ...
    const next = acquireKoluxdInstanceLock(root, hooks())
