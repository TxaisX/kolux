/**
 * koluxd's half of the supervision contract: it spawns and supervises the terminal daemon.
 *
 ...
 * Without a daemon here, every koluxd restart, update and rollback is a SIGKILL for every
 * running terminal — on the host whose selling point is that work survives the client going
 * away.
 *
 * The constraint that makes a restart non-destructive lives in `stopKoluxdDaemon` below: the
...
export type KoluxdDaemonStartup =
 ...
 */
export async function startKoluxdDaemon(): Promise<KoluxdDaemonStartup> {
  try {
    // Why no login-session watch: that retires the daemon when the spawning macOS GUI login
    // session dies. A koluxd daemon must survive its SSH session ending — that is the point.
    await initDaemonPtyProvider(undefined, { macosLoginSessionWatch: false })
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error)
    console.error(
      `[koluxd] The terminal daemon did not start: ${reason}\n` +
        '[koluxd] Terminals will run in-process and WILL NOT survive a koluxd restart.'
    )
  ...
  if (!daemonOwnsFreshPersistentPtys()) {
    ...
    console.warn(
      `[koluxd] ${reason}. Existing daemon sessions keep working, but new terminals will not ` +
        'survive a koluxd restart until the daemon is restarted.'
    )
 ...
 * every PTY under it. Calling it here would make koluxd's own restart destructive, which is
 * the exact property this whole item exists to buy.
 */
export async function stopKoluxdDaemon(): Promise<void> {
