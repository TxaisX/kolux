...
import type { RuntimeBrowserCommandHost } from '../runtime/kolux-runtime-browser'
...
const FAKE_SIDECAR = join(import.meta.dirname, '__fixtures__', 'fake-koluxd-electron-sidecar.cjs')
const INSTALLED_EXECUTABLE = join('/Applications', 'Kolux.app', 'Contents', 'MacOS', 'Kolux')

/** Every key the provider must strip so the sidecar cannot inherit koluxd's own browser config. */
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'koluxd-electron-harness-'))
  ...
  spawnProcessMock.mockImplementation((spec: RunProcessModule.ProcessSpec) =>
    actual.spawnProcess({
      ...
      env: {
        ...spec.env,
        KOLUX_FAKE_SIDECAR_LOG: logPath,
        KOLUX_FAKE_SIDECAR_CONTROL: controlPath,
        ...(sidecarMode ? { KOLUX_FAKE_SIDECAR_MODE: sidecarMode } : {})
      }
    })
  )
})
...
describe('ElectronServeBrowserProcess start-up', () => {
  it('launches the installed app in headless serve mode without koluxd browser env', async () => {
    for (const key of AGENT_BROWSER_ENVIRONMENT_KEYS) {
      vi.stubEnv(key, `leaked-${key}`)
    }
    vi.stubEnv('KOLUX_HARNESS_UNRELATED', 'preserved')
    ...
    expect(spec.env?.KOLUX_HARNESS_UNRELATED).toBe('preserved')
  ...
  it('stops the sidecar process and removes its user data directory', async () => {
    ...
    const metadata = JSON.parse(
      await readFile(join(userDataPath, 'kolux-runtime.json'), 'utf8')
    ) as { pid: number }
