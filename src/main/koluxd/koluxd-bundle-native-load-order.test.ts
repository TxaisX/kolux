 ...
 * lazy initializer because `koluxd-entry` reaches it through `await import('../ipc/pty')`,
 ...
 *
 * Why this builds the bundle instead of skipping without one: no CI job builds koluxd and
 * runs vitest. `smoke:koluxd-terminal` builds it in the static-analysis job, which never
 * runs vitest; the `koluxd_browser` job runs vitest but deliberately does not build koluxd.
 * A `runIf(existsSync(...))` guard therefore skips in every shard, forever — the same way
 * an unset KOLUX_BROWSER_EXECUTABLE kept the browser provider uncovered.
 *
...
const BUNDLE = join(REPO_ROOT, 'out', 'koluxd', 'koluxd.js')
const BUILD_SCRIPT = join(REPO_ROOT, 'config', 'scripts', 'build-koluxd.mjs')
 ...
 */
function ensureKoluxdBundle(): void {
...
describe('koluxd bundle native load order', () => {
  const dirs: string[] = []
  afterEach(() => {
  ...
  it('does not require node-pty in-process before the entry rejects its argv', () => {
    ensureKoluxdBundle()
    const dir = mkdtempSync(join(tmpdir(), 'koluxd-load-order-'))
    dirs.push(dir)
    ...
    writeFileSync(
      ...
      [
        ...
        '}',
        "process.argv.push('--koluxd-load-order-check')",
    ...
    expect(output).toContain('Unknown argument: --koluxd-load-order-check')
