...
const { collectKoluxdHealth, collectTerminalDaemonHealth, computeKoluxdBuildHash } =
  await import('./koluxd-health')
...
describe('collectTerminalDaemonHealth', () => {
  it('reports live only when the daemon answered its own PTY spawn probe', async () => {
    ...
    expect(health.entryPath).toBe('/opt/koluxd/daemon-entry.js')
...
describe('computeKoluxdBuildHash', () => {
  it('changes when the bundle bytes change, even at the same version string', () => {
    const dir = mkdtempSync(join(tmpdir(), 'koluxd-build-hash-'))
    const entry = join(dir, 'koluxd.js')
    writeFileSync(entry, 'build-a')
    const first = computeKoluxdBuildHash(entry)
    ...
    expect(computeKoluxdBuildHash(entry)).not.toBe(first)
  })

  it('answers unknown rather than throwing when the entry cannot be read', () => {
    expect(computeKoluxdBuildHash(join(tmpdir(), 'definitely-absent-koluxd.js'))).toBe('unknown')
    // A process with no argv[1] (an embedded host) still has to publish a readiness payload.
    expect(computeKoluxdBuildHash('')).toBe('unknown')
  })
})
