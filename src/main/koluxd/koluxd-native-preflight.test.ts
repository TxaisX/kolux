...
import {
  KOLUXD_NATIVE_PRECONDITION_EXIT_CODE,
  runKoluxdNativePreflight
} from './koluxd-native-preflight'
import {
...
const harness = (given: NodePtyPreconditionVerdict) => {
  ...
  const continued = runKoluxdNativePreflight({
    check: () => given,
...
describe('runKoluxdNativePreflight', () => {
  it('stops the boot on a proven-unloadable binary instead of reaching the require', () => {
    ...
    expect(exit).toHaveBeenCalledWith(KOLUXD_NATIVE_PRECONDITION_EXIT_CODE)
  ...
  it('exits with EX_CONFIG so a supervisor does not restart an unequippable host forever', () => {
    expect(KOLUXD_NATIVE_PRECONDITION_EXIT_CODE).toBe(78)
    expect(KOLUXD_NATIVE_PRECONDITION_EXIT_CODE).not.toBe(1)
  })
