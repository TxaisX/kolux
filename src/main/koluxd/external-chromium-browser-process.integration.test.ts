...
import { resolveKoluxdBrowserProvider } from './koluxd-browser-provider'

const executablePath = process.env.KOLUX_BROWSER_EXECUTABLE
...
describe('ExternalChromiumBrowserProcess integration', () => {
  it.runIf(Boolean(executablePath))(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'koluxd-external-browser-'))
      ...
      const provider = await resolveKoluxdBrowserProvider({
        userDataPath: root,
        environment: { KOLUX_BROWSER_EXECUTABLE: executablePath },
