/**
 * koluxd's health surface: the facts a supervisor needs to decide whether this deployment
 * is actually serving, as opposed to merely listening.
 *
 * The load-bearing one is the terminal-daemon verdict. koluxd answers RPC from its own
 ...
 * to cross the process boundary: koluxd drives it, the daemon performs it, and the verdict
...
export type TerminalDaemonHealth = {
  ...
  /** True only when FRESH terminals are daemon-owned, i.e. survive a koluxd restart. */
  ownsFreshSessions: boolean
  pid: number | null
  /** The build the LIVE daemon was forked from, which may predate this koluxd after an update. */
...
export type KoluxdHealth = {
  /** Content hash of the running koluxd bundle — the deployed build's identity. */
  buildHash: string
  buildVersion: string
  nodeVersion: string
 ...
 *
 * Why hash the entry and not read a version string: `KOLUX_VERSION` is whatever the deploy
 ...
 */
export function computeKoluxdBuildHash(entryPath = process.argv[1]): string {
  if (!entryPath) {
    return 'unknown'
  }
  try {
...
export async function collectKoluxdHealth(buildVersion: string): Promise<KoluxdHealth> {
  return {
    buildHash: computeKoluxdBuildHash(),
    buildVersion,
