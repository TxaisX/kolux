/** Executable entry for `koluxd`. See `./koluxd-entry.ts`. */
import process from 'node:process'
import { main, resolveKoluxdExitCode } from './koluxd-entry'
import { runKoluxdNativePreflight } from './koluxd-native-preflight'
...
// machine.
if (process.argv.includes('--koluxd-smoke-load-check')) {
  process.exit(0)
}

// Why here and not inside startKoluxd: this must run before anything requires node-pty,
// and `koluxd-entry` reaches it through `await import('../ipc/pty')`. Static imports are
...
runKoluxdNativePreflight()

main().catch((error: unknown) => {
  console.error('koluxd: failed to start:', error)
  ...
  process.exit(resolveKoluxdExitCode(error))
})
