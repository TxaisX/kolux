...
const BASE = {
  executablePath: '/opt/kolux/chromium',
  profilePath: '/state/browser-chromium',
  sessionName: 'kolux-koluxd-0123456789abcdef'
}
...
describe('koluxd external-chromium agent-browser environment', () => {
  beforeEach(() => {
  ...
  // Why (#16367): koluxd is the backend a remote user's Chromium hangs off, and this session
  // never passes --cdp, so a daemon an earlier koluxd left behind is not bound to the dead
  ...
  // that is the killed-koluxd case the stable session name exists to recover.
