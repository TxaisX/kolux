 ...
 * emits it — and Kolux's send acknowledgement depends on the replay.
 */
...
export type ClaudeStructuredLaunch = {
  /** Always Kolux's resolved user CLI: the SDK's bundled binaries are excluded from the install. */
...
export function claudeSessionIdForKoluxSession(sessionId: string): string {
  const bytes = createHash('sha256').update(`kolux-claude:${sessionId}`).digest().subarray(0, 16)
  ...
  return async ({ identity }) => {
    ...
    const providerSessionId =
      head?.handle.provider === 'claude'
        ...
        : claudeSessionIdForKoluxSession(identity.sessionId)
    ...
    const env = withCliRuntimeOnPath(
      command,
      // Only a dispatched structured worker gets the orchestration identity and the Kolux CLI on
