...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.txais.kolux.computer-use'
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'kolux-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'kolux-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Kolux Computer Use, then retry get-app-state.`
}
