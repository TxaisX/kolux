...
function makeHome(): string {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'kolux-commit-env-'))
  tempDirs.push(dir)
...
describe('prepareLocalCommitMessageAgentEnv', () => {
  ...
  it('prefers the original Pi agent root over inherited PTY overlays', async () => {
    process.env.PI_CODING_AGENT_DIR = '/tmp/kolux-pi-overlay'
    process.env.KOLUX_PI_SOURCE_AGENT_DIR = '/Users/tester/.pi/agent'
  ...
  it('sets CODEX_HOME for host managed Codex accounts', async () => {
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        'C:\\Users\\tester\\AppData\\Roaming\\Kolux\\codex-accounts\\a\\home'
    })

    expect(result).toEqual({
      ok: true,
      env: expect.objectContaining({
        CODEX_HOME: 'C:\\Users\\tester\\AppData\\Roaming\\Kolux\\codex-accounts\\a\\home'
      })
    })
  })
  ...
  it('strips a nested-Kolux CODEX_HOME override when the launch resolves to the real home', async () => {
    process.env.CODEX_HOME = '/managed/runtime/home'
    process.env.KOLUX_CODEX_HOME = '/managed/runtime/home'
  ...
  it('does not pass WSL managed Codex homes to host-local commit generation', async () => {
    ...
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        '\\\\wsl.localhost\\Ubuntu\\home\\tester\\.local\\share\\kolux\\codex-accounts\\a\\home'
    })
