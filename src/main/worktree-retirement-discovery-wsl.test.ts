...
const DISTRO_ROOT = '\\\\wsl.localhost\\Ubuntu\\home\\ada\\kolux\\workspaces'
...
describe('retirement discovery on WSL', () => {
  ...
  it('reports a gate refusal as incomplete but still returns what the host side could read', async () => {
    ...
    const hostHome = await mkdtemp(join(tmpdir(), 'kolux-wsl-host-home-'))
    try {
      await mkdir(
        join(
          hostHome,
  ...
  it('accepts a UNC ENOENT as absence once the parent directory still lists', async () => {
    ...
    const missing = new Set([
      DISTRO_ROOT,
      '\\\\wsl.localhost\\Ubuntu\\home\\ada\\kolux',
  ...
  it('keeps a deleted WSL workspace name spent, so the next create cannot reuse its cwd', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'kolux-wsl-distro-home-'))
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'kolux-wsl-workspaces-'))
    try {
      await mkdir(join(distroHome, '.claude', 'projects', `-home-ada-kolux-workspaces-${FIRST}`), {
        recursive: true
      })
  ...
  it('reaches distro discovery from the real backfill, not only when called directly', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'kolux-wsl-backfill-home-'))
    try {
      await mkdir(join(distroHome, '.claude', 'projects', `-home-ada-kolux-workspaces-${FIRST}`), {
        recursive: true
      })
