...
const MANAGED_CODEX_HOME = 'C:\\Kolux Managed\\codex-runtime-home'
...
for (const shell of WINDOWS_POWERSHELLS) {
  describe.runIf(isAvailable(shell))(`${shell} managed home bootstrap`, () => {
    it.each(['FullLanguage', 'ConstrainedLanguage'] as const)(
      ...
      (languageMode) => {
        const cwd = mkdtempSync(join(tmpdir(), 'kolux-powershell-clm-'))
        try {
          expect(runBootstrap(shell, languageMode, cwd)).toContain(
            `mode=${languageMode};codexHome=${MANAGED_CODEX_HOME};koluxHome=${MANAGED_CODEX_HOME};startupCount=2;cwd=${cwd}`
          )
        } finally {
...
function runBootstrap(
  ...
  const launch = resolveWindowsShellLaunchArgs(
    ...
    undefined,
    '$env:KOLUX_TEST_STARTUP_COUNT = 1 + [int]$env:KOLUX_TEST_STARTUP_COUNT'
  )
  ...
  return execFileSync(
    ...
    ['-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', harness],
    {
      ...
      env: {
        ...
        KOLUX_CODEX_HOME: MANAGED_CODEX_HOME,
        KOLUX_TEST_BOOTSTRAP: encodedCommand,
        KOLUX_TEST_LANGUAGE_MODE: languageMode
      },
      windowsHide: true
    }
  )
...
$initialState.LanguageMode = $env:KOLUX_TEST_LANGUAGE_MODE
...
$bootstrap = [Text.Encoding]::Unicode.GetString(
  [Convert]::FromBase64String($env:KOLUX_TEST_BOOTSTRAP)
)
...
$runner.Commands.Clear()
$runner.AddScript(
  '"mode=$($ExecutionContext.SessionState.LanguageMode);codexHome=$env:CODEX_HOME;koluxHome=$env:KOLUX_CODEX_HOME;startupCount=$env:KOLUX_TEST_STARTUP_COUNT;cwd=$($PWD.Path)"'
).Invoke()
$runner.Dispose()
$runspace.Dispose()
`)
