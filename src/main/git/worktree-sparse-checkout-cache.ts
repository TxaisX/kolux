...
// already skips rows where that's set), but Kolux's compatibility baseline is Git 2.25, which predates it —
...
//  - Kolux-driven remove/move: explicit calls below (worktree-removal.ts, worktree-move.ts).
...
/** Drop one worktree's cached state; call when Kolux itself removes or moves a worktree path. */
