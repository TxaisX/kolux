...
function fencedProbeStdout(command: unknown, payload: string): string {
  const nonce = /__KOLUX_WSL_CAPTURE_BEGIN_([^_]+)__/.exec(String(command))?.[1] ?? ''
  return `profile banner\n__KOLUX_WSL_CAPTURE_BEGIN_${nonce}__${payload}__KOLUX_WSL_CAPTURE_END_${nonce}__`
}
...
describe('WSL direct Git reads', () => {
  ...
  it('fences parsed output from a barrier Git login-shell fallback', async () => {
    await withPlatform('win32', async () => {
      ...
      spawnMock.mockImplementation((_command, args) => {
        ...
        queueMicrotask(() => {
          const capturedCommand = args?.find((arg) =>
            String(arg).includes('__KOLUX_WSL_CAPTURE_BEGIN_')
          )
          const fenced = fencedProbeStdout(capturedCommand, 'fork-point\n')
          const echoedMarker = fenced.match(/__KOLUX_WSL_CAPTURE_BEGIN_[^_]+__/)?.[0] ?? ''
      ...
      expect(spawnMock.mock.calls[0]?.[1]?.join(' ')).toContain('__KOLUX_WSL_CAPTURE_BEGIN_')
    })
  })
        ...
        if (spawnMock.mock.calls.length === 1) {
          queueMicrotask(() => {
            const marker = String(args?.join(' ')).match(
              /(__KOLUX_WSL_PROCESS_GROUP_[0-9a-f-]+__=)/
            )?.[1]
  ...
  it('waits for a cold probe so the first read already skips the shell', async () => {
    await withPlatform('win32', async () => {
      execFileMock.mockImplementation((_command, args, _options, callback) => {
        const child = createMockChild()
        if (String(args).includes('_kolux_git_path')) {
          setTimeout(
