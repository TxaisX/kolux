...
// accepts `worktree remove --force` on a path Kolux just renamed away.
...
beforeEach(async () => {
  // realpath: macOS hands out /var/... temp paths while Git reports /private/var/..., and Kolux
  // matches the worktree it is removing against Git's own list.
  scratchDir = await realpath(await mkdtemp(join(tmpdir(), 'kolux-deferred-worktree-removal-')))
