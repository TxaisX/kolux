// resolveWorktreeAddTimeoutMs: KOLUX_WORKTREE_ADD_TIMEOUT_MS parsing, clamping, and warnings.
...
describe('resolveWorktreeAddTimeoutMs', () => {
  ...
  it('falls back to the default when the override is unset, blank, or unparseable', () => {
    ...
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '   ' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: 'nope' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
  })

  it('raises the timeout up to the closed maximum', () => {
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '300000' })).toBe(300_000)
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '300000.9' })).toBe(300_000)
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '999999999' })).toBe(
      1_800_000
    )
  })
  ...
  it('treats an infinite override as the maximum, not the default', () => {
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: 'Infinity' })).toBe(
      1_800_000
    )
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '1e400' })).toBe(1_800_000)
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '-Infinity' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
  ...
  it('stays quiet when the value is unset, blank, or lands in range', () => {
    resolveWorktreeAddTimeoutMs({})
    resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '   ' })
    resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '600000' })
    resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '300000.9' })
  ...
  it('warns with the range when it clamps an out-of-range value', () => {
    resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '300' })

    expect(warnSpy).toHaveBeenCalledTimes(1)
    // Why: pin the prefix and variable name too — a warning nobody can grep for is not a diagnostic.
    expect(warnSpy).toHaveBeenCalledWith(
      '[git/worktree] KOLUX_WORKTREE_ADD_TIMEOUT_MS="300" is outside [180000, 1800000]ms; using 180000ms'
    )
  })
  ...
  it('warns that an unparseable value is not a number rather than out of range', () => {
    resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '600_000' })
  ...
  it('never lowers the timeout below the stall-guard default', () => {
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '300' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '0' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
    expect(resolveWorktreeAddTimeoutMs({ KOLUX_WORKTREE_ADD_TIMEOUT_MS: '-1' })).toBe(
      WORKTREE_ADD_TIMEOUT_MS
    )
  })
})
