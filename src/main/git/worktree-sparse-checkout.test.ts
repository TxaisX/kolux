...
async function createRepoWithTwoDirs(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), 'kolux-sparse-checkout-'))
  tempRoots.push(root)
...
describe('sparse-checkout cache invalidation across the worktree lifecycle', () => {
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      // Removing through Kolux's own removeWorktree (not a raw `git worktree remove`) exercises the
