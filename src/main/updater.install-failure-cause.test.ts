...
const DEB_ELEVATION_ERROR =
  'Error: Command failed: /usr/bin/pkexec --disable-internal-agent "/bin/bash" "-c" "dpkg -i \'/home/u/.cache/kolux-updater/pending/kolux-ide_1.4.163_amd64.deb\'"\npkexec must be setuid root'
...
const WINDOWS_SIGNATURE_MISMATCH_ERROR =
  'New version 1.4.163 is not signed by the application owner: publisherNames: Kolux, Inc.'
 ...
 * re-dispatches it through the 'error' event *synchronously* inside `quitAndInstall()`. Kolux
  ...
  it('redacts the cause the same way the retained-package card does', async () => {
    ...
    autoUpdaterMock.quitAndInstall.mockImplementation(() => {
      autoUpdaterMock.emit(
        'error',
        new Error(
          `${escape}[31mdpkg -i '${home}/.cache/kolux-updater/pending/kolux.deb'${escape}[0m`
        )
      )
    })
    ...
    expect(message).toContain("<home>/.cache/kolux-updater/pending/kolux.deb'")
  })
