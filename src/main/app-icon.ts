...
import watercolorIcon from '../../resources/app-icons/kolux-watercolor.png?asset'
import watercolorMacDockIcon from '../../resources/app-icons/kolux-watercolor.png?asset&asarUnpack'
import blueIcon from '../../resources/app-icons/kolux-blue.png?asset'
import blueMacDockIcon from '../../resources/app-icons/kolux-blue.png?asset&asarUnpack'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'set appPath to system attribute "KOLUX_APP_BUNDLE_PATH"',
  'set iconPath to system attribute "KOLUX_APP_ICON_PATH"',
  "set image to current application's NSImage's alloc()'s initWithContentsOfFile:iconPath",
  'if image is missing value then error "Kolux app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Kolux app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Kolux app icon could not be cleared"'
]
...
function runMacCustomIconCommand(
  ...
  return runBoundedMacDockIconCommand({
    ...
    options: {
      env: {
        ...process.env,
        KOLUX_APP_BUNDLE_PATH: appBundlePath,
        KOLUX_APP_ICON_PATH: iconPath
      }
    },
