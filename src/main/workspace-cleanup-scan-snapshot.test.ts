...
const SNAPSHOT_FILE = 'kolux-workspace-cleanup-scan.json'
...
describe('workspace cleanup scan snapshot', () => {
  beforeEach(async () => {
    ...
    userDataDirHolder.dir = await mkdtemp(join(tmpdir(), 'kolux-cleanup-snapshot-'))
  })
  ...
  it('keeps profile snapshots isolated', async () => {
    const otherProfile = await mkdtemp(join(tmpdir(), 'kolux-cleanup-snapshot-other-'))
    try {
      await persistWorkspaceCleanupScanResult(
        userDataDirHolder.dir,
        { includeAllWorkspaces: true },
