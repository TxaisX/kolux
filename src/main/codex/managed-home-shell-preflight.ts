...
function resolveAccountManagedHome(candidate: string, userDataPath: string): string | null {
  ...
  try {
    return readFileSync(join(candidate, '.kolux-managed-home'), 'utf-8').trim() === accountId
      ? expectedHome
      : null
  } catch {
    return null
...
export function resolveManagedCodexShellPreflightHome(
  ...
  if (!codexHome || !koluxCodexHome || !pathsEqual(codexHome, koluxCodexHome)) {
    return null
  }
...
export async function prepareManagedCodexHomeBeforeShellLaunch(args: {
  ...
  const env = args.env ?? {
    ...
    KOLUX_CODEX_HOME: process.env.KOLUX_CODEX_HOME
  }
