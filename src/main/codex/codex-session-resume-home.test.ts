...
describe('resolveTrustedCodexSessionResumeHome', () => {
  ...
  it('requires the transcript provenance to name a regular rollout file', () => {
    const homePath = mkdtempSync(join(tmpdir(), 'kolux-codex-resume-home-'))
...
describe('claimsCodexRolloutLayout', () => {
  ...
  it('is true for a rollout under a home Kolux no longer trusts, so resume cannot silently fall through to the selected account', () => {
    expect(
...
describe('resolveCodexSessionResumeProvenance', () => {
  function writeRollout(sessionId: string): { homePath: string; rolloutPath: string } {
    const homePath = mkdtempSync(join(tmpdir(), 'kolux-codex-resume-provenance-'))
    tempRoots.push(homePath)
  ...
  it('starts fresh for a rollout file that really exists under a home Kolux no longer trusts', async () => {
    ...
    const trusted = mkdtempSync(join(tmpdir(), 'kolux-codex-resume-provenance-'))
