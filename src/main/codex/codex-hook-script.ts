...
export function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current Kolux so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  unset KOLUX_AGENT_HOOK_TRANSPORT',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only Kolux's known assignments since cmd.exe `set` lines aren't shell syntax.
    ...
    '          "set KOLUX_AGENT_HOOK_PORT="*) KOLUX_AGENT_HOOK_PORT=${endpoint_line#*=} ;;',
    '          "set KOLUX_AGENT_HOOK_TOKEN="*) KOLUX_AGENT_HOOK_TOKEN=${endpoint_line#*=} ;;',
    '          "set KOLUX_AGENT_HOOK_ENV="*) KOLUX_AGENT_HOOK_ENV=${endpoint_line#*=} ;;',
    '          "set KOLUX_AGENT_HOOK_VERSION="*) KOLUX_AGENT_HOOK_VERSION=${endpoint_line#*=} ;;',
    '          "set KOLUX_AGENT_HOOK_TRANSPORT="*) KOLUX_AGENT_HOOK_TRANSPORT=${endpoint_line#*=} ;;',
    '        esac',
    '      done < "$endpoint_path"',
    '      ;;',
    '    *)',
    ...
    '  load_hook_endpoint "$KOLUX_AGENT_HOOK_ENDPOINT"',
    'fi',
