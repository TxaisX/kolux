...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'kolux-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'kolux-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses KOLUX_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
    ...
    process.env.KOLUX_USER_DATA_PATH = userDataDir
    try {
      const { getKoluxManagedCodexHomePath: getCliSafeManagedPath } =
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(managedHomePath, '.kolux-resource-copies', 'AGENTS.md.json'), {
        recursive: true
      })
  ...
  it('removes an unowned copy when recording copy ownership fails', () => {
    ...
    mkdirSync(managedHomePath, { recursive: true })
    writeFileSync(join(managedHomePath, '.kolux-resource-copies'), 'blocks marker directory\n')
