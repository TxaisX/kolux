...
const LEGACY_KOLUX_PROFILE_LINES = [
  '# BEGIN KOLUX AGENT STATUS HOOKS',
  '[[hooks.PermissionRequest]]',
  '[[hooks.PermissionRequest.hooks]]',
  'type = "command"',
  'command = "codex-hook"',
  '# END KOLUX AGENT STATUS HOOKS',
  ''
]
...
describe('CodexHookService', () => {
  it('removes legacy Kolux-managed hooks from system ~/.codex during install', async () => {
  ...
  it('removes very large legacy Kolux-managed hook lists from system ~/.codex', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyCommand = legacyManagedHookCommand()
    mkdirSync(systemCodexHome, { recursive: true })
  ...
  it('removes the legacy Kolux Codex profile file when it only contains managed hooks', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'kolux-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(profilePath, LEGACY_KOLUX_PROFILE_LINES.join('\n'), 'utf-8')
  ...
  it('removes only the legacy Kolux block from a user-edited Codex profile file', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'kolux-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      profilePath,
      ['model = "gpt-5.5"', '', ...LEGACY_KOLUX_PROFILE_LINES].join('\n'),
      'utf-8'
    )
    ...
    expect(profileConfig).not.toContain('KOLUX AGENT STATUS HOOKS')
  ...
  it('cleans duplicate Codex hook representations while keeping status hooks in runtime CODEX_HOME', async () => {
    ...
    const legacyProfilePath = join(systemCodexHome, 'kolux-agent-status.config.toml')
    const legacyCommand = legacyManagedHookCommand()
    ...
    writeFileSync(legacyProfilePath, LEGACY_KOLUX_PROFILE_LINES.join('\n'), 'utf-8')
