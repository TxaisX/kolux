// Opt in: KOLUX_WORKSPACE_SNAPSHOT_PRUNE_BENCH=1 pnpm exec vitest run --config config/vitest.config.ts src/main/workspace-snapshot-pruning.bench.test.ts
...
const describeBench = process.env.KOLUX_WORKSPACE_SNAPSHOT_PRUNE_BENCH ? describe : describe.skip
const ROW_COUNT = 610
...
describeBench('workspace snapshot bulk pruning', () => {
  ...
  beforeAll(async () => {
    snapshotDirectory = await mkdtemp(join(tmpdir(), 'kolux-snapshot-prune-bench-'))
  })
  ...
  it('prunes the measured 610-row/100-delete shape with one write per sidecar', async () => {
    ...
    const cleanupPayload = JSON.parse(
      await readFile(join(snapshotDirectory, 'kolux-workspace-cleanup-scan.json'), 'utf-8')
    ) as { result: { candidates: unknown[] } }
    const spacePayload = JSON.parse(
      await readFile(join(snapshotDirectory, 'kolux-workspace-space-analysis.json'), 'utf-8')
