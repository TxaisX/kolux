...
describe('ephemeral VM runtime service', () => {
  ...
  it('persists a successful recipe-created runtime and cleans it up', async () => {
    const userDataPath = makeDir('kolux-ephemeral-vm-service-user-data-')
    const repoPath = makeDir('kolux-ephemeral-vm-service-repo-')
    ...
    upsertEphemeralVmRuntime(userDataPath, {
      ...
      recipeResult: {
        ...
        connection: {
          type: 'ssh',
          projectRoot: '/workspace/repo',
          target: { label: 'VM', host: 'host', port: 22, username: 'kolux' }
        }
      }
    })
    const cleanupArgs = {
