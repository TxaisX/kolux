...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('KOLUX_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('KOLUX_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after Kolux itself creates a PR so the
