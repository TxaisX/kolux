...
describe('Gitea client', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV }
    process.env.KOLUX_GITEA_TOKEN = 'gitea-token'
  ...
  it('uses an API base URL override for subpath or non-standard deployments', async () => {
    process.env.KOLUX_GITEA_API_BASE_URL = 'https://git.example.com/code'
  ...
  it('verifies token auth when a global API base URL is configured', async () => {
    process.env.KOLUX_GITEA_API_BASE_URL = 'https://git.example.com'
