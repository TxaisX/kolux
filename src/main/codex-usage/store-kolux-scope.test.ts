...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for Kolux scope when only non-Kolux Codex usage exists', async () => {
  ...
  it('uses only Kolux-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('kolux', '30d', 'model')
    const recentSessions = await store.getRecentSessions('kolux', '30d', 10)
