...
const PRODUCTION_ARTIFACTS_API_URL = 'https://share.kolux.invalid'
...
export function resolveArtifactCloudApiUrl(
  ...
  const candidate = override?.trim() || env.KOLUX_ARTIFACTS_API_URL?.trim()
  const url = new URL(candidate || PRODUCTION_ARTIFACTS_API_URL)
  const loopback = ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname)
  const firstParty = url.hostname === 'kolux.invalid' || url.hostname.endsWith('.kolux.invalid')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use a kolux.invalid or loopback host.')
  }
