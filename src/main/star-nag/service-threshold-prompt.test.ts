...
describe('StarNagService', () => {
  ...
  it('shows the browser fallback when checkKoluxStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkKoluxStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkKoluxStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkKoluxStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
