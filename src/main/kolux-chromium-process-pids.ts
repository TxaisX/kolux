...
/**
 * PIDs of Kolux's own Chromium processes — browser, renderers, GPU, utilities.
 *
 ...
 * kill in every field Kolux records (#10680). A pid in this set is proof the
 ...
 * (main-process-preflight). The standalone daemon installs none and `koluxd`
 ...
 * the *killing* process's own pid. Kolux's Chromium processes are children of
 * Electron main, so they never classify `own` from a daemon or koluxd host, and
 * on an SSH/serve host there is no Chromium on the machine at all.
 */
export function readKoluxChromiumProcessPids(): ReadonlySet<number> {
