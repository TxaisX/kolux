...
// socket (nodejs/undici#5360, kolux#8695). This applies to every Node process
...
const AUDITED_GLOBAL_FETCH_LINES = new Map<string, number>([
  ...
  ['main/kolux-profiles/profile-cloud-client.ts', 1],
  ['main/kolux-profiles/profile-cloud-org-members-client.ts', 1],
  ...
  ['main/mimo/status-plugin/status-plugin-post-source.ts', 1],
  ...
  it('keeps every global-fetch line audited with its expected count', () => {
    ...
    expect(
      ...
      'Global fetch (bare, globalThis.fetch, or global.fetch) uses undici, ' +
        'where an unread response body can crash the whole process (kolux#8695). ' +
