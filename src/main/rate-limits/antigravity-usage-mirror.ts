...
// fetcher reads, so Kolux never actually queries Antigravity. Only a *successful* Gemini read
...
const ANTIGRAVITY_NO_SIGN_IN_REASON =
  'Antigravity usage is not available. Kolux can only show shared Google Code Assist quota while a Gemini CLI sign-in is connected.'
...
const ANTIGRAVITY_QUOTA_UNREADABLE_REASON =
  'Antigravity usage is not available. Kolux reads it from the shared Google Code Assist quota, which could not be read right now.'
