...
import { RuntimeEmulatorCommands } from '../runtime/kolux-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Kolux-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Kolux-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Kolux-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
