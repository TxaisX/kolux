...
const MAC_OPEN_SHIM_DIR = join(tmpdir(), 'kolux-serve-sim-open-shim')
...
function getServeSimEnv(executable: ServeSimExecutable): NodeJS.ProcessEnv {
  ...
  if (openShimDir) {
    // Why: serve-sim needs Simulator.app attached for display/rotation, but Kolux embeds the stream.
...
export async function execServeSimCommand(
  ...
  if (result.code !== 0 || result.timedOut) {
    ...
    if (/no serve-sim server|not running/i.test(message)) {
      throw new EmulatorError(
        'emulator_no_active',
        'No active emulator for this worktree — use kolux emulator list/attach or open the pane'
      )
    }
