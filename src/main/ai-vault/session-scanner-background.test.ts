...
const originalBackend = process.env.KOLUX_AI_VAULT_SERVICE_PROCESS
const originalNodeEnv = process.env.NODE_ENV

afterEach(() => {
  if (originalBackend === undefined) {
    delete process.env.KOLUX_AI_VAULT_SERVICE_PROCESS
  } else {
    process.env.KOLUX_AI_VAULT_SERVICE_PROCESS = originalBackend
  }
...
describe('shouldUseAiVaultServiceProcess', () => {
  ...
  it.each([
    ...
    process.env.KOLUX_AI_VAULT_SERVICE_PROCESS = value
