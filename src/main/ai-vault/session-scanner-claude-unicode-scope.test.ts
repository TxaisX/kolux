...
// production encoder: these tests exist to catch Kolux drifting away from how
...
describe('scanAiVaultSessions — non-ASCII scope paths', () => {
  it('lists Claude sessions when the workspace path is NFD and the transcript is NFC', async () => {
    // Regression for #10832: macOS hands Kolux decomposed (NFD) workspace paths
    ...
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-nfc-scope-'))
    tempRoots.push(root)
  ...
  it('lists Claude sessions for a Windows workspace whose path has uppercase segments', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-win-scope-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
