...
describe('parseCodexSessionFile', () => {
  it('uses completed user items for paginated session metadata', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-codex-paginated-'))
    tempRoots.push(root)
  ...
  it('uses user-message events instead of later injected user-role records', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-codex-last-prompt-'))
    tempRoots.push(root)
  ...
  it('does not double-count usage when token count formats switch', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-codex-token-switch-'))
    tempRoots.push(root)
  ...
  it('extracts the model from turn context, latest turn winning', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-ai-vault-codex-model-'))
    tempRoots.push(root)
