...
describe('shell startup identity scanner', () => {
  it('strips a split identity marker and returns its shell pid', () => {
    ...
    expect(scanForShellStartupIdentity(state, 'before\x1b]777;kolux-shell-st')).toEqual({
      output: 'before',
      shellPid: null
    })
  ...
  it('forwards lookalikes unchanged', () => {
    const state = createShellStartupIdentityScanState()
    const input = 'a\x1b]777;kolux-shell-start:nope\x07b'
  ...
  it('releases an incomplete marker on teardown', () => {
    const state = createShellStartupIdentityScanState()
    scanForShellStartupIdentity(state, '\x1b]777;kolux-shell-start:12')
    expect(drainShellStartupIdentityHeldBytes(state)).toBe('\x1b]777;kolux-shell-start:12')
  })

  it('does not retain an unbounded digit stream', () => {
    const state = createShellStartupIdentityScanState()
    const input = `\x1b]777;kolux-shell-start:${'1'.repeat(100)}`
