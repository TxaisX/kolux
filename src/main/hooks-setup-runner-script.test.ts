...
describe('createSetupRunnerScript', () => {
  ...
  it('writes POSIX setup runners for shebang-declared scripts on native Windows paths', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\kolux\\setup-runner.sh\n')
  ...
  it('keeps batch setup scripts on cmd.exe when the terminal is Git Bash', async () => {
    ...
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\kolux\\setup-runner.cmd\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    writeFileSyncMock.mockClear()
    const originalPlatform = process.platform
    ...
    try {
      ...
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:\\repo\\.git\\kolux\\setup-runner.cmd',
  ...
  it('keeps POSIX runner behavior on POSIX platforms', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('/test/repo/.git/kolux/setup-runner.sh\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    const chmodSyncMock = vi.mocked(fs.chmodSync)
    writeFileSyncMock.mockClear()
    ...
    try {
      ...
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        '/test/repo/.git/kolux/setup-runner.sh',
        '#!/usr/bin/env bash\nset -e\npnpm install\n',
        'utf-8'
      )
      expect(chmodSyncMock).toHaveBeenCalledWith('/test/repo/.git/kolux/setup-runner.sh', 0o755)
  ...
  it('marks setup-runner terminals for the always-on credential guard', async () => {
    ...
    expect(setup.envVars).toMatchObject({
      ...
      KOLUX_WORKTREE_PATH: '/test/worktree',
      KOLUX_INTERNAL_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY: 'guard'
    })
  })
})
