/**
 * Agent/tool preflight detection. Split out of `ipc/preflight.ts` so the Kolux
 ...
 * Refresh — handles the "installed a new CLI, Kolux doesn't see it yet" case
...
async function executePreflightCheck(
  ...
  if (force) {
    ...
    // runs `glab auth login` for a self-hosted host after Kolux starts would
