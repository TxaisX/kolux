...
const CHILD = process.env.KOLUX_SKILL_BUNDLE_PROCESS_CHILD === '1'
...
describe.runIf(CHILD)('skill bundle process termination child', () => {
  it('extracts a bundle until the parent terminates this process', async () => {
    const root = process.env.KOLUX_SKILL_BUNDLE_CRASH_ROOT
    const marker = process.env.KOLUX_SKILL_BUNDLE_CRASH_MARKER
    if (!root || !marker) {
    ...
    await installSkillBundle({
      ...
      koluxStateDirectory: join(root, 'state'),
      detectedProviders: [],
