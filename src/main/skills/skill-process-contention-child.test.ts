...
const CHILD = process.env.KOLUX_SKILL_CONTENTION_CHILD === '1'
...
describe.runIf(CHILD)('skill process contention child', () => {
  it('runs one transaction process', async () => {
    const root = process.env.KOLUX_SKILL_CONTENTION_ROOT
    const archivePath = process.env.KOLUX_SKILL_CONTENTION_ARCHIVE
    const resultPath = process.env.KOLUX_SKILL_CONTENTION_RESULT
    const role = process.env.KOLUX_SKILL_CONTENTION_ROLE
    ...
    const result = await installLocalSkillPackage(
      ...
      },
      {
        onJournalTransition: async (phase, boundary) => {
          ...
          const readyPath = process.env.KOLUX_SKILL_CONTENTION_READY
          const releasePath = process.env.KOLUX_SKILL_CONTENTION_RELEASE
