...
const CHILD = process.env.KOLUX_SKILL_UPLOAD_PROCESS_CHILD === '1'
...
async function stopAtBoundary(uploadId: string): Promise<void> {
  const marker = process.env.KOLUX_SKILL_UPLOAD_RESTART_MARKER
  if (!marker) {
...
describe.runIf(CHILD)('skill upload process restart child', () => {
  it('stops at the requested upload boundary', async () => {
    const root = process.env.KOLUX_SKILL_UPLOAD_RESTART_ROOT
    const boundary = process.env.KOLUX_SKILL_UPLOAD_RESTART_BOUNDARY
    if (!root || !boundary) {
