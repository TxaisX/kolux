...
describe('plugin skill candidate scan', () => {
  it('stops at the package candidate budget and marks the scan incomplete', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-skill-scan-'))
    temporaryDirectories.push(root)
    await Promise.all(
      ['one', 'two'].map(async (vendor) => {
        await mkdir(join(root, vendor, 'kolux-cli'), { recursive: true })
        await writeFile(join(root, vendor, 'kolux-cli', 'SKILL.md'), '# Kolux CLI\n')
      })
    )

    const result = await scanKnownPluginSkillCandidates(root, new Set(['kolux-cli']), {
      maximumCandidates: 1
    })
  ...
  it('stops at the entry budget and reports the truncation at the scan root', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-entry-limit-'))
    temporaryDirectories.push(root)
    await Promise.all(
      ['one', 'two'].map(async (vendor) => {
        await mkdir(join(root, vendor, 'kolux-cli'), { recursive: true })
    ...
    expect(result.candidates).toEqual([{ name: 'kolux-cli', path: join(root, 'one', 'kolux-cli') }])
  ...
  it('stops walking the directory whose read crossed the entry budget', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-entry-limit-stop-'))
    temporaryDirectories.push(root)
  ...
  async function createManifestWithMissingSkillRoots(): Promise<{
    ...
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-entry-limit-declared-'))
    temporaryDirectories.push(root)
    ...
    const candidate = join(packageRoot, 'a-skills', 'kolux-cli')
    ...
    await writeFile(join(candidate, 'SKILL.md'), '# Kolux CLI\n')
  ...
  it('stops at the entry budget while resolving declared skill roots', async () => {
    ...
    expect(result.candidates).toEqual([{ name: 'kolux-cli', path: candidate }])
    expect(result.issues).toEqual([{ path: root, reason: 'entry-limit', errorCode: null }])
  })
  ...
  it('completes a real-shaped Codex cache without reporting coverage issues', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-real-shape-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'openai-bundled', 'sites', '0.1.31')
    const skill = join(packageRoot, 'skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
    ...
    await writeFile(join(skill, 'SKILL.md'), '# Kolux CLI\n')

    const result = await scanKnownPluginSkillCandidates(root, new Set(['kolux-cli']))

    expect(result).toEqual({ candidates: [{ name: 'kolux-cli', path: skill }], issues: [] })
  })

  it('does not emit a plugin directory that only shares a skill name', async () => {
    // The cached plugin is itself called kolux-cli. Only the SKILL.md below it is a skill.
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-name-collision-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'openai-bundled', 'kolux-cli', '1.0.0')
    const skill = join(packageRoot, 'skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
  ...
  it('does not emit a bare known-name directory that carries no SKILL.md', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-bare-name-'))
    temporaryDirectories.push(root)
    await mkdir(join(root, 'vendor', 'kolux-cli', 'assets'), { recursive: true })
  ...
  it.each([
    ...
    async (intermediateDepth, expectFound) => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-nested-depth-'))
      temporaryDirectories.push(root)
      const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
      ...
      const candidate = join(parent, 'kolux-cli')
      await mkdir(candidate, { recursive: true })
      await writeFile(join(candidate, 'SKILL.md'), '# Kolux CLI\n')

      const result = await scanKnownPluginSkillCandidates(root, new Set(['kolux-cli']))

      expect(result.candidates).toEqual(expectFound ? [{ name: 'kolux-cli', path: candidate }] : [])
  ...
  it('reports a depth-truncated subtree as scan coverage instead of a skill candidate', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-skill-depth-'))
    temporaryDirectories.push(root)
    const segments = Array.from({ length: 11 }, (_, index) => `level-${index}`)
    const hiddenSkill = join(root, ...segments, 'kolux-cli')
  ...
  it('does not scan dependency packages for plugin skill entrypoints', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-dependencies-'))
    temporaryDirectories.push(root)
    await mkdir(
      join(
        root,
  ...
  it('scans only declared Codex plugin skill roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-roots-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'group', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await mkdir(
    ...
    expect(result).toEqual({
      candidates: [{ name: 'kolux-cli', path: candidate }],
      issues: []
    })
  })

  it('uses the default skills root for compatible manifests without a skills field', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-default-root-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'skills', 'nested', 'kolux-cli')
  ...
  it('falls back to traversal when a manifest skills path is invalid', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-invalid-declared-root-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(
      join(packageRoot, '.codex-plugin', 'plugin.json'),
  ...
  it('does not traverse plugin payload when the default skills root is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-missing-default-root-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
  ...
  it('does not traverse plugin payload when the manifest declares no skill roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-empty-skill-roots-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
  ...
  it('discovers nested skill packages recursively within declared roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-skill-boundary-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const skillRoot = join(packageRoot, 'skills', 'sites-building')
    await mkdir(join(packageRoot, '.claude-plugin'), { recursive: true })
    await mkdir(join(skillRoot, 'templates', 'kolux-cli'), { recursive: true })
    ...
    await writeFile(join(skillRoot, 'templates', 'kolux-cli', 'SKILL.md'), '# Kolux CLI\n')

    const result = await scanKnownPluginSkillCandidates(root, new Set(['kolux-cli']))

    expect(result).toEqual({
      candidates: [{ name: 'kolux-cli', path: join(skillRoot, 'templates', 'kolux-cli') }],
      issues: []
    })
  })

  it('rejects Windows parent traversal without hiding the default skills root', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-windows-parent-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(
      join(packageRoot, '.codex-plugin', 'plugin.json'),
  ...
  it('falls through empty manifest directories to the first manifest file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-precedence-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
  ...
  it('bounds how many skill roots one manifest can declare', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-budget-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const manifestPath = join(packageRoot, '.codex-plugin', 'plugin.json')
    const candidate = join(packageRoot, 'r0000', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(join(candidate, 'SKILL.md'), '# Kolux CLI\n')
    await writeFile(
  ...
  it('keeps valid roots when a skills array contains an invalid value', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-invalid-root-array-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const defaultCandidate = join(packageRoot, 'skills', 'kolux-cli')
    const declaredCandidate = join(packageRoot, 'custom-skills', 'kolux-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    ...
    await writeFile(join(declaredCandidate, 'SKILL.md'), '# Kolux CLI\n')

    const result = await scanKnownPluginSkillCandidates(root, new Set(['kolux-cli']))

    expect(result).toEqual({
      candidates: [{ name: 'kolux-cli', path: declaredCandidate }],
      issues: []
    })
  })

  it('does not reset the depth budget across nested plugin manifests', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-nested-manifests-'))
    temporaryDirectories.push(root)
    ...
    const hiddenCandidate = join(pluginRoot, 'kolux-cli')
    await mkdir(hiddenCandidate, { recursive: true })
    await writeFile(join(hiddenCandidate, 'SKILL.md'), '# Kolux CLI\n')
  ...
  it('ignores non-directory manifest markers when selecting precedence', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-marker-file-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'kolux-cli')
  ...
  it('reports manifests that exceed the bounded read limit', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-large-manifest-'))
    temporaryDirectories.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-symlink-error-'))
      temporaryDirectories.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-issue-limit-'))
      temporaryDirectories.push(root)
      await Promise.all(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await createCacheBehindSpentBudget('kolux-plugin-attention-eviction-', 1)
  ...
  it('reports the bound that ended the walk even with the issue budget spent', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-truncating-issue-'))
    temporaryDirectories.push(root)
    await Promise.all(
  ...
  it('keeps scanning past the issue budget', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-issue-budget-coverage-'))
    temporaryDirectories.push(root)
    const candidate = join(root, 'zz-package', 'skills', 'kolux-cli')
    await Promise.all(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-issue-budget-candidate-'))
      temporaryDirectories.push(root)
      const packageRoot = join(root, 'zz-package')
      const candidate = join(packageRoot, 'skills', 'kolux-cli')
      await Promise.all(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-declared-dangling-symlink-'))
      temporaryDirectories.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const parent = await mkdtemp(join(tmpdir(), 'kolux-plugin-symlink-outside-'))
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
      const outside = join(parent, 'outside')
      const linkPath = join(root, 'vendor')
      await mkdir(join(outside, 'kolux-cli'), { recursive: true })
      await mkdir(root, { recursive: true })
      await writeFile(join(outside, 'kolux-cli', 'SKILL.md'), '# Kolux CLI\n')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const parent = await mkdtemp(join(tmpdir(), 'kolux-plugin-skill-file-outside-'))
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
      const skill = join(root, 'vendor', 'kolux-cli')
      const outsideSkillFile = join(parent, 'outside', 'SKILL.md')
      await mkdir(skill, { recursive: true })
      await mkdir(join(parent, 'outside'), { recursive: true })
      await writeFile(outsideSkillFile, '# Kolux CLI\n')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const parent = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-outside-'))
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-plugin-manifest-fifo-'))
      temporaryDirectories.push(root)
      const manifestPath = join(root, '.codex-plugin', 'plugin.json')
      await mkdir(join(root, '.codex-plugin'), { recursive: true })
