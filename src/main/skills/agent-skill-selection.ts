  ...
  for (const selector of selectors) {
    ...
    if (named.length === 0) {
      throw new AgentSkillSharingError(
        AGENT_SKILL_SELECTOR_NOT_FOUND_CODE,
        `Installed skill "${selector}" was not found. Run \`kolux skills installed\` to list valid selectors.`,
        { selector }
      )
    }
    if (named.length > 1) {
      throw new AgentSkillSharingError(
        AGENT_SKILL_SELECTOR_AMBIGUOUS_CODE,
        `More than one installed skill is named "${selector}". Use its discovery ID from \`kolux skills installed\`.`,
