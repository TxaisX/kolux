...
const CHILD = process.env.KOLUX_SKILL_PROCESS_CHILD === '1'
...
async function stopAtBoundary(phase: string, boundary: string): Promise<void> {
  if (
    phase !== process.env.KOLUX_SKILL_CRASH_PHASE ||
    boundary !== process.env.KOLUX_SKILL_CRASH_BOUNDARY
  ) {
    return
  }
  const marker = process.env.KOLUX_SKILL_CRASH_MARKER
  if (!marker) {
...
describe.runIf(CHILD)('skill process termination child', () => {
  it('stops at the requested durable boundary', async () => {
    const root = process.env.KOLUX_SKILL_CRASH_ROOT
    if (!root) {
      throw new Error('missing-crash-root')
    }
    const destinationRoot = join(root, 'skills')
    const stateDirectory = join(root, 'state')
    if (process.env.KOLUX_SKILL_CRASH_OPERATION === 'extract') {
    ...
    if (process.env.KOLUX_SKILL_CRASH_OPERATION === 'remove') {
