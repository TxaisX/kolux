...
export async function scanKnownPluginSkillCandidates(
  ...
  function recordIssue(
    ...
    // date", so evicting one for display budget makes Kolux report all-clear over a read
    ...
    // Kolux's own bounds no longer raise attention, it would do so silently.
  ...
  async function visit(
    ...
    // silent — recording it would put Kolux's own traversal rules in the user's dialog.
