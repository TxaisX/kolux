...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'kolux-skill-bundle-test-'))
...
describe('skill bundle creation and extraction', () => {
  ...
  it('rejects conflicting staging roots without changing their imported namespace', async () => {
    ...
    const importedManifest = join(destination, 'dev.kolux.skill-sharing', 'manifest.json')
    await mkdir(join(destination, 'dev.kolux.skill-sharing'), { recursive: true })
  ...
  it('rejects unknown top-level extension namespaces and removes fresh staging', async () => {
    ...
    await writeSkillTarGzip(archivePath, [
      ...
      {
        path: 'dev.kolux.unexpected/manifest.json',
        size: unknown.length,
        executable: false,
        bytes: unknown
      }
