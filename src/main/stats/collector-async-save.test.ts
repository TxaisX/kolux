...
const statsPath = (): string => join(userDataDir, 'kolux-stats.json')
...
describe('StatsCollector async debounced save', () => {
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'kolux-stats-test-'))
    gate.blocked = false
  ...
  it('writes valid JSON via the async fs/promises path on the debounced timer, no stray temp files', async () => {
    ...
    await vi.waitFor(() => {
      expect(readdirSync(userDataDir)).toContain('kolux-stats.json')
    })
