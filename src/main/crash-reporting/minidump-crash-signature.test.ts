...
describe('parseMinidumpCrashSignature', () => {
      ...
      modules: [
        {
          ...
          size: 0x1000,
          name: 'C:\\Program Files\\Kolux\\Kolux.exe'
        },
        {
          base: 0x7ff8_0000_0000n,
          size: 0x10_0000,
          name: 'C:\\Program Files\\Kolux\\chrome_elf.dll'
        }
      ]
    })
  ...
  it('resolves a faulting module past index 1024 on a real macOS image count', () => {
    ...
    const modules = Array.from({ length: 1042 }, (_, index) => ({
      ...
      size: 0x1000,
      name: `/Applications/Kolux.app/Contents/Frameworks/lib${index}.dylib`
    }))
    const { dump } = buildDump({
  ...
  it('still drops the module list when the claimed module count is absurd', () => {
    const { dump } = buildDump({
      ...
      modules: [{ base: 0x7ff7_0000_0000n, size: 0x1000, name: '/opt/kolux/kolux' }]
    })
