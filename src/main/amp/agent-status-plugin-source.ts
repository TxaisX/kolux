export const AMP_PLUGIN_FILE = 'kolux-agent-status.ts'
export const AMP_PLUGIN_MARKER = 'Managed by Kolux. Do not edit; changes may be overwritten.'
...
export function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.KOLUX_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'KOLUX_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'KOLUX_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'KOLUX_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'KOLUX_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    "      console.warn('[kolux-hook] failed to parse Amp endpoint file:', (error as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '  // Why: Amp sessions can outlive a Kolux restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.KOLUX_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.KOLUX_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.KOLUX_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.KOLUX_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '    })',
    '  } catch {',
    '    // Why: Kolux status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Kolux is down and each POST waits for timeout.',
