...
const runRealWsl = process.platform === 'win32' && process.env.KOLUX_REAL_WSL_BANNER_TEST === '1'
...
describe.skipIf(!runRealWsl)('WSL worktree reads carry no shell chatter', () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await wsl("mktemp -d -p /tmp 'kolux-wsl-banner.XXXXXX'")
    await wsl('mkdir -p "$1/dir" && printf \'%s\' "$2" > "$1/file.txt"', fixtureRoot, FILE_CONTENTS)
  }, 120_000)

  afterAll(async () => {
    if (/^\/tmp\/kolux-wsl-banner\.[A-Za-z0-9]+$/u.test(fixtureRoot)) {
