...
describe('readIssueCommand', () => {
  it('prefers the local override over the shared kolux.yaml command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_ISSUE_COMMAND_PATH || path === TEST_REPO_KOLUX_YAML_PATH
    )
  ...
  it('falls back to the shared kolux.yaml command when no local override exists', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation((path) => path === TEST_REPO_KOLUX_YAML_PATH)
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_KOLUX_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
...
describe('writeIssueCommand', () => {
  it('writes only the local override file and keeps .kolux ignored locally', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_GITIGNORE_PATH || path === join(TEST_REPO_PATH, '.kolux')
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX issue-command runner when a shebang declares bash and setup resolves to Git Bash', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\kolux\\issue-command-runner.sh\n')
    const fs = await import('node:fs')
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'kolux/issue-command-runner.sh'],
        { cwd: 'C:\\repo-worktree' }
      )
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:\\repo\\.git\\kolux\\issue-command-runner.sh',
        '#!/usr/bin/env bash\nset -e\ngh issue view 42\n',
        'utf-8'
      )
  ...
  it('keeps a plain issue command on the cmd runner under a Git Bash terminal', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\kolux\\issue-command-runner.cmd\n')
    const originalPlatform = process.platform
    Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'kolux/issue-command-runner.cmd'],
        { cwd: 'C:\\repo-worktree' }
      )
