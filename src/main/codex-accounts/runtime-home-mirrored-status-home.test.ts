...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'kolux-codex-status-home-ud-'))
  testState.home = mkdtempSync(join(tmpdir(), 'kolux-codex-status-home-'))
  ...
  for (const key of ['KOLUX_USER_DATA_PATH', 'CODEX_HOME', 'KOLUX_CODEX_HOME']) {
...
describe('CodexRuntimeHomeService.getMirroredHostHomePathForStatus', () => {
  ...
  it('returns the shared runtime home when a custom CODEX_HOME keeps the mirror lane', async () => {
    ...
    const { CodexRuntimeHomeService } = await import('./runtime-home-service')
    const { getKoluxManagedCodexHomePath } = await import('../codex/codex-home-paths')
    const service = new CodexRuntimeHomeService(createStore([], null) as never)
    ...
    expect(service.getMirroredHostHomePathForStatus()).toEqual({
      kind: 'ready',
      homePath: getKoluxManagedCodexHomePath()
    })
  })
})
