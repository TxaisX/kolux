...
describe('assertOwnedHostCodexManagedHomePath', () => {
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'kolux-managed-home-ownership-'))
  ...
  it('accepts the expected marked account home', () => {
    ...
    writeFileSync(join(accountHome, '.kolux-managed-home'), 'account-1\n', 'utf-8')
  ...
  it('rejects an account path redirected into the sandboxed system Codex home', () => {
    ...
    writeFileSync(join(systemAccountHome, '.kolux-managed-home'), 'account-1\n', 'utf-8')
  ...
  it('rejects a persisted home or marker belonging to another account', () => {
    ...
    mkdirSync(accountHome, { recursive: true })
    writeFileSync(join(accountHome, '.kolux-managed-home'), 'account-2\n', 'utf-8')
