...
describe('STA-4735 an unreadable runtime auth.json must not be written over', () => {
  ...
  it('refuses the write instead of overwriting a token it could not read', async () => {
    ...
    const STALE = createCodexAuthJson('user@example.com', 'acct-1', 'refresh-stale-kolux-copy')
    ...
    // below replaced a freshly rotated refresh token with Kolux's stale copy.
