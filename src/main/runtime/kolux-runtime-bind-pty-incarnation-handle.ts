...
import { KoluxRuntimeWithBuildPtyTerminalSummary } from './kolux-runtime-build-pty-terminal-summary'
import type { PtyIncarnationHandleRecord } from './kolux-runtime-core'
import type { RuntimeLeafRecord, RuntimePtyWorktreeRecord } from './runtime-terminal-state-records'
import { randomUUID } from 'node:crypto'

export class KoluxRuntimeWithBindPtyIncarnationHandle extends KoluxRuntimeWithBuildPtyTerminalSummary {
  ...
  protected rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: KOLUX_TERMINAL_HANDLE is an agent identity, so CLI control survives renderer graph loss while the PTY is alive.
