...
describe('KoluxRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (kolux serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-runtime-rpc-'))
    const server = new KoluxRuntimeRpcServer({
      runtime: new KoluxRuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new KoluxRuntimeRpcServer({
      runtime: new KoluxRuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
  ...
  it('honours a pinned bind host over exposeNetworkByDefault (koluxd --bind)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-runtime-rpc-'))
    const server = new KoluxRuntimeRpcServer({
      runtime: new KoluxRuntimeService(),
      userDataPath,
  ...
  it('stays pinned to loopback even after a device has connected once', async () => {
    ...
    // network-reach device has connected. A loopback koluxd would therefore go wide one
