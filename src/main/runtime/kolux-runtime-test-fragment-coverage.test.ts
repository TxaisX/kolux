...
// because kolux-runtime.test.ts imports them. A fragment left out of that list never runs and
// nothing fails, so the import list is the coverage boundary and has to be checked.
const ENTRYPOINT = 'kolux-runtime.test.ts'
const FRAGMENT_DIR = 'kolux-runtime-tests'

function importedFragments(): string[] {
  const source = readFileSync(join(import.meta.dirname, ENTRYPOINT), 'utf8')
  return [...source.matchAll(/await import\('\.\/kolux-runtime-tests\/([\w-]+)\.spec'\)/g)]
...
describe('Kolux runtime test fragments', () => {
