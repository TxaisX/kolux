...
import { KoluxRuntimeWithGetAgentSessionExecutionNamespace } from './kolux-runtime-get-agent-session-execution-namespace'
import type {
...
export class KoluxRuntimeWithCreateAgentSession extends KoluxRuntimeWithGetAgentSessionExecutionNamespace {
    ...
    const operation = (async (): Promise<RuntimeCreateAgentSessionResult> => {
      ...
      const { agentArgs: resolvedAgentArgs, agentEnv: resolvedAgentEnv } =
        resolveWorktreeAgentLaunchArgs({
          agent: request.agent,
          explicitAgentArgs: request.agentArgs,
      ...
      // KOLUX_TERMINAL_HANDLE, so it is the only name a lost spawn can be re-found by.
      reclaim.identity = {
