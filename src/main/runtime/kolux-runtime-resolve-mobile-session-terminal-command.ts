...
import { KoluxRuntimeWithRunCreateMobileSessionTerminal } from './kolux-runtime-run-create-mobile-session-terminal'
...
export class KoluxRuntimeWithResolveMobileSessionTerminalCommand extends KoluxRuntimeWithRunCreateMobileSessionTerminal {
  protected async resolveMobileSessionTerminalCommand(
    ...
    // Why: SSH runs the CLI through the relay shim (plain `kolux`), so the Linux-only `kolux-ide` rename must not apply.
