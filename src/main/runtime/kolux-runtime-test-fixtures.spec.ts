...
import { InMemoryOrchestrationMessages } from './kolux-runtime-test-orchestration-messages.spec'
import type { OrchestrationDb } from './orchestration/db'
import type { PtyProcessInspection } from '../providers/pty-process-inspection'

type RuntimeService = InstanceType<typeof KoluxRuntimeService>
...
function antigravityReadyScreen(model = 'Gemini 3.5 Flash (High)'): string {
  return [
    ...
    model,
    '~/kolux/workspaces/kolux/agy-dispatch-issue',
    '>'
  ].join('\n')
}
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.KOLUX_TAB_ID).toMatch(UUID_RE)
  const leafId = env.KOLUX_PANE_KEY?.slice(`${env.KOLUX_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.KOLUX_PANE_KEY).toBe(`${env.KOLUX_TAB_ID}:${leafId}`)
  return env.KOLUX_PANE_KEY
}

function createRuntime(): RuntimeService {
  return new KoluxRuntimeService(store)
}
