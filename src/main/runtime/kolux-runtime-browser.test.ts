...
import type { RuntimeBrowserCommandHost } from './kolux-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:kolux-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
  ...
  it('sends the resolved isolated profile partition when creating a renderer tab', async () => {
    ...
    expect(send).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        sessionPartition: 'persist:kolux-browser-session-profile-isolated'
      })
    )
  })
