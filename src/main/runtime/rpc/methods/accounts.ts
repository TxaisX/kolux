...
// Why: `kolux account list` prints only emails and the active ids, so it opts out
...
// `kolux account add` CLI can register accounts on a headless host; it is gated
...
export const ACCOUNT_METHODS: readonly RpcAnyMethod[] = [
  ...
  defineMethod({
    ...
    handler: async (params, { runtime, clientKind }) => {
      ...
      if (clientKind !== undefined) {
        throw new Error('Adding Claude accounts is only available on the Kolux host runtime.')
      }
  ...
  defineMethod({
    ...
    handler: async (params, { runtime, clientKind }) => {
      if (clientKind !== undefined) {
        throw new Error('Adding Codex accounts is only available on the Kolux host runtime.')
      }
