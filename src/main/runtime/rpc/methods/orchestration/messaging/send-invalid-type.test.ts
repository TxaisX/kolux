  ...
  it('explains valid message types and the question reply path', async () => {
    ...
    expect(response).toMatchObject({
      ...
      error: {
        ...
        message:
          'Invalid --type. Expected one of: status, dispatch, worker_done, merge_ready, escalation, handoff, decision_gate, question, heartbeat. To answer a worker question, use the same Kolux CLI executable with orchestration reply --id <msg_id> --body <text>.'
      }
    })
  })
})
