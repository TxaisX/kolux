...
export async function inspectWorkerTerminal(
  ...
  if (structured) {
    ...
    //
    // `agentWait` is deliberately ABSENT rather than null. Null is the contract's "Kolux looked and
...
export function resolvePinnedFederatedServer(
  ...
  if (server.peerFingerprint !== federated.peer_fingerprint) {
    throw new OrchestrationError(
      'peer_changed',
      `Saved environment ${federated.environment_name} now identifies a different Kolux server.`
    )
  }
  return server
}
