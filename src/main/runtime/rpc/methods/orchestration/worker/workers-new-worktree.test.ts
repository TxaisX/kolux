...
describe('orchestration new-worktree workers', () => {
  type CreateWorktreeResult = Awaited<ReturnType<KoluxRuntimeService['createManagedWorktree']>>
  ...
  it('injects the execution host CLI command and Dispatch capability together', async () => {
    mockCreatedWorktree()
    vi.mocked(runtime.getTerminalOrchestrationCliCommand).mockReturnValue('kolux-ide')

    await startWorker({ worktree: 'new-top-level' })

    const prompt = vi.mocked(runtime.sendTerminalAgentPrompt).mock.calls[0]?.[1] ?? ''
    expect(prompt).toContain('kolux-ide orchestration send')
    expect(prompt).toMatch(/--dispatch-capability dcap_[A-Za-z0-9_-]+/)
    expect(prompt).not.toMatch(/(^|\s)kolux orchestration send/)
  })
  ...
  it('replays a dispatch-input failure after restart without creating another worker', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-worker-start-replay-'))
    paths.push(dir)
    db.close()
    ...
    db = new OrchestrationDb(join(dir, 'orchestration.db'))
    const restartedRuntime = new KoluxRuntimeService()
  ...
  it('persists pre-effect, post-effect, and post-input stages in order', async () => {
    mockCreatedWorktree({ hookFound: false })
    let finishWait:
      | ((value: Awaited<ReturnType<KoluxRuntimeService['waitForTerminal']>>) => void)
      | undefined
    let finishPrompt:
      | ((value: Awaited<ReturnType<KoluxRuntimeService['sendTerminalAgentPrompt']>>) => void)
      | undefined
