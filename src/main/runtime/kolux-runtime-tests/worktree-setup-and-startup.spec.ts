...
import { UUID_RE, expectStablePaneKeyEnv, store } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes the selected Windows setup shell into runtime runner generation', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        KOLUX_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        // Why: createTerminal stamps KOLUX_PANE_KEY/TAB_ID/WORKTREE_ID so hook-based agent status can attribute events to a stable pane.
        env: expect.objectContaining({
          KOLUX_ROOT_PATH: '/tmp/repo',
          KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          KOLUX_TAB_ID: expect.stringMatching(UUID_RE),
    ...
    const setupLeafId = setupSpawnEnv.KOLUX_PANE_KEY.slice(`${setupSpawnEnv.KOLUX_TAB_ID}:`.length)
    ...
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: setupSpawnEnv.KOLUX_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('uses returned WSL setup shell metadata when runtime spawns setup', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: 'C:\\repo',
        KOLUX_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /mnt/c/repo/.git/kolux/setup-runner.sh',
        env: expect.objectContaining({
          KOLUX_ROOT_PATH: 'C:\\repo',
          KOLUX_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl',
          KOLUX_TAB_ID: expect.stringMatching(UUID_RE),
  ...
  it('uses the shell-aware setup runner for windowless creates without a startup command', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: 'C:\\repo',
        KOLUX_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-windowless'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /c/repo/.git/kolux/setup-runner.sh',
        worktreeId: result.worktree.id
      })
    )
  })
