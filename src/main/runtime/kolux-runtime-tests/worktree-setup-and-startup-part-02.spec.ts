...
import { expectStablePaneKeyEnv, store } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  it('sequences setup before startup for opted-in local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: 'C:\\tmp\\repo\\.git\\kolux\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-startup-setup'
      },
    ...
    expect(startupScript).toContain('/mnt/c/tmp/repo/.git/kolux/setup-runner.sh')
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/kolux/setup-runner.sh')
  ...
  it('starts setup and startup side by side by default for local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-parallel'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        command: expect.stringContaining('__KOLUX_SETUP_COMPLETE__:')
      })
    )
  ...
  it('observes setup completion through the launch shell the runner was written for', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-observed-wsl-shell'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    const setupCommand = (spawn.mock.calls[1]![0] as { command: string }).command
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/kolux/setup-runner.sh')
    expect(setupCommand).toContain('__KOLUX_SETUP_COMPLETE__:')
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.KOLUX_PANE_KEY.slice(
      `${initialSpawnEnv.KOLUX_TAB_ID}:`.length
    )
    ...
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: initialSpawnEnv.KOLUX_TAB_ID,
  ...
  it('does not surface the new workspace when a background CLI create launches its agent', async () => {
    ...
    // `kolux worktree create --agent claude` without --activate: the agent tab is
  ...
  it('honors split setup placement for CLI-created worktrees without startup agents', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-cli-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(setupEnv.KOLUX_TAB_ID).toBe(mainEnv.KOLUX_TAB_ID)
    const mainLeafId = mainEnv.KOLUX_PANE_KEY!.slice(`${mainEnv.KOLUX_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-cli-setup-setup',
        tabId: mainEnv.KOLUX_TAB_ID,
        activate: false,
