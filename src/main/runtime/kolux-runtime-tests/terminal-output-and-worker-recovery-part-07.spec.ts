...
import { KoluxRuntimeService, getDefaultWorkspaceSession } from '../kolux-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('KoluxRuntimeService', () => {
  it('keeps current-generation tab and leaf identity across a host restart', async () => {
    ...
    const makeRuntime = (): KoluxRuntimeService => {
  ...
  it('rejects connection mismatch and reused handles while allowing a WSL-owned orphan', async () => {
    const makeRuntime = (): KoluxRuntimeService => {
      ...
      return new KoluxRuntimeService({ ...runtimeStore, flushOrThrow: vi.fn() } as never)
    }
    ...
    await withPlatform('win32', async () => {
      const makeWslRuntime = (reportedWslDistro?: string | null): KoluxRuntimeService => {
        const { runtimeStore } = makeRuntimeStoreWithWorkspaceSession({
          ...getDefaultWorkspaceSession(),
          tabsByWorktree: { [TEST_WORKTREE_ID]: [] }
        })
        const wsl = new KoluxRuntimeService({
          ...runtimeStore,
          flushOrThrow: vi.fn(),
          getProjects: () => [
            {
