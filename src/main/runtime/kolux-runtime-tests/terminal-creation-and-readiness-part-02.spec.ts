...
describe('KoluxRuntimeService', () => {
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    expect(spawnCall?.env?.KOLUX_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  // the CLI Kolux can host (issue #11926).
  ...
  it.each([
    { platform: 'win32' as const, expected: 'kolux.cmd claude-teams' },
    { platform: 'linux' as const, expected: 'kolux-ide claude-teams' },
    { platform: 'darwin' as const, expected: 'kolux claude-teams' }
  ])(
  ...
  it('resolves a startupAgent in a repo-less folder workspace', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'kolux-runtime-folder-startup-agent-'))
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-bg' })
