...
import { listWorktrees } from '../kolux-runtime-test-mocks.spec'
import { TEST_REPO_ID, createRuntime, syncSinglePty } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  // clear stripping the spinner off Kolux's synthesized title after 3s of quiet output, and
  ...
  it('only records the bare Cursor native title via the stale-working clear', async () => {
    ...
    try {
      ...
      // Kolux's synthesized spinner, then quiet output: the clear strips it to the bare title.
      runtime.onPtyData(ptyId, '\x1b]0;⠋ Cursor Agent\x07', 101)
