...
describe('KoluxRuntimeService', () => {
  ...
  it('rejects SSH lineage updates when Kolux worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Kolux worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.kolux'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.kolux'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
