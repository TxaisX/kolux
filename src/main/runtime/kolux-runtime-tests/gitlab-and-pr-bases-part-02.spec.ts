...
describe('KoluxRuntimeService', () => {
  ...
  it('resolves SSH GitHub fork PR heads through the write-capable fetch RPC', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (
          args[0] === 'rev-parse' &&
          args[2] === `refs/kolux/pull/${ORIGIN_HEAD_COMPONENT}/42^{commit}`
        ) {
      ...
      fetchGitHubPullRequestHead: vi
        .fn()
        .mockResolvedValue(`refs/kolux/pull/${ORIGIN_HEAD_COMPONENT}/42`),
  ...
  it('resolves local GitLab fork MR bases from the target project MR head ref', async () => {
    ...
    const gitSpy = vi.spyOn(gitRunner, 'gitExecFileAsync').mockImplementation(async (args) => {
      ...
      if (
        args[0] === 'rev-parse' &&
        args[1] === '--verify' &&
        args[2] === `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/42^{commit}`
      ) {
    ...
    try {
      ...
      expect(gitSpy).toHaveBeenCalledWith(
        [
          ...
          '--no-tags',
          'origin',
          `+refs/merge-requests/42/head:refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/42`
        ],
      ...
      expect(gitSpy).toHaveBeenCalledWith(
        ['rev-parse', '--verify', `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/42^{commit}`],
        { cwd: TEST_REPO_PATH }
      )
    } finally {
      gitSpy.mockRestore()
  ...
  it('captures the fork MR head from a dedicated ref, not the shared FETCH_HEAD', async () => {
    ...
    // default-branch tip. The resolved base must come from the durable Kolux MR ref.
    const gitSpy = vi.spyOn(gitRunner, 'gitExecFileAsync').mockImplementation(async (args) => {
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: `${ORIGIN_REMOTE_URL}\n`, stderr: '' }
      }
      ...
      if (args[0] === 'rev-parse') {
        ...
        if (ref === `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/42^{commit}`) {
  ...
  it('keeps the durable MR head when the head fetch fails but the local ref resolves', async () => {
    ...
    // fail the resolve when a prior fetch already pinned refs/kolux/merge-requests/<iid>.
    const localRepo = {
      id: TEST_REPO_ID,
      path: TEST_REPO_PATH,
      displayName: 'repo',
  ...
  it.each([
    ...
    [
      'This SSH host is running an older Kolux relay that cannot fetch merge request heads. Reconnect to deploy the latest relay, then try again.',
      'stale relay'
    ]
