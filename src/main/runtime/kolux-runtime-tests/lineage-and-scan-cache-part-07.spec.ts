...
import { TEST_WORKTREE_ID, store } from '../kolux-runtime-test-fixtures.spec'
 ...
 * KOLUX_TERMINAL_HANDLE alive so its dispatch still resolves; the coordinator's handle in
...
describe('KoluxRuntimeService orchestration lineage across restart', () => {
  it('projects the coordinator pane key as the worker parent after the handles are reminted', () => {
    ...
    const before = new KoluxRuntimeService(store)
    try {
      ...
      // (KOLUX_TERMINAL_HANDLE) re-adopted for the still-live worker PTYs. The coordinator did not
      // run an orchestration command yet, so its handle is fresh and the Run still names the old one.
      const after = new KoluxRuntimeService(store)
