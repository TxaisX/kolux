...
import { createWorktreeRemovalRuntime } from '../kolux-runtime-test-scenario-builders.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kolux-runtime-remove-'))
  ...
  it('routes already-missing SSH runtime history cleanup through the PTY owner', async () => {
    ...
    const metaById: Record<string, WorktreeMeta> = {
      [worktreeId]: makeWorktreeMeta({ hostId: 'ssh:ssh-1', koluxCreationSource: 'ssh' })
    }
    ...
    const runtime = new KoluxRuntimeService(runtimeStore as never, undefined, {
  ...
  it('force-removes a legacy Kolux-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kolux-runtime-orphan-'))
  ...
  it('prompts then force-removes a Kolux-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kolux-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects a Kolux-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kolux-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
