...
import { syncSinglePty } from '../kolux-runtime-test-fixtures.spec'
...
// Kolux's own synthetic agent spinner: one frame per pane every 80ms while an agent works.
...
describe('decorative title fact throttle', () => {
  ...
  it('collapses spinner ticks with an unchanged underlying title to the heartbeat rate', () => {
    ...
    const ticks = 125 // 10s of Kolux's 80ms synthetic spinner timer
