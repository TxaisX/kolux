...
describe('KoluxRuntimeService', () => {
  ...
  it('matches canonical bare agent commands when a command override is configured', async () => {
    ...
    const runtime = new KoluxRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.KOLUX_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        KOLUX_AGENT_HOOK_TRANSPORT: 'stale-transport',
        KOLUX_AGENT_HOOK_ENDPOINT: '/tmp/stale-endpoint.env'
      },
      envToDelete: ['CODEX_HOME', 'KOLUX_CODEX_HOME']
    })
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        KOLUX_AGENT_HOOK_ENV: 'remote',
        KOLUX_AGENT_HOOK_VERSION: '1',
        KOLUX_PANE_KEY: expect.any(String),
        KOLUX_TAB_ID: expect.any(String),
        KOLUX_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.KOLUX_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(spawnCall?.env?.KOLUX_AGENT_HOOK_TRANSPORT).toBeUndefined()
    expect(spawnCall?.envToDelete).toEqual(['CODEX_HOME', 'KOLUX_CODEX_HOME'])
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'kolux-runtime-folder-workspace-'))
    ...
    expect(spawnedEnv.KOLUX_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.KOLUX_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.KOLUX_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.KOLUX_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.KOLUX_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.KOLUX_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.KOLUX_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.KOLUX_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `kolux-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `kolux-missing-folder-update-${randomUUID()}`)
  ...
  it('reveals Claude Agent Teams launches with the rewritten launch config', async () => {
    ...
    const spawnedEnv = spawnCall?.env ?? {}
    const spawnedLeafId = spawnedEnv.KOLUX_PANE_KEY.slice(`${spawnedEnv.KOLUX_TAB_ID}:`.length)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.KOLUX_AGENT_LAUNCH_TOKEN,
      launchAgent: 'claude',
      activate: false,
      tabId: spawnedEnv.KOLUX_TAB_ID,
