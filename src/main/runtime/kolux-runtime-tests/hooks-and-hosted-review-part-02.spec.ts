...
describe('KoluxRuntimeService', () => {
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new KoluxRuntimeService(multiRepoStore as never)
  ...
  it('routes local WSL project hosted review flows through runtime git options', async () => {
    ...
    const runtime = new KoluxRuntimeService(wslStore as never)
    getHostedReviewForBranchMock.mockResolvedValueOnce({
      provider: 'github',
      number: 76,
      title: 'Feature WSL',
      state: 'open',
      url: 'https://github.com/acme/kolux/pull/76',
      status: 'success',
    ...
    createHostedReviewMock.mockResolvedValueOnce({
      ok: true,
      number: 77,
      url: 'https://github.com/acme/kolux/pull/77'
    })
