...
describe('KoluxRuntimeService', () => {
  it('resolves SSH GitLab fork MR bases from the target project MR head ref', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (
          args[0] === 'rev-parse' &&
          args[1] === '--verify' &&
          args[2] === `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/77^{commit}`
        ) {
      ...
      fetchGitLabMergeRequestHead: vi
        .fn()
        .mockResolvedValue(`refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/77`),
    ...
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--verify', `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/77^{commit}`],
      '/remote/repo'
    )
  ...
  it('keeps a cross-repo fork MR compare base when the fetch fails but the local ref resolves', async () => {
    ...
    const durableLocalRef = `refs/kolux/merge-requests/${ORIGIN_HEAD_COMPONENT}/77`
    const provider = {
      exec: vi.fn(async (args: string[]) => {
