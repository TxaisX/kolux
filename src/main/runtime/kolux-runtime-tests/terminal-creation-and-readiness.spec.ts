...
import { KoluxRuntimeService, makePaneKey } from '../kolux-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('KoluxRuntimeService', () => {
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    expect(spawnedEnv.KOLUX_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.KOLUX_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  it('retires inherited launch authority when the agent command exits', async () => {
    ...
    const evidence = {
      ...
      paneKey: spawnEnv.KOLUX_PANE_KEY,
      launchToken: spawnEnv.KOLUX_AGENT_LAUNCH_TOKEN
    }

    expect(runtime.verifyOrchestrationCompatibilityCaller(evidence)).not.toBeNull()
    expect(
      runtime.getAgentStatusLaunchConfigForPaneKey(spawnEnv.KOLUX_PANE_KEY, {
        launchToken: spawnEnv.KOLUX_AGENT_LAUNCH_TOKEN
      })
    ).toBeDefined()
    ...
    expect(retireAuthority).toHaveBeenCalledWith(spawnEnv.KOLUX_PANE_KEY)
