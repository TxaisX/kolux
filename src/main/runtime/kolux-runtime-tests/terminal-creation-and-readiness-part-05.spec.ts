...
describe('KoluxRuntimeService', () => {
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceLeafId = sourceEnv.KOLUX_PANE_KEY.slice(`${sourceEnv.KOLUX_TAB_ID}:`.length)

    const split = await runtime.splitTerminal(handle, { direction: 'vertical' })
    expect(split).toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.KOLUX_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.KOLUX_PANE_KEY.slice(`${sourceEnv.KOLUX_TAB_ID}:`.length)
    expect(split.leafId).toBe(splitLeafId)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.KOLUX_TAB_ID).toBe(sourceEnv.KOLUX_TAB_ID)
    expect(splitEnv.KOLUX_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    ...
    const siblingSurfaces = publishedTabs.filter(
      (tab): tab is Extract<typeof tab, { type: 'terminal' }> =>
        tab.type === 'terminal' && tab.parentTabId === sourceEnv.KOLUX_TAB_ID
    )
  ...
  it('splits folder workspace pty-backed terminal sessions with folder cwd and env', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'kolux-runtime-folder-split-'))
    const spawn = vi
      .fn()
    ...
    expect(splitEnv.KOLUX_TAB_ID).toBe(sourceEnv.KOLUX_TAB_ID)
    expect(splitEnv.KOLUX_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(splitEnv.KOLUX_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(splitEnv.KOLUX_WORKSPACE_ROOT).toBe(folderPath)
    expect(splitEnv.KOLUX_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  ...
  it('returns an actionable discoverability warning when default adoption fails after spawn', async () => {
    ...
    try {
      ...
      expect(created.warning).toContain(`kolux terminal focus --terminal ${created.handle}`)
