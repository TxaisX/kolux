...
import { store } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  it('deduplicates runtime repo paths with Windows/UNC comparison semantics', async () => {
    ...
    const runtime = new KoluxRuntimeService(uncStore as never)
  ...
  it('browses runtime server directories before projects are added', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-browse-'))
    try {
  ...
  it('sets up a project whose identity exists only on the requesting host', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-cross-host-project-'))
    const repos: Record<string, unknown>[] = []
    ...
    try {
      execFileSync('git', ['init'], { cwd: tempRoot, stdio: 'ignore' })
      const result = await runtime.setupProjectExistingFolder({
        projectId: 'github:github.acme.test/acme/kolux',
      ...
      expect(result.project).toMatchObject({
        id: 'github:github.acme.test/acme/kolux',
        providerIdentity: {
          provider: 'github',
          owner: 'acme',
          repo: 'kolux',
  ...
  it('rolls back a new runtime repo when project alignment fails', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-project-rollback-'))
    const repos: Record<string, unknown>[] = []
    getRepoUpstreamMock.mockResolvedValueOnce(null)
    const runtimeStore = {
      ...store,
    ...
    try {
      ...
      await expect(
        runtime.setupProjectExistingFolder({
          projectId: 'git:git.example.test/acme/kolux',
          hostId: 'runtime:env-1',
          path: tempRoot,
          kind: 'git'
        })
  ...
  it('rolls back a newly cloned repo when project alignment fails', async () => {
    ...
    await expect(
      runtime.setupProjectClone({
        projectId: 'git:git.example.test/acme/kolux',
        hostId: 'runtime:env-1',
        url: 'https://git.example.test/acme/kolux.git',
        destination: '/tmp'
      })
    ).rejects.toThrow('Imported folder does not match the selected project identity.')
  ...
  it('never sets an SSH-hosted project up on the local machine', async () => {
    ...
    const destination = await mkdtemp(join(tmpdir(), 'kolux-runtime-ssh-guard-'))
    const existingFolder = join(destination, 'kolux')
