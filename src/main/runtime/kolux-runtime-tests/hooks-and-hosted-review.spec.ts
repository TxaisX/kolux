...
import { TEST_REPO_ID, TEST_REPO_PATH, store } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  describe('checkRepoHooks status', () => {
    ...
    it('reports ok for a missing remote kolux.yaml and error for any other read failure', async () => {
  ...
  it('resolves SSH issue commands from shared kolux.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseKoluxYaml).mockReturnValue({
      scripts: {},
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        ...
        if (filePath.endsWith('kolux.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.kolux/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.kolux/issue-command', false)
  ...
  it('allows host integration slug helpers for SSH repos through provider-aware GitHub clients', async () => {
    const prRepo = { owner: 'acme', repo: 'kolux', host: 'github.acme.test' }
  ...
  it('routes runtime GitHub repo identity helpers through the selected WSL project runtime', async () => {
    ...
    getRepoSlugMock.mockResolvedValueOnce({ owner: 'acme', repo: 'kolux' })
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/kolux/issues/12'
    })
