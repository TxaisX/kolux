...
import { KoluxRuntimeService, join, listWorktrees, tmpdir } from '../kolux-runtime-test-mocks.spec'
import type { WorktreeLineage, WorktreeMeta } from '../kolux-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WORKTREE_ID,
  makeWorktreeInfo,
...
describe('KoluxRuntimeService', () => {
  it('strips Kolux provenance fields from runtime metadata updates', async () => {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      koluxCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
