...
describe('KoluxRuntimeService', () => {
  it('launches SSH setup terminals for runtime task-created worktrees', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup/kolux/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          ...
          command: expect.stringContaining(
            'bash /remote/repo/.git/worktrees/mobile-setup/kolux/setup-runner.sh'
          ),
          worktreeId: result.worktree.id
        })
      )
      ...
      const markerPath = `/remote/repo/.git/worktrees/mobile-setup/kolux/setup-runner.sh.${nonceMatch![1]}.done`
  ...
  it('honors split setup placement for SSH worktrees without startup agents', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup-split/kolux/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    try {
      ...
      expect(setupEnv.KOLUX_TAB_ID).toBe(initialEnv.KOLUX_TAB_ID)
      const initialLeafId = initialEnv.KOLUX_PANE_KEY!.slice(`${initialEnv.KOLUX_TAB_ID!}:`.length)
      expect(revealTerminalSession).toHaveBeenLastCalledWith(
        result.worktree.id,
        expect.objectContaining({
          ptyId: 'pty-remote-setup-split',
          tabId: initialEnv.KOLUX_TAB_ID,
          activate: false,
  ...
  it('removes SSH-backed runtime worktrees through the SSH git provider', async () => {
    ...
    const runtime = new KoluxRuntimeService(remoteStore as never, undefined, {
  ...
  it('reads SSH repo hooks through the SSH filesystem provider', async () => {
    ...
    vi.mocked(parseKoluxYaml).mockReturnValue({ scripts: { setup: 'pnpm install' } })
    registerSshFilesystemProvider('ssh-1', fsProvider as never)
    const runtime = new KoluxRuntimeService(remoteStore as never)
  ...
  it('hashes only the shared kolux.yaml setup script for local run-both hooks', async () => {
