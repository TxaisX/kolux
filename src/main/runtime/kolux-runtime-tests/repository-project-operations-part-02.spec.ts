...
describe('KoluxRuntimeService', () => {
  it('adopts public clone repos into host-qualified project setup', async () => {
    const destination = await mkdtemp(join(tmpdir(), 'kolux-runtime-project-clone-'))
    ...
    try {
      const cloned = await runtime.cloneRepo('https://example.com/kolux.git', destination)
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new KoluxRuntimeService(colorStore as never)
    const parentDir = await mkdtemp('/tmp/kolux-runtime-create-')
    try {
  ...
  it('creates a missing runtime parent before creating the project directory', async () => {
    ...
    const runtime = new KoluxRuntimeService(createStore as never)
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-create-parent-'))
    const parentDir = join(tempRoot, 'kolux', 'projects')
    try {
  ...
  it('prepares the runtime worktree root when creating a repo', async () => {
    ...
    const runtime = new KoluxRuntimeService(runtimeStore as never)
    const parentDir = await mkdtemp(join(tmpdir(), 'kolux-runtime-create-root-prep-'))
    try {
  ...
  it('drops a same-path negative submodule cache before runtime cloneRepo', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'gitSpawnAfterWindowsEnvironmentReady')
    const destination = await mkdtemp(join(tmpdir(), 'kolux-runtime-reclone-'))
    ...
    const runtime = new KoluxRuntimeService(cloneStore as never)
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    ...
    const tempRoot = await mkdtemp(join(tmpdir(), 'kolux-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'kolux-runtime-clone-'))
