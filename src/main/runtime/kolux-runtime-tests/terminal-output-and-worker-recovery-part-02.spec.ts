...
import type { WorkspaceSessionState } from '../kolux-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
import { publishLegacyWorkerReveal } from '../kolux-runtime-test-scenario-builders.spec'

describe('KoluxRuntimeService', () => {
  it('recovers exported KOLUX_TERMINAL_HANDLE from discovered live PTY sessions', async () => {
  ...
  it('reveals one exact live legacy worker without stealing focus', async () => {
    ...
    const runtime = new KoluxRuntimeService({ ...runtimeStore, flushOrThrow } as never, undefined, {
