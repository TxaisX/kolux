...
import { KoluxRuntimeService, OrchestrationDb } from '../kolux-runtime-test-mocks.spec'
import {
...
describe('KoluxRuntimeService', () => {
  ...
  it('points a Run mailbox at its live-idle coordinator without replaying pending rows', async () => {
    ...
    try {
      ...
      expect(write).toHaveBeenCalledWith(
        'pty-1',
        '\nYou have 1 orchestration message. Run `kolux-dev orchestration check --run run_mailbox`.\n'
      )
  ...
  it('points already-idle Run mail after Codex replaces its completion title', async () => {
    ...
    db.insertMessage({
      ...
      body: 'The package name is kolux.',
      type: 'worker_done'
    })
    ...
    await vi.waitFor(() => {
      expect(write).toHaveBeenCalledWith(
        'pty-1',
        '\nYou have 1 orchestration message. Run `kolux-dev orchestration check --run run_codex_native_title`.\n'
      )
    })
    db.close()
  })
