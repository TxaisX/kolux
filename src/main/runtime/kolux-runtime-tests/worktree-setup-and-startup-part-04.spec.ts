...
import type { WorktreeMeta } from '../kolux-runtime-test-mocks.spec'
import { TEST_REPO_ID, makeWorktreeMeta, store } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/TxaisX/kolux/issues/123',
      setupDecision: 'run',
      activate: true,
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-startup-setup-split',
        command: expect.stringContaining('bash /tmp/repo/.git/kolux/setup-runner.sh'),
        env: expect.objectContaining({
          KOLUX_ROOT_PATH: '/tmp/repo',
          KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split',
          KOLUX_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const markerPath = `/tmp/repo/.git/kolux/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    expect(mainEnv.KOLUX_TAB_ID).toBeDefined()
    expect(mainEnv.KOLUX_PANE_KEY).toBeDefined()
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KOLUX_ROOT_PATH: '/tmp/repo',
        KOLUX_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    expect(activateWorktree).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        runnerScriptPath: 'C:\\tmp\\repo\\.git\\kolux\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/tmp/repo/.git/kolux/setup-runner.sh')
      }),
      undefined,
      undefined
    )
  ...
  it('lets explicit startup draft agents override the desktop default', async () => {
    ...
    const draftUrl = 'https://github.com/TxaisX/kolux/issues/789'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
