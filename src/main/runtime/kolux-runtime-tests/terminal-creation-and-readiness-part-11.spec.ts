...
import { store, syncSinglePty } from '../kolux-runtime-test-fixtures.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/kolux report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'kolux-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/kolux report/result.json:12:3',
        '/tmp/kolux report/result.json',
        '/tmp/kolux report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/kolux%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/kolux report/result.json',
        '/tmp/kolux report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/kolux report/other.json',
        '/tmp/kolux report/result.json',
        '/tmp/kolux report/result.json'
      )
    ).toBe(false)
