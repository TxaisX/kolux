...
import type { RuntimeMobileSessionTabsResult } from '../kolux-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WINDOW_ID,
  TEST_WORKTREE_ID,
...
} from '../kolux-runtime-test-scenario-builders.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  it('reports the driver as missing instead of telling a configured operator to configure it', () => {
    ...
    expect(degradation?.message).not.toMatch(/set KOLUX_BROWSER_EXECUTABLE/)
  })
  ...
  it('keeps the degradation wire-safe for peers that predate structured causes', () => {
    ...
    expect(degradation?.message).toBe(
      'KOLUX_BROWSER_EXECUTABLE points at a path that does not exist. (/nope/chromium)'
    )
  })
