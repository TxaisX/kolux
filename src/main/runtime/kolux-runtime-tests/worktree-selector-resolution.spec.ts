...
describe('KoluxRuntimeService', () => {
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/kolux-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'kolux-runtime-folder-files-'))
  ...
  it('guides registered SSH repo ids without probing the provider', async () => {
    ...
    const runtime = new KoluxRuntimeService(remoteStore)
  ...
  it('retires a generated name before a post-create listing failure', async () => {
    const addRetiredWorktreeName = vi.fn()
    const runtime = new KoluxRuntimeService({ ...store, addRetiredWorktreeName })
