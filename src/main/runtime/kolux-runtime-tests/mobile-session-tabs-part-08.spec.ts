...
import { KoluxRuntimeService, electronMocks } from '../kolux-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
import { makePendingAgentTabActivationRuntime } from '../kolux-runtime-test-scenario-builders.spec'
...
describe('KoluxRuntimeService', () => {
  ...
  it('durably tears down a runtime-owned SSH headless tab when renderer cleanup fails', async () => {
    ...
    const runtime = new KoluxRuntimeService({ ...runtimeStore, flushOrThrow } as never)
    runtime.setPtyController({
      write: () => true,
      kill,
      getForegroundProcess: async () => null,
  ...
  it('retires an SSH-owned surface when a stale renderer acknowledges close after relay recovery', async () => {
    ...
    runtime = new KoluxRuntimeService(runtimeStore as never)
    runtime.setNotifier({ closeTerminal, closeTerminalTab } as never)
    runtime.setPtyController({
      write: () => true,
      kill,
