...
import { TEST_WORKTREE_ID, TEST_WORKTREE_PATH, store } from '../kolux-runtime-test-fixtures.spec'
import { wireHeadlessServeRuntime } from '../kolux-runtime-test-scenario-builders.spec'
...
describe('KoluxRuntimeService', () => {
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        KOLUX_AGENT_TEAMS_SHIM_BIN: '/opt/kolux/bin/kolux-ide',
        KOLUX_AGENT_TEAMS_TEAM_ID: 'stale-team',
        KOLUX_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/kolux-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.KOLUX_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.KOLUX_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.KOLUX_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/kolux-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.KOLUX_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.KOLUX_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    expect(spawnedEnv.KOLUX_TAB_ID).toBe(tabId)
    expect(spawnedEnv.KOLUX_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.KOLUX_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.KOLUX_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.KOLUX_PANE_KEY).toMatch(`${spawnedEnv.KOLUX_TAB_ID}:`)
  })
  ...
  it('spawns focus-requested CLI terminal creates in background on headless serve', async () => {
    ...
    // `kolux terminal create --worktree <wt> --command "echo test" --focus`
    await expect(
      runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
        command: 'echo test',
        focus: true
