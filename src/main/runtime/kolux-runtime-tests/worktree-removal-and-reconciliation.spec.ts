...
describe('KoluxRuntimeService', () => {
    ...
    try {
      ...
      expect(gitSpy).not.toHaveBeenCalledWith(
        [
          ...
          '+refs/heads/contributor/runtime-wsl*:refs/remotes/pr-contributor-kolux/contributor/runtime-wsl*'
        ],
        expect.anything()
      )
      expect(gitSpy).not.toHaveBeenCalledWith(
        [
          'branch',
          '--set-upstream-to',
          'pr-contributor-kolux/contributor/runtime-wsl',
          'runtime-wsl'
        ],
        expect.anything()
      )
  ...
  it('skips archive hooks for CLI worktree removal by default', async () => {
    ...
    expect(result.warning).toBe(
      `kolux.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
    )
  })
  ...
  it('scopes a runtime-host orphan PTY sweep to its environment, not the local host', async () => {
    ...
    const runtime = new KoluxRuntimeService(orphanStore as never, undefined, {
