...
describe('KoluxRuntimeService', () => {
  it('does not inspect or delete a local path when SSH runtime orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'kolux-runtime-ssh-missing-fs-'))
    const repo = {
  ...
  it('still rejects forced runtime unregistered delete paths that exist on disk', async () => {
    const existingWorktreePath = await mkdtemp(join(tmpdir(), 'kolux-runtime-remove-existing-'))
