...
import { KoluxRuntimeService } from '../kolux-runtime-test-mocks.spec'
import {
  TEST_WORKTREE_ID,
  TEST_WORKTREE_PATH,
...
describe('KoluxRuntimeService', () => {
  ...
  it('observes setup command completion without waiting for its interactive shell to exit', async () => {
    ...
    runtime.onPtyData(
      'pty-setup',
      'setup failed\r\n__KOLUX_SETUP_COMPLETE__:token-live:17\r\nPS>',
      100
    )
  ...
  it('replays fast setup completion emitted before its observer is registered', async () => {
    ...
    runtime.onPtyData(
      'pty-fast-setup',
      '__KOLUX_SETUP_COMPLETE__:wrong:9\r\n__KOLUX_SETUP_COMPLETE__:token-fast:0\r\n$',
      100
    )
  ...
  it('keeps observing after an uncertain setup terminal status', async () => {
    ...
    runtime.onPtyData('pty-uncertain-setup', '__KOLUX_SETUP_COMPLETE__:token-uncertain:0\r\n', 100)
