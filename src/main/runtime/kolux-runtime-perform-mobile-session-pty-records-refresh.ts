...
import { KoluxRuntimeWithBuildHeadlessMobileSessionBrowserTabs } from './kolux-runtime-build-headless-mobile-session-browser-tabs'
...
export class KoluxRuntimeWithPerformMobileSessionPtyRecordsRefresh extends KoluxRuntimeWithBuildHeadlessMobileSessionBrowserTabs {
    ...
    if (tab.type === 'terminal') {
      ...
      if (shouldMaterializePendingTerminal) {
        ...
        let agentStartup: Awaited<
          ReturnType<KoluxRuntimeService['resolveMobileSessionTerminalCommand']>
        > = {}
        if (tab.launchAgent) {
          try {
