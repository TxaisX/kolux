 ...
 *
 * The harness is the close-continuity pattern: a real KoluxRuntimeService with
...
async function listTerminalSurfaceIds(runtime: KoluxRuntimeService): Promise<string[]> {
...
async function listParentTabIds(runtime: KoluxRuntimeService): Promise<string[]> {
  const listed = await runtime.listMobileSessionTabs(`id:${WORKTREE_ID}`)
  return [
    ...new Set(
...
describe('a committed paired close stays closed while its PTY lingers', () => {
  ...
  it('red: a host-side close (no paired close RPC) must not be resurrected by the preserved-merge', async () => {
    ...
    // The host closes the tab itself — host user gesture or `kolux terminal`
