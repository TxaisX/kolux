...
import { resolveAuthorizedPathMock } from './kolux-runtime-files-mock-registry'
import {
  createRuntimeFileCommands,
  useRuntimeFileCommandsLifecycle
} from './kolux-runtime-files-test-harness'
import { getSshFilesystemProvider } from '../providers/ssh-filesystem-dispatch'
import { RUNTIME_PREVIEWABLE_BINARY_MAX_BYTES } from './kolux-runtime-files'
...
describe('RuntimeFileCommands', () => {
  ...
  describe('previewable binary budget', () => {
    ...
    async function previewFixture(size = Buffer.byteLength('fake-png')): Promise<string> {
      const dir = await mkdtemp(join(tmpdir(), 'kolux-preview-budget-'))
