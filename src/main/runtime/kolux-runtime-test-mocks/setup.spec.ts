...
import type { KoluxRuntimeService as KoluxRuntimeServiceConstructor } from '../kolux-runtime'
...
export function acknowledgeAgentPromptSubmit(
  runtime: InstanceType<typeof KoluxRuntimeServiceConstructor>,
  ptyId: string,
  data: string
): void {
  if (data === '\r') {
...
const {
  ...
  return {
    ...
    markClaudeProjectTrustedMock: vi.fn(async () => undefined) as TestMock,
...
vi.mock('../../hooks', () => ({
  ...
  parseKoluxYaml: vi.fn().mockReturnValue(null)
}))
