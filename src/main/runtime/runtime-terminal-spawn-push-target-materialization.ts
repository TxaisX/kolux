...
// Kolux first" isn't an option mid-task. Fires on every terminal spawn into the worktree;
...
function materializeOverSsh(
  ...
  if (!provider) {
    // Why: connection dropped -- the next Kolux-driven sync action will retry via its own dispatch.
