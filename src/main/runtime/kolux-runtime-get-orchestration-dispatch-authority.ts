...
import { KoluxRuntimeWithVerifyOrchestrationCompatibilityCaller } from './kolux-runtime-verify-orchestration-compatibility-caller'
...
export class KoluxRuntimeWithGetOrchestrationDispatchAuthority extends KoluxRuntimeWithVerifyOrchestrationCompatibilityCaller {
  ...
  getTerminalOrchestrationCliCommand(handle: string): OrchestrationCliCommand {
    ...
    return resolveTerminalOrchestrationCliCommand({
      ...
      isWsl: pty.isWsl,
      worktreeId: pty.worktreeId,
      // Dev builds run the CLI as `kolux-dev`; a packaged app must not advertise it.
      runtimeCliCommand: getAppEnvironment().isPackaged() ? undefined : 'kolux-dev',
