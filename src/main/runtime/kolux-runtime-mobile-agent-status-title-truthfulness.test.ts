...
async function createRuntime(rows: AgentStatusIpcPayload[] = []): Promise<KoluxRuntimeService> {
...
function publishRendererWorkingPane(runtime: KoluxRuntimeService, paneTitle: string): void {
...
function publishRendererReleasedPane(runtime: KoluxRuntimeService, title: string): void {
  runtime.attachWindow(1)
  runtime.syncWindowGraph(1, {
    tabs: [
      {
