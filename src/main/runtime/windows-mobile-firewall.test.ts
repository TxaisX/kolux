...
function environment(
  ...
  return {
    platform: 'win32',
    isPackaged: true,
    executablePath: "C:\\Users\\O'Brien\\Kolux\\Kolux.exe",
...
describe('windows mobile firewall', () => {
  it('inspects the exact executable, port, and selected interface profile', async () => {
    ...
    expect(script).toContain(
      "Get-NetFirewallApplicationFilter -PolicyStore ActiveStore -Program 'C:\\Users\\O''Brien\\Kolux\\Kolux.exe'"
    )
    ...
    expect(script).toContain("C:\\Users\\O''Brien\\Kolux\\Kolux.exe")
  ...
  it('repairs only Kolux mobile pairing on private networks after elevation', async () => {
    ...
    expect(repairScript).toContain("-Name 'Kolux.MobilePairing'")
    ...
    expect(repairScript).toContain("-Program 'C:\\Users\\O''Brien\\Kolux\\Kolux.exe'")
  ...
  it('keeps the elevated child encoded because Start-Process re-splits its ArgumentList', async () => {
    ...
    // of spaces. Measured on Windows 11: a `-Command` payload turned `C:\My  App\Kolux.exe`
    // into `C:\My App\Kolux.exe`, i.e. a firewall rule for the wrong program. Base64 is the
    // only form that survives that hop, so this site must not follow the local runner.
    const runPowerShell = vi.fn().mockResolvedValue('{"launched":true,"exitCode":0}')
    await repairWindowsMobileFirewall(
      6769,
      environment(runPowerShell, { executablePath: 'C:\\My  App\\Kolux.exe' })
    )
    ...
    const encoded = outerScript.match(/'-EncodedCommand', '([^']+)'/)?.[1]
    const repairScript = Buffer.from(encoded!, 'base64').toString('utf16le')
    expect(repairScript).toContain("-Program 'C:\\My  App\\Kolux.exe'")
  })
  ...
  it('runs the local PowerShell over argv with a plain -Command script', async () => {
    ...
    await inspectWindowsMobileFirewall(6768, undefined, {
      platform: 'win32',
      isPackaged: true,
      executablePath: 'C:\\My  App\\Kolux.exe',
      systemRoot: 'C:\\Windows'
    })
    ...
    expect(args![3]).toContain("-Program 'C:\\My  App\\Kolux.exe'")
