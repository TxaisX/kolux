...
function runtimeWith(settings: Record<string, unknown>): KoluxRuntimeService {
  return new KoluxRuntimeService({ getSettings: () => settings } as never)
}
...
describe('structured agent-session launch args wiring', () => {
  ...
  it('supplies the Claude env overlay so the launch resolver does not fall back to process.env', async () => {
    const deps = await installedDeps({
      agentDefaultArgs: {},
      agentDefaultEnv: {
        claude: { KOLUX_CLAUDE_OVERLAY: 'claude-value' },
        codex: { KOLUX_CODEX_OVERLAY: 'codex-value' }
      }
    })

    expect(deps.resolveClaudeLaunchEnv).toBeTypeOf('function')
    expect(deps.resolveClaudeLaunchEnv?.()).toMatchObject({
      KOLUX_CLAUDE_OVERLAY: 'claude-value'
    })
    expect(deps.resolveClaudeLaunchEnv?.()).not.toHaveProperty('KOLUX_CODEX_OVERLAY')
    expect(deps.resolveLaunchEnvOverlay()).toMatchObject({
      KOLUX_CODEX_OVERLAY: 'codex-value'
    })
  })
})
