...
import { KoluxRuntimeWithSubscribeToTerminalResize } from './kolux-runtime-subscribe-to-terminal-resize'
import type {
...
export class KoluxRuntimeWithAdoptTerminalOrphansFromInventory extends KoluxRuntimeWithSubscribeToTerminalResize {
  ...
  async resolveActiveTerminal(
    ...
    // structured worker is covered by the `KOLUX_TERMINAL_HANDLE` its child is spawned with. That
    ...
    // pane the guess resolves. Such a child now carries `KOLUX_STRUCTURED_SESSION` and the CLI
