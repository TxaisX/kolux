// #11761: on a headless `kolux serve` host the HTTP agent hook is the only carrier of
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: KoluxRuntimeService) => void
...
function observePaneTitle(runtime: KoluxRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: KoluxRuntimeService): number {
  const pty = (
