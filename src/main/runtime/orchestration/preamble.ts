...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `kolux-ide`;
  // other execution hosts keep their existing bare `kolux` bridge.
  ...
  // shells have no agent prompt for Kolux to reuse.
...
// Why: the dispatch preamble teaches agents about Kolux's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use kolux-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Kolux instance (Section 6.4).
  const cli = params.devMode ? 'kolux-dev' : (params.cliCommand ?? 'kolux')
  ...
  const header = `You are working inside Kolux, a multi-agent IDE. You are a dispatched worker.
...
prompt for Kolux to reuse; if the coordinator has more for you it will
