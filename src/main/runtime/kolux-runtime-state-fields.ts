...
import { KoluxRuntimeWithLinearCommands } from './kolux-runtime-linear-commands'
...
export class KoluxRuntimeWithStateFields extends KoluxRuntimeWithLinearCommands {
  constructor(
    ...
    deps?: {
      ...
      // runs under `kolux serve`, so remote/SSH hosts would silently drop
    ...
    // even on headless `kolux serve` hosts where registerCoreHandlers never runs.
