import { KoluxRuntimeWithGetRuntimeId } from './kolux-runtime-get-runtime-id'
...
import type { RuntimeWorktreeLifecycleEvent } from './kolux-runtime-core'
import { WORKTREE_CREATE_RESULT_TTL_MS } from './kolux-runtime-core'
...
export class KoluxRuntimeWithGetStatus extends KoluxRuntimeWithGetRuntimeId {
  ...
  getStatus(): RuntimeStatus {
    ...
    const capabilities: RuntimeCapability[] = RUNTIME_CAPABILITIES.filter(
      (capability) =>
        ...
        (process.env.KOLUX_E2E_DISABLE_RUNTIME_SHARED_CONTROL !== '1' ||
          capability !== REMOTE_RUNTIME_SHARED_CONTROL_CAPABILITY) &&
        (process.env.KOLUX_E2E_DISABLE_PAIRED_TERMINAL_PARKING !== '1' ||
          capability !== TERMINAL_PAIRED_PARKING_RUNTIME_CAPABILITY) &&
        (process.env.KOLUX_E2E_DISABLE_AUTHORITATIVE_SESSION_TABS_INVENTORY !== '1' ||
    ...
    // that actually applies, and a host that says "set KOLUX_BROWSER_EXECUTABLE" to someone
    ...
    return {
      ...
      // Why: headless kolux serve cannot create/stream BrowserViews, so clients
