import * as mocks from './kolux-runtime-test-mocks.spec'
import type { Mock } from 'vitest'

const { HEADLESS_RUNTIME_WINDOW_ID, KoluxRuntimeService, electronMocks } = mocks
const { getBrowserHostLeaseRegistry, getDefaultWorkspaceSession, getRuntimeBrowserPageRegistry } =
  mocks
const { gitRunner, vi } = mocks

import * as fixtures from './kolux-runtime-test-fixtures.spec'
...
// Why (#10333): `kolux serve` publishes a ready graph under
...
function createWorktreeRemovalRuntime(runtimeStore: unknown = store): RuntimeService {
  ...
  return new KoluxRuntimeService(runtimeStore as never, undefined, {
