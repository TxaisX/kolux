...
function syncSplit(runtime: KoluxRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('KoluxRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new KoluxRuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
