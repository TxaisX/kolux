...
import { enoent, resolveAuthorizedPathMock, statMock } from './kolux-runtime-files-mock-registry'
...
} from './kolux-runtime-files-terminal-artifact-fixtures'
...
describe('RuntimeFileCommands', () => {
  ...
  describe('resolveTerminalPath', () => {
    ...
    it('does not mint an absolute artifact grant from a relative path observed under stale cwd', async () => {
      const dir = await mkdtemp(join(tmpdir(), 'kolux-terminal-artifact-'))
      tempDirs.push(dir)
