...
export const MAIN_THREAD_DIAGNOSTICS_ENV = 'KOLUX_MAIN_THREAD_DIAGNOSTICS'
...
export function classifySubprocessCommand(command: string, args: readonly string[]): string {
  ...
  const rest = [...args]
  if (binary === 'wsl') {
    // Kolux spawns guest commands with `--exec`; `--`/`-e` still appear on
    // wsl.exe processes started outside Kolux, so unwrap either separator.
    while (rest.length > 0) {
 ...
 * No-op unless KOLUX_MAIN_THREAD_DIAGNOSTICS=1.
 */
