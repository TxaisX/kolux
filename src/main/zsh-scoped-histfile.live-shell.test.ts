 ...
 * that the rest of Kolux's startup features arrive with it.
 *
 * macOS `/etc/zshrc` assigns `HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history` with no
 * check-before-set. Kolux used to fight that by keeping its own ZDOTDIR in place
 ...
 * Kolux would not wrap cannot pass here by construction.
 */
...
const systemZshrcClobbersHistfile = (() => {
  ...
  const home = mkdtempSync(join(tmpdir(), 'kolux-histfile-probe-'))
  try {
...
/** The launch config Kolux produces for a pane with exactly these features. */
function launchPane(
  home: string,
  ...
  const env: Record<string, string> = {
    HOME: home,
    ...(scopedHistfile ? { HISTFILE: scopedHistfile, KOLUX_HISTFILE: scopedHistfile } : {})
  }
  ...
  return {
    features,
    launch,
    // Why KOLUX_ORIG_ZDOTDIR overridden: the launch config resolves the user's
    // config dir from the real process env, and these runs must resolve against
    // the sandbox home instead.
    env: { PATH: '/usr/bin:/bin', ...env, ...launch.env, KOLUX_ORIG_ZDOTDIR: home }
  }
}
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    itWithZsh(
      'wraps a plain pane once Kolux injected a worktree HISTFILE',
      withHome(USER_FILES, async (home) => {
        ...
        expect(launch.env.KOLUX_SHELL_FEATURES).toBe('history')
      })
    )
    ...
    itWithZsh(
      ...
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'kolux-history', 'zsh_history'))

        const { values } = await runZshPty({
          env,
          report: [
            'ZDOTDIR',
            'KOLUX_TEST_USER_ZSHENV',
        ...
        expect(values.KOLUX_TEST_USER_ZSHENV).toBe('1')
    ...
    itWithZsh(
      'never leaves history inside Kolux’s own wrapper directory',
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'kolux-history', 'zsh_history'))
    ...
    itWithZsh(
      'consumes KOLUX_HISTFILE so nothing the shell spawns can inherit it',
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'kolux-history', 'zsh_history')
        const { env } = launchPane(home, scoped)
        ...
        const { values } = await runZshPty({
          env,
          report: ['KOLUX_HISTFILE', 'KOLUX_SHELL_FEATURES', 'HISTFILE']
        })

        // Root-cause fix for #11146: the variables no longer exist after use.
        expect(values.KOLUX_HISTFILE).toBe('UNSET')
    ...
    itWithZsh(
      'leaves HISTFILE exactly as an unwrapped zsh would when Kolux injects nothing',
      withHome(USER_FILES, async (home) => {
        ...
        // The contract is that Kolux's wrapper does not change it either way.
        const overlayEnv = { KOLUX_CODEX_HOME: join(home, 'codex') }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Kolux’s overlay values after the user’s config overwrites them',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        const overlayEnv = {
          KOLUX_CODEX_HOME: '/kolux/codex',
          KOLUX_AGENT_TEAMS_SHIM_DIR: '/kolux/shim'
        }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
          env: { HOME: home, ...overlayEnv },
        ...
        // the spawn env did, and Kolux's values still win.
        expect(values.CODEX_HOME).toBe('/kolux/codex')
        expect(values.PATH.startsWith('/kolux/shim:')).toBe(true)
      }
    )
  )
})
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
     ...
     * `.zshenv` or `.zprofile` cost the pane all of Kolux's features. Only one
    ...
    it.each([
      ...
      try {
        ...
        const { values } = await runZshPty({ env, report: ['HISTFILE', 'KOLUX_HISTFILE'] })
    ...
    itWithZsh(
      ...
      withHome({ ...USER_FILES, '.zshrc': 'precmd_functions=()\n' }, async (home) => {
        ...
        const report = ['HISTFILE', 'KOLUX_HISTFILE', 'ZDOTDIR']
        ...
        // host's call, not Kolux's. macOS /etc/zshrc overwrites HISTFILE, so it
        ...
        expect(values.HISTFILE).not.toContain(launch.env.ZDOTDIR)
        // KOLUX_HISTFILE was consumed in .zshenv precisely so a dropped hook
    ...
    itWithZsh(
      ...
      withHome(
        ...
        async (home) => {
          const { env } = launchPane(home, join(home, 'kolux-history', 'zsh_history'))

          const { values } = await runZshPty({
            env,
            report: ['ZDOTDIR', 'KOLUX_TEST_USER_ZSHRC']
          })
...
describe.skipIf(process.platform === 'win32')('the relay variant of the hook', () => {
  itWithZsh(
    ...
    withHome(USER_FILES, async (home) => {
      const relayRoot = mkdtempSync(join(tmpdir(), 'kolux-relay-wrapper-'))
      try {
        ...
        const { output, values } = await runZshPty({
          env: {
            ...
            KOLUX_ORIG_ZDOTDIR: home,
            KOLUX_SHELL_FEATURES: 'overlay,history,ready',
            KOLUX_HISTFILE: scoped,
            KOLUX_REMOTE_CLI_BIN_DIR: '/kolux/remote-bin'
          },
          commands: ['true'],
          report: ['HISTFILE', 'ZDOTDIR', 'PATH', 'KOLUX_HISTFILE']
        })

        expect(values.HISTFILE).toBe(scoped)
        expect(values.ZDOTDIR).toBe(home)
        expect(values.KOLUX_HISTFILE).toBe('UNSET')
        expect(values.PATH.startsWith('/kolux/remote-bin:')).toBe(true)
        expect(output).toContain(MARKERS.ready)
