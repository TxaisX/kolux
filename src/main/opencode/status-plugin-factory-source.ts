export function getStatusPluginFactorySource(options: { emitSessionStart: boolean }): string[] {
  return [
    ...
    '// Why: V1 hands the factory `{ client }` while V2 hands `setup` the plugin context',
    '// itself, whose domain methods (`ctx.session`, `ctx.permission`, …) are exactly what',
    '// `client` was — fall back to the argument when there is no `.client`.',
    'export const KoluxOpenCodeStatusPlugin = async (_ctx) => {',
    '  const client = _ctx?.client ?? _ctx;',
    ...
    '};',
    '',
    '// Why: both generations are supported from one export because they call different',
    '// members — 1.x only ever calls `server()`, 2.x only ever calls `setup()`.',
    '// Dropping `server()` breaks the pinned 1.x CLI; dropping `setup()` makes 2.x',
    '// reject the module with `Missing key at ["default"]["setup"]`, which is what',
    '// surfaced as "Plugin failed: …kolux-opencode-status.js".',
    'export default {',
    '  id: "kolux-opencode-status",',
    '  server: KoluxOpenCodeStatusPlugin,',
    '  setup: async (ctx) => {',
    '    const controller = new AbortController();',
    '    const hooks = await KoluxOpenCodeStatusPlugin(ctx);',
    '    void (async () => {',
    '      try {',
    ...
    '          // abort() closes the iterator synchronously, so no dispatch can start',
    '          // after setup\'s cleanup returns.',
    '          for (const v1Event of v2ToV1Events(event)) dispatchV1Event(hooks, v1Event);',
    '        }',
    '      } catch {',
    '        // An aborted or unavailable subscription is not a plugin error.',
    '      }',
    '    })();',
    '    return async () => {',
    '      controller.abort();',
    '      if (typeof hooks?.dispose !== "function") return;',
    '      try {',
    '        await hooks.dispose();',
    '      } catch {',
    '        // Teardown must never reject into OpenCode.',
    '      }',
    '    };',
    '  },',
    '};',
