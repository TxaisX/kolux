export function getStatusPluginSessionLineageSource(): string[] {
  return [
    ...
    '// flip the root Kolux pane to done or overwrite the parent turn preview.',
    ...
    '        // Why: 1.x returns `{ data: session }`, 2.x returns the session itself;',
    '        // unwrap either shape or every 2.x lookup misses and lineage goes unknown.',
    '        const info = result?.data && typeof result.data === "object" ? result.data : result;',
    '        if (info?.id === sessionID) return { data: [info] };',
    '      } catch {',
