...
describe('MiMo Code hook normalization', () => {
  it('SessionBusy maps to working', () => {
    ...
    expect(result?.payload.agentType).toBe('mimo-code')
  })

  it('SessionBusy does NOT clear the cached user prompt', () => {
    // Why: MiMo Code caches the user's MessagePart before SessionBusy fires, so the cached prompt is this turn's; clearing it would clobber the dashboard.
  ...
  it('MessagePart with role=user surfaces text as the prompt and stays working', () => {
    ...
    expect(result?.promptInteractionKey).toBe('mimo-code-message-msg-1')
  })
