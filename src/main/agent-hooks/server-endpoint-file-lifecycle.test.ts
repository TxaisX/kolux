...
describe('Endpoint file lifecycle', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kolux-endpoint-'))
  })
  ...
  it('writes the endpoint file with the expected shell-sourceable shape', async () => {
    ...
    try {
      ...
      const expectedPort = server.buildPtyEnv().KOLUX_AGENT_HOOK_PORT
      const expectedToken = server.buildPtyEnv().KOLUX_AGENT_HOOK_TOKEN
      const prefix = process.platform === 'win32' ? 'set ' : ''
      expect(contents).toContain(`${prefix}KOLUX_AGENT_HOOK_PORT=${expectedPort}`)
      expect(contents).toContain(`${prefix}KOLUX_AGENT_HOOK_TOKEN=${expectedToken}`)
      expect(contents).toContain(`${prefix}KOLUX_AGENT_HOOK_ENV=development`)
      expect(contents).toContain(`${prefix}KOLUX_AGENT_HOOK_VERSION=1`)
      expect(contents).toContain(`${prefix}KOLUX_AGENT_HOOK_TRANSPORT=raw-json-v1`)
    } finally {
      server.stop()
    }
  })
  ...
  it('rewrites the endpoint file with a new port after restart on the same path', async () => {
    ...
    const firstToken = server.buildPtyEnv().KOLUX_AGENT_HOOK_TOKEN
    server.stop()

    await server.start({ env: 'production', userDataPath })
    try {
      const secondPath = server.endpointFilePath
      const secondPort = server.buildPtyEnv().KOLUX_AGENT_HOOK_PORT
      const secondToken = server.buildPtyEnv().KOLUX_AGENT_HOOK_TOKEN
      ...
      expect(contents).toContain(`KOLUX_AGENT_HOOK_PORT=${secondPort}`)
      expect(contents).toContain(`KOLUX_AGENT_HOOK_TOKEN=${secondToken}`)
      expect(contents).not.toContain(`KOLUX_AGENT_HOOK_TOKEN=${firstToken}`)
    } finally {
      server.stop()
    }
  })
  ...
  it('leaves the endpoint file in place on stop()', async () => {
    // Why: stop() leaves the file (stale = fail-open); unlinking would race a concurrent Kolux instance rewriting it between token-check and unlink (TOCTOU).
    const server = new AgentHookServer()
    await server.start({ env: 'production', userDataPath })
    const filePath = server.endpointFilePath!
    expect(existsSync(filePath)).toBe(true)
  ...
  it('buildPtyEnv includes KOLUX_AGENT_HOOK_ENDPOINT when the server is running', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production', userDataPath })
    try {
      const env = server.buildPtyEnv()
      expect(env.KOLUX_AGENT_HOOK_ENDPOINT).toBe(server.endpointFilePath)
    } finally {
      server.stop()
    }
  })

  it('buildPtyEnv includes namespaced KOLUX_AGENT_HOOK_ENDPOINT for development servers', async () => {
    const server = new AgentHookServer()
    await server.start({
      env: 'development',
      userDataPath,
      endpointNamespace: 'com.txais.kolux.dev.test123'
    })
    try {
      const env = server.buildPtyEnv()
      expect(env.KOLUX_AGENT_HOOK_ENDPOINT).toBe(server.endpointFilePath)
      expect(env.KOLUX_AGENT_HOOK_ENDPOINT).toContain('com.txais.kolux.dev.test123')
  ...
  it('keeps endpoint files separate for parallel dev namespaces', async () => {
    ...
    try {
      ...
      expect(firstServer.buildPtyEnv().KOLUX_AGENT_HOOK_ENDPOINT).toBe(firstServer.endpointFilePath)
      expect(secondServer.buildPtyEnv().KOLUX_AGENT_HOOK_ENDPOINT).toBe(
  ...
  it('buildPtyEnv omits KOLUX_AGENT_HOOK_ENDPOINT when no userDataPath was provided', async () => {
  ...
  it('refuses to write the endpoint file when a value contains shell metacharacters', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().KOLUX_AGENT_HOOK_PORT).toBeTruthy()
      expect(server.buildPtyEnv().KOLUX_AGENT_HOOK_TOKEN).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('endpoint file contents are re-parseable by /bin/sh', async () => {
    ...
    try {
      ...
      const out = execFileSync('/bin/sh', [
        '-c',
        `. "${filePath}" && echo "$KOLUX_AGENT_HOOK_PORT"`
      ])
        .toString()
        .trim()
