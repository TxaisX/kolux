...
async function createTempHome(): Promise<string> {
  const home = await mkdtemp(join(tempRoot, 'kolux-managed-hook-runtime-'))
  tempHomes.push(home)
  return home
}
...
describe.runIf(process.platform !== 'win32')('resolveRelayGrokHome', () => {
  it('uses the login-shell GROK_HOME and normalizes trailing separators', async () => {
    ...
    await expect(resolveRelayGrokHome('/home/kolux')).resolves.toBe('/srv/grok')
  ...
  it('falls back when the login-shell GROK_HOME is not an absolute POSIX path', async () => {
    ...
    await expect(resolveRelayGrokHome('/home/kolux')).resolves.toBe('/home/kolux/.grok')
  })
...
describe.runIf(process.platform !== 'win32')('installManagedHooks', () => {
  it.each([
    ...
    async (_label, options) => {
      ...
      // Why: no agent config home, no ~/.kolux install lock, and no GROK_HOME login-shell probe.
  ...
  it('installs only the allowlisted agent, under the install lock', async () => {
    ...
    expect((await readdir(home)).sort()).toEqual(['.claude', '.kolux', SHELL_NAME, SHELL_RUNS_NAME])
  })
})
