...
vi.mock('electron', () => ({ app: { getPath: () => '/tmp/kolux-user-data' } }))
...
describe('managed hook script refresh stays off the main thread', () => {
  beforeEach(async () => {
    state.home = await mkdtemp(join(tmpdir(), 'kolux-hook-refresh-main-thread-'))
    state.syncCalls = []
  ...
  it('uses no synchronous HOME filesystem calls for missing or stale scripts', async () => {
    const hooksDir = join(state.home, '.kolux', 'agent-hooks')
