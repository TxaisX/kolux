...
async function createTempHome(): Promise<string> {
  const home = await mkdtemp(join(tmpdir(), 'kolux-managed-hook-lock-'))
  tempHomes.push(home)
  return home
}
...
async function createOwnedLock(
  ...
  const lockParent = join(home, '.kolux')
...
describe.skipIf(process.platform === 'win32')('withManagedHookInstallLock', () => {
  ...
  it('fails fast instead of stealing an unverifiable legacy directory lock', async () => {
    const home = await createTempHome()
    await mkdir(join(home, '.kolux', 'managed-hook-install.lock'), { recursive: true })
    const run = vi.fn()
  ...
  it('does not compare PID identities or steal a lock owned by another SSH host', async () => {
    const home = await createTempHome()
    const lockPath = join(home, '.kolux', 'managed-hook-install.lock')
