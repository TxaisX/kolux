...
export function wrapRuntimeHomeHookCommand(
  ...
  const windowsScript = `"\${HOME-}/.kolux/agent-hooks/${scriptBaseName}.cmd"`
  const posixScript = `"\${HOME-}/.kolux/agent-hooks/${scriptBaseName}.sh"`
  ...
  // answer comes first and the drain only runs with a Kolux env behind it (#11549).
  ...
  const powershellCommand = `$homePath = $env:HOME -replace '^/([A-Za-z])/', '$1:/'; $scriptPath = Join-Path $homePath '.kolux\\agent-hooks\\${scriptBaseName}.cmd'; if (Test-Path -LiteralPath $scriptPath -PathType Leaf) { & $scriptPath; exit $LASTEXITCODE }${powershellFallback}; ${WINDOWS_POWERSHELL_HOOK_ENVIRONMENT_GUARD}; [Console]::In.ReadToEnd() | Out-Null; exit 0`
