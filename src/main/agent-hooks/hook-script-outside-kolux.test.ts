...
describe('managed hook outside a Kolux terminal', () => {
  ...
  it('pane key present but no endpoint: still silent and writes nothing', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-outside-partial-'))
    const res = runHook(dir, { KOLUX_PANE_KEY: 'tab:0', KOLUX_TAB_ID: 'tab' })
    expect(res.status).toBe(0)
    expect(res.stdout).toBe('')
    expect(res.stderr).toBe('')
    expect(readdirSync(dir)).toEqual(['codex-hook.sh'])
  ...
  it('endpoint points at a path that does not exist: silent, exit 0', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-outside-stale-'))
    const res = runHook(dir, {
      KOLUX_AGENT_HOOK_ENDPOINT: join(dir, 'gone', 'deeper', 'endpoint.env'),
      KOLUX_PANE_KEY: 'tab:0'
    })
    expect(res.status).toBe(0)
    expect(res.stdout).toBe('')
    expect(res.stderr).toBe('')
    // a stale env var must not create a spool tree for a Kolux that is not installed here
    expect(readdirSync(dir)).toEqual(['codex-hook.sh'])
  })

  it('readable endpoint without a pane key: silent, writes nothing', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kolux-outside-readable-'))
    const endpoint = join(dir, 'endpoint.env')
    writeFileSync(endpoint, 'KOLUX_AGENT_HOOK_PORT=9\nKOLUX_AGENT_HOOK_TOKEN=stale\n')
    const res = runHook(dir, { KOLUX_AGENT_HOOK_ENDPOINT: endpoint })
    expect(res.status).toBe(0)
    expect(res.stdout).toBe('')
    expect(res.stderr).toBe('')
...
describe('antigravity out-of-band event name', () => {
  it('records hookEventName and filters tool progress on it', () => {
    const lines = buildPosixHookSpoolLines('antigravity', 'KOLUX_ANTIGRAVITY_EVENT').join('\n')
    expect(lines).toContain('"hookEventName":"%s"')
    expect(lines).toContain('${KOLUX_ANTIGRAVITY_EVENT:-}')
