...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Kolux-created worktrees whose branch Kolux is allowed to rename. */
  canRenameKoluxCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameKoluxCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Kolux auto-named — never one the user chose.
