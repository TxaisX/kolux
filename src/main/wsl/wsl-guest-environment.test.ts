...
function respondWithPayload(payload: string, code = 0): void {
  runProcessMock.mockImplementation(async (spec: { args: string[] }) => {
    ...
    const begin = /__KOLUX_WSL_CAPTURE_BEGIN_[a-z0-9]+__/.exec(script)?.[0] ?? ''
    const end = /__KOLUX_WSL_CAPTURE_END_[a-z0-9]+__/.exec(script)?.[0] ?? ''
    return {
      code,
      signal: null,
...
describe('invalidation', () => {
  it('re-probes after the caller invalidates, so a newly installed tool appears', async () => {
    ...
    // pre-install PATH until Kolux restarts, and read that as the detection bug
