 ...
 *    status both times.)
 * 2. Behaviour parity — a pane wrapped only because Kolux injected a worktree
...
describe('shell startup feature selection', () => {
  it('ignores an inherited KOLUX_SHELL_FEATURES in the spawn env', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KOLUX_SHELL_FEATURES: 'overlay,markers,ready,identity,history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
  ...
  it('cannot be turned off for a pane that needs a feature', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KOLUX_SHELL_FEATURES: '', KOLUX_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
    ).toEqual(['history'])
  })
  ...
  it('gives a history-only pane no command markers', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KOLUX_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
  ...
  it('keeps markers on a pane that already had them', () => {
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KOLUX_CODEX_HOME: '/tmp/codex' },
        ...PLAIN_PANE
      })
    ).toContain('markers')
  })
  ...
  it('does not wrap bash for history alone', () => {
    ...
    // replace its login startup-file chain with Kolux's approximation.
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/bash',
        env: { KOLUX_HISTFILE: '/tmp/wt/bash_history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
...
describePosix('zsh launch config', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kolux-feature-channel-'))
    setTestUserDataPath(userDataPath)
    previousFeatures = process.env.KOLUX_SHELL_FEATURES
  })
  ...
  it('publishes exactly the selected features, whatever process.env holds', async () => {
    process.env.KOLUX_SHELL_FEATURES = 'overlay,markers,ready,identity'
    const { getShellLaunchConfig } = await importFreshLocalPtyShellReady()

    const config = getShellLaunchConfig('/bin/zsh', ['history'])

    expect(config.env.KOLUX_SHELL_FEATURES).toBe('history')
  })
  ...
  it('wraps a startup-only command without enabling shell readiness', async () => {
    ...
    expect(config.env.KOLUX_SHELL_FEATURES).toBe('startup')
  ...
  it('does not treat another terminal’s hijacked ZDOTDIR as the user config dir', async () => {
    // Why: Kolux can be launched from a terminal that already owns ZDOTDIR. Only
    // a dir Kolux can prove is its own, or one holding no zsh startup file at
    // all, may be rejected — never a vendor guessed at by name.
    const home = mkdtempSync(join(tmpdir(), 'kolux-stacked-zdotdir-'))
    ...
    try {
      ...
      expect(getShellLaunchConfig('/bin/zsh', ['history']).env.KOLUX_ORIG_ZDOTDIR).toBeUndefined()

      // Stamped as Kolux-owned: rejected by positive identification, even though
      // the path shape is not one of Kolux's.
      ...
      expect(
        stamped.getShellLaunchConfig('/bin/zsh', ['history']).env.KOLUX_ORIG_ZDOTDIR
      ).toBeUndefined()
      ...
      expect(real.getShellLaunchConfig('/bin/zsh', ['history']).env.KOLUX_ORIG_ZDOTDIR).toBe(
        foreignWrapper
      )
    } finally {
...
describePosix('epilogue under hostile user shell options', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kolux-feature-channel-options-'))
    setTestUserDataPath(userDataPath)
    home = mkdtempSync(join(tmpdir(), 'kolux-feature-options-home-'))
  })
...
function makeUserHome(): string {
  const home = mkdtempSync(join(tmpdir(), 'kolux-feature-home-'))
...
describePosix('history-only pane in a real zsh', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kolux-feature-channel-live-'))
    setTestUserDataPath(userDataPath)
    home = makeUserHome()
  })
  ...
  function withoutInheritedZdotdir(env: Record<string, string>): Record<string, string> {
    const { KOLUX_ORIG_ZDOTDIR: _inherited, ...rest } = env
    return rest
  }
  ...
  async function launchHistoryOnly(): Promise<{
    ...
    return {
      ...
      env: {
        PATH: '/usr/bin:/bin',
        ...spawnEnv,
        // Why KOLUX_ORIG_ZDOTDIR is stripped: the launch config computes it from
  ...
  itWithZsh('unsets the feature channel for the shell and for a grandchild', async () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        '-i',
        '-c',
        `print -r -- "SELF=[\${KOLUX_SHELL_FEATURES:-}]"; zsh -c 'printenv KOLUX_SHELL_FEATURES; print -r -- CHILD_CLEAN'`
      ],
  ...
  itWithZsh(
    ...
    async () => {
      ...
      // started with -c would report a pane Kolux had not finished setting up.
      const { env } = await launchHistoryOnly()
      const capture = [
        'PRECMD="${precmd_functions[*]}"; PREEXEC="${preexec_functions[*]}"',
        'LINEINIT="${widgets[zle-line-init]:-none}"'
      ]
      const report = ['PRECMD', 'PREEXEC', 'LINEINIT', 'ZDOTDIR', 'KOLUX_SHELL_FEATURES']
      ...
      // place: a history-only pane leaves no Kolux name in the user's hook arrays.
      ...
      // installs its own zle-line-init widget has one either way, and what Kolux
      // owes is that it looks the same wrapped as unwrapped.
      expect(wrapped.values.LINEINIT).toBe(unwrapped.values.LINEINIT)
      expect(wrapped.values.PRECMD).not.toContain('kolux')
      ...
      expect(wrapped.values.KOLUX_SHELL_FEATURES).toBe('UNSET')
    }
  )
})
