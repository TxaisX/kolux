...
const ROLLOUT_LINUX =
  '/home/ada/.local/share/kolux/codex-runtime-home/home/sessions/2026/07/24/rollout-sess.jsonl'
const ROLLOUT_UNC =
  '\\\\wsl.localhost\\Ubuntu\\home\\ada\\.local\\share\\kolux\\codex-runtime-home\\home\\sessions\\2026\\07\\24\\rollout-sess.jsonl'
...
describe('wslCodexSessionsDirs', () => {
  ...
  it('lists the managed and system Codex roots per distro home', async () => {
    await expect(
      ...
      `${UBUNTU_HOME}\\.local\\share\\kolux\\codex-runtime-home\\home\\sessions`,
      `${UBUNTU_HOME}\\.codex\\sessions`
    ])
  })

  it('includes WSL managed-account session roots supplied by the runtime', async () => {
    const accountHome = `${UBUNTU_HOME}\\.local\\share\\kolux\\codex-accounts\\account-1\\home`
    configureHostReadableTranscriptPathSources({
