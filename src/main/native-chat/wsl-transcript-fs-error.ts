export const WSL_TRANSCRIPT_FS_SLOW_MESSAGE =
  'WSL transcript files are temporarily unavailable because filesystem access is taking too long. Try again shortly or restart Kolux if the issue continues.'
export const WSL_TRANSCRIPT_FS_CAPACITY_MESSAGE =
  'WSL transcript discovery is temporarily unavailable because too many filesystem requests are already waiting. Try again shortly or restart Kolux if the issue continues.'
...
export function wslTranscriptFsProcessFailureError(detail: unknown): WslTranscriptFsError {
  ...
  return new WslTranscriptFsError(
    'unavailable',
    `${WSL_TRANSCRIPT_FS_PROCESS_FAILURE_PREFIX} (${text}). Try again shortly or restart Kolux if the issue continues.`
  )
}
