...
// This is Kolux's own `log.jsonl`, which no build after the SQLite move reads.
  ...
  return {
    ...
    body: {
      ...
      text: boundJournalStatusText(
        `This chat's history was saved in an older format Kolux no longer reads, so it starts ` +
