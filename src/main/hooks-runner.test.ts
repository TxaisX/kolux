...
describe('createSetupRunnerScript', () => {
  ...
  it('writes a fail-fast Windows runner that returns after batch commands', async () => {
    ...
    execFileSyncMock.mockReturnValue('C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.cmd')
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.cmd',
        envVars: expect.objectContaining({
          KOLUX_ROOT_PATH: '/test/repo',
          KOLUX_WORKTREE_PATH: 'C:\\repo\\feature\\',
          KOLUX_WORKSPACE_NAME: 'feature'
        }),
        // Why: native Windows worktrees without a configured setup shell keep the cmd runner.
        shell: { family: 'cmd' }
      })
      expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith(
        'C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.cmd',
        [
          '@echo off',
  ...
  it('converts path env vars to MSYS form for a native Windows Git Bash runner', async () => {
    ...
    execFileSyncMock.mockReturnValue('C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.sh')
    Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })

    try {
      const { createSetupRunnerScript } = await import('./worktree-runner-script')
      const result = createSetupRunnerScript(
        { ...makeRepo(), path: 'C:\\Users\\jinwo\\git\\kolux' },
        'C:\\repo\\feature',
        '#!/usr/bin/env bash\npnpm install',
        undefined,
        { family: 'posix' }
      ...
      expect(result).toEqual({
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.sh',
        envVars: expect.objectContaining({
          KOLUX_ROOT_PATH: '/c/Users/jinwo/git/kolux',
          KOLUX_WORKTREE_PATH: '/c/repo/feature',
          CONDUCTOR_ROOT_PATH: '/c/Users/jinwo/git/kolux',
          GHOSTX_ROOT_PATH: '/c/Users/jinwo/git/kolux',
      ...
      expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith(
        'C:\\repo\\.git\\worktrees\\feature\\kolux\\setup-runner.sh',
        '#!/usr/bin/env bash\nset -e\npnpm install\n',
        'utf-8'
      )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const dir = mkdtempSync(join(tmpdir(), 'kolux-posix-runner-'))
      try {
  ...
  it('derives KOLUX_WORKSPACE_NAME from a POSIX worktree path', async () => {
    const originalPlatform = process.platform

    execFileSyncMock.mockReturnValue('/test/repo/.git/worktrees/feature/kolux/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    })
    ...
    try {
      ...
      expect(result.envVars).toEqual(
        expect.objectContaining({
          KOLUX_WORKTREE_PATH: '/test/repo-feature',
          KOLUX_WORKSPACE_NAME: 'repo-feature'
        })
      )
    } finally {
      Object.defineProperty(process, 'platform', {
  ...
  it('translates WSL runner paths and env vars to Linux form on Windows', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/.git/worktrees/feature/kolux/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath:
          '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.git\\worktrees\\feature\\kolux\\setup-runner.sh',
        envVars: expect.objectContaining({
          KOLUX_ROOT_PATH: '/mnt/c/Users/jinwo/git/kolux',
          KOLUX_WORKTREE_PATH: '/home/jin/feature',
          KOLUX_WORKSPACE_NAME: 'feature',
          CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/kolux',
          GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/kolux'
        })
      })
      expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith(
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.git\\worktrees\\feature\\kolux\\setup-runner.sh',
  ...
  it('translates WSL env vars to Linux paths when the worktree lives on a WSL UNC path', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/repo/.git/worktrees/feature/kolux/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        envVars: expect.objectContaining({
          KOLUX_ROOT_PATH: '/test/repo',
          KOLUX_WORKTREE_PATH: '/home/jin/repo/feature',
          KOLUX_WORKSPACE_NAME: 'feature',
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX runner under the worktree git dir for long issue commands', async () => {
    ...
    execFileSyncMock.mockReturnValue(
      '/test/repo/.git/worktrees/feature/kolux/issue-command-runner.sh'
    )
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: '/test/repo/.git/worktrees/feature/kolux/issue-command-runner.sh',
        envVars: expect.objectContaining({
          KOLUX_ROOT_PATH: '/test/repo',
          KOLUX_WORKTREE_PATH: '/test/repo-feature'
        })
  ...
  it('carries the WSL launch shell for a Windows-drive worktree routed through WSL', async () => {
    ...
    execFileSyncMock.mockReturnValue('/mnt/c/repo/.git/kolux/issue-command-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result.runnerScriptPath).toBe('C:\\repo\\.git\\kolux\\issue-command-runner.sh')
  ...
  it('keeps native Windows issue runners on the cmd launch shell', async () => {
    ...
    execFileSyncMock.mockReturnValue('C:\\repo\\.git\\kolux\\issue-command-runner.cmd')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
