...
const SNAPSHOT_FILE = 'kolux-workspace-space-analysis.json'
...
describe('workspace space analysis snapshot', () => {
  beforeEach(async () => {
    ...
    userDataDirHolder.dir = await mkdtemp(join(tmpdir(), 'kolux-space-snapshot-'))
  })
  ...
  it('keeps profile snapshots isolated', async () => {
    const otherProfile = await mkdtemp(join(tmpdir(), 'kolux-space-snapshot-other-'))
    try {
      await persistWorkspaceSpaceAnalysisSnapshot(
        userDataDirHolder.dir,
