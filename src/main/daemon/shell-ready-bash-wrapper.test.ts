...
describePosix('daemon shell-ready bash wrapper', () => {
  ...
  it('emits OSC 133 C/D markers in the daemon bash wrapper', async () => {
    ...
    expect(bashRc).toContain('__kolux_append_prompt_command "__kolux_osc133_epilogue"')
    ...
    expect(bashRc.lastIndexOf("trap '__kolux_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('__kolux_append_prompt_command "__kolux_osc133_epilogue"')
    )
  ...
  itWithBash(
    ...
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      // Minimal bash-preexec imitation: re-arms its own DEBUG trap from PROMPT_COMMAND at first prompt, silencing Kolux's trap.
      writeFileSync(
        join(userDataPath, '.bash_profile'),
        [
          'preexec_functions=()',
  ...
  itWithBash(
    'dispatches a non-empty preexec_functions against the real command, not Kolux hooks',
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      ...
      expect(output).not.toContain('USER_PREEXEC:__kolux_osc133')
