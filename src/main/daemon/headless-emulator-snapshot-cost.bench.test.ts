...
//   KOLUX_TERMINAL_PERF_BENCH=1 pnpm vitest run \
...
const benchEnabled = process.env.KOLUX_TERMINAL_PERF_BENCH === '1'
...
describe.skipIf(!benchEnabled)('headless emulator snapshot cost (issue #5096 harness)', () => {
  it('measures daemon checkpoint cost across history sizes', () => {
    ...
    writeBenchReport('kolux-headless-snapshot-bench.json', {
      interpretation:
  ...
  it('measures PTY pump stall when a checkpoint lands mid-stream', async () => {
    ...
    writeBenchReport('kolux-checkpoint-interference-bench.json', {
      interpretation:
