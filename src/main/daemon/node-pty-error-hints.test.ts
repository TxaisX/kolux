...
describe('node-pty diagnostic error hints', () => {
  it('parses the native step and errno without dropping the original message', () => {
    ...
    expect(addNodePtyRecoveryHint(message)).toBe(
      `Daemon's node-pty install is gone (worktree deleted?). Restart Kolux. ${message}`
    )
  })
  ...
  it('hints local wrapped spawn errors from pty allocation failures', () => {
    const message =
      'Failed to spawn shell "/bin/zsh": node-pty: open_slave failed: EMFILE (errno 24, Too many open files) - slave=\'/dev/ttys003\' (shell: /bin/zsh, cwd: /tmp, arch: arm64, platform: darwin 25.0.0, kolux: 1.4.178). If this persists, please file an issue.'
