  ...
  describe('ensureRunning', () => {
    ...
    it('uses protocol-scoped socket and token paths', () => {
      ...
      if (process.platform === 'win32') {
        expect(socketPath).toContain(`kolux-terminal-host-v${PROTOCOL_VERSION}`)
      } else {
    ...
    it('starts daemon and returns connection info', async () => {
      ...
      if (process.platform === 'win32') {
        expect(info.socketPath).toContain(`kolux-terminal-host-v${PROTOCOL_VERSION}`)
      } else {
...
describe('daemon socket publication', () => {
  it('keeps every scratch namespace outside the released sweeper pattern', () => {
    ...
    for (let i = 0; i < 20; i++) {
      const names = [
        basename(getDaemonSocketBindPath(getDaemonSocketPath('/tmp/kolux-daemon'))),
        basename(getDaemonPidSwapClaimPath('/tmp/kolux-daemon/daemon-v32.pid')),
        basename(getDaemonArtifactHoldClaimPath('/tmp/kolux-daemon/daemon-v32.token'))
      ]
  ...
  it('keeps the bind name shorter than the canonical endpoint', () => {
    ...
    const canonicalPath = getDaemonSocketPath('/tmp/kolux-daemon-runtime')
