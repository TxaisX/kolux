...
describe('createMainWindow', () => {
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.KOLUX_E2E_HEADLESS
    process.env.KOLUX_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.KOLUX_E2E_HEADLESS
      } else {
        process.env.KOLUX_E2E_HEADLESS = previousHeadless
      }
    }
  })
