...
describe('attachMainWindowServices', () => {
  ...
  // Why deleted: "passes injected update quit cleanup to the auto-updater" and "flushes the
  // store before update quit when no cleanup is injected" asserted that attachMainWindowServices
  // itself wires a ready-to-show listener into setupAutoUpdater. It doesn't — see the
  // "fork: no auto-updater scheduling" comment in attach-main-window-services.ts, present since
  // this repo's root commit (21231c88). That wiring now lives in main-window-updater.ts's
  // ensureAutoUpdaterConfigured (exported from this same module but not called by
  // attachMainWindowServices), which currently has no dedicated test file.
