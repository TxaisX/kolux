...
const STAGING_ROOT = join(TEMP_ROOT, `kolux-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `kolux-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `kolux-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('kolux-clipboard-file-lookalike'),
