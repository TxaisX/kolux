...
function userDataWithProfiles(
  ...
  const root = mkdtempSync(join(tmpdir(), 'kolux-gc-profiles-'))
  roots.push(root)
  mkdirSync(join(root, 'profiles'), { recursive: true })
  writeFileSync(
    join(root, 'kolux-profile-index.json'),
    JSON.stringify({
      activeProfileId,
  ...
  for (const profile of profiles) {
    ...
    if (profile.raw !== undefined) {
      writeFileSync(join(root, 'profiles', profile.id, 'kolux-data.json'), profile.raw)
    } else if (profile.state !== undefined) {
      writeFileSync(
        join(root, 'profiles', profile.id, 'kolux-data.json'),
