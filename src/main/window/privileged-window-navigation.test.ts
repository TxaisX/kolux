...
describe('privileged window navigation policy', () => {
  ...
  it('lets the packaged renderer document reload itself', () => {
    const appUrl =
      'file:///Applications/Kolux.app/Contents/Resources/app.asar/out/renderer/index.html'
  ...
  it('blocks a foreign file host and a data: URL that reuse the renderer path', () => {
    ...
    expect(
      fixture.willNavigate(
        'file://evil.example/Applications/Kolux.app/Contents/Resources/app.asar/out/renderer/index.html'
      ).preventDefault
    ).toHaveBeenCalled()
    expect(
