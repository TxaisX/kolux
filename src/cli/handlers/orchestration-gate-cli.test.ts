...
describe('orchestration gate commands carry caller identity', () => {
  ...
  afterEach(() => {
    ...
    restoreEnv('KOLUX_PANE_KEY', originalPaneKey)
    process.exitCode = 0
  })
  ...
  it('sends the bound coordinator handle to gateCreate', async () => {
    process.env.KOLUX_TERMINAL_HANDLE = 'term_coord'
    queueFixtures(
      callMock,
  ...
  it('remints a stale environment handle before authorizing gateCreate', async () => {
    process.env.KOLUX_TERMINAL_HANDLE = 'term_stale'
  ...
  it('reports idempotent recovery when a mutation connection drops', async () => {
    ...
    callMock
      ...
      .mockRejectedValueOnce(
        new RuntimeClientError(
          'runtime_unavailable',
          'The Kolux runtime closed the connection before responding. Restart Kolux and try again. Orchestration mutation request ID: mutation_1.',
          {
    ...
    expect(output.error.message).not.toMatch(/restart Kolux/i)
