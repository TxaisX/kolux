...
describe('absolute file CLI paths', () => {
  ...
  it('reproduces the issue positional command without invalid_relative_path', async () => {
    const issuePath = '/root/kolux/workspaces/xxx/xxx/xxx.ts'
    callMock.mockImplementation(async (method: string, params: { relativePath?: string }) => {
      if (method === 'worktree.list') {
        return worktreeListFixture([buildWorktree('/root/kolux/workspaces/xxx', 'feature')])
      }
      if (method === 'worktree.show') {
        return okFixture('req_show', {
          worktree: buildWorktree('/root/kolux/workspaces/xxx', 'feature')
        })
      }
    ...
    await main(['file', 'open', issuePath], '/root/kolux/workspaces/xxx')
    ...
    expect(callMock).toHaveBeenNthCalledWith(2, 'worktree.show', {
      worktree: 'id:repo::/root/kolux/workspaces/xxx'
    })
  ...
  describe('WSL UNC worktree roots', () => {
    const uncRoot = '//wsl.localhost/Ubuntu/root/kolux/workspaces/xxx'
    ...
    it('relativizes a Linux path typed inside the distro against the UNC root', async () => {
      ...
      await openPath('/root/kolux/workspaces/xxx/xxx/xxx.ts')
    ...
    it('relativizes against a legacy backslash wsl$ root', async () => {
      vi.stubEnv('WSL_DISTRO_NAME', 'Ubuntu')
      mockWorktreeShow('\\\\wsl$\\Ubuntu\\root\\kolux\\workspaces\\xxx')
    ...
    it('leaves a Linux path containing a backslash for the runtime guard', async () => {
      ...
      await openPath('/root/kolux/workspaces/xxx/a\\b.ts')

      expectOpenedWith('/root/kolux/workspaces/xxx/a\\b.ts')
    })
    ...
    it('leaves a Linux path from another distro for the runtime guard', async () => {
      ...
      expectOpenedWith('/root/kolux/workspaces/xxx/xxx/xxx.ts')
    })
    ...
    it('keeps relativizing POSIX roots while WSL_DISTRO_NAME is set', async () => {
      vi.stubEnv('WSL_DISTRO_NAME', 'Ubuntu')
      mockWorktreeShow('/root/kolux/workspaces/xxx')
    ...
    it('does not relativize a sibling directory that merely shares the root prefix', async () => {
      ...
      await openPath('/root/kolux/workspaces/xxx-2/xxx.ts')

      expectOpenedWith('/root/kolux/workspaces/xxx-2/xxx.ts')
    })
    ...
    it('leaves a Linux path whose own case does not match the root', async () => {
      ...
      await openPath('/root/kolux/Workspaces/xxx/xxx.ts')

      expectOpenedWith('/root/kolux/Workspaces/xxx/xxx.ts')
    })
    ...
    // always sets KOLUX_CLI_CWD, which reaches the handler as the invocation cwd.
    it('falls back to the distro named by a UNC cwd', async () => {
      mockWorktreeShow(uncRoot)

      await openPath(
        '/root/kolux/workspaces/xxx/xxx/xxx.ts',
        '\\\\wsl.localhost\\Ubuntu\\root\\kolux\\workspaces\\xxx\\xxx'
      )
    ...
    it('rejects the UNC worktree root itself typed as a Linux path', async () => {
      ...
      await openPath('/root/kolux/workspaces/xxx')
