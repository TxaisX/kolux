...
// It fails when `kolux serve` grows a flag that a direct `<binary> serve …` launch would silently drop (#12677).
...
describe('serve flag parity between the CLI spec and the Electron argv rewrite', () => {
  ...
  it.each(translatedFlags)('rewrites CLI-form --%s into the --serve-* form', (flag) => {
    ...
    if (takesValue) {
      // The equals form is the other shape `kolux serve` accepts; normalize it to the internal shape.
    ...
    // Idempotent: `kolux serve` spawns the app already in this shape, and the rewrite runs over it too.
  ...
  it('emits the same --serve-* names the CLI spawns with and the main process reads', () => {
    // Why source text: serveKoluxApp spawns a real process; keeping both names visible here makes
