...
// Why: the SSH relay bridge executes this CLI on the Kolux host while the
// caller's shell cwd lives on the remote machine (which cannot be chdir'd
// into). KOLUX_CLI_CWD carries that remote cwd so cwd-based selectors like
// `--worktree active` resolve against the caller's directory.
function resolveInvocationCwd(): string {
  const override = process.env.KOLUX_CLI_CWD
...
export async function main(
  ...
  if (argv.length === 1 && (argv[0] === '--version' || argv[0] === '-v')) {
    const version = readKoluxCliVersion()
    if (!version) {
      process.stderr.write('Could not determine the Kolux version for this build.\n')
      process.exitCode = 1
      return
    }
  ...
  try {
    ...
    // lookup so users do not get misleading "Kolux is not running" failures for
    ...
    // Why: only the explicit flag is asserted eagerly. An ambient KOLUX_ENVIRONMENT is background
    ...
    // An ambient KOLUX_ENVIRONMENT is checked for disagreement too — silently
      ...
      : await resolveHostFlagEnvironmentId(parsed.flags, {
          ...
          environmentSelector:
            typeof environmentSelector === 'string'
              ...
              : process.env.KOLUX_ENVIRONMENT
                ? { value: process.env.KOLUX_ENVIRONMENT, label: 'KOLUX_ENVIRONMENT' }
                : null
        })
    ...
    // KOLUX_PAIRING_CODE / KOLUX_ENVIRONMENT env-var fallback for commands
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `kolux claude-teams` belongs to Claude Code, not
    // Kolux's own flag parser, so new Claude flags work without Kolux changes.
...
async function runAgentTeamsTmuxShim(argv: string[]): Promise<void> {
  try {
    ...
    const response = await client.call<{
      ...
      {
        teamId: process.env.KOLUX_AGENT_TEAMS_TEAM_ID,
        token: process.env.KOLUX_AGENT_TEAMS_TOKEN,
