export const ROOT_HELP_TEXT_SECONDARY = [
  ...
  '  kolux status [--json]',
  '  kolux diagnostics memory [--json]',
  '  kolux agent-context [--json]',
  '  kolux account add [--agent claude|codex] [--json]',
  '  kolux account list [--json]',
  '  kolux host list [--json]',
  '  kolux environment add --name <name> --pairing-code <code> [--json]',
  '  kolux environment list [--json]',
  '  kolux environment show --environment <selector> [--json]',
  '  kolux environment rm --environment <selector> [--json]',
  '  kolux worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  '  kolux worktree rm --worktree <selector> [--force] [--run-hooks] [--json]',
  '  kolux worktree ps [--limit <n>] [--json]',
  '  kolux file open <path> [--worktree <selector>] [--json]',
  '  kolux file diff <path> [--staged] [--worktree <selector>] [--json]',
  '  kolux file open-changed [--mode edit|diff|both] [--worktree <selector>] [--json]',
  '  kolux terminal list [--worktree <selector>] [--limit <n>] [--include-visual-layouts] [--json]',
  '  kolux terminal show [--terminal <handle>] [--json]',
  '  kolux terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]',
  ...
  '  kolux terminal split [--terminal <handle>] [--direction horizontal|vertical] [--json]',
  '  kolux terminal switch [--terminal <handle>] [--json]',
  '  kolux terminal close ([--terminal <handle>] [--tab] | --worktree <selector> --all) [--json]',
  '  kolux project list [--json]',
  '  kolux project setups [--project <id>] [--host <host-id>] [--json]',
  ...
  '  kolux project setup-delete --setup <setup-id> [--json]',
  '  kolux repo list [--json]',
  '  kolux repo add --path <path> [--json]',
  '  kolux repo show --repo <selector> [--json]',
  '  kolux repo set-base-ref --repo <selector> --ref <ref> [--json]',
  '  kolux repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
  '',
  'Selectors:',
  ...
  '  --terminal <handle>       Runtime-issued terminal handle returned by `kolux terminal list --json`',
  ...
  '  --pairing-code <code>      Connect to a remote Kolux runtime using a kolux://pair?... code',
  '  --environment <selector>   Connect using a saved environment id or name',
  '  --help                    Show this help message',
  '',
  'Behavior:',
  '  Most commands require a running Kolux runtime. If Kolux is not open yet, run `kolux open` first.',
  '  Remote runtime access can also be supplied with KOLUX_PAIRING_CODE or KOLUX_ENVIRONMENT.',
  ...
  '',
  'Browser Workflow:',
  '  1. Create or navigate:  kolux tab create --url https://example.com',
  '                          kolux goto --url https://example.com',
  '  2. Inspect the page:    kolux snapshot',
  '     (Returns an accessibility tree with element refs like e1, e2, e3)',
  '     For concurrent workflows, prefer: kolux tab list --json',
  '     then reuse tabs[].browserPageId with --page <id> on later commands.',
  '  3. Interact:            kolux click --element e2',
  '                          kolux fill --element e5 --value "search query"',
  '                          kolux keypress --key Enter',
  '  4. Re-inspect:          kolux snapshot',
  ...
  '  $ kolux diagnostics memory --json',
  '  $ kolux repo list',
  '  $ kolux worktree create --name agent-task --agent codex --prompt "hi"',
  '  $ kolux worktree create --repo name:kolux --name cli-test-1 --issue 273',
  '  $ kolux worktree create --repo name:kolux --name linear-task --linear-issue https://linear.app/stably/issue/STA-335/test-issue',
  '  $ kolux worktree create --name linear-task --linear-issue STA-335',
  '  $ kolux worktree show --worktree branch:Jinwoo-H/cli',
  '  $ kolux worktree current',
  '  $ kolux worktree set --worktree active --comment "waiting on review"',
  '  $ kolux worktree set --worktree active --linear-issue null',
  '  $ kolux worktree ps --limit 10',
  '  $ kolux file open-changed --mode diff',
  '  $ kolux file open src/App.tsx',
  '  $ kolux terminal create --worktree active --command "codex"',
  '  $ kolux terminal list --worktree path:/Users/me/kolux/workspaces/kolux/cli-test-1 --json',
  '  $ kolux terminal send --terminal term_123 --text "hi" --enter',
  '  $ kolux terminal wait --terminal term_123 --for exit --timeout-ms 60000 --json',
  '  $ kolux tab current --json',
  '  $ kolux tab show --page page_123 --json',
  '  $ kolux tab create --url https://example.com --profile work',
  '  $ kolux tab profile clone --page page_123 --profile work --json',
  '  $ kolux snapshot',
  '  $ kolux click --element e3',
  '  $ kolux fill --element e5 --value "hello"',
  '  $ kolux goto --url https://example.com/login',
  '  $ kolux keypress --key Enter',
  '  $ kolux eval --expression "document.title"',
  '  $ kolux tab list --json'
].join('\n')
