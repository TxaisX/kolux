...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Kolux host',
    usage: 'kolux account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Kolux runtime.',
      ...
      '--agent defaults to claude. Requires the Kolux runtime to be running on this machine.'
    ],
    examples: ['kolux account add', 'kolux account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Kolux host',
    usage: 'kolux account list [--json]',
