...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    summary: 'Start one supervised worker on the Run home or a connected Kolux server',
    usage:
      'kolux orchestration worker-start (--task <task_id> | --spec <text>) [--on <saved-environment>] [--worktree <current|selector|new-child|new-top-level>] (--agent <agent> | --terminal <handle> | --tier <major|deep|build|light>) [--task-title <text>] [--deps <json_array>] [--parent <task_id>] [--model <id>] [--effort <level>] [--name <name>] [--repo <selector>] [--base-branch <ref>] [--display-name <text>] [--comment <text>] [--setup <run|skip|inherit>] [--retry-of <dispatch_id>] [--timeout-ms <n>] [--run <run_id>] [--from <handle>] [--retry-request <id>] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'task',
      'spec',
    ...
    notes: [
      ...
      "Providers never mix: a worker always runs on the coordinator's own agent when that agent is known, whether started with --agent or by reusing --terminal. A mismatch is rejected.",
      "--tier classifies the task (major/deep/build/light) and resolves model and effort from Settings > Agents > Model routing for the coordinator's own agent; it cannot combine with --model, --effort, or --terminal. --agent may still be passed alongside --tier, but only to name the same agent as the coordinator (or to supply one when the coordinator's agent is unknown) — it never picks a different provider.",
      ...
      'Not every worker has a terminal. Read output with worker-read --source auto or --source transcript, which always work; --source terminal is refused when there is none, and kolux terminal verbs do not accept every worker handle. Nothing above needs you to know which kind you have — the orchestration verbs cover all of them.',
      '--on selects only the worker server; the Run and this command remain on the current Kolux server.',
  ...
  },
  {
    ...
    usage: 'kolux orchestration worker-show --dispatch <dispatch_id> [--json]',
    ...
    notes: [
      ...
      'observation.agentWait names a worker parked on a prompt only a human can answer, with the evidence that proved it (hook, prompt-text, or title). Null means Kolux looked and found no wait. An absent field means it never looked — an older host, an unverifiable worker identity, an unreadable pane, or an agent probe that did not answer in time — and never means the worker is not waiting. A waiting worker is healthy, not failed.'
    ]
  },
  {
    ...
    usage:
      'kolux orchestration worker-read --dispatch <dispatch_id> [--source <auto|transcript|terminal>] [--cursor <cursor>] [--limit <n>] [--json]',
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Kolux reports source_changed.'
    ]
  },
  {
    ...
    usage:
      'kolux orchestration worker-stop --dispatch <dispatch_id> [--retry-request <id>] [--json]',
  ...
  },
  {
    ...
    usage:
      'kolux orchestration worker-abandon --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
  ...
  },
  {
    ...
    usage:
      'kolux orchestration worker-release --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'kolux orchestration worker-retain --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'kolux orchestration worker-list [--run <run_id>] [--terminal-state <active|reclaimable|retained|release_pending|release_unknown|released>] [--include-remote] [--cursor <cursor>] [--limit <1-100>] [--json]',
