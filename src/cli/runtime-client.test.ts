...
import { launchKoluxApp } from './runtime/launch'
...
describe.skipIf(process.platform === 'win32')('RuntimeClient', () => {
  it('adds an opaque durable request ID only to orchestration mutations', async () => {
    ...
    const priorLaunchToken = process.env.KOLUX_AGENT_LAUNCH_TOKEN
    process.env.KOLUX_AGENT_LAUNCH_TOKEN = 'launch-secret'
    ...
    try {
      ...
      if (priorLaunchToken === undefined) {
        delete process.env.KOLUX_AGENT_LAUNCH_TOKEN
      } else {
        process.env.KOLUX_AGENT_LAUNCH_TOKEN = priorLaunchToken
      }
    }
  ...
  it('openKolux activates the app even when a desktop runtime is already reachable', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openKolux(100)

    expect(status.result.runtime.state).toBe('ready')
    expect(status.result.runtime.reachable).toBe(true)
    expect(launchKoluxApp).toHaveBeenCalledOnce()
  })

  it('openKolux waits for a reachable headless runtime to expose a desktop window', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openKolux(1_000)
  ...
  it('openKolux fails explicitly when the serve owner cannot promote safely', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kolux-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    await expect(client.openKolux(100)).rejects.toMatchObject({
