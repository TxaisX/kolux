...
import { launchKoluxApp, serveKoluxApp } from './launch'
...
const SERVE_INSTALL_STATUS = '[serve] kolux CLI install: installed'
...
describe('serveKoluxApp', () => {
  const temporaryDirectories: string[] = []

  beforeEach(() => {
    spawnMock.mockReset()
    spawnSyncMock.mockReset()
    process.env.KOLUX_APP_EXECUTABLE = '/Applications/Kolux.app/Contents/MacOS/Kolux'
  })

  afterEach(() => {
    vi.restoreAllMocks()
    delete process.env.KOLUX_APP_EXECUTABLE
    delete process.env.KOLUX_APP_EXECUTABLE_NEEDS_APP_ROOT
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-serve-update-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Kolux.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Kolux')
      ...
      process.env.KOLUX_APP_EXECUTABLE = executable
      ...
      const supervisor = serveKoluxApp({ json: true }).then((code) => {
        supervisorExited = true
        return code
      })
      const childEnv = spawnMock.mock.calls[0]?.[2]?.env as NodeJS.ProcessEnv | undefined
      const handoffPath = childEnv?.KOLUX_SERVE_UPDATE_HANDOFF_PATH
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-serve-update-mismatch-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Kolux.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Kolux')
      const userDataPath = join(root, 'user-data')
      ...
      const supervisor = serveKoluxApp({ json: true })
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-serve-update-spawn-failure-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Kolux.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Kolux')
      const userDataPath = join(root, 'user-data')
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      vi.useFakeTimers()
      const root = await mkdtemp(join(tmpdir(), 'kolux-serve-update-no-readiness-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'Kolux.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'Kolux')
      const userDataPath = join(root, 'user-data')
  ...
  it('pins the Electron child cwd to the app root instead of the caller cwd', async () => {
    ...
    await expect(serveKoluxApp({ json: true })).resolves.toBe(0)
  ...
  it('passes the app root before serve flags for dev Electron executables', async () => {
    process.env.KOLUX_APP_EXECUTABLE = '/repo/node_modules/.bin/electron'
    process.env.KOLUX_APP_EXECUTABLE_NEEDS_APP_ROOT = '1'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
    ...
    await expect(serveKoluxApp({ json: true, port: '6768' })).resolves.toBe(0)
  ...
  it.each([
    ...
    async ({ result: userNamespaceResult, expectedPrefix }) => {
      ...
      const root = await mkdtemp(join(tmpdir(), 'kolux-extracted-appimage-'))
      temporaryDirectories.push(root)
      const executable = join(root, 'kolux-ide')
      ...
      try {
        const result = serveKoluxApp({ json: true })
  ...
  it('rejects when the server exits without valid recipe JSON', async () => {
    ...
    const untrustedLines = [
      'KOLUX://pair?code=UPPER-SECRET',
      'kolux://pair/?code=SLASH-SECRET',
      'kolux://pair#LEGACY-SECRET',
    ...
    await expect(result).rejects.toMatchObject({
      ...
      message: 'Kolux serve exited before printing valid recipe JSON with code 0.'
    })
  ...
  it('uses a shell when a Windows npm command shim is the Electron executable', async () => {
    ...
    process.env.KOLUX_APP_EXECUTABLE = 'C:\\repo\\node_modules\\.bin\\electron.cmd'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
...
describe('launchKoluxApp', () => {
  ...
  afterEach(() => {
    delete process.env.KOLUX_OPEN_COMMAND
    delete process.env.KOLUX_APP_EXECUTABLE
    delete process.env.KOLUX_APP_EXECUTABLE_NEEDS_APP_ROOT
  })

  it('handles asynchronous detached spawn errors without throwing', async () => {
    process.env.KOLUX_APP_EXECUTABLE = '/missing/Kolux'
    const child = new FakeChildProcess()
    spawnMock.mockReturnValue(child)
  ...
  it('adds the extracted-AppImage sandbox fallback for open launches', async () => {
    const platformDescriptor = Object.getOwnPropertyDescriptor(process, 'platform')
    const getuidDescriptor = Object.getOwnPropertyDescriptor(process, 'getuid')
    const root = await mkdtemp(join(tmpdir(), 'kolux-open-extracted-appimage-'))
    const executable = join(root, 'kolux-ide')
