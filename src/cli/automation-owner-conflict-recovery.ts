...
const RECOVERY: Record<string, readonly string[]> = {
  ...
  [AUTOMATION_OWNER_CONFLICT_CODES.targetRemoved]: [
    ...
    'Re-add that SSH host, or delete the automation with `kolux automations remove --id <id>`.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.fencingRequired]: [
    ...
    'Update Kolux on the host, then run the command again.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.invalidDestination]: [
    ...
    'Run `kolux automations show --id <id>` to see where it lives, then target a host that authority knows.'
  ]
}
