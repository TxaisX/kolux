...
const pairingInput = process.env.KOLUX_REMOTE_ACCESS_PAIRING_INPUT
const proxyEndpoint = process.env.KOLUX_REMOTE_ACCESS_PROXY_ENDPOINT
const secondPairingInput = process.env.KOLUX_REMOTE_ACCESS_SECOND_PAIRING_INPUT
...
async function exerciseCatalogAndFilesystem(client: WebRuntimeClient): Promise<void> {
  ...
  expect(preview.content).toContain('"name": "kolux"')
...
describe.runIf(Boolean(pairingInput && proxyEndpoint))(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      if (!parsed) {
        throw new Error('KOLUX_REMOTE_ACCESS_PAIRING_INPUT is not a valid pairing input.')
      }
      directOffer = parsed
    ...
    it.runIf(Boolean(secondPairingInput))(
      ...
      async () => {
        ...
        if (!secondOffer) {
          throw new Error('KOLUX_REMOTE_ACCESS_SECOND_PAIRING_INPUT is not valid.')
        }
