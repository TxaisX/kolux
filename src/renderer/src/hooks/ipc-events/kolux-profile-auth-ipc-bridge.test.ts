...
import { registerKoluxProfileAuthIpcBridge } from './kolux-profile-auth-ipc-bridge'
...
const reconnectRequiredAuthStatus: KoluxProfileAuthStatus = {
  activeProfileId: 'local-default',
  configured: true,
...
describe('kolux profile auth IPC bridge', () => {
  ...
  it('re-fetches auth status on the push, flipping connected to reconnect-required', async () => {
    ...
    await store.getState().fetchKoluxProfileAuthStatus()
    expect(store.getState().koluxProfileAuthStatus).toEqual(connectedAuthStatus)

    const unsubs: (() => void)[] = []
    registerKoluxProfileAuthIpcBridge(unsubs)
    authStatus.mockResolvedValue(reconnectRequiredAuthStatus)
    listener?.()
    await vi.waitFor(() =>
      expect(store.getState().koluxProfileAuthStatus).toEqual(reconnectRequiredAuthStatus)
    )
  ...
  it('skips registration when the preload bridge does not expose the event', () => {
    vi.stubGlobal('window', { api: { koluxProfiles: { authStatus } } })
    const unsubs: (() => void)[] = []
