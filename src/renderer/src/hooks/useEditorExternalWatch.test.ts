...
describe('createExternalWatchEventHandler tombstone coalescing', () => {
  ...
  it('does not drop external edits that arrive inside the self-write TTL', async () => {
    ...
    recordSelfWrite('/repo/notes.md', 'kolux save')
  ...
  it('still suppresses the watcher echo from Kolux self-writes', async () => {
    vi.mocked(useAppStore.getState).mockReturnValue({
      openFiles: [fileNotes],
      setExternalMutation
    } as never)
    vi.mocked(getOpenFilesForExternalFileChange).mockReturnValue([fileNotes] as never)
    const readFile = vi.fn().mockResolvedValue({ content: 'kolux save', isBinary: false })
    vi.stubGlobal('window', { api: { fs: { readFile } } })
    const { handleFsChanged, dispose } = createExternalWatchEventHandler(findTarget)
  ...
  it('does not mark a dirty tab for the echo of Kolux’s own save', async () => {
    const dirtyFile = {
      ...fileNotes,
      isDirty: true
    }
