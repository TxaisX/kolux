...
const LINEAR_AGENT_SKILL_NAMES = ['kolux-linear', 'linear-tickets'] as const
...
describe('useInstalledAgentSkill', () => {
  ...
  it('does not flag an incomplete scan once the skill is found anyway', async () => {
    ...
    await renderProbe()
    scan.resolve(discoveryResult([skill({ name: 'kolux-linear' })], [unavailableSource()]))
    await act(async () => {
      await scan.promise
    })
  ...
  it('serves a focus rescan from cache so window switching does not walk disk', async () => {
    ...
    firstScan.resolve(discoveryResult([skill({ name: 'kolux-linear' })]))
    await act(async () => {
  ...
  it('clears loading when a silent refresh supersedes an in-flight forced rescan', async () => {
    ...
    forcedScan.resolve(discoveryResult([skill({ name: 'kolux-linear' })]))
    await act(async () => {
  ...
  it('reports unsettled again when the discovery target changes', async () => {
    ...
    await renderProbe()
    hostScan.resolve(discoveryResult([skill({ name: 'kolux-linear' })]))
    await act(async () => {
      await hostScan.promise
    })
    expect(latestState?.settled).toBe(true)
