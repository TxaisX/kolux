...
describe('normalizeRightSidebarRoute', () => {
  ...
  it('preserves well-formed plugin panel tabs', () => {
    expect(normalizeRightSidebarRoute('plugin:kolux-samples.my-plugin/dashboard')).toEqual({
      rightSidebarTab: 'plugin:kolux-samples.my-plugin/dashboard',
      rightSidebarExplorerView: 'files'
    })
  })
  ...
  it('drops a well-formed tab once the installed plugin list proves it is stale', () => {
    expect(
      normalizeRightSidebarRoute('plugin:kolux-samples.removed/dashboard', undefined, {
        installedPluginTabKeys: new Set(['plugin:kolux-samples.present/dashboard'])
      })
    ).toEqual({
      rightSidebarTab: 'explorer',
      rightSidebarExplorerView: 'files'
  ...
  it('normalizes malformed plugin tabs to Explorer files', () => {
    expect(normalizeRightSidebarRoute('plugin:kolux-samples.my-plugin')).toEqual({
      rightSidebarTab: 'explorer',
      rightSidebarExplorerView: 'files'
    })
    expect(normalizeRightSidebarRoute('plugin:kolux-samples.my-plugin/panel/extra')).toEqual({
      rightSidebarTab: 'explorer',
      rightSidebarExplorerView: 'files'
    })
