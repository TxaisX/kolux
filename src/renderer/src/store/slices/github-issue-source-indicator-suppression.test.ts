...
describe('IssueSourceIndicator suppression', () => {
  it('hides when sources deep-equal, shows when they differ, hides when either is null', async () => {
    ...
    // Case-insensitive equality — the parent design doc calls out that `TXAISX/Kolux`
    // and `TxaisX/kolux` resolve to the same repo and must suppress.
    expect(
      sameGitHubOwnerRepo({ owner: 'TXAISX', repo: 'Kolux' }, { owner: 'TxaisX', repo: 'kolux' })
    ).toBe(true)
    expect(
      sameGitHubOwnerRepo(
        { owner: 'TxaisX', repo: 'kolux', host: 'github.com' },
        { owner: 'TxaisX', repo: 'kolux', host: 'ghe.example.test' }
      )
    ).toBe(false)
