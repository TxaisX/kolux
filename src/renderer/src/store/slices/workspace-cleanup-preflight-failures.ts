  ...
  return {
    ...
    message: error.executionHostId
      ? translate(
          ...
          "Kolux couldn't check this workspace's git status. Try again, or delete it from its host-specific sidebar or project list."
        )
      : translate(
          'auto.store.slices.workspace.cleanup.gitStatusUnavailableOlderPeer',
          "Kolux couldn't match this git-status failure to a host. Update the older connected peer, or delete the workspace from its host-specific sidebar or project list."
        ),
