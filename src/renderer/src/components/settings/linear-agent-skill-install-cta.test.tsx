...
describe('LinearAgentSkillInstallCta', () => {
  ...
  it('shows the install command and explanation when the skill is missing', async () => {
    ...
    expect(rendered.textContent).toContain('kolux-linear')
    ...
    expect(rendered.textContent).toContain(
      'npx skills add https://github.com/TxaisX/nightshift --skill kolux-linear --global'
    )
  })
  ...
  it('shows a subtle confirmation and the update command when installed', async () => {
    ...
    expect(rendered.textContent).toContain('npx skills update kolux-linear --global')
