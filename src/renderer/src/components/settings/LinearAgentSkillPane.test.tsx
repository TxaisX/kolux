...
import {
  KOLUX_LINEAR_SKILL_INSTALL_COMMAND,
...
const UPDATE_COMMAND = 'npx skills update kolux-linear --global'

const mocks = vi.hoisted(() => ({
  panelProps: [] as Record<string, unknown>[],
  runtime: 'native' as 'native' | 'wsl',
  skillInstalled: true,
  updateSkillName: 'kolux-linear',
...
describe('LinearAgentSkillPane', () => {
  afterEach(async () => {
    ...
    mocks.updateSkillName = 'kolux-linear'
  ...
  it('renders checklist with inlined skill panel and usage examples', () => {
    ...
    for (const example of examples) {
      ...
      expect(example.prompt).toContain('/kolux-linear')
  ...
  it('passes the kolux-linear install/update commands and freshness on a local runtime', async () => {
    await renderPane()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        command: KOLUX_LINEAR_SKILL_INSTALL_COMMAND,
        installedCommand: UPDATE_COMMAND,
        freshnessSkillName: KOLUX_LINEAR_SKILL_NAME
      })
    )
  })
