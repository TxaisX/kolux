...
vi.mock('@/store', () => ({
  useAppStore: (selector: (state: Record<string, unknown>) => unknown) =>
    selector({
      ...
      signOutCurrentKoluxProfile: mocks.signOut
    })
}))

vi.mock('../kolux-profiles/KoluxProfileSignOutConfirmDialog', () => ({
...
describe('KoluxAccountSettingsPane', () => {
  ...
  it('shows the connected identity and confirms sign out', async () => {
    ...
    render(<KoluxAccountSettingsPane />)

    expect(screen.getByText('Ada Lovelace')).toBeInTheDocument()
    expect(screen.getByText('ada@example.com')).toBeInTheDocument()
    expect(screen.getByText('Artifact sharing')).toBeInTheDocument()
    expect(screen.getByText('Kolux Relay')).toBeInTheDocument()
