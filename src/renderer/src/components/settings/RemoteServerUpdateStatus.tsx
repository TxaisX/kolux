...
export function getRemoteServerManualUpdateHelp(entry: RemoteServerUpdateEntry): string {
  if (entry.support?.reason === 'manual-service-update-required') {
    return translate(
      ...
      'Update Kolux on the server host — through its system package manager if it was installed from a .deb or .rpm, otherwise through the service manager that starts it.'
    )
  }
