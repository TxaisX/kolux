...
describe('browser settings search copy', () => {
  ...
  it('keeps the pre-feature wording while inverting is off', () => {
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toBe(
      "Open http(s) links in Kolux's built-in browser — from the terminal, markdown, and the editor. ⇧⌘-click always uses your system browser."
    )
  ...
  // Why: "always" would be a lie once the chord can land in Kolux, so the nested row
...
describe('browser link routing modifier copy', () => {
  ...
  it('names the destination the modifier actually reaches', () => {
    expect(getLinkRoutingModifierTitle(false)).toBe('Hold Shift to open in Kolux')
  ...
  it('points the description at Kolux only when links currently open externally', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      "Kolux's built-in browser"
    )
  ...
  // Why: the toggle is off by default, so present-tense "opens one in Kolux" would
  // describe behavior the user does not have yet.
  it('phrases the Kolux branch as enabled-state copy', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      'When enabled'
    )
  })
...
describe('Link Routing description localization', () => {
  ...
  it('renders the Korean copy with the shortcut interpolated', async () => {
    ...
    expect(description).not.toContain("Kolux's built-in browser")
    ...
    await i18n.changeLanguage('en')
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toContain("Kolux's built-in browser")
  })
