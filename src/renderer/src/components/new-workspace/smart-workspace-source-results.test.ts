...
describe('Branch source results', () => {
  ...
  it('gives Jira intent exclusive ownership of Smart results', () => {
    ...
    expect(rows).toEqual([{ kind: 'jira', value: 'jira-site-1-KOLUX-123', issue: jiraIssue }])
  })
  ...
  it('keeps a resolved Jira row when ignored URL data exceeds the generic search limit', () => {
    ...
    expect(
      buildSmartWorkspaceSourceRows({
        mode: 'smart',
        value: `https://company.atlassian.net/browse/KOLUX-123#${'x'.repeat(2100)}`,
        branches: [],
        githubItems: [],
        gitlabItems: [],
        linearIssues: [],
    ...
    ).toEqual([{ kind: 'jira', value: 'jira-site-1-KOLUX-123', issue: jiraIssue }])
  })
...
describe('Jira issue search', () => {
  it('keeps valid Jira URL intent blocking in both Smart and Jira modes', () => {
    const firstUrl = 'https://company.atlassian.net/browse/KOLUX-1'
    const secondUrl = 'https://company.atlassian.net/browse/KOLUX-2'
  ...
  it('builds text and exact-key JQL without accepting oversized input', () => {
    ...
    expect(buildJiraIssueSearchJql('kolux-123')).toBe('key = "KOLUX-123"')
