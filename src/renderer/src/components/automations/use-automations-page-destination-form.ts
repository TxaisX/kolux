...
export function useAutomationsPageDestinationForm({
  ...
  const isKoluxForm = createTarget === 'kolux' && local.editingExternalTarget === null
  ...
  useEffect(() => {
    if (!createOpen || editingAutomationId !== null || createTarget !== 'kolux') {
      return
    }
    setDraft((current) =>
