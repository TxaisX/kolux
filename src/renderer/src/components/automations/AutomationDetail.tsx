...
import { formatUiAutomationOnEventLabel, formatUiAutomationTrigger } from './automation-schedule-label'
    ...
    <div className="flex w-full flex-col gap-4">
      ...
      {automation.executionTargetType === 'ssh' ? (
        <div className="rounded-md border border-border/50 bg-muted/50 p-3 text-sm text-muted-foreground shadow-sm">
          {translate(
            ...
            'This SSH automation runs only while Kolux can reach the SSH host. If reconnect needs interactive credentials or the host is unavailable, the run is recorded as skipped.'
          )}
        </div>
      ) : null}
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(9rem,1fr))] gap-5 rounded-md border border-border/50 bg-muted/30 px-4 py-3 shadow-sm">
        <DetailMetric
          ...
          value={formatUiAutomationTrigger(automation)}
        />
        <DetailMetric
