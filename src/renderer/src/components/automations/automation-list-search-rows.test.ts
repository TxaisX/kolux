...
const repo = { id: REPO_ID, displayName: 'kolux', path: '/src/kolux' } as Repo
...
describe('automation search row index', () => {
  it('builds one index per row and matches in a single pass', () => {
    ...
    expect(matchAutomationListSearchRowKeys(rows, 'kolux')).toEqual([first, second])
