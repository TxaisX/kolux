...
export function LinearApiKeyDialog({
  ...
  const resolvedDescription =
    description ??
    (workspace
      ? `Paste a Personal API key for ${workspace.organizationName}. If this workspace is already connected, Kolux replaces its stored key.`
      : 'Paste a Personal API key for the Linear workspace you want Kolux to use. If that workspace is already connected, Kolux replaces its stored key.')
  const storageCopy =
          ...
          <div className="space-y-2 text-xs leading-relaxed text-muted-foreground">
            ...
            <p>
              {translate(
                ...
                'Prefer full access when Kolux should show every team the account can access in that workspace. Restricted keys only expose permitted teams, and private teams require the key owner to have access.'
              )}
            </p>
            <p>
              {translate(
