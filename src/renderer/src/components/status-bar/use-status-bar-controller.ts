...
export function useStatusBarController(floatingTerminalOpen: boolean) {
  ...
  const { claude, codex, gemini, kimi, antigravity, minimax, grok } = rateLimits
  ...
  const hasVisibleUsageMeters =
    showClaude || showCodex || showGemini || showKimi || showAntigravity || showMiniMax || showGrok
  ...
  const isEmptyUsageState = isUsageEmptyState(
    { claude, codex, gemini, kimi, antigravity, minimax, grok },
    usageSettings
  )
