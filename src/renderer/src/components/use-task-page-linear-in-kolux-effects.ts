...
import {
  ...
  filterLinearIssuesForInKoluxWorkspace
...
export function useTaskPageLinearInKoluxEffects(model: TaskPageLinearListEffectsModel) {
  const {
    fetchLinearIssue,
    refreshLinearIssue,
    linearConnected,
    ...
    inKoluxLinkedLinearRefsSignature,
    inKoluxLinkedLinearRefsRef
  } = model
  ...
  useEffect(() => {
    ...
    if (taskSource !== 'linear' || linearMode !== 'in-kolux' || !linearConnected) {
      return
    }
    let cancelled = false
    const linkedRefs = inKoluxLinkedLinearRefsRef.current
    const requestSignature = `in-kolux::${selectedLinearWorkspaceId ?? 'default'}::${inKoluxLinkedLinearRefsSignature}`
        ...
        if (loaded.length === 0) {
          setLinearError(
            translate(
              ...
              'Unable to load Linear issues linked to a Kolux workspace.'
            )
          )
          setLinearIssues([])
          setLinearLoading(false)
        ...
        if (loaded.length !== results.length) {
          setLinearError(
            translate(
              ...
              'Some Linear issues linked to a Kolux workspace could not be loaded. Refresh to try again.'
            )
          )
        }
        setLinearIssues(filterLinearIssuesForInKoluxWorkspace(loaded, selectedLinearWorkspaceId))
        setLinearLoading(false)
      })
      .catch((err) => {
        if (
...
export type TaskPageLinearInKoluxEffectsModel = ReturnType<typeof useTaskPageLinearInKoluxEffects>
