...
export function buildUpdateCardErrorModel({
  ...
  if (isHttp2ProtocolError(status.message)) {
    return {
      ...
      summary: 'Kolux can retry through HTTP/1.1 compatibility mode.',
      explainer: translate(
  ...
  if (isWindowsSignatureMismatchFailure(status.message)) {
    return {
      ...
      summary: translate(
        ...
        "The installer's publisher doesn't match Kolux, so we stopped the update. Don't install this download; check official releases for a corrected version."
      ),
      detail: status.message,
