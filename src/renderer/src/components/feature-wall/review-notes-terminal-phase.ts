...
// "Loaded N review notes from Kolux", lists each ack with its line number,
...
export async function runTerminalPhase(ctx: TerminalPhaseContext): Promise<void> {
  ...
  if (loadedEl) {
    loadedEl.innerHTML = `<span class="ravs-term-check">✓</span><span class="ravs-term-muted">Loaded ${NOTE_TARGETS.length} review notes from Kolux</span>`
  }
  await wait(520)
  if (isCancelled()) {
    return
