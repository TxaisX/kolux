...
import { KOLUX_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isKoluxCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  const cliEnabled = isKoluxCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isKoluxCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Kolux CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Kolux CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensureKoluxCliAvailableForAgentSkillTerminal({
