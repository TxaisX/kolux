...
const SUMMARY =
  'Kolux downloaded the system package. Quit Kolux before finishing the update from a terminal.'
const COPIED_NOTE =
  `Command copied. Quit Kolux, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Kolux.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Kolux is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/Copy the command, quit Kolux/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(/Kolux checks the downloaded file against the release metadata/)
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Kolux cannot vouch for the file ' +
        'after that point.'
    )
  })
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Kolux before finishing the update/)).toBeTruthy()
