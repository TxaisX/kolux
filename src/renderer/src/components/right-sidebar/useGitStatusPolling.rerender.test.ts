...
import { KOLUX_WORKTREE_FILE_CHANGE_EVENT } from '@/hooks/worktree-file-change-event'
...
describe('useGitStatusPolling rerender stability', () => {
  ...
  it('keeps the refresh scheduler stable when openFiles changes mid-signal debounce', async () => {
    ...
    expect(addSpy).toHaveBeenCalledWith(KOLUX_WORKTREE_FILE_CHANGE_EVENT, expect.any(Function))

    // Fire a file-watch event to drive fetchStatus during the cooldown.
    window.dispatchEvent(
      new CustomEvent(KOLUX_WORKTREE_FILE_CHANGE_EVENT, {
        detail: {
          payload: {
