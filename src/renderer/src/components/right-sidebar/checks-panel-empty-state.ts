...
export function getChecksPanelReviewState(
  ...
  if (reviewFetchInFlight && hasPendingReviewEvidence && pendingReviewLookupOwnsState) {
    return {
      ...
      description: translate(
        ...
        'Kolux is checking {{provider}} for a {{reviewLabel}} on this branch.',
  ...
  if (input.reviewLookup === 'positive_unresolved' && !isBranchBlocker(blockedReason)) {
    return {
      ...
      description: translate(
        ...
        'Kolux has saved {{reviewLabel}} information for this branch but could not confirm its current status.',
        { reviewLabel }
      ),
      composerMode: 'hidden',
      workflowAction: null,
  ...
  if (input.gitStatusPhase === 'loading' && input.hasUpstream === undefined) {
    return {
      ...
      description: translate(
        ...
        'Kolux is checking this branch before showing create or publish actions.'
      ),
      composerMode: 'hidden',
      workflowAction: null,
      recovery: []
  ...
  if (input.gitStatusPhase === 'error' && input.hasUpstream === undefined) {
    return {
      ...
      description: translate(
        ...
        "Kolux could not confirm this branch's upstream from this environment. Retry before publishing or creating a {{reviewLabel}}.",
        { reviewLabel }
      ),
      composerMode: 'hidden',
      workflowAction: null,
  ...
  return {
    ...
    description: translate(
      ...
      'Kolux has not confirmed the {{reviewLabel}} status for this branch. Retry to check again.',
      { reviewLabel }
    ),
    composerMode: 'hidden',
    workflowAction: null,
