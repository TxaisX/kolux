...
describe('createPanelBridgeMessageHandler', () => {
  it('relays a valid request and posts the success result back into the panel', async () => {
    ...
    expect(panelWindow.postMessage).toHaveBeenCalledWith(
      {
        type: 'kolux-panel-action-result',
        requestId: 'req-1',
        ok: true,
        value: { accepted: true }
      },
  ...
  it('refuses a valid request after malformed and pong traffic exhaust the budget', () => {
    ...
    handler(messageEvent({ type: 'kolux-panel-pong', pingId: 7 }, panelWindow))
    handler(messageEvent(VALID_DATA, panelWindow))
  ...
  it('charges a near-miss pong to the data budget only, sparing the reserved lane', () => {
    ...
    handler(messageEvent({ type: 'kolux-panel-pong', pingId: -1 }, panelWindow))
    handler(messageEvent({ type: 'kolux-panel-pong', pingId: 'seven' }, panelWindow))
    handler(messageEvent({ type: 'kolux-panel-pong' }, panelWindow))
  ...
  it('never lets a panel starve its own watchdog with self-sent pong traffic', () => {
    ...
    for (let i = 0; i < 500; i += 1) {
      handler(messageEvent({ type: 'kolux-panel-pong', pingId: i }, panelWindow))
      clock += 1
    }
    handler(messageEvent({ type: 'kolux-panel-pong', pingId: 99 }, panelWindow))
  ...
  it('refuses an oversized frame on the reserved lane', () => {
    ...
    handler(
      messageEvent(
        { type: 'kolux-panel-pong', pingId: 7, padding: 'x'.repeat(4 * 1024) },
        panelWindow
      )
    )
