...
export function reloadKoluxRenderer(): void {
...
function unsetEnvVarCommand(varName: string): { label: string; command: string } {
  if (IS_WINDOWS) {
    ...
    // shell/Kolux relaunch for the change to take effect.
    return {
      label: translate(
...
export function buildRemediation(
  ...
  if (!diag.ghAvailable) {
    return {
      ...
      detail:
        'Kolux uses `gh` to talk to GitHub Projects. Install it from cli.github.com, then sign in.',
      commands: [
        {
          label: translate(
            'auto.components.github.project.GhAuthErrorHelp.9c2da6353b',
  ...
  if (active?.envToken) {
    ...
    return {
      ...
      detail: IS_WINDOWS
        ? `Find where \`${varName}\` is set (System or User environment variables, or your PowerShell profile), remove it, then restart Kolux so the new environment is picked up.${fallback}`
        : `Find where \`${varName}\` is exported (commonly \`~/.zshrc\`, \`~/.zshenv\`, \`~/.bashrc\`, \`~/.profile\`, or your shell's secrets manager), remove it, then restart Kolux so the new environment is picked up.${fallback}`,
  ...
  // separate terminal, Kolux's gh subprocess sees the env var and uses it.
  if (diag.envTokenInProcess && (!active || diag.missingScopes.length > 0)) {
    const varName = diag.envTokenInProcess
    return {
      summary: `Kolux inherited \`${varName}\` from your shell, and \`gh\` is using that token. \`gh auth refresh\` doesn't apply to env-supplied tokens.`,
      detail: `Unset \`${varName}\` in the shell that launches Kolux${
  ...
  return (
    <div className="flex flex-col gap-2 text-sm">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {/* Why: after running the refresh command in a terminal, users need to
            reload the renderer to pick up the new gh token state. */}
        <Button size="sm" variant="outline" onClick={reloadKoluxRenderer}>
