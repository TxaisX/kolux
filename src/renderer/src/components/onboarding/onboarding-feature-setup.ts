...
import { showKoluxCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
...
export function createOnboardingFeatureSetupDeps(
  ...
  // Register `kolux` on the same PATH used by the skill install (#12103).
  ...
  return {
    ...
    showCliRegistrationPrompt: showKoluxCliRegistrationPromptToast,
    installCli: () =>
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Kolux CLI registration is not available on this platform.'
      })
    } else if (status.pathConfigured === null) {
      // Why: an unknown registry read cannot safely drive a PATH read-modify-write.
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Kolux could not check your Windows user PATH.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Kolux CLI registration needs attention.'
        })
