...
describe('deriveInboxItems', () => {
  it('puts a fresh blocked entry with a plain prompt into needs as a question', () => {
    ...
    expect(groups.needs[0]).toMatchObject({ kind: 'question', title: 'Which branch?', worktreeId: worktree.id })
  })
  ...
  it('classifies a question-shaped interactivePrompt as question, using its text', () => {
    const entry = makeEntry({
      paneKey: paneKey(),
      state: 'blocked',
      interactivePrompt: JSON.stringify({ questions: [{ question: 'Pick a color', options: ['red', 'blue'] }] })
    })
    const groups = deriveInboxItems(baseInput({ agentStatusByPaneKey: { [entry.paneKey]: entry } }))
  ...
  it('sorts needs freshest (most recently entered) first', () => {
    const older = makeEntry({ paneKey: paneKey(LEAF_1), state: 'blocked', stateStartedAt: NOW - 10_000 })
    const newer = makeEntry({ paneKey: paneKey(LEAF_2), state: 'blocked', stateStartedAt: NOW - 1_000 })
    const groups = deriveInboxItems(
      baseInput({
  ...
  it('attaches a diff only when a branch line total exists for the worktree', () => {
    ...
    const withoutDiff = deriveInboxItems(baseInput({ agentStatusByPaneKey: { [entry.paneKey]: entry } }))
  ...
  it('falls back to the entry worktreeId when no tab maps the pane', () => {
    ...
    const groups = deriveInboxItems(baseInput({ agentStatusByPaneKey: { [entry.paneKey]: entry }, tabsByWorktree: {} }))
    expect(groups.needs).toHaveLength(1)
  ...
  it('drops an entry whose worktree is unknown', () => {
    const entry = makeEntry({ paneKey: paneKey(), state: 'blocked' })
    const groups = deriveInboxItems(baseInput({ agentStatusByPaneKey: { [entry.paneKey]: entry }, worktrees: [] }))
    expect(groups.needs).toHaveLength(0)
  })
})
