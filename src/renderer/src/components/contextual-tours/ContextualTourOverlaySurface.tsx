...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __koluxContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__koluxContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__koluxContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'kolux-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="kolux-contextual-tour-target-rings fixed z-[75]"
