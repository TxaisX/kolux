...
} from './task-page-linear-in-kolux-issues'
...
describe('filterLinearIssuesForInKoluxWorkspace', () => {
  it('keeps issues without workspace metadata when a workspace is selected', () => {
    ...
    expect(
      filterLinearIssuesForInKoluxWorkspace(issues, 'ws-a').map((item) => item.identifier)
    ).toEqual(['ENG-1', 'ENG-3'])
  })
})
