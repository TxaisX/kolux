...
/** True when an existing Kolux host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInKolux` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In Kolux" is still savable as a second target. */
