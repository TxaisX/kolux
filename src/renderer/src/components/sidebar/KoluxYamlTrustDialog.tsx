...
type ScriptKind = KoluxHookScriptKind
...
const KoluxYamlTrustDialog = React.memo(function KoluxYamlTrustDialog() {
  const activeModal = useAppStore((s) => s.activeModal)
  const modalData = useAppStore((s) => s.modalData)
  const closeModal = useAppStore((s) => s.closeModal)
  const markKoluxHookScriptConfirmed = useAppStore((s) => s.markKoluxHookScriptConfirmed)
  const markKoluxHookRepoAlwaysTrusted = useAppStore((s) => s.markKoluxHookRepoAlwaysTrusted)

  const isOpen = activeModal === 'confirm-kolux-yaml-hooks'
  ...
  const resolveAndClose = useCallback(
    (decision: 'run' | 'skip') => {
      if (decision === 'run' && repoId) {
        if (alwaysTrust) {
          ...
          markKoluxHookScriptConfirmed(repoId, scriptKind, contentHash)
        }
      }
      onResolve?.(decision)
      closeModal()
    ...
    [
      ...
      closeModal,
      contentHash,
      markKoluxHookRepoAlwaysTrusted,
      ...
      <DialogContent className="max-w-md sm:max-w-md" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle className="text-sm">
            {previouslyApproved
              ? translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.02b0ede5ad',
                  "{{value0}}'s {{value1}} changed — run the new version?",
                  { value0: repoName, value1: SCRIPT_KIND_LABEL[scriptKind] }
                )
              : translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.e4a51dc4b3',
          ...
          <DialogDescription className="text-xs">
            {previouslyApproved ? (
              <>
                <code>
                  {translate(
                    'auto.components.sidebar.KoluxYamlTrustDialog.79afc6772b',
                    'kolux.yaml'
                  )}
                </code>{' '}
                {translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.c55beddbf8',
              ...
              <>
                {translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.aa3ffb33fb',
                  "This repository's"
                )}{' '}
                <code>
                  {translate(
                ...
                {translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.831f2cd9f0',
                  'runs on your machine'
                )}{' '}
                {SCRIPT_KIND_TRIGGER[scriptKind]}
                {translate(
                  'auto.components.sidebar.KoluxYamlTrustDialog.bf800b7e04',
        ...
        {scriptContent && (
          <div className="rounded-md border border-border/70 bg-muted/35 px-3 py-2">
            <div className="mb-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
              {previouslyApproved
                ? translate(
                    'auto.components.sidebar.KoluxYamlTrustDialog.9e52effffd',
                    'New {{value0}} script',
                    { value0: scriptKind }
                  )
                : translate(
                    'auto.components.sidebar.KoluxYamlTrustDialog.95bf974a1a',
                    '{{value0}} script',
                    { value0: scriptKind }
                  )}
            </div>
        ...
        >
          ...
          <span className="text-xs font-medium text-foreground">
            {translate('auto.components.sidebar.KoluxYamlTrustDialog.531689199b', 'Always trust')}{' '}
            <code>
              {translate('auto.components.sidebar.KoluxYamlTrustDialog.79afc6772b', 'kolux.yaml')}
            </code>{' '}
            {translate('auto.components.sidebar.KoluxYamlTrustDialog.c494b3ccb1', 'in')} {repoName}
          </span>
        </label>

        <DialogFooter>
          <Button variant="outline" onClick={() => resolveAndClose('skip')}>
            {translate('auto.components.sidebar.KoluxYamlTrustDialog.43b7bec4cd', "Don't run")}
          </Button>
          <Button onClick={() => resolveAndClose('run')}>
            {translate('auto.components.sidebar.KoluxYamlTrustDialog.f3e2b868fb', 'Run hooks')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default KoluxYamlTrustDialog
