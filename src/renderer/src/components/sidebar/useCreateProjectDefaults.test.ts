...
describe('useCreateProjectDefaults', () => {
  beforeEach(() => {
    ...
    mocks.getDefaultCreateProjectParent.mockResolvedValue('/Users/alice/kolux/projects')
  })
  ...
  it('auto-fills the local default parent and reports Git availability', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/Users/alice/kolux/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/Users/alice/kolux/projects')
  ...
  it('keeps the local default marker after the auto-filled parent rerenders the hook', async () => {
    ...
    useHarness()
    await flushAsync()
    useHarness({ createParent: '/Users/alice/kolux/projects' })
  ...
  it('resolves the runtime default parent from the host home directory', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/home/alice/kolux/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/kolux/projects')
  ...
  it('replaces an untouched local default when switching to a runtime target', async () => {
    ...
    await flushAsync()
    expect(local.setCreateParent).toHaveBeenCalledWith('/Users/alice/kolux/projects')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/kolux/projects'
    })
    ...
    expect(mocks.browseRuntimeServerDirectory).toHaveBeenCalledWith('env-1', '~')
    expect(resolvedRuntime.setCreateParent).toHaveBeenCalledWith('/home/alice/kolux/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/kolux/projects')
  ...
  it('does not replace a touched parent when switching to a runtime target', async () => {
    ...
    const local = useHarness({ createParent: '/Users/alice/kolux/projects' })
    local.result.markCreateParentTouched('/Users/alice/kolux/projects/pr5115-target-switch')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/kolux/projects/pr5115-target-switch'
    })
    await flushAsync()
