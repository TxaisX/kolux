  ...
  return (
    <WorktreeCardDetailSection>
      <DetailHeader
        ...
        label={translate('auto.components.sidebar.WorktreeCardMeta.cliHeader', 'Kolux CLI')}
      />
        ...
        <div className="text-[13px] font-semibold leading-snug text-foreground break-words">
          {provenance.callerTerminalHandle
            ? translate(
                ...
                'Created by an agent via `kolux worktree create`'
              )
            : translate(
                'auto.components.sidebar.WorktreeCardMeta.cliCreatedFromShell',
                'Created via `kolux worktree create`'
              )}
        </div>
        {agentLabel ? (
