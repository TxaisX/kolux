...
describe('buildTitleDerivedAgentRows', () => {
  ...
  it('still resolves Claude from a title that presents Claude, owner or not', () => {
    ...
    // Pane reuse: the user exited Goose and ran claude in the same pane.
    expect(rowsFor('✳ Claude Code', 'goose').map((row) => row.agentType)).toEqual(['claude'])
    ...
    expect(rowsFor('zsh', 'goose')).toHaveLength(0)
  })

  it('does not brand a split pane with the tab-scoped launch agent', () => {
    const rows = buildWorktreeAgentRows({
      tabs: [makeTab('tab-1', { launchAgent: 'goose' })],
      entries: [],
      retained: [],
