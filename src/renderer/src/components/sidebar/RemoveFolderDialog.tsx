...
const RemoveFolderDialog = React.memo(function RemoveFolderDialog() {
  ...
  const description = isRuntimeOwnedSshTargetId(sshConnectionId)
    ? translate(
        ...
        'This removes {{name}} from Kolux. Its VM recipe determines whether the environment and its files are permanently deleted.',
        { name: NAME_TOKEN }
      )
    : sshHostLabel
      ? translate(
          'auto.components.sidebar.RemoveFolderDialog.removeDescriptionSsh',
          'This only removes {{name}} from Kolux. Its files stay on {{host}} — re-add that SSH host to recover it.',
          { name: NAME_TOKEN, host: sshHostLabel }
        )
      : translate(
          'auto.components.sidebar.RemoveFolderDialog.removeDescriptionLocal',
          'This only removes {{name}} from Kolux. It is still on your disk.',
          { name: NAME_TOKEN }
        )
