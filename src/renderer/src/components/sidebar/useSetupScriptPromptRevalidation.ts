...
/**
 * Re-runs the setup-script prompt inspection when a shared `kolux.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: kolux.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to Kolux detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable kolux.yaml into a readable one,
