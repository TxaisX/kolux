...
function getPreservedBranchDescription(
  ...
  if (!targetName) {
    return translate(
      ...
      'Git could not safely delete branch "{{value0}}", so Kolux kept it to avoid losing local commits.',
      { value0: branch }
    )
  }
  return isWorkspace
    ? translate(
        ...
        'Git could not safely delete branch "{{value0}}" after deleting workspace "{{value1}}", so Kolux kept it to avoid losing local commits.',
        { value0: branch, value1: targetName }
      )
    : translate(
        'auto.store.slices.worktrees.81f13f48d2',
        'Git could not safely delete branch "{{value0}}" after deleting worktree "{{value1}}", so Kolux kept it to avoid losing local commits.',
        { value0: branch, value1: targetName }
      )
}
