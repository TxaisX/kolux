...
export function getLinearAgentSkillSetupMissingLabel(
  ...
  if (!cliAvailable && !skillInstalled) {
    return translate(
      ...
      'Kolux CLI and Linear agent skill are missing.'
    )
  }
  if (!cliAvailable) {
    return translate(
...
function getLinearAgentSkillSetupToastBaseDescription(
  ...
  if (!cliAvailable) {
    return translate(
      ...
      'Install the Kolux CLI to enable your agents to read and edit Linear tasks.'
    )
  }
  return translate(
    'auto.components.sidebar.LinearAgentSkillSetupPrompt.toastInstallSkillDescription',
    'Install the Linear skill to enable your agents to read and edit Linear tasks through the Kolux CLI.'
  )
}
