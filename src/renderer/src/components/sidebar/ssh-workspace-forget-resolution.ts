 ...
 * workspace from Kolux because the SSH target is gone entirely.
 */
export type SshWorkspaceForgetResolution =
  ...
  // impossible, so forget-from-Kolux is the only path.
...
export function resolveSshWorkspaceForget(args: {
  ...
  // the only escape is to forget it from Kolux.
  if (!isConfigured) {
