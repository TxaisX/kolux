  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Kolux. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Kolux. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Kolux workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Kolux. The project folder on disk will not be deleted.'
