/**
 * Real-fish regression for #9993: Kolux must never write `CSI ?997;Nn` into a PTY
...
describe('fish never receives a color-scheme report it did not query (#9993)', () => {
  ...
  itWithFish(
    ...
    async () => {
      ...
      configHome = mkdtempSync(path.join(tmpdir(), 'kolux-fish-2031-'))
