...
beforeEach(() => {
  ...
  environmentMocks.resolveFooter.mockResolvedValue(
    ['---', 'Kolux: 1.4.178-rc.2', 'OS: darwin 25.0.0 (arm64)', 'Shell: /bin/zsh'].join('\n')
  )
})
...
const LEGACY_HOST_GONE =
  "Error invoking remote method 'pty:spawn': Error: connect ENOENT \\\\?\\pipe\\kolux-terminal-host-v30-14cb7f94b511"
...
describe('humanizeTerminalError', () => {
  it('replaces the pane-owner-unverified code with actionable copy', () => {
    ...
    expect(humanized).toContain('Kolux left the saved session unchanged')
  ...
  it('humanizes an owner marker without classifying mixed errors as safe warnings', () => {
    ...
    expect(mixed).toContain("Kolux couldn't verify this terminal's owner.")
  ...
  it('humanizes a legacy host raw named-pipe error', () => {
    ...
    expect(humanized).not.toContain('kolux-terminal-host-v30')
    expect(humanized).toContain('Open a new terminal to continue')
  })
  ...
  it('replaces a raw relay PTY-not-found string and its quoted id', () => {
    const humanized = humanizeTerminalError(
      'Error invoking remote method \'pty:spawn\': Error: PTY "kolux:2f1c@@pty-7" not found'
    )
  ...
  it('replaces the identity-mismatch form of PTY-not-found', () => {
    const humanized = humanizeTerminalError('PTY "kolux:2f1c@@pty-7" not found (identity mismatch)')
  ...
  it('does not claim the remote shell is still running or dead', () => {
    const humanized = humanizeTerminalError('SSH_SESSION_EXPIRED: kolux:2f1c@@pty-7')
  ...
  it.each(['ENOENT', 'ECONNREFUSED'])(
    ...
    (code) => {
      const aggregated =
        `connect ${code} \\\\?\\pipe\\unrelated\n` + 'kolux-terminal-host-v30-14cb7f94b511'
...
describe('isExplainedTerminalError', () => {
  it('suppresses the issue link for a provably dead terminal host', () => {
    ...
    expect(
      isExplainedTerminalError(
        'connect ECONNREFUSED /tmp/kolux-terminal-host-v30-14cb7f94b511.sock'
      )
    ).toBe(true)
  })
  ...
  it('suppresses the issue link for a session the host cannot reattach', () => {
    expect(isExplainedTerminalError('SSH_SESSION_EXPIRED: kolux:2f1c@@pty-7')).toBe(true)
    expect(
      isExplainedTerminalError(
        'Error invoking remote method \'pty:spawn\': Error: PTY "kolux:2f1c@@pty-7" not found'
      )
  ...
  it('keeps the issue link for errors Kolux cannot explain', () => {
    ...
    expect(isExplainedTerminalError('open ENOENT \\\\?\\pipe\\kolux-terminal-host-v30-dead')).toBe(
      false
    )
    expect(
      isExplainedTerminalError('connect ETIMEDOUT \\\\?\\pipe\\kolux-terminal-host-v30-dead')
    ).toBe(false)
...
describe('shouldOfferDaemonRestart', () => {
  it('matches stale daemon node-pty install failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's node-pty install is gone (worktree deleted?). Restart Kolux. node-pty: posix_spawn failed: ENOENT (errno 2, No such file or directory) - helper='/Applications/Kolux.app/Contents/Resources/app.asar.unpacked/node_modules/node-pty/build/Release/spawn-helper'"
      )
    ).toBe(true)
  })
  ...
  it('matches stale daemon cwd failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's working directory is gone (worktree deleted?). Restart Kolux. node-pty: daemon_cwd failed: ENOENT (errno 2, No such file or directory) - cwd='<unavailable>'"
      )
    ).toBe(true)
  })
...
describe('TerminalErrorToast environment footer', () => {
  it('appends client environment details to local errors', async () => {
    ...
    await waitFor(() => expect(view.container.textContent).toContain('Kolux: 1.4.178-rc.2'))
  })
  ...
  it('does not retain a prior async footer when the next error already has one', async () => {
    ...
    expect(view.container.textContent).toContain('Kolux: embedded')
    expect(view.container.textContent).not.toContain('Kolux: 1.4.178-rc.2')
  })
  ...
  it('renders owner-unverified as a warning without an issue link', () => {
    ...
    expect(toast?.textContent).toContain('Kolux left the saved session unchanged')
