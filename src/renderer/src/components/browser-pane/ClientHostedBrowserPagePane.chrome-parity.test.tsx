...
describe('ClientHostedBrowserPagePane chrome parity', () => {
  ...
  it('keeps a Kagi session token out of the stored page and the address bar', () => {
    ...
    act(() => {
      fireEvent.change(addressBar, {
        target: { value: 'https://kagi.com/search?q=kolux&token=secret-token' }
      })
    })
    act(() => {
      fireEvent.submit(addressBar.closest('form') ?? addressBar)
    ...
    // The guest still gets the real link — redaction is about what Kolux keeps, not what loads.
    expect(webview.loadURL).toHaveBeenCalledWith(
      'https://kagi.com/search?q=kolux&token=secret-token'
    )
  })
  ...
  it('says so when the page asks for a permission Kolux denied', () => {
    renderPane()
    ...
    expect(toastMocks.message).toHaveBeenCalledWith(
      'https://example.internal asked for camera or microphone access, and Kolux denied it.',
