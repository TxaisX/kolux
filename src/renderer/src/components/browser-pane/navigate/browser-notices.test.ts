...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Kolux denied it.')
    expect(
      formatPermissionNotice({
        browserPageId: 'browser-1',
    ...
    ).toBe('this page asked for your location, and Kolux denied it.')
  })
  ...
  it('names the storage permission in words rather than its raw token', () => {
    ...
    expect(notice).toBe(
      'https://example.com asked for cookie access on behalf of an embedded site, and Kolux denied it.'
    )
  })
  ...
  it.each([
    ...
    ['openExternal', 'permission to open a link outside Kolux'],
    ...
    ).toBe(`https://example.com asked for ${description}, and Kolux denied it.`)
  })
  ...
  it('falls back to the raw permission name for anything unmapped', () => {
    ...
    ).toBe('https://example.com asked for some-future-permission, and Kolux denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('A site tried to open a popup Kolux does not support here.')
  })
  ...
  it('formats certificate failures without local-server recovery advice', () => {
    ...
    expect(formatLoadFailureDescription(loadError(-202), meta)).toBe(
      "Kolux doesn't trust the authority that issued the certificate for localhost:3443."
    )
    expect(formatLoadFailureDescription(loadError(-208), meta)).toBe(
      "Kolux couldn't verify the certificate for localhost:3443."
    )
