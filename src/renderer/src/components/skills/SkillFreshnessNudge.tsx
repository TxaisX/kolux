...
export function SkillFreshnessNudge(): null {
  ...
  useEffect(() => {
    ...
    const candidates = inventory.installations.flatMap((installation) =>
      ...
      // dismissed — and spends the bounded dismissal budget on copies Kolux cannot update.
    ...
    nextActive.id = toast.info(
      names.size === 1
        ? translate(
            ...
            'An installed Kolux skill is out of date'
          )
        : translate(
            'auto.components.skills.SkillFreshnessNudge.titleMany',
            '{{value0}} installed Kolux skills are out of date',
            { value0: names.size }
          ),
      {
        description: translate(
          'auto.components.skills.SkillFreshnessNudge.description',
          'Update {{value0}} so agents follow the current instructions for this version of Kolux.',
