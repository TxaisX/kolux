...
describe('rich Markdown HTML superscript links', () => {
  ...
  it('preserves authored current-key and legacy transport-looking text', () => {
    ...
    const authored = `${transport.create('inline-html', '<b>authored</b>')} [[KOLUX_RAW_HTML_INLINE:%3Ci%3Ex%3C%2Fi%3E]]`
  ...
  it('renders inert live DOM while exposing link semantics through Kolux context', () => {
  ...
  it('rejects oversized clipboard source before UTF-8 encoding', () => {
    ...
    try {
      editor.commands.setContent(
        `<sup data-rich-markdown-html-superscript-link="1" data-kolux-superscript-link-source="${oversizedSource}"><a>x</a></sup>`,
        { contentType: 'html' }
      )
      expect(nodeNames(editor)).not.toContain('richMarkdownHtmlSuperscriptLink')
  ...
  it('maps citation labels into search, review, and empty-selection Cut', () => {
    ...
    try {
      ...
      try {
        ...
        expect(clipboard.get('text/html')).toContain('data-kolux-superscript-link-source')
