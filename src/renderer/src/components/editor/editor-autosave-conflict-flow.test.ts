...
import { KOLUX_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, requestEditorFileSave } from './editor-autosave'
...
describe('editor autosave changed-on-disk conflict flow', () => {
  ...
  it('does not backstop-mark a dirty tab for the echo of its own save', () => {
    ...
    // event is Kolux's own echo, not an external change.
    recordSelfWrite('/repo/file.ts', 'kolux save')
