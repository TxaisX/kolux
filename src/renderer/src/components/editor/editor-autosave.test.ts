...
describe('requestEditorSaveQuiesce', () => {
  ...
  it('waits for a claiming listener to finish quiescing', async () => {
    ...
    window.addEventListener(KOLUX_EDITOR_QUIESCE_FILE_SAVES_EVENT, handler as EventListener)
    try {
      ...
      window.removeEventListener(KOLUX_EDITOR_QUIESCE_FILE_SAVES_EVENT, handler as EventListener)
    }
  })
})
...
describe('requestEditorFileClose', () => {
  it('dispatches a close request event with the file id', () => {
    ...
    window.addEventListener(KOLUX_EDITOR_REQUEST_FILE_CLOSE_EVENT, listener as EventListener)
    try {
      ...
      window.removeEventListener(KOLUX_EDITOR_REQUEST_FILE_CLOSE_EVENT, listener as EventListener)
    }
  })
})
