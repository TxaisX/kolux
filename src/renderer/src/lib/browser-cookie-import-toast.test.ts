...
describe('emitBrowserCookieImportToast', () => {
  ...
  it('offers the in-app file import without recommending an exporter', () => {
    ...
    expect(message).toBe(
      "Kolux cannot decrypt 3 of this browser's cookies because they use app-bound encryption. You can import cookies from a file using “From File…”."
    )
  ...
  it('warns separately about cookies skipped for an unreadable partition', () => {
    ...
    expect(successToastMock).toHaveBeenCalledWith('Imported 2 cookies.')
    expect(warningToastMock).toHaveBeenCalledWith(
      '1 cookies were not imported because their site-partition could not be read. Sign in to those sites again in Kolux.',
      { duration: 12000 }
    )
  })
