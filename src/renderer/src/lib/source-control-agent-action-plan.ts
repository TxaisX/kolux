...
export function planSourceControlAgentActionLaunch(args: {
  ...
  const summary =
    delivery === 'paste-submit'
      ? 'The agent starts with no prompt, then Kolux pastes and submits the command input after the TUI is ready.'
      : delivery === 'draft-native'
        ? 'The command input is prefilled as an editable draft by the agent launch command.'
        : delivery === 'draft-paste'
          ? 'The agent starts with no prompt, then Kolux pastes the command input as an editable draft after the TUI is ready.'
  ...
  return {
    ...
    summary,
    caveat:
      'This check builds Kolux’s launch plan only. PATH, binary availability, account setup, and terminal startup failures are still caught by the real launch watchdog.'
  }
}
