 ...
 *
 * Kolux's live PTY write path performs NO scroll-intent enforcement — it
 ...
 * CSI 3 J, so the erase itself releases a pinned reader to the bottom. Kolux's pin still
...
describe('xterm native user-scrolling contract (vendored 6.1.0-beta.303)', () => {
  ...
  it('applies scrollOnUserInput before notifying onData listeners', async () => {
    ...
    // Why: Kolux resyncs typing intent synchronously from onData, so this
  ...
  it('exposes the isUserScrolling flag the structural restore paths depend on', async () => {
    ...
    // scrollLines/scrollToBottom self-manage the flag, so Kolux's programmatic
