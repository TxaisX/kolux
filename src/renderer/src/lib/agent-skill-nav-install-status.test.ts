  ...
  it('keeps loading and missing states ahead of freshness', () => {
    expect(
      getAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: true,
        inventory: inventory([placement('kolux-linear', 'current')])
      })
    ).toBe('checking')
    expect(
      getAgentSkillNavInstallStatus({
...
describe('getLinearAgentSkillNavInstallStatus', () => {
  it('reports canonical installs under kolux-linear', () => {
    const skills = [skill('kolux-linear')]
    ...
    expect(
      getLinearAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: false,
        inventory: inventory([placement('kolux-linear', 'outdated')], ['kolux-linear'])
      })
  ...
  it('prefers canonical freshness when both names are installed', () => {
    expect(
      getLinearAgentSkillNavInstallStatus({
        skills: [skill('kolux-linear'), skill('linear-tickets')],
        installed: true,
        loading: false,
        inventory: inventory(
          [placement('kolux-linear', 'current'), placement('linear-tickets', 'outdated')],
          ['linear-tickets']
        )
      })
    ).toBe('up-to-date')
