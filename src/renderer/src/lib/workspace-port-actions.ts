...
export function shouldOpenWorkspacePortInKoluxBrowser(
...
export function resolvePortOpenInKoluxBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInKoluxBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInKoluxBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
