...
export function getSkillFreshnessDisplayStatus(
  ...
  for (const installation of inventory?.installations ?? []) {
    ...
    // drift Kolux has no way to fix. Skipped before `hasPlacement` so a repo-only skill
    // reports presence, not a freshness claim about a copy Kolux does not manage.
  ...
  // fail-closed — but only for skills Kolux actually found somewhere. Flagging a skill
