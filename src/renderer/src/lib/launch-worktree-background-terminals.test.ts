...
const setupLaunch = {
  ...
  envVars: { KOLUX_WORKTREE_PATH: '/repo/worktree' }
}
...
describe('launchWorktreeBackgroundTerminals', () => {
  ...
  it('spawns default tabs and setup as inactive background terminals', async () => {
    ...
    expect(mockSpawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({ KOLUX_WORKTREE_PATH: '/repo/worktree' }),
        tabId: 'tab-2',
  ...
  it('uses Windows setup commands for Windows SSH runner paths', async () => {
    ...
    await launchWorktreeBackgroundTerminals({
      worktreeId: 'wt-1',
      setup: {
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\wt\\kolux\\setup-runner.cmd',
        envVars: { KOLUX_WORKTREE_PATH: 'C:\\repo\\worktree' }
      }
    })
    ...
    expect(mockSpawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'cmd.exe /c "C:\\repo\\.git\\worktrees\\wt\\kolux\\setup-runner.cmd"',
  ...
  it('uses configured WSL setup commands for Windows bash runner paths', async () => {
    ...
    await launchWorktreeBackgroundTerminals({
      worktreeId: 'wt-1',
      setup: {
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\wt\\kolux\\setup-runner.sh',
    ...
    expect(mockSpawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /mnt/c/repo/.git/worktrees/wt/kolux/setup-runner.sh',
        connectionId: null
      })
    )
  })
