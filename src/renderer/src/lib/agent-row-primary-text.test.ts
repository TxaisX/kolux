...
=== CLI COMMANDS ===
kolux orchestration send --to term_parent
...
${taskBody}`
    ...
    expect(preview).not.toContain('You are working inside Kolux')
...
Implement the detailed worker instructions that should not stay as the final label`
  ...
  it('does not surface preamble boilerplate when the TASK marker is absent', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Kolux, a multi-agent IDE. You are a dispatched worker. Your task ID is: task_no_body CLI noise only'
      })
    ).toBe('')
  })
  ...
  // mentions `=== TASK ===` wins over Kolux's real separator.
  ...
  it('still reads the task body from a normalized single-line compact prompt', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Kolux, a multi-agent IDE. Your task ID is: task_inline === TASK === Compact body preview'
      })
    ).toBe('Compact body preview')
  })
})

describe('getKoluxDispatchTaskId', () => {
...
body`
    ...
    expect(getKoluxDispatchTaskId(normalized!.prompt)).toBe('task_9f3ab2')
  })
...
describe('isKoluxDispatchPrompt / getAgentRowGeneratedTitleText', () => {
  it('treats leading whitespace as still a dispatch preamble', () => {
    expect(
      isKoluxDispatchPrompt('  You are working inside Kolux, a multi-agent IDE. Worker task')
    ).toBe(true)
  })
