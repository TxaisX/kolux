...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Kolux pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Kolux session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Kolux status hints:', ...statusHints] : []),
    '',
