...
describe('getSkillFreshnessDisplayStatus', () => {
  ...
  it.each([
    ...
    // Why: these are Kolux's own bounds. A large but healthy plugin cache would
  ...
  // bound Kolux chose, so the only one a person can actually clear.
...
describe('hasSkillCopyNeedingAttention', () => {
  ...
  // copy inside a work directory. Kolux only updates global skills, so the project copy
  ...
  // presence rather than a freshness claim about a copy Kolux does not manage.
  ...
  // Why: an unreadable plugin path could hide a copy of anything, but a skill Kolux
