...
describe('prepareEphemeralVmWorkspaceTarget', () => {
  ...
  it('rejects and cleans up a Kolux-server provisioned root before project import', async () => {
    vi.mocked(window.api.ephemeralVm.provision).mockResolvedValue({
      ok: true,
      connectionType: 'kolux-server',
      stderr: 'creating sandbox',
  ...
  it('cleans up the runtime when required project setup capability is missing', async () => {
    vi.mocked(assertRuntimeEnvironmentCapability).mockRejectedValue(
      new Error('The recipe-created Kolux server does not support project setup.')
    )
    vi.mocked(window.api.ephemeralVm.provision).mockResolvedValue({
      ok: true,
      connectionType: 'kolux-server',
    ...
    expect(result).toEqual({
      ok: false,
      error: 'The recipe-created Kolux server does not support project setup.',
      stderr: 'creating sandbox'
    })
  })
