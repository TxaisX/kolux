...
describe('detectAgentStatusFromTitle', () => {
  ...
  // Why: cursor-agent's native title stays "Cursor Agent" all turn, so Kolux synthesizes decorated titles for the spinner/unread pipeline.
...
describe('getAgentLabel', () => {
  ...
  it('labels Cursor by identity, not a bare "cursor" token in another agent title', () => {
    ...
    expect(getAgentLabel('Terminal Cursor and Kolux slows down')).toBeNull()
  })
})
