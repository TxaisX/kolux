...
type EnsureKoluxCliAvailableOptions = {
...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Kolux may show a system prompt to register the Kolux CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Kolux needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Kolux CLI command.'

export function isKoluxCliAvailableOnPath(status: CliInstallStatus | null | undefined): boolean {
  return status?.state === 'installed' && status.pathConfigured === true
}

export async function ensureKoluxCliAvailableForAgentSkillTerminal({
  onStatusChange,
  registrationPromptDelayMs = 700
}: EnsureKoluxCliAvailableOptions = {}): Promise<CliInstallStatus | null> {
  try {
    ...
    if (status.state !== 'installed' || status.pathConfigured === false) {
      ...
      await showKoluxCliRegistrationPromptToast(registrationPromptDelayMs)
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Kolux CLI in PATH.'
          )
    )
    return null
  }
...
export async function showKoluxCliRegistrationPromptToast(delayMs = 700): Promise<void> {
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Kolux CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Kolux CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Kolux could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    // Kolux affordances when that shell can resolve the Kolux CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Kolux CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Kolux CLI directory to PATH before setup.'
          )
      }
    )
  }
