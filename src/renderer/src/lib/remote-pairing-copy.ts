  ...
  switch (kind) {
    case 'invalid-input':
      return translate(
        ...
        'Enter a Kolux access link or bare pairing code.'
      )
    case 'mobile-only':
      return translate(
        'auto.lib.remotePairingCopy.mobileOnly',
        'This link grants mobile-only access. Generate a link for another Kolux client.'
      )
  ...
  switch (kind) {
    case 'host-identity-mismatch':
      return translate(
        ...
        'Kolux reached {{endpoint}}, but that host does not match this link. Generate a new link on the other host.',
        { endpoint: endpoint ?? 'Kolux' }
      )
    ...
    case 'protocol-incompatible':
      return translate(
        ...
        'Update Kolux on this device and the other host, then try again.'
      )
    ...
    case 'environment-save-failed':
      return translate(
        ...
        'The host was verified, but Kolux could not save it. Check the name and local settings storage, then try again.'
      )
    case 'host-unreachable':
      return translate(
        'auto.components.settings.RuntimeHostAccessForm.unavailableHelp',
        'Make sure Kolux is running on the other host and that the network or SSH tunnel can reach {{endpoint}}.',
        { endpoint: endpoint ?? 'Kolux' }
      )
  }
}
