...
import { hashKoluxHookScript } from './kolux-hook-trust'
import {
...
function createTestState(overrides?: Partial<AppState>): {
  ...
  const trust: PersistedTrustedKoluxHooks = {}
  const state = {
    trustedKoluxHooks: trust,
...
describe('ensureHooksConfirmed', () => {
  ...
  it('short-circuits to run when the persisted content hash matches the current script', async () => {
    ...
    const hash = await hashKoluxHookScript(script)
    state.trustedKoluxHooks['repo-1'] = {
  ...
  it('re-prompts when the script content differs from the persisted hash', async () => {
    const { state, pending } = createTestState()
    const staleHash = await hashKoluxHookScript('old script')
    state.trustedKoluxHooks['repo-1'] = {
    ...
    // kolux.yaml changed, not because they've never approved this hook.
  ...
  it('includes default tab commands in the setup trust prompt', async () => {
    ...
    expect(pending[0].data.contentHash).toBe(await hashKoluxHookScript(expectedContent))
  ...
  it('inspects the requested host when duplicate repo ids exist', async () => {
    const { state } = createTestState({
      settings: { activeRuntimeEnvironmentId: 'env-1' },
      trustedKoluxHooks: { 'repo-1': { all: { approvedAt: 1 } } },
      repos: [
  ...
  it('does not prompt for kolux.yaml when the repo uses local commands only', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('does not prompt for kolux.yaml when local commands are the implicit default', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('opens a modal with the computed content hash and resolves with the user decision', async () => {
    ...
    expect(pending[0].data).toMatchObject({
      ...
      contentHash: await hashKoluxHookScript('pnpm install'),
