...
// parsed hook envelope to Kolux over this process's own stdin/stdout using the
...
async function main(): Promise<void> {
  const windowsPort = Number(process.env.KOLUX_AGENT_HOOK_PORT ?? '')
  const token = process.env.KOLUX_AGENT_HOOK_TOKEN ?? ''
  if (!Number.isInteger(windowsPort) || windowsPort <= 0 || token.length === 0) {
    process.stderr.write('[wsl-hook-relay] missing KOLUX_AGENT_HOOK_PORT/TOKEN in env\n')
    process.exit(1)
  }
