...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // Why no KOLUX_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('KOLUX_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets KOLUX_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      const env = buildSshPtySpawnEnv({
        env: { HOME: homeDir, PATH: '/usr/bin:/bin' },
        remoteCliBridgeEnv: {
          binDir: '/home/remote/.kolux-relay/bin',
          relayDir: '/home/remote/.kolux-relay',
          nodePath: '/home/remote/.kolux-relay/node',
          sockPath: '/home/remote/.kolux-relay/relay.sock'
        }
      })
      env.KOLUX_HISTFILE = join(homeDir, 'kolux-history', 'zsh_history')

      const config = getRelayShellLaunchConfig('/bin/zsh', env)

      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.kolux-relay', 'shell-ready', 'zsh'))
      expect(config.env.KOLUX_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'keeps a remote zsh with nothing Kolux-owned on the plain login path',
    () => {
      const env = buildSshPtySpawnEnv({ env: { HOME: homeDir, PATH: '/usr/bin:/bin' } })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ KOLUX_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.KOLUX_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset KOLUX_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.supportsReadyMarker).toBe(true)
      expect(config.env.KOLUX_SHELL_FEATURES).toContain('ready')
      expect(bashRc).toContain('printf "\\033]777;kolux-shell-start:%s\\007" "$$"')
      expect(bashRc).toContain('builtin unset KOLUX_SHELL_FEATURES')
      expect(bashRc).toContain('__kolux_append_prompt_command "__kolux_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;kolux-shell-ready\\007"')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.kolux-relay', 'shell-ready', 'zsh'))
      expect(config.env.KOLUX_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.KOLUX_SHELL_FEATURES).not.toContain('ready')
  ...
  itWithBash('runs the relay bash wrapper without fake C/D markers before the first prompt', () => {
    ...
    expect(bashRc).toContain('[[ -z "${__kolux_in_command:-}" ]] || return 0')
    expectBashOsc133Lifecycle(output)
  })

  itWithBash('emits lifecycle for foreground text ending like an internal hook', () => {
    const config = getRelayShellLaunchConfig('/bin/bash', { HOME: homeDir })
    const input = 'echo user:__kolux_osc133_prompt_done\nfalse\nexit 0\n'
    const output = runInteractiveBashRcfile(config.args[1] as string, homeDir, input)

    expect(output).toContain('user:__kolux_osc133_prompt_done')
    expectBashOsc133Lifecycle(output)
  })
  ...
  itWithBash('preserves relay bash prompt hooks and DEBUG traps without fake markers', () => {
    ...
    expect(output).not.toContain('USER_DEBUG_AFTER:<(( __kolux_exit_code == 0 ))>')
    expect(output).not.toContain('USER_DEBUG_AFTER:<__kolux_restore_prompt_status')
    expectBashOsc133Lifecycle(output)
  })
  ...
  // Kolux inherits ends in a ";"+whitespace separator. Prepend/append must not
