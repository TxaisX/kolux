...
describe('Subprocess: Relay entry point', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawn(
        ...
        { ...process.env, KOLUX_RELAY_EMPTY_STARTUP_GRACE_MS: '100' }
      )
      await relay.sentinelReceived
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawn(['--detached', '--grace-time', '1', '--sock-path', sockPath], {
        ...process.env,
        KOLUX_RELAY_EMPTY_STARTUP_GRACE_MS: '500'
      })
      await relay.sentinelReceived
  ...
  function spawnIdleGraceDaemon(
    ...
    const daemon = spawnRelayEntry(
      ...
      { ...process.env, KOLUX_RELAY_IDLE_GRACE_MS: idleGraceMs }
    )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawnRelayEntry(
        ...
        { ...process.env, KOLUX_RELAY_IDLE_GRACE_MS: '200' }
      )
      await relay.sentinelReceived
