...
export async function runRelayKoluxCliChannel(
  sockPath: string,
  argv: string[],
  ...
  const stdin =
    ...
    (shouldReadRemoteCliStdin(argv) ? await readKoluxCliStdin() : undefined)
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[kolux-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (error) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[kolux-cli] Relay socket error: ${error.message}\n`)
    process.exit(1)
  })
}

async function readKoluxCliStdin(): Promise<string | undefined> {
