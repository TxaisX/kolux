 ...
 *
 * Wires Kolux's main-side SshChannelMultiplexer to the relay-side
...
describe('Integration: relay hook server → mux → AgentHookServer.ingestRemote', () => {
  ...
  let koluxServer: AgentHookServer
  ...
  beforeEach(async () => {
    ...
    // Why: Kolux-side never starts an HTTP server in this test — `ingestRemote`
  ...
  it('clears remote Claude permission when the approved tool starts with matching identity', async () => {
    ...
    const start = Date.now()
    while (koluxServer.getStatusSnapshot()[0]?.state !== 'working' && Date.now() - start < 1500) {
      await new Promise((r) => setImmediate(r))
    }
    expect(koluxServer.getStatusSnapshot()).toEqual([
  ...
  it('clears remote Claude permission when its teammate idles', async () => {
    ...
    expect(koluxServer.getStatusSnapshot()[0]).toMatchObject({
      paneKey: `tab-9:${LEAF_9}`,
      connectionId: 'conn-test',
      state: 'working',
      agentType: 'claude',
    ...
    expect(koluxServer.getStatusSnapshot()[0]?.toolName).toBeUndefined()
  })
  ...
  it('clears remote Claude permission when approved PostToolUse matches the preceding tool use id', async () => {
    ...
    await expect(
      postClaude({
        hook_event_name: 'PreToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'rm -rf /tmp/kolux-2824-permission-target' },
    ...
    expect(koluxServer.getStatusSnapshot()).toEqual([
      expect.objectContaining({
        ...
        agentType: 'claude',
        toolName: 'Bash',
        toolInput: 'rm -rf /tmp/kolux-2824-permission-target'
      })
    ])
  })
