...
export function buildPosixCommandPathLookupScript(
  ...
  const mountPrelude = options.skipWindowsMountDirs
    ? [
        '[ "${_kolux_win_mounts+set}" = set ] || _kolux_win_mounts=$(awk \'$3 == "drvfs" || $3 == "9p" || $3 == "virtiofs" { print $2 }\' /proc/mounts 2>/dev/null)'
      ]
    : []
  const skipMountComponent = options.skipWindowsMountDirs
    ? [
        '      for _kolux_win_mount in $_kolux_win_mounts; do',
        '        case "$_kolux_lookup_component/" in',
        '          "$_kolux_win_mount"/*) _kolux_lookup_component= ;;',
        '        esac',
        '      done',
        '      if [ -z "$_kolux_lookup_component" ]; then',
        '        [ -n "$_kolux_lookup_has_more" ] || break',
        '        continue',
        '      fi'
      ]
    : []
  ...
  return [
    `_kolux_lookup_command=${commandAssignment}`,
    'resolved=',
    ...mountPrelude,
    'case "$_kolux_lookup_command" in',
    '  */*)',
    '    case "$_kolux_lookup_command" in',
    '      /*) _kolux_lookup_candidate=$_kolux_lookup_command ;;',
    '      *) _kolux_lookup_candidate=${PWD%/}/$_kolux_lookup_command ;;',
    '    esac',
    '    if [ -x "$_kolux_lookup_candidate" ] && [ ! -d "$_kolux_lookup_candidate" ]; then',
    '      resolved=$_kolux_lookup_candidate',
    '    fi',
    '    ;;',
    '  *)',
    '    _kolux_lookup_remaining=${PATH-}',
    '    while :; do',
    '      case "$_kolux_lookup_remaining" in',
    '        *:*)',
    '          _kolux_lookup_component=${_kolux_lookup_remaining%%:*}',
    '          _kolux_lookup_remaining=${_kolux_lookup_remaining#*:}',
    '          _kolux_lookup_has_more=1',
    '          ;;',
    '        *)',
    '          _kolux_lookup_component=$_kolux_lookup_remaining',
    '          _kolux_lookup_has_more=',
    '          ;;',
    '      esac',
    ...skipMountComponent,
    '      [ -n "$_kolux_lookup_component" ] || _kolux_lookup_component=.',
    '      case "$_kolux_lookup_component" in',
    '        /*) _kolux_lookup_candidate=$_kolux_lookup_component/$_kolux_lookup_command ;;',
    '        *) _kolux_lookup_candidate=${PWD%/}/$_kolux_lookup_component/$_kolux_lookup_command ;;',
    '      esac',
    '      if [ -x "$_kolux_lookup_candidate" ] && [ ! -d "$_kolux_lookup_candidate" ]; then',
    '        resolved=$_kolux_lookup_candidate',
    '        break',
