...
describe('buildSetupRunnerCommand', () => {
  ...
  it('keeps generic forward-slash UNC runner scripts on cmd.exe', () => {
    expect(
      buildSetupRunnerCommand('//server/share/repo/.git/kolux/setup-runner.cmd', 'windows')
    ).toBe('cmd.exe /c "//server/share/repo/.git/kolux/setup-runner.cmd"')
  })

  it('uses POSIX launch semantics for native Windows runners when the setup shell is POSIX', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\.git\\kolux\\setup-runner.sh', 'windows', {
        family: 'posix'
      })
    ).toBe('bash /c/repo/.git/kolux/setup-runner.sh')
  })
  ...
  it('uses the active WSL shell with WSL paths for native Windows POSIX runners', () => {
    ...
    ).toBe('bash /mnt/c/repo/.git/kolux/setup-runner.sh')
  })

  it('keeps cmd.exe launch semantics for cmd setup runners', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\.git\\kolux\\setup-runner.cmd', 'windows', {
        family: 'cmd'
      })
    ).toBe('cmd.exe /c "C:\\repo\\.git\\kolux\\setup-runner.cmd"')
  })

  it('infers generated POSIX runner shell semantics from extension when metadata is absent', () => {
    expect(buildSetupRunnerCommand('C:\\repo\\.git\\kolux\\setup-runner.sh', 'windows')).toBe(
      'bash /c/repo/.git/kolux/setup-runner.sh'
    )
  })
  ...
  it('never hands a batch runner to bash, even from a Git Bash pane', () => {
    ...
    const command = buildSetupRunnerCommand('C:\\repo\\.git\\kolux\\setup-runner.cmd', 'windows', {
      family: 'posix'
    })
  ...
  it('keeps the batch runner path in native form for a POSIX pane launch', () => {
    ...
    expect(
      resolveSetupRunnerCommand('C:\\repo\\.git\\kolux\\setup-runner.cmd', 'windows', {
        family: 'posix'
      })
    ).toMatchObject({
      runnerScriptPathForShell: 'C:\\repo\\.git\\kolux\\setup-runner.cmd',
      shell: 'windows'
    })
  })
...
describe('buildSetupRunnerCommand cmd metacharacter guard', () => {
  const cmdRunner = (segment: string) => `C:\\repo${segment}\\.git\\kolux\\setup-runner.cmd`
  ...
  it.each([
    ['plain', 'C:\\repo\\.git\\kolux\\setup-runner.cmd'],
    ['spaces', 'C:\\Program Files\\repo\\.git\\kolux\\setup-runner.cmd'],
    ['single quote', "C:\\o'brien\\.git\\kolux\\setup-runner.cmd"],
    ['brackets and dash', 'C:\\repo-[2]\\.git\\kolux\\setup-runner.cmd']
  ...
  it('passes the runner path through the environment rather than the cmd argument string', () => {
    ...
    expect(script).toContain('$processInfo.EnvironmentVariables["KOLUX_SETUP_RUNNER"] = $runner')
  ...
  it('escapes single quotes when embedding the path in the PowerShell literal', () => {
    const script = decodePowerShellCommand(
      buildSetupRunnerCommand("C:\\o'brien&co\\.git\\kolux\\setup-runner.cmd", 'windows', {
        family: 'cmd'
      })
    )

    expect(script).toContain("$runner = 'C:\\o''brien&co\\.git\\kolux\\setup-runner.cmd'")
  })
  ...
  it.each([
    ['native POSIX runner', 'C:\\repo\\a&b\\.git\\kolux\\setup-runner.sh', undefined],
    ['WSL UNC runner', '\\\\wsl.localhost\\Ubuntu\\home\\a&b\\kolux\\setup-runner.sh', undefined]
  ...
  it('does not disturb the wsl.exe POSIX launch', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\a&b\\.git\\kolux\\setup-runner.sh', 'windows', {
        family: 'posix',
        executable: 'wsl.exe'
      })
    ).toBe("bash '/mnt/c/repo/a&b/.git/kolux/setup-runner.sh'")
  })
})
...
describe('getSetupRunnerCommandPlatformForPath', () => {
  it('prefers POSIX for absolute POSIX runner paths even from Windows clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('/remote/repo/.git/kolux/setup-runner.sh', 'windows')
    ).toBe('posix')
  })

  it('prefers Windows for native Windows runner paths even from POSIX clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('C:\\repo\\.git\\kolux\\setup-runner.cmd', 'posix')
    ).toBe('windows')
  })
  ...
  it('keeps WSL UNC paths on the Windows resolver so they can be converted', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\kolux\\setup-runner.sh',
        'posix'
      )
    ).toBe('windows')
  })
  ...
  it('keeps forward-slash UNC paths on the Windows resolver', () => {
    ...
    expect(
      getSetupRunnerCommandPlatformForPath(
        '//server/share/repo/.git/kolux/setup-runner.cmd',
        'posix'
      )
    ).toBe('windows')
  })
