...
export function computerUseErrorRecoveryData(
  ...
  switch (code) {
    ...
    case 'window_not_found':
      return recoverWith(
        'Run `kolux computer list-windows --app <app> --json` and target one of the listed windows.',
        'If the app is listed but no usable window is visible, retry observation once with `--restore-window`.',
        'If no window is listed, open or focus the app first; `kolux computer` does not launch closed desktop apps.'
      )
    case 'window_not_focused':
      if (message?.includes('may already have been delivered')) {
        return recoverWith(
          'Run `kolux computer get-app-state --app <app> --json` and verify whether the intended action already occurred before retrying.',
    ...
    case 'window_stale':
      return recoverWith(
        'Run `kolux computer list-windows --app <app> --json` and choose a current window selector.',
        'Then rerun `kolux computer get-app-state --app <app> --json` before acting.'
      )
    case 'provider_incompatible':
      return recoverWith(
        'Run `kolux computer capabilities --json` and verify the local provider supports the requested operation.',
        'Update Kolux or use a supported platform/provider path before retrying.'
      )
    case 'unsupported_capability':
      return recoverWith(
        'Run `kolux computer capabilities --json` and choose a supported action.',
    ...
    case 'permission_denied':
      return recoverWith(
        'Run `kolux computer permissions --json`, or `kolux computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one missing permission.',
        'For remote or SSH targets, verify the command is running inside an active graphical desktop session.'
      )
    case 'element_not_found':
      return recoverWith(
        'Run `kolux computer get-app-state --app <app> --json` again and use an element index from the fresh tree.',
    ...
    case 'action_timeout':
      return recoverWith(
        'Run `kolux computer get-app-state --app <app> --json` before retrying so you know whether the UI changed.',
    ...
    case 'screenshot_failed':
      return recoverWith(
        ...
        'If the message mentions Screen Recording or screenshots permission, run `kolux computer permissions --id screenshots --json` and grant access before retrying.',
        'If the message mentions the payload cap, target a smaller/current window or use `--no-screenshot`.'
      )
    case 'accessibility_error':
      return recoverWith(
        'Run `kolux computer capabilities --json` to confirm the provider is available before retrying.',
        'If the message mentions permissions, run `kolux computer permissions --id accessibility --json` and grant access.',
