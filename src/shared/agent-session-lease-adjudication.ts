...
export type AgentSessionOwnerProbe =
  /** Kolux watched this exact process exit. */
...
export function evaluateAgentSessionAcquisition(args: {
  ...
  if (lease.ownerProcess !== null) {
    if (!isProvenDeadProbe(probe)) {
      // Why: a lapsed deadline means Kolux stopped hearing from the owner, not that the child
...
/**
 * A process carrying a Kolux spawn token with no matching lease is an orphan: stop it, never
