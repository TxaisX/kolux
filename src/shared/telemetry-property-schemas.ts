...
// Small set: only failures Kolux's PTY-typed-command launch can observe (`binary_not_found` = shell ENOENT, `paste_readiness_timeout`, `unknown`).
// Provider-side errors (auth/rate-limit/network) happen inside the agent CLI subprocess and are invisible to Kolux. See telemetry-plan.md §Defer per-incident error fields.
...
// Whitelist of settings emittable on `settings_changed`. `kolux_channel` (build-time, not user-togglable) is absent.
