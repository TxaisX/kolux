...
// Only the detail after it is Kolux's own wording, so verdict matchers anchor on the boundary
...
// making a different decision than one killing a terminal Kolux just saw running. The marker
...
export function classifyWorktreeForceDeleteReason(
  ...
  if (isLockedWorktreeRemovalError(error)) {
    ...
    // unlocked explicitly rather than folded into Kolux's dirty-file force path.
    return null
  }
