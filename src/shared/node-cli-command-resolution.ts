...
function getBaseVersionManagerDirectories(platform: NodeJS.Platform, homePath: string): string[] {
  ...
  if (platform === 'win32') {
    ...
    // GUI-launched Kolux may not inherit the user's PATH entry for it.
...
function getNvmVersionDirectories(homePath: string): string[] {
  ...
  // (TxaisX/kolux#10932). The rest stay behind it as fallbacks, so a CLI
 ...
 * require (TxaisX/kolux#10932). Pair the binary with the runtime it was
