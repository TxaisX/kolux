...
import {
  ...
  MAX_KOLUX_YAML_BYTES,
  MAX_KOLUX_YAML_COLLECTION_ENTRIES,
  MAX_KOLUX_YAML_FIELD_BYTES,
  MAX_KOLUX_YAML_FIELD_CODE_UNITS
...
describe('kolux.yaml parse bounds', () => {
  beforeEach(() => {
  ...
  it('admits the exact UTF-8 input boundary and rejects +1 before YAML parsing', () => {
    expect(parseKoluxYaml(' '.repeat(MAX_KOLUX_YAML_BYTES))).toMatchObject({
      scripts: { setup: 'pnpm install' }
    })
    expect(parseDocumentMock).toHaveBeenCalledOnce()

    parseDocumentMock.mockClear()
    expect(parseKoluxYaml(' '.repeat(MAX_KOLUX_YAML_BYTES + 1))).toBeNull()
    expect(parseDocumentMock).not.toHaveBeenCalled()
  })

  it('rejects a multibyte input over the byte cap before YAML parsing', () => {
    const content = 'é'.repeat(MAX_KOLUX_YAML_BYTES / 2 + 1)

    expect(parseKoluxYaml(content)).toBeNull()
    expect(parseDocumentMock).not.toHaveBeenCalled()
  })
  ...
  it('passes an explicit alias expansion cap to YAML conversion', () => {
    ...
    expect(parseKoluxYaml('scripts: {}')).not.toBeNull()
    expect(toJS).toHaveBeenCalledWith({ maxAliasCount: MAX_KOLUX_YAML_ALIAS_COUNT })
  })

  it('preserves exact-size fields and drops a field at +1 code unit', () => {
    const exact = 'x'.repeat(MAX_KOLUX_YAML_FIELD_CODE_UNITS)
    returnYamlRoot({ scripts: { setup: exact } })
    expect(parseKoluxYaml('exact')).toMatchObject({ scripts: { setup: exact } })

    returnYamlRoot({ scripts: { setup: `${exact}x` } })
    expect(parseKoluxYaml('overflow')).toBeNull()

    const exactUtf8 = 'é'.repeat(MAX_KOLUX_YAML_FIELD_BYTES / 2)
    returnYamlRoot({ scripts: { setup: exactUtf8 } })
    expect(parseKoluxYaml('exact-utf8')).toMatchObject({ scripts: { setup: exactUtf8 } })

    returnYamlRoot({ scripts: { setup: `${exactUtf8}é` } })
    expect(parseKoluxYaml('overflow-utf8')).toBeNull()
  })

  it('admits the exact collection boundary and rejects +1 entries', () => {
    const tabs = Array.from({ length: MAX_KOLUX_YAML_COLLECTION_ENTRIES }, (_, index) => ({
      title: `tab-${index}`
    }))
    returnYamlRoot({ defaultTabs: tabs })
    expect(parseKoluxYaml('exact')?.defaultTabs).toHaveLength(MAX_KOLUX_YAML_COLLECTION_ENTRIES)
