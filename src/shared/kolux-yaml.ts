...
import type {
  KoluxDefaultTabTemplate,
  KoluxHooks,
  KoluxVmRecipe,
  KoluxVmRecipeDiagnostic
} from './kolux-yaml-hook-types'
import {
  isKoluxYamlFieldWithinLimit,
  isKoluxYamlTextWithinLimit,
  MAX_KOLUX_YAML_ALIAS_COUNT,
  MAX_KOLUX_YAML_COLLECTION_ENTRIES
} from './kolux-yaml-file-limit'
import { normalizeLegacyCheckoutModeSpelling } from './ephemeral-vm-recipe-checkout-mode'
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isKoluxYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const KOLUX_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const KOLUX_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): KoluxDefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_KOLUX_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is KoluxDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: KoluxVmRecipe[]
  diagnostics: KoluxVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: KoluxVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_KOLUX_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_KOLUX_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_KOLUX_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!KOLUX_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${KOLUX_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
      ...
      const rawCheckoutMode = asTrimmedString(record.checkoutMode)
      const checkoutMode = rawCheckoutMode
        ? normalizeLegacyCheckoutModeSpelling(rawCheckoutMode)
        : rawCheckoutMode
      if (
        checkoutMode &&
        ...
        diagnostics.push({
          index,
          field: 'checkoutMode',
          message: `Recipe "${id}" checkoutMode must be "kolux-worktree" or "provisioned-root".`
        })
        return null
      }
    ...
    .filter((entry): entry is KoluxVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `kolux.yaml`.
 */
export function parseKoluxYaml(content: string): KoluxHooks | null {
  if (!isKoluxYamlTextWithinLimit(content)) {
    return null
  }
  ...
  try {
    ...
    root = document.toJS({ maxAliasCount: MAX_KOLUX_YAML_ALIAS_COUNT })
  } catch {
    return null
  }
