...
describe('createTerminalTitleTracker pr-link facts', () => {
  it('emits one fact per PR URL including multiple links in one chunk', () => {
    ...
    tracker.handleChunk(
      'see https://github.com/acme/kolux/pull/42 and https://github.com/acme/kolux/pull/43 \r\n'
    )

    expect(events).toEqual([
      ['pr', 'https://github.com/acme/kolux/pull/42', 42],
      ['pr', 'https://github.com/acme/kolux/pull/43', 43]
    ])
  })
  ...
  it('waits for a boundary when a URL splits across chunks and dedupes repeats', () => {
    ...
    tracker.handleChunk('PR: https://github.com/acme/kolux/pull/4')
    expect(events).toEqual([])
    tracker.handleChunk('2\r\n')
    tracker.handleChunk('again https://github.com/acme/kolux/pull/42\r\n')

    expect(events).toEqual([['pr', 'https://github.com/acme/kolux/pull/42', 42]])
  })
  ...
  it('skips the 133/URL scans entirely when no consumer is registered', () => {
    ...
    tracker.handleChunk(`${ESC}]133;D;0${BEL}https://github.com/acme/kolux/pull/42\r\n`)
...
describe('createTerminalTitleTracker synthetic-frame isolation', () => {
  it('never feeds synthetic frames to the 133/PR scanners', () => {
    ...
    tracker.applySyntheticTitleFrame(
      `${ESC}]0;⠋ Cursor Agent${BEL}${ESC}]133;D;0${BEL}https://github.com/acme/kolux/pull/42\r\n`
    )
...
describe('createTerminalTitleTracker transient-fact scanning suppression', () => {
  it('skips bell/133/pr-link/2031 while suppressed but keeps title processing', () => {
    ...
    tracker.handleChunk(
      `${ESC}]0;zsh${BEL}${ESC}]133;D;0${BEL}ding${BEL}https://github.com/acme/kolux/pull/42\r\n${ESC}[?2031h`
    )
