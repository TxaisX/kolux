...
const BROWSER_UNAVAILABLE_MESSAGES: Record<RuntimeBrowserUnavailableReason, string> = {
  unconfigured:
    'Browser automation has no backend on this host. Install the Kolux desktop app, or set KOLUX_BROWSER_EXECUTABLE to a Chromium executable.',
  driver_missing:
    'KOLUX_BROWSER_EXECUTABLE is set, but the bundled agent-browser driver is missing or not executable on this host, so Chromium cannot be driven.',
  executable_not_found: 'KOLUX_BROWSER_EXECUTABLE points at a path that does not exist.',
  executable_not_executable:
    'KOLUX_BROWSER_EXECUTABLE points at a file that is not executable by this host.',
  electron_start_failed: 'The installed Electron browser provider failed to start.',
  chromium_start_failed:
    'The Chromium browser provider named by KOLUX_BROWSER_EXECUTABLE failed to start.',
