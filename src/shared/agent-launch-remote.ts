...
/**
 * Why: a repo reached over SSH runs the Kolux CLI through the relay shim, which is always deployed
 * as plain `kolux` (Unix) / `kolux.cmd` (Windows). The Linux-only `kolux-ide` rename — which exists
 * solely to avoid shadowing the GNOME Orca screen reader on a local desktop — must not be applied
 * to those remotes, or `kolux-ide claude-teams` lands on a PATH where it does not exist.
 *
 ...
 *
 *   - a row carrying only `executionHostId: 'ssh:<target>'` reads as local and gets the `kolux-ide`
 ...
 * shim), while a runtime host with no nested target is a full Kolux install and stays false — as do
