...
describe('createTerminalGitHubPRLinkDetector', () => {
  it('extracts GitHub pull request URLs from terminal output', () => {
    ...
    expect(observe('Created https://github.com/acme/kolux/pull/42\r\n')).toEqual([
      {
  ...
  it('waits for a boundary when the URL is split across PTY chunks', () => {
    ...
    expect(observe('https://github.com/acme/kolux/pull/4')).toEqual([])
    expect(observe('2\r\n')).toEqual([
      {
        url: 'https://github.com/acme/kolux/pull/42',
        slug: { owner: 'acme', repo: 'kolux', host: 'github.com' },
  ...
  it('detects a URL split inside the GitHub prefix', () => {
    ...
    expect(observe('ub.com/acme/kolux/pull/42\n')).toEqual([
      {
        url: 'https://github.com/acme/kolux/pull/42',
        slug: { owner: 'acme', repo: 'kolux', host: 'github.com' },
        number: 42
  ...
  it('trims terminal punctuation around printed URLs', () => {
    ...
    expect(observe('Opened (https://github.com/acme/kolux/pull/42).\n')[0]?.url).toBe(
      'https://github.com/acme/kolux/pull/42'
    )
  })
  ...
  it('does not repeat the same PR URL from overlapping carry text', () => {
    ...
    expect(observe('https://github.com/acme/kolux/pull/42\n')).toHaveLength(1)
  ...
  it('ignores non-PR GitHub-shaped links', () => {
    ...
    expect(observe('https://github.com/acme/kolux/issues/42\n')).toEqual([])
  })
  ...
  it('scans huge terminal chunks containing pull markers without global regex iteration', () => {
    ...
    expect(observe(`${noise}Created https://github.com/acme/kolux/pull/42\r\n`)).toEqual([
      {
        url: 'https://github.com/acme/kolux/pull/42',
        slug: { owner: 'acme', repo: 'kolux', host: 'github.com' },
        number: 42
  ...
  it('drops overlong incomplete URL carry instead of retaining pasted megabytes', () => {
    ...
    expect(observe(`https://github.com/acme/kolux/pull/${'4'.repeat(10_000)}`)).toEqual([])
    expect(observe('2\r\n')).toEqual([])
  })
  ...
  it('still joins a PR URL split across chunks after a large scheme-free chunk', () => {
    ...
    expect(
      observe(`${'filler output\n'.repeat(5_000)}https://github.com/acme/kolux/pull/`)
    ).toEqual([])
    expect(observe('7\r\n')).toEqual([
      {
  ...
  it('drops carry when the scheme sits further back than the carry cap', () => {
    ...
    expect(observe(`https://github.com/acme/kolux/pull/${'4'.repeat(600)}`)).toEqual([])
    expect(observe('2\r\n')).toEqual([])
  })
  ...
  it('drops a trailing scheme fragment when a scheme sits behind the window', () => {
    ...
    expect(observe(`https://github.com/acme/kolux/pull/1${'x'.repeat(600)}https`)).toEqual([])
    expect(observe('://github.com/acme/kolux/pull/12\n')).toEqual([])
  })
  ...
  it('does not fabricate a link by splicing a stale fragment onto later output', () => {
    ...
    expect(observe(`https://github.com/acme/kolux/pull/1${'x'.repeat(600)}h`)).toEqual([])
