...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('TxaisX/kolux-hourly')
    expect(getReleaseRepoForChannel('daily')).toBe('TxaisX/kolux-daily')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('TxaisX/kolux-adhoc')
    expect(getReleaseRepoForChannel('stable')).toBe('TxaisX/kolux')
    expect(getReleaseRepoForChannel('rc')).toBe('TxaisX/kolux')
  })
  ...
  it('builds release-notes links against the repo that published the version', () => {
    expect(getReleaseNotesUrlForVersion('1.4.160-hourly.202607281400')).toBe(
      'https://github.com/TxaisX/kolux-hourly/releases/tag/v1.4.160-hourly.202607281400'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/TxaisX/kolux-daily/releases/tag/v1.4.160-daily.202607281300'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160')).toBe(
      'https://github.com/TxaisX/kolux/releases/tag/v1.4.160'
    )
    expect(getReleaseNotesUrlForVersion('v1.4.160-rc.3')).toBe(
      'https://github.com/TxaisX/kolux/releases/tag/v1.4.160-rc.3'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/TxaisX/kolux-adhoc/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe('https://github.com/TxaisX/kolux/releases')
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/TxaisX/kolux-hourly/releases/tag/v${version}`,
      installerUrl: null
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/TxaisX/kolux-adhoc/releases/tag/v${version}`,
      installerUrl: null
    })
    const sorted = sortReleaseBuildsNewestFirst([
