...
// or a plugin's bundled copy. Kolux's global update never writes here, so the owner's
...
// Why: Kolux's updater only ever passes --global, so a copy a project owns is not
// something Kolux can act on — it has no remedy by design. Reporting it as global drift
...
// Why: a real read failure is a fact about the user's disk and stays actionable. Kolux's
