// Why: full Kolux dispatch preambles are multi-KB (CLI instructions before
...
export const KOLUX_DISPATCH_STATUS_PREAMBLE_PREFIX =
  'You are working inside Kolux, a multi-agent IDE.'
export const KOLUX_DISPATCH_STATUS_TASK_MARKER = '=== TASK ==='
const KOLUX_DISPATCH_STATUS_TASK_ID_MARKER = 'Your task ID is:'
...
const KOLUX_DISPATCH_STATUS_SOURCE_SCAN_LIMIT = 24_576

export function isKoluxDispatchStatusPrompt(value: string): boolean {
  ...
  const scanEnd = Math.min(value.length, KOLUX_DISPATCH_STATUS_SOURCE_SCAN_LIMIT)
  let start = 0
  while (start < scanEnd && isEcmaTrimWhitespace(value.charCodeAt(start))) {
    start++
  }
  return (
    start + KOLUX_DISPATCH_STATUS_PREAMBLE_PREFIX.length <= scanEnd &&
    value.startsWith(KOLUX_DISPATCH_STATUS_PREAMBLE_PREFIX, start)
  )
}
...
export function compactDispatchPromptForStatus(
  ...
  let taskId = ''
  const idMarkerIndex = scan.indexOf(KOLUX_DISPATCH_STATUS_TASK_ID_MARKER)
  if (idMarkerIndex !== -1) {
    const afterId = scan.slice(idMarkerIndex + KOLUX_DISPATCH_STATUS_TASK_ID_MARKER.length)
    let idStart = 0
  ...
  if (taskMarkerIndex !== -1) {
    const body = scan.slice(taskMarkerIndex + KOLUX_DISPATCH_STATUS_TASK_MARKER.length)
  ...
  // Why: keep the dispatch prefix (isKoluxDispatchPrompt) + task id (label match)
  // + task body (fallback preview) so UI helpers still work on the 200-char field.
  let compact = KOLUX_DISPATCH_STATUS_PREAMBLE_PREFIX
  if (taskId) {
    compact += ` ${KOLUX_DISPATCH_STATUS_TASK_ID_MARKER} ${taskId}`
  }
  if (taskBody) {
    compact += ` ${KOLUX_DISPATCH_STATUS_TASK_MARKER} ${taskBody}`
  }
  return normalizeSingleLine(compact, maxLength)
}

/**
 * Locate the Kolux task separator in a dispatch prompt scan window.
 * Why: base-drift commit subjects are repository-controlled and may mention
 * `=== TASK ===`. Raw multi-line preambles must use the standalone line Kolux
 ...
 */
export function findKoluxDispatchTaskMarkerIndex(value: string): number {
  let searchFrom = 0
  while (searchFrom < value.length) {
    const markerIndex = value.indexOf(KOLUX_DISPATCH_STATUS_TASK_MARKER, searchFrom)
    if (markerIndex === -1) {
      break
    }
    const markerEnd = markerIndex + KOLUX_DISPATCH_STATUS_TASK_MARKER.length
  ...
  return value.includes('\n') || value.includes('\r')
    ? -1
    : value.indexOf(KOLUX_DISPATCH_STATUS_TASK_MARKER)
}
