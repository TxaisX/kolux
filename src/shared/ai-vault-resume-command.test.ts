...
describe('buildAiVaultResumeCommand', () => {
  ...
  it('carries non-default Codex homes in copied resume commands', () => {
    expect(
      ...
      "cd '/repo/app' && CODEX_HOME='/Users/ada/Library/Application Support/Kolux/codex-runtime-home/home' codex resume 'session-1'"
    )
    ...
    expect(
      ...
      'cmd /d /s /c "cd /d ""C:\\Users\\Ada Lovelace\\repo"" && set ""CODEX_HOME=C:\\Users\\Ada\\AppData\\Roaming\\Kolux\\codex-runtime-home\\home"" && codex resume ""session-1"""'
    )
  })
...
describe('buildAiVaultResumeShellCommand env removal', () => {
  ...
  it('keeps a pinned CODEX_HOME authoritative instead of stripping it', () => {
    ...
    expect(command).toBe(
      "cd '/repo' && CODEX_HOME='/home/a/.codex-work' env -u KOLUX_CODEX_HOME codex 'resume' 'sid'"
    )
  ...
  it('keeps a pinned CODEX_HOME authoritative under a git-bash shell too', () => {
    expect(
      ...
      "cd '/repo' && CODEX_HOME='/c/users/a/.codex-work' env -u KOLUX_CODEX_HOME codex 'resume' 'sid'"
    )
  })
  ...
  it('uses PowerShell grammar for a PowerShell shell on a non-Windows host', () => {
    ...
    expect(command).toBe(
      'Remove-Item Env:CODEX_HOME -ErrorAction SilentlyContinue; ' +
        'Remove-Item Env:KOLUX_CODEX_HOME -ErrorAction SilentlyContinue; ' +
  ...
  it('keeps the cmd clear ahead of the cd so a failed cd cannot launch the agent', () => {
    expect(
      ...
      'set "CODEX_HOME=" & set "KOLUX_CODEX_HOME=" & cd /d "C:\\repo" && codex \'resume\' \'sid\''
    )
  })
})
