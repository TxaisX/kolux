...
/**
 * MiMo Code reports an approval prompt as `permission.asked`; the status plugin forwards
 * that event's `properties` under `hook_event_name: 'PermissionRequest'`. The shape is
 * fixed by the underlying SDK (`EventPermissionAsked`): `permission` names what is
 ...
 * inside it come from each tool. The payloads below were captured from a live session
 * rather than invented, because the first version of this suite guessed a shape
 * (`metadata: {}`) that the CLI never emits and so never exercised the real `edit` path.
 */
describe('MiMo Code permission request status', () => {
  ...
  const SOURCES = ['mimo-code'] as const
  ...
  it.each(SOURCES)('names the edited file for %s', (source) => {
    // Captured verbatim from a live session — an `edit` request keys its path as
  ...
  it.each(SOURCES)('never previews a diff as the blocked target for %s', (source) => {
    ...
    // Why: `diff` is the largest field the tool sends and would swamp a one-line row.
  ...
  it.each(SOURCES)('falls back to patterns when metadata is unrecognized for %s', (source) => {
    // Why: `metadata` is typed `Record<string, unknown>` by the SDK, so a tool Kolux has
