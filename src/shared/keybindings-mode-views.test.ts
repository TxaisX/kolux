...
describe('mode view keybindings', () => {
  ...
  it('match the shifted punctuation a US layout reports for Shift+digit', () => {
    expect(keybindingMatchesAction('view.inbox', shifted('!', 'Digit1', 'win32'), 'win32')).toBe(true)
    expect(keybindingMatchesAction('view.floor', shifted('@', 'Digit2', 'win32'), 'win32')).toBe(true)
    expect(keybindingMatchesAction('view.code', shifted('#', 'Digit3', 'linux'), 'linux')).toBe(true)
  ...
  it('match when the platform reports the plain digit as the logical key', () => {
    expect(keybindingMatchesAction('view.floor', shifted('2', 'Digit2', 'win32'), 'win32')).toBe(true)
  })
