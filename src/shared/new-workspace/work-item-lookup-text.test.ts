...
describe('isWorkItemLookupText Jira URLs', () => {
  it('recognizes absolute Jira browse URLs for auto-name replacement', () => {
    expect(isWorkItemLookupText('https://company.atlassian.net/browse/KOLUX-123')).toBe(true)
  ...
  it('does not claim bare Jira-shaped keys or malformed browse URLs', () => {
    expect(isWorkItemLookupText('KOLUX-123')).toBe(false)
    expect(isWorkItemLookupText('https://jira.example.com/browse/KOLUX-123/extra')).toBe(false)
  })
})
