...
export const LOCAL_BUILD_COMPATIBILITY_FILENAME = 'kolux-local-build.json'
export const LOCAL_BUILD_COMPATIBILITY_FORMAT_VERSION =
  LOCAL_BUILD_COMPATIBILITY_CONTRACT.formatVersion
export const KOLUX_APP_ID = LOCAL_BUILD_COMPATIBILITY_CONTRACT.appId
...
export function getLocalBuildCompatibilityError(
  ...
  if (!target.readableStateSchemaVersions.includes(currentStateSchemaVersion)) {
    return `This build cannot read Kolux workspace state schema ${currentStateSchemaVersion}. Your workspace was not changed.`
  }
