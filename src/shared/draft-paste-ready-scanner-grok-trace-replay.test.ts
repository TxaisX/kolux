 ...
 *
 * Mirrors the callers in agent-draft-readiness.ts and kolux-runtime.ts: the quiet
...
describe('grok startup trace replay (inline mode, no alternate screen)', () => {
  it('still delivers through the quiet window when the marker never anchors', () => {
    ...
    // launch with NO delivery path — kolux-runtime drops the draft when readiness
