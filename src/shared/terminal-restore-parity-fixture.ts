...
import { activateKoluxTerminalUnicodeProvider } from './terminal-unicode-provider'
 ...
 *  (pane-dom-creation.ts) and the Kolux ZWJ provider (pane-lifecycle.ts).
...
export function createRendererParityTerminal(dims: { cols: number; rows: number }): ParityTerminal {
  ...
  activateKoluxTerminalUnicodeProvider(terminal)
 ...
 *    while Kolux's WebGL glyph renderer skips decorations on null cells.
 ...
 *  KoluxRuntime.serializeHeadlessTerminalBuffer (normal buffer separated from
