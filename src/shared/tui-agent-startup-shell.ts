...
/**
 * `'posix'` covers every Unix shell Kolux can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Kolux has to detect is a dialect it
...
export function tokenizeStartupCommand(
  ...
  // Why one Unix parse: the input is a string the user typed into a Kolux
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Kolux can
 ...
 * of them, and Kolux never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Kolux's shell wrappers: Kolux only wraps
 ...
 * shells Kolux never spawned — in all of those a helper would be `command not
 ...
 * clipboard command, which a user may run with no Kolux-injected value at all.
