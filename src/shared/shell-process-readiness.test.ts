...
describe('shell process readiness', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-shell-path-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-relative-shell-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-shadowed-shell-'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-installed-shells-'))
      const first = join(root, 'first')
      const second = join(root, 'second')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kolux-offpath-shell-'))
