...
const EPHEMERAL_VM_RUNTIMES_FILE = 'kolux-ephemeral-vm-runtimes.json'
...
export function upsertEphemeralVmRuntimeRollbackRecovery(
  ...
  try {
    ...
    if (error instanceof JsonStringifyByteLimitError) {
      throw new EphemeralVmRuntimeStoreError(
        'runtime_error',
        `Could not write Kolux ephemeral VM runtimes at ${path}; the store exceeds its durable capacity.`
      )
    }
    throw error
  }
...
function readEphemeralVmRuntimeStore(userDataPath: string): LoadedEphemeralVmRuntimeStore {
  ...
  try {
    ...
    throw new EphemeralVmRuntimeStoreError(
      'runtime_error',
      `Could not read Kolux ephemeral VM runtimes at ${path}; the file is invalid.`
    )
  }
}
