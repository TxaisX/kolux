...
Kolux can put one skill or a bundle of skills behind one unlisted, revocable link. Shared bundles
...
Publishing and link management require a Kolux account in the desktop app.
...
Kolux publishes an immutable version. Later changes do not silently alter a link's current bytes;
...
Opening a Kolux skill link shows a preview before changing any files. You can also open **Skills**,
...
Supported destinations include the local machine, paired Kolux runtimes, WSL, and SSH hosts. The
...
Kolux keeps one canonical installed copy and places it where supported agents can discover it.
...
**Keep local** is the default when an existing skill differs. Kolux replaces modified content only
...
Kolux updates only the selected skills that still exist in that version.

An interrupted install is recovered on restart. If Kolux reports a conflict or partial result,
review the named skill and retry; completed skills do not need to be installed again.

## Remove an installed skill

Use **Skills → Manage installs → Remove**. Kolux removes only copies and provider placements that it
...
executables may run later when a person or agent uses the skill. Kolux validates the package and
...
Kolux records bounded operational identifiers and outcomes. Normal logs, telemetry, and support
...
links intentionally show the same response so Kolux does not disclose private package existence.
