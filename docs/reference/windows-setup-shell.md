...
On native Windows, Kolux writes the `kolux.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `kolux.yaml`, so no project
...
A `#!` line only takes effect when Kolux can actually launch bash from the configured terminal — the
