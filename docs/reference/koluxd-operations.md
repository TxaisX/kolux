...
`koluxd` is the Kolux runtime served from plain Node. This is the contract between it and
...
A deployment is **koluxd** plus **the terminal daemon**.

|            | koluxd                            | terminal daemon                       |
| ---------- | -------------------------------- | ------------------------------------- |
| Started by | the supervisor                   | koluxd, detached                       |
| Owns       | RPC, git, worktrees, persistence | every local PTY                       |
| Lifetime   | one supervised run               | detached from koluxd, not its service  |
| Endpoint   | `ws://<bind>:<port>`             | `<data-root>/daemon/daemon-v<N>.sock` |

koluxd detaches the daemon and calls `disconnectDaemon()`, never `shutdownDaemon()`. The
built-in remote deployment path stops only the recorded koluxd PID, so the daemon and its PTYs
...
Process detachment is not service isolation. A daemon forked by koluxd, and every PTY it owns,
...
`kolux serve`'s wide default, and a startup where some device has connected before — and an
...
Under the shipping design a client reaches a remote koluxd over an SSH local port-forward, so
loopback is the correct default and the pairing credential travels over SSH.

## Data root and the instance lock

The data root is `$KOLUX_USER_DATA`, else `$XDG_DATA_HOME/Kolux`, else `~/.kolux`.

Before the profile index or the store is touched, koluxd takes `<data-root>/koluxd.lock`.
It refuses to start when:

| Code                                   | Meaning                                                       |
| -------------------------------------- | ------------------------------------------------------------- |
| `koluxd_data_root_wrong_owner`          | the root is owned by another uid (POSIX)                      |
| `koluxd_data_root_shared`               | the root is group/world accessible and could not be tightened |
| `koluxd_instance_lock_held`             | another live koluxd owns this root                             |
| `koluxd_instance_lock_foreign_identity` | the lock belongs to a different identity                      |
| `koluxd_data_root_unusable`             | the root cannot be created, stat'd or written                 |

A root that is merely too permissive and that we own is tightened to `0700` rather than
refused — koluxd stores credentials there unsealed (no OS keyring on this host), so the goal
...
Before a cgroup-wide stop, obtain a fresh `kolux-ide terminal list --json` result using the same OS
...
`sudo -Hu kolux /home/kolux/.local/bin/kolux-ide terminal list --json`). Replace both `kolux` and
`/home/kolux` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/kolux-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
...
not admit new work after the census. Kolux does not yet provide an atomic census-and-stop fence.

### Who supervises koluxd

An external supervisor (systemd, launchd, a process manager). koluxd conforms to it:

- **Readiness.** One JSON line on stdout (`--json`), `type: "kolux_server_ready"`, published
...
- **Shutdown.** `SIGTERM` or `SIGINT` starts a graceful stop. A **second** signal exits
  ...
  the one teardown that skips the daemon handoff. koluxd also imposes its own 15s deadline
...
- **Logs.** koluxd writes human-readable diagnostics to **stderr** and its readiness contract
  ...
  `KOLUX_DIAGNOSTICS_DISABLED=1`). Rotation of that file is not implemented — see
  [What is not covered](#what-is-not-covered).

### koluxd supervising the daemon

- **Launch.** Forked detached from `daemon-entry.js` beside `koluxd.js`, with its own PID
...
- **Crash-loop containment.** At most **5 launches per 60s rolling window** per koluxd run;
  ...
  recovers without restarting koluxd. An operator-initiated daemon restart clears it — that
  is the deliberate "try again".
- **No macOS login-session watch.** That watch retires the daemon when the spawning GUI login
  session dies. A koluxd daemon must survive its SSH session ending.
- **Shutdown.** koluxd never stops the daemon. A daemon that was never adopted retires itself
  after its adoption window; an adopted one stays resident (see Decommissioning).

### Decommissioning

After a PID-scoped stop, an adopted daemon stays resident so the next koluxd can reattach.
A combined-unit systemd stop kills it instead. To retire a process-scoped deployment, apply
the census rule above, stop koluxd, then stop the daemon named by `health.terminalDaemon.pid`.
...
```
buildHash    sha256 (16 hex) of the running koluxd bundle — build identity that a version
             string cannot give, so a rollback that did not replace the file is visible
buildVersion KOLUX_VERSION
...
terminalDaemon:
  ...
  buildVersion       the build the LIVE daemon was forked from (may legitimately predate
                     this koluxd after an update — reporting koluxd's version for both would
                     hide exactly that)
...
processes: koluxd drives it, the daemon performs it, the verdict crosses the socket.
...
because those terminals die with koluxd. A daemon that answered and then failed its spawn
...
- **A continuous health endpoint.** `health` is published once, in the readiness payload. A
  ...
  `collectKoluxdHealth()`; that surface does not exist yet.
- **Systemd-isolated daemon supervision.** koluxd and its daemon currently share one service
