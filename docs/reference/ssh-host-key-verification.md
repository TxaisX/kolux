...
`HostKeyAlias` if set, else the **resolved hostname** — keying on the Kolux label would miss every
existing entry.
...
`StrictHostKeyChecking=no` did not help. `KOLUX_SSH_FORCE_SYSTEM_TRANSPORT=1` is read from the
...
> **Ours disagreed** — "The host key for `build-01` changed since you last connected from Kolux. If you
...
2. **We keyed on the Kolux label.** `ssh -G` echoes its own argument back as `hostname` when no Host
...
- **The web user never sees the reason.** `runtime/rpc/methods/ssh.ts` rethrows
  ...
  _desktop_ user viewing a remote-Kolux-server-owned host gets the same generic string.
...
**Rollout:** the first release carrying this is the first time Kolux can refuse an SSH connection at
