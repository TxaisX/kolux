...
Kolux selects a Relay region in the Electron main process before requesting a new assignment. The
director publishes an allowlisted region catalog containing only HTTPS cell subdomains of that
director. Kolux discards one warm-up `/health` request per probe origin — a cold request pays TCP and
...
be measured, Kolux sends no hint rather than selecting the sole survivor. Sending no hint is not
...
After a control socket registers, Kolux probes the cell it actually landed on, once per cell URL per
...
For deterministic local diagnostics, set `KOLUX_RELAY_REGION_OVERRIDE` to `us-central1` or
`asia-east2` before launching Kolux. The override is not an end-user setting and is not written to
