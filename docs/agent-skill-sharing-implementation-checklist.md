...
- Kolux implementation: `skills-share` through `607e31dd04`; no PR.
- Kolux Cloud: bundle smoke PR `#329` merged as `eddb144afe`; generation-aware recovery PR `#330`
...
- Final bearer-link changes pass 143 Kolux Cloud API tests with one opt-in skip; 58 focused Kolux
...
- The opt-in desktop staging harness now publishes a bearer share without retired audience fields;
  Node typecheck and Playwright test discovery pass without starting Kolux or a browser login flow.
- Local Node and web typechecks, changed-code quality gates, 94 skill-domain files with 770 tests
  passed and 3 skipped, 134 Kolux Cloud API tests with one opt-in integration skip, the full Cloud
  monorepo test/typecheck/lint/build gates, and isolated Terraform formatting and validation.
- Final Kolux validation passed repository-wide lint, all Node/CLI/web typechecks, the production
  ...
  after removing Kolux-injected Git config variables and rerunning the slow macOS PTY case.
...
- Standalone and bundled package names reject Windows reserved device names in both Kolux and Cloud,
...
- The final combined physical `windows 2` run at `44d1266641` enabled native-Windows, real-process,
  ...
  aliases with `ln -sT`. The checkout remained clean and no `kolux-skill-*` directory remained in
  WSL `/tmp`.
...
- The exact x64 Electron directory package at `f1dccb4f42` passed its after-pack ABI scan for all
  ...
  `kolux-ide` executable and `resources/node_modules` layout. A native ARM64 package built from
...
- Real paired-runtime installation passed from an isolated Electron client to both a headed
  desktop host and headless `kolux serve` host. The host owned global and Git-worktree installs;
  the headed host also owned a plain-folder install. The client's home remained untouched.
- A fresh post-resilience E2E run passed all three headed paired-runtime, headless `kolux serve`,
...
- Isolated staging bucket, IAM, `kolux_skills` database and principal, enabled secret version, 11
  log metrics, four alerts, and one dashboard in `kolux-cloud-staging`. The complete targeted
...
- The route-disabled baseline, API revision `kolux-cloud-api-staging-00039-rek`, first proved the
...
- Auth deploy run `31535179937` promoted revision `kolux-cloud-auth-staging-00017-dug` at 100%
...
- The historical ACL-era API deploy run `31535438327` promoted revision
  `kolux-cloud-api-staging-00042-hef` at 100%
...
- Cloud PR `#317` passed both required checks and merged as `be00db10`. Its historical ACL-era
  ...
  promoted revision `kolux-cloud-api-staging-00048-xom` at 100% traffic with
...
- Guarded wake run `31564370807` prepared the reviewed staging topology. Deploy run `31564803943`
  promoted `kolux-cloud-api-staging-00051-yoq` at 100% traffic with immutable image digest
...
- Cloud PR `#330` passed required run `31578720429`, merged as `8045c85dad`, and passed merged-main
  ...
  deploy `31579844413` promoted `kolux-cloud-api-staging-00057-kat` at 100% traffic with immutable
...
- Auth deploy `31584318896` promoted `kolux-cloud-auth-staging-00021-tuq` with digest
...
- Guarded wake `31589384191` restored SQL and the two configured Relay cells without waking C3.
  Auth deploy `31589963244` promoted `kolux-cloud-auth-staging-00025-zuz` at 100% traffic with
...
- Guarded wake `31603249983` restored only the two configured staging cells. A disposable
  ...
  package deletion. The remote install, Kolux SSH target, encrypted credential artifact, one-time
...
- Guarded wake `31605729090` restored the same configured topology. An isolated headless Kolux host
...
- Cloud PR `#355` removed internal publisher and organization identifiers from anonymous skill
  ...
  `31640677572` promoted `kolux-cloud-api-staging-00060-qay` at 100% traffic with immutable digest
...
- [x] A user can select one or many private local skills, publish one immutable Skill Bundle, and
      receive one durable Kolux share URL.
- [x] Anyone with an active unlisted link can inspect a bundle without signing in, choose all or a
      ...
      workspace.
- [x] Installation works on local macOS, Linux, native Windows, WSL, paired Kolux runtimes, and
      supported SSH targets.
- [x] The portable archive root conforms to Agent Plugins 1.0.0 for skills-only packages and keeps
      Kolux integrity metadata in `dev.kolux.skill-sharing/manifest.json`.
...
- [x] Decide Kolux will not depend on the upstream CLI or an unsupported programmatic API.
- [x] Decide V1 accepts Kolux package sources only; Git, npm, and community registries remain with
      existing tools.
...
- [x] Treat the archive as a Skill Bundle, not a Kolux plugin product or executable plugin runtime.
...
- [x] Confirm private means unlisted and protected by an unpredictable revocable bearer link, not
      end-to-end encryption from Kolux Cloud operators.
...
- [x] Replace the unpublished single-skill envelope with root `plugin.json`, `skills/<name>/`, and
      `dev.kolux.skill-sharing/manifest.json`; do not add an outer `bundle/` directory.
...
- [x] Create fresh staging roots and reject unknown, malformed, or conflicting imported
      `dev.kolux.skill-sharing` namespaces without overwriting them. The extractor now exclusively
...
- [x] Generate deterministic `plugin.json`, `skills/`, and Kolux extension metadata.
...
- [x] Validate the portable root, Kolux extension namespace, bundle manifest, and every selected
...
- [x] Allow only configured Kolux skill-bucket origins and reject credential-bearing cross-host
      redirects.
...
- [x] Permit clean updates only when installed bytes still match Kolux provenance.
- [x] Return a modified conflict when installed bytes drift from their receipt.
- [x] Return an unowned conflict when a destination exists without Kolux provenance.
...
- [x] Add data-only records for Kolux agent ID, canonical-root support, global/project resolvers,
...
- [x] Repair broken Kolux-owned aliases.
...
- [x] Remove the canonical copy only when Kolux owns it, it is unmodified, and no retained receipt
      depends on it.
...
- [x] Inspect the active `kolux-cloud` project read-only and record the existing Cloud Run, GCS,
      Cloud SQL, IAM, and enabled-service baseline.
- [x] Locate the authoritative Kolux Cloud Terraform worktree, state, modules, and deployment
      pipeline.
...
- [x] Confirm production names and quotas, including availability of
      `kolux-cloud-skill-packages`. A read-only 2026-08-12 check returned `404` for the global
      bucket name, found no production `kolux_skills` database or skill-database secret collision,
...
- [x] Declare private bucket `kolux-cloud-skill-packages`, subject to Terraform validation.
...
- [x] Add exact approved production and development Kolux origins to CORS; never use `*`.
...
- [x] Declare database `kolux_skills` on existing regional PostgreSQL 17 instance
      `kolux-cloud-auth-db`.
- [x] Declare dedicated principal `kolux_skills_app` with access only to `kolux_skills`.
- [x] Store its connection URL in Secret Manager as `kolux-cloud-skills-database-url`.
- [x] Attach the existing Cloud SQL instance to `kolux-cloud-api` without replacing the service.
- [x] Inject only the skill database secret into the API service.
- [x] Verify backups and point-in-time recovery cover the new database. `kolux_skills` shares the
...
- [x] Grant `kolux-cloud-api@kolux-cloud.iam.gserviceaccount.com` bucket-scoped
...
- [x] Extend `kolux-cloud-api` in `us-central1`; do not create a separate V1 worker service.
...
- [x] Make immutable API candidates atomically replace the complete Terraform-owned literal
      environment and the exact `KOLUX_SKILLS_DATABASE_URL` Secret Manager reference.
...
- [x] Test tenant-scoped deduplication without cross-tenant timing or response disclosure. Kolux
...
- [x] Return a copyable durable Kolux URL after publication.
...
- [x] Show whether a package came from Kolux Cloud and its accessible version history.
...
- [x] Prefer destination-runtime direct download when it can reach approved Kolux Cloud storage.
...
- [x] Run the same contract against a paired headless `kolux serve` host for global and
...
The connected Kolux environment `windows 2` was reachable on 2026-08-11 and ra Kolux `1.4.180` with
runtime ID `68b5e70d-baaf-40a5-b384-be09cc088880`. Validation used the isolated checkout
`C:\Users\neil\kolux\skills-share-validation` and WSL distro `Ubuntu-24.04`; no production skill
...
`C:\Users\neil\kolux\workspaces\kolux\skills-share-staging-validation` on branch
`KoluxWin/skills-share-staging-validation`. At `44d1266641`, native-Windows, real-process, and real
...
distro filesystem. No standalone Kolux CLI is installed inside that distro; WSL skill operations
are owned by the connected Windows Kolux runtime. A reproducible second-distro setup is
`wsl --install -d Debian --no-launch`, followed by first launch and an isolated Kolux test user/home
...
matching Kolux's supported Windows placement topology.
...
- [x] Confirm `windows 2` is saved and reachable through `kolux environment list` and
      `kolux status --environment "windows 2" --json`.
...
- [x] Discover installed WSL version and distros, each distro's running state, default user, home,
      filesystem, and Kolux host support.
...
- [x] Record exact Kolux client/server versions and capabilities with each test run.
...
- [x] Test an unowned junction, external link, broken junction, and provider parent that is already
      linked. All four real NTFS junction cases passed on `windows 2` at `c7c8a2e32d`; Kolux repairs
...
- [x] Temporary local test installations, workspaces, and packages are removed through verified,
      recoverable cleanup. The final checkout was clean and WSL `/tmp` had no `kolux-skill-*` debris.

## 11. Implement and validate SSH targets

The 2026-08-11 live Kolux inventory contained only the connected `windows 2` environment and no
non-local worktree or repository. Later staging journeys registered disposable Ubuntu 20.04 and
native macOS ARM64 hosts directly through Kolux's SSH provider and removed them afterward. A
...
- [x] Add signed-policy/URL generation and IAM Credentials failure alerts. Grant-signing failures
      ...
      Data Access audit logging; evidence is in Kolux Cloud `69b388e`.
- [ ] Add budget alerts for GCS storage/egress, Cloud Run growth, and Cloud SQL storage.

Dashboard evidence in Kolux Cloud `41ef335` and `f67ee2c`: upload-grant, finalize, share, and
...
- [x] Test malicious redirects, DNS/host confusion, expired grants, mismatched generations, and
      ...
      GCS generation pinning are covered in Kolux `534abb8908` and Kolux Cloud `81a581d`.
...
- [ ] Test canonical/provider paths as regular files, directories, aliases, junctions, external
      ...
      Kolux-owned broken-link repair, aliases, and verified copy fallback. Physical Windows
...
- [x] Run Linux against Ubuntu 20.04/glibc 2.31 and verify bundled native binaries respect the
      ...
      `kolux-node-pty-floor-ok` from an ARM64 Ubuntu 20.04 container on native `aarch64` hardware.
...
- [x] Modify installed bytes, publish an update, and prove Kolux refuses silent replacement. The
...
- [x] Add deterministic fixtures authored for Kolux without copying upstream fixtures.
...
- [x] Deploy `kolux-cloud-api` skill routes disabled by server-side flags.
...
- [x] Deploy the API with all skill flags disabled. Bootstrap revision `kolux-cloud-api-00016-yem`
...
- [x] After every first-release gate passes, enable the feature for all accounts in one launch.
      ...
      artifact plus skill smoke and promoted `kolux-cloud-api-00025-qup` at 100% traffic with digest
...
- [ ] Review error budgets, cost, authorization denials, saturation, orphan counts, and support
      ...
      alerts route to the enabled dedicated `Kolux skill sharing alerts` channel, and aggregate
      monitor execution `kolux-cloud-skill-storage-monitor-lfd26` passed on the serving API digest
...
## 16. Agent and Kolux CLI publishing
...
- [x] Add `kolux skills installed --json` with discovery IDs and names but no local filesystem paths.
- [x] Add `kolux skills share` with repeated explicit `--skill` selectors, a required bundle name,
