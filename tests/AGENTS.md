...
whenever `KOLUX_E2E_HEADLESS=1`, `KOLUX_E2E_HEADFUL=1`, or `KOLUX_BACKGROUND_LAUNCH=1`:
...
- Launch through `tests/e2e/helpers/kolux-app.ts` (or `kolux-restart.ts`) — they already set the env.
- A raw `electron.launch()` outside those helpers must pass `KOLUX_BACKGROUND_LAUNCH: '1'`.
...
- Native-focus tests belong on an isolated display or CI. Do not set `KOLUX_E2E_FOREGROUND=1`
