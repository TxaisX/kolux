...
test.describe('Settings search responsiveness', () => {
  test('renders only the active settings pane when many projects match search', async ({
    ...
    await expect
      .poll(() => koluxPage.evaluate(() => window.__store?.getState().settingsSearchQuery ?? ''), {
        timeout: 5_000,
        message: 'settings search query did not apply'
      })
      .toBe('Project Long Search')

    await expect(koluxPage.getByRole('button', { name: 'Project Long Search 000' })).toBeVisible()
    await expect
      .poll(() => koluxPage.locator('section.scroll-mt-8[data-settings-section]').count(), {
        timeout: 5_000,
        message: 'settings search rendered more than the active pane'
      })
      .toBe(1)

    const renderedSectionId = await koluxPage
