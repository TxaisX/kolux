...
test('recreates a fresh profile index on disk with a restrictive umask @posix-profile-index-golden', async ({
  ...
  const indexPath = path.join(userDataDir, 'kolux-profile-index.json')
  ...
  try {
    ...
    // Why: koluxProfiles:list reads the index from disk on every call and rebuilds
    // it when missing, so this drives the real create + fsync + rename path.
    const listed = await koluxPage.evaluate(async () => {
      const result = await window.api.koluxProfiles.list()
