/**
 * Regression proof for #8291: a real alt-screen TUI survives a Kolux quit/relaunch, and after the
...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and Kolux canonicalizes repo.path.
  const repoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'kolux-mouse-reattach-repo-')))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Kolux mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches Kolux against one userDataDir, and the
...
test.describe('terminal reattach mouse mode', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why a screenshot and not the video fixture: this spec quits and relaunches Kolux,
      ...
      const proofShot = process.env.KOLUX_E2E_PROOF_SCREENSHOT
      if (proofShot) {
