...
test.describe('Terminal Panes', () => {
  ...
  test('closing a split pane prunes its leaf-keyed PTY binding without remapping siblings', async ({
    ...
    const beforeClose = await waitForPaneIdentitySnapshot(koluxPage, 3)
    ...
    const afterClose = await waitForPaneIdentitySnapshot(koluxPage, 2)
  ...
  test('closing and remaking right/down splits keeps surviving leaf-keyed bindings stable', async ({
    ...
    const afterRemake = await waitForPaneIdentitySnapshot(koluxPage, 3)
  ...
  test('moving panes through the drag-drop handler preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeMove = await waitForPaneIdentitySnapshot(koluxPage, 3)
    ...
    await moveTerminalPaneByLeafId(koluxPage, source.leafId, target.leafId, 'left')
    ...
    const afterMove = await waitForPaneIdentitySnapshot(koluxPage, 3)
  ...
  test('@headful dragging terminal panes around preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeDrag = await waitForPaneIdentitySnapshot(koluxPage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(koluxPage)
    ...
    const sourceHandle = koluxPage.locator(
    ...
    await koluxPage.mouse.move(targetBox!.x + 8, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    ...
    const afterDrag = await waitForPaneIdentitySnapshot(koluxPage, 3)
