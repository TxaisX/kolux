...
} from './artificial-tui-active-terminal-scroll'
import {
  formatScrollAttempts,
  getResponsiveScrollPath,
  type ScrollAttemptMeasurement
} from './artificial-tui-scroll-measurement'
...
export async function seedActiveTerminalScrollback(
  ...
  const marker = `TUI_SCROLL_READY_${runId}`
  const script = [
    `for (let i = 0; i < 420; i++) console.log('TUI_SCROLL_${runId}_' + i)`,
    `console.log('${marker}')`
  ].join(';')
