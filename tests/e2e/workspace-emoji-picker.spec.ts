...
test.describe('Workspace emoji picker', () => {
  ...
  test('inserts emoji in sidebar rename, worktree details, and Cmd+J', async ({
    ...
    const title = koluxPage.locator('[data-worktree-title-inline-rename=""]').first()
    await expect(title).toBeVisible()
    await title.dblclick()

    const inlineInput = koluxPage.locator('[data-worktree-title-rename-input="true"]')
    await expect(inlineInput).toBeVisible()
    await inlineInput.fill('Sidebar proof')
    await captureProof(koluxPage, testInfo, 'sidebar-rename-before.png')
    await inlineInput.pressSequentially(' :wink', { delay: 60 })
    const inlineSuggestions = koluxPage.locator('[data-workspace-emoji-suggestions="true"]')
    await expect(inlineSuggestions.getByRole('option', { name: ':wink:' })).toBeVisible()
    await captureProof(koluxPage, testInfo, 'sidebar-rename-picker.png')
    await inlineInput.press('Enter')
    await expect(inlineInput).toHaveValue('Sidebar proof 😉 ')
    await inlineInput.press('Enter')
    await expect(koluxPage.getByText('Sidebar proof 😉', { exact: true }).first()).toBeVisible()
    ...
    const detailsDialog = koluxPage.getByRole('dialog', { name: 'Edit Worktree Details' })
    ...
    await captureProof(koluxPage, testInfo, 'worktree-details-before.png')
    ...
    await captureProof(koluxPage, testInfo, 'worktree-details-picker.png')
    await displayNameInput.press('Enter')
    await expect(displayNameInput).toHaveValue('Details proof 😉 ')
    await detailsDialog.getByRole('button', { name: 'Cancel' }).click()

    await koluxPage.evaluate(() => window.__store!.getState().openModal('worktree-palette'))
    ...
    await captureProof(koluxPage, testInfo, 'cmd-j-before.png')
    ...
    await captureProof(koluxPage, testInfo, 'cmd-j-picker.png')
