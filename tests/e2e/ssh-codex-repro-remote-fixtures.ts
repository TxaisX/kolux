...
const REMOTE_TUI_PATH = '/tmp/kolux-codex-display-artifacts-repro.mjs'
export const REMOTE_TUI_DONE = 'KOLUX_REMOTE_CODEX_ARTIFACT_TUI_DONE'
...
const REMOTE_CODEX_FIXTURE_FRAMES = parseEnvNumber(process.env.KOLUX_E2E_CODEX_FIXTURE_FRAMES, 34)
const REMOTE_CODEX_FIXTURE_FRAME_DELAY_MS = parseEnvNumber(
  process.env.KOLUX_E2E_CODEX_FIXTURE_FRAME_DELAY_MS,
  45
)
...
await write('If Kolux leaves wide gray rectangles here, they are stale remote-PTY render artifacts.\\r\\n')
...
const marker = process.argv.join(' ').match(/KOLUX_REMOTE_CODEX_ARTIFACT_TUI_DONE|KOLUX_REAL_REMOTE_CODEX_DONE_[0-9]+/)?.[0] || '${REMOTE_TUI_DONE}'
const scrollTop = 1
...
await write('Tip: deterministic fixture for Kolux SSH Codex display artifacts.\\r\\n\\r\\n')
