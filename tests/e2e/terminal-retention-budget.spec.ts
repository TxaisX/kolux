...
test.describe('terminal hidden-worktree retention budget', () => {
  ...
  test('force-parks the older hidden un-parkable worktree and spares the newest', async ({
    ...
    try {
      ...
      const older = await connectDockerSshRelayTarget(koluxPage, target)
      await expect
      ...
      const olderPtyId = await waitForActivePanePtyId(koluxPage, 60_000)
      const olderTabId = await getActiveTabId(koluxPage)
      if (!olderTabId) {
      ...
      await sendToTerminal(koluxPage, olderPtyId, `echo "${olderMarker}:"\r`)
      await expect
        .poll(() => getTerminalContent(koluxPage, 20_000), {
          timeout: 30_000,
      ...
      const newerTabId = await getActiveTabId(koluxPage)
      if (!newerTabId) {
      ...
      await waitForTabParked(koluxPage, olderTabId, { parkDelayMs: PARKING_DELAY_MS })
      // …while the newest hidden worktree keeps its mounted panes (last-active exemption).
      const newerStillMounted = await koluxPage.evaluate(
