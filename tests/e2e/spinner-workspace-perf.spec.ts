...
const enabled = process.env.KOLUX_SPINNER_BENCH === '1'
const sampleMs = Number(process.env.KOLUX_SPINNER_SAMPLE_MS ?? 10000)
const rounds = Number(process.env.KOLUX_SPINNER_ROUNDS ?? 4)
const keyCount = Number(process.env.KOLUX_SPINNER_KEYS ?? 48)
// Avoid phase-locking keystrokes to the 200 ms status burst or 60 Hz frames.
const keyCadenceMs = Number(process.env.KOLUX_SPINNER_KEY_CADENCE_MS ?? 113)
const variants = (process.env.KOLUX_SPINNER_VARIANTS ?? 'original,long').split(',')
if (enabled) {
...
test.use({
  seedTestRepo: false,
  koluxAppExtraEnv: { KOLUX_BACKGROUND_LAUNCH: '1' },
  koluxAppExtraArgs: [
...
for (const scenario of scenarios) {
  test(`spinner performance ${scenario.name}`, async ({
    ...
    const output = path.resolve(process.env.KOLUX_SPINNER_OUTPUT ?? '.bench-fixtures/spinner-app')
    ...
    for (let round = 0; round < (process.env.KOLUX_SPINNER_CPU === '0' ? 0 : rounds); round++) {
