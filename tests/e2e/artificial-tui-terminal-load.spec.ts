...
import { runHiddenRealPtyPressureScenario } from './artificial-tui-hidden-pressure-scenario'
import type { HiddenPressureOutputMode } from './artificial-tui-hidden-pressure-script'
import { runMainPressureScenario } from './artificial-tui-main-pressure-scenario'
import { runRendererBackpressureRevisitScenario } from './artificial-tui-revisit-pressure-scenario'
import { startSyntheticTuiInjection } from './artificial-tui-synthetic-injection'
...
type SyntheticTuiWindow = Window & {
...
// 100-pane TUI-scale runs are below 50ms worst-key latency; keep enough
...
const SAME_WORKSPACE_PANES = readPositiveInt(
  'KOLUX_E2E_TUI_SAME_WORKSPACE_PANES',
  DEFAULT_SAME_WORKSPACE_PANES
)
const CROSS_WORKSPACE_PANES_PER_WORKTREE = readPositiveInt(
  'KOLUX_E2E_TUI_CROSS_WORKSPACE_PANES',
  DEFAULT_CROSS_WORKSPACE_PANES_PER_WORKTREE
)
const PRESSURE_BACKGROUND_PANES = readPositiveInt(
  'KOLUX_E2E_TUI_PRESSURE_BACKGROUND_PANES',
  DEFAULT_PRESSURE_BACKGROUND_PANES
)
const PRESSURE_OUTPUT_CHARS = readPositiveInt(
  'KOLUX_E2E_TUI_PRESSURE_OUTPUT_CHARS',
  DEFAULT_PRESSURE_OUTPUT_CHARS
)
const HIDDEN_PRESSURE_PANES = readPositiveInt(
  'KOLUX_E2E_TUI_HIDDEN_PRESSURE_PANES',
  DEFAULT_HIDDEN_PRESSURE_PANES
)
const FRAME_COUNT = readPositiveInt('KOLUX_E2E_TUI_FRAME_COUNT', DEFAULT_FRAME_COUNT)
const FRAME_INTERVAL_MS = readPositiveInt(
  'KOLUX_E2E_TUI_FRAME_INTERVAL_MS',
  DEFAULT_FRAME_INTERVAL_MS
)
const SCALE_SAME_WORKSPACE_PANES = readPositiveIntList('KOLUX_E2E_TUI_SCALE_PANES')
const SCALE_CROSS_WORKSPACE_PANES = readPositiveIntList('KOLUX_E2E_TUI_SCALE_CROSS_WORKSPACE_PANES')
const SCALE_PRESSURE_PANES = readPositiveIntList('KOLUX_E2E_TUI_SCALE_PRESSURE_PANES')
const SCALE_HIDDEN_PRESSURE_PANES = readPositiveIntList('KOLUX_E2E_TUI_SCALE_HIDDEN_PRESSURE_PANES')
...
process.stdout.write('\\x1b]0;TUI load typing benchmark\\x07')
process.stdout.write('TUI_TYPING_READY_${runId}\\n')
process.stdin.on('data', (chunk) => {
  ...
  for (const char of chunk) {
    if (char === '\\r' || char === '\\n') continue
    seq += 1
    process.stdout.write('\\r\\x1b[2KTui load prompt ' + seq + ': ' + char + ' TUI_TYPING_KEY_${runId}_' + seq + '\\n')
  }
})
`
}
...
async function measureTypingDuringLoad(
  ...
  await waitForTerminalOutputForPtyId(page, ptyId, `TUI_TYPING_READY_${runId}`, 10_000)
  ...
  for (const [index, char] of [...KEY_LATENCY_SAMPLES].entries()) {
    const marker = `TUI_TYPING_KEY_${runId}_${index + 1}`
...
async function resetTerminalPtyOutputDebug(page: Page): Promise<void> {
  await page.evaluate(async () => {
    ;(window as SyntheticTuiWindow).__terminalPtyOutputDebug?.reset()
    ;(window as SyntheticTuiWindow).__terminalOutputSchedulerDebug?.reset()
...
async function readTerminalPtyOutputDebug(
  ...
  return page.evaluate(() => {
    return (window as SyntheticTuiWindow).__terminalPtyOutputDebug?.snapshot() ?? null
  })
}
...
async function readTerminalOutputSchedulerDebug(
  ...
  return page.evaluate(() => {
    return (window as SyntheticTuiWindow).__terminalOutputSchedulerDebug?.snapshot() ?? null
  })
}
...
async function holdTerminalAckGate(page: Page, ptyIds: string[]): Promise<void> {
  await page.evaluate((ids) => {
    const gate = (window as SyntheticTuiWindow).__terminalPtyAckGate
    if (!gate) {
...
async function releaseTerminalAckGate(page: Page): Promise<void> {
  await page.evaluate(() => {
    ;(window as SyntheticTuiWindow).__terminalPtyAckGate?.release()
  })
}

async function readTerminalAckGateDebug(page: Page): Promise<TerminalPtyAckGateSnapshot | null> {
  return page.evaluate(() => {
    return (window as SyntheticTuiWindow).__terminalPtyAckGate?.snapshot() ?? null
  })
}
...
async function measureCrossWorkspaceTypingDuringHiddenLoad({
  ...
  test.skip(!secondWorktreeId, 'TUI cross-workspace load needs the seeded secondary worktree')
  if (!secondWorktreeId) {
    return
  }
  ...
  const scriptPath = path.join(testRepoPath, `.kolux-tui-cross-${hiddenPaneCount}-${runId}.mjs`)
  writeInteractivePromptScript(scriptPath, runId)
  await resetTerminalPtyOutputDebug(koluxPage)
  const load = await startSyntheticTuiInjection({
    frameCount: FRAME_COUNT,
...
test.describe('Artificial TUI terminal load', () => {
  ...
  test('measures baseline typing responsiveness with one active terminal', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kolux-tui-baseline-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(koluxPage)
    try {
      const measurement = await measureTypingDuringLoad(koluxPage, scriptPath, typingPtyId, runId)
  ...
  test('keeps typing responsive while same-workspace panes redraw simultaneously', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kolux-tui-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(koluxPage)
    const load = await startSyntheticTuiInjection({
      frameCount: FRAME_COUNT,
  ...
  for (const paneCount of SCALE_PRESSURE_PANES) {
    test(`keeps active interactions responsive at ${paneCount} ACK-backpressured TUI PTYs`, async ({
      koluxPage,
      testRepoPath
    }, testInfo) => {
      await runConfiguredMainPressureScenario({
  ...
  for (const paneCount of SCALE_SAME_WORKSPACE_PANES) {
    test(`keeps typing responsive at ${paneCount} same-workspace TUI panes`, async ({
      koluxPage,
      testRepoPath
    }, testInfo) => {
      await waitForSessionReady(koluxPage)
      ...
      const runId = randomUUID()
      const scriptPath = path.join(testRepoPath, `.kolux-tui-scale-${paneCount}-${runId}.mjs`)
      writeInteractivePromptScript(scriptPath, runId)
      await resetTerminalPtyOutputDebug(koluxPage)
      const load = await startSyntheticTuiInjection({
        frameCount: FRAME_COUNT,
      ...
      try {
        ...
        annotateTypingMeasurement(
          testInfo,
          `tui-scale-same-workspace-${paneCount}`,
          panes.length,
          measurement,
          await readTerminalPtyOutputDebug(koluxPage),
          await readTerminalOutputSchedulerDebug(koluxPage),
  ...
  test('keeps typing responsive while another workspace streams TUI-style output', async ({
    koluxPage,
    testRepoPath
  }, testInfo) => {
    await measureCrossWorkspaceTypingDuringHiddenLoad({
      ...
      annotationType: 'tui-cross-workspace-typing',
      testInfo
    })
  })
  ...
  for (const paneCount of SCALE_CROSS_WORKSPACE_PANES) {
    test(`keeps typing responsive with ${paneCount} hidden cross-workspace TUI panes`, async ({
      koluxPage,
      testRepoPath
    }, testInfo) => {
      await measureCrossWorkspaceTypingDuringHiddenLoad({
        ...
        annotationType: `tui-scale-cross-workspace-${paneCount}`,
        testInfo
      })
    })
  }
