...
test.describe('TUI emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kolux-tui-emoji-table-${runId}.mjs`)
    ...
    try {
      ...
      testInfo.annotations.push({
        type: 'tui-emoji-table-rendering',
  ...
  test('local real TUI demo keeps table rendering and cursor visible', async ({
    koluxPage
  }, testInfo) => {
    test.skip(
      process.env.KOLUX_E2E_REAL_TUI !== '1',
      'Set KOLUX_E2E_REAL_TUI=1 to exercise the locally installed TUI'
    )
    ...
    await sendToTerminal(
      koluxPage,
      ptyId,
      'tui run --demo --interactive "Give me markdown table dummy data a long table with emojis in it"\r'
    )
    try {
      ...
      await testInfo.attach('real-tui-demo-table', {
