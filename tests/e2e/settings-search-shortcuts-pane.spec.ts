...
test.describe('Settings sidebar search on the Shortcuts pane', () => {
  test('pane-title-only query keeps rows visible and local search usable', async ({
    ...
    const searchInput = koluxPage.getByPlaceholder('Search settings')
    ...
    await expect(koluxPage.getByRole('heading', { name: 'Shortcuts', exact: true })).toBeVisible()

    // Regression: a pane-title-only query used to blank the whole list (0/112).
    await expect(koluxPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(koluxPage.getByText('No shortcuts match those filters.')).not.toBeVisible()
    ...
    const localSearch = koluxPage.getByPlaceholder('Search command or keys')
    await localSearch.fill('go to')
    await expect(koluxPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(koluxPage.getByText('Force Reload', { exact: true })).not.toBeVisible()

    // A row-matching global query still narrows the list as before.
    await localSearch.clear()
    await searchInput.fill('worktree')
    await expect(koluxPage.getByText('Create worktree', { exact: true })).toBeVisible()
    await expect(koluxPage.getByText('Go to File', { exact: true })).not.toBeVisible()
  })
})
