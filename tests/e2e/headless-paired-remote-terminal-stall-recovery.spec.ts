...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kolux-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless Kolux host @headful', async ({
  testRepoPath
}) => {
