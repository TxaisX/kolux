...
test.describe('reattach mouse-mode leak', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      await execInTerminal(firstLaunch.page, ptyId, 'echo KOLUX_MOUSE_READY_$((21+21))')
      const shellExecutes = await waitForTerminalOutput(
        firstLaunch.page,
        'KOLUX_MOUSE_READY_42',
        15_000
      )
        .then(() => true)
        .catch(() => false)
      ...
      await execInTerminal(secondLaunch.page, reattachedPtyId, 'echo KOLUX_REATTACHED_$((21+21))')
      await waitForTerminalOutput(secondLaunch.page, 'KOLUX_REATTACHED_42', 15_000)
