...
async function openRemoteFileExplorer(page: Page, target: DockerSshRelayTarget): Promise<void> {
  ...
  await expect(page.locator('[data-kolux-explorer-shell]')).toBeVisible({ timeout: 15_000 })
...
test.describe('Docker SSH relay watcher isolation', () => {
  ...
  test('keeps the relay, terminal, and explorer alive when only relay-watcher.js crashes', async ({
    ...
    try {
      ...
      await openRemoteFileExplorer(koluxPage, target)

      const beforeFile = 'before-crash.txt'
      writeDockerSshRelayTargetFile(target, remoteRepoFile(beforeFile), 'before watcher crash\n')
      await expect(fileExplorerRow(koluxPage, beforeFile)).toBeVisible({ timeout: 30_000 })
      ...
      await expect(fileExplorerRow(koluxPage, afterFile)).toBeVisible({ timeout: 30_000 })
    } finally {
  ...
  test('repairs a missing deployed relay-watcher.js on reconnect', async ({
    ...
    try {
      ...
      await closeRemoteFileExplorer(koluxPage)
      ...
      await expect(fileExplorerRow(koluxPage, repairedFile)).toBeVisible({ timeout: 30_000 })
    } finally {
      cleanupDockerSshRelayTarget(target)
    }
  })
