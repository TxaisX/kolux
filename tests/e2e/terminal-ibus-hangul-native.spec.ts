...
function nativeRepetitions(): number {
  const parsed = Number(process.env.KOLUX_E2E_NATIVE_IBUS_REPETITIONS ?? DEFAULT_REPETITIONS)
...
function nativeKeyDelayMs(): number {
  const parsed = Number(process.env.KOLUX_E2E_NATIVE_IBUS_KEY_DELAY_MS ?? DEFAULT_KEY_DELAY_MS)
  return Number.isInteger(parsed) && parsed >= 0
...
test.describe('Native IBus Hangul terminal input @headful', () => {
  test.skip(
    process.env.KOLUX_E2E_NATIVE_IBUS_HANGUL !== '1',
  ...
  test('forwards the issue exact-byte sequence without loss or duplication', async ({
    ...
    await runNativeIbusScenario(koluxPage, testInfo, testRepoPath, '한abc글', typeExactByteSequence)
  })
