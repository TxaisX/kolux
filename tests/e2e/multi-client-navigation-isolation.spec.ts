...
const isPairedBrowserRun = process.env.KOLUX_E2E_WEB_CLIENT === '1'
...
test('keeps two paired browser clients and the host on independent worktrees', async ({
  ...
  await expect
    .poll(() => loadTestWorktreeIds(koluxPage, branchA, branchB), {
      timeout: 30_000,
  ...
  const ids = await loadTestWorktreeIds(koluxPage, branchA, branchB)
  if (!ids) {
    throw new Error('Test worktrees disappeared after discovery')
  }

  await selectWorktree(koluxPage, ids.host)

  let clientA: Page | null = null
  let clientB: Page | null = null
  try {
    const offerA = await createPairingOffer(koluxPage)
    ...
    const offerB = await createPairingOffer(koluxPage)
    ...
    await expectActiveWorktree(koluxPage, ids.host)
...
test('shows only provider-backed creation actions in paired web', async ({
  ...
  const visibleWorktreeId = await koluxPage.evaluate(
  ...
  const offer = await createPairingOffer(koluxPage)
...
test('routes Add Project folder browsing through the paired host', async ({
  ...
  const hostFolder = mkdtempSync(path.join(os.tmpdir(), 'kolux-paired-web-folder-'))
