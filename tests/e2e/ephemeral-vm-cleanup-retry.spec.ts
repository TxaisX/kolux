...
test('shows interrupted hidden SSH cleanup as retryable', async ({ electronApp, koluxPage }) => {
  const userDataPath = await electronApp.evaluate(({ app }) => app.getPath('userData'))
  writeFileSync(
    path.join(userDataPath, 'kolux-ephemeral-vm-runtimes.json'),
    JSON.stringify({
      version: 1,
      runtimes: [
        {
  ...
  const runtimes = koluxPage.locator('[data-settings-section="temporary-vm-runtimes"]')
...
test('stops long-running cleanup and keeps it retryable', async ({ electronApp, koluxPage }) => {
  const repoPath = mkdtempSync(path.join(tmpdir(), 'kolux-cleanup-stop-'))
  ...
  try {
    ...
    const repoId = await koluxPage.evaluate(async (repo) => {
    ...
    await openCloudVmRuntimes(koluxPage)
    const runtimes = koluxPage.locator('[data-settings-section="temporary-vm-runtimes"]')
    ...
    const dialog = koluxPage.getByRole('dialog', { name: 'Stop cleanup?' })
    ...
    await expect(runtimes.getByRole('button', { name: 'Retry cleanup' })).toBeVisible()
    await expect(koluxPage.getByText('Cleanup stopped by user.')).toBeVisible()
