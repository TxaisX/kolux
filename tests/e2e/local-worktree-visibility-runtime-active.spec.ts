/**
 * Regression: a worktree created via the CLI (`kolux worktree
...
test.describe('worktree visibility with a remote runtime active', () => {
  test('a CLI-created worktree appears in the sidebar while a remote runtime is active', async ({
    ...
    const repoId = await koluxPage.evaluate(() => {
    ...
    // dir — exactly what `kolux worktree create` does from a terminal.
    ...
    const worktreeRow = (worktreeId: string) =>
      koluxPage.locator(`[data-worktree-id=${JSON.stringify(worktreeId)}]`).first()
