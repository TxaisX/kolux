...
test.skip(
  process.env.KOLUX_E2E_LOCAL_SSH_BROWSER !== '1',
  'Run with KOLUX_E2E_LOCAL_SSH_BROWSER=1 (requires Docker)'
)
...
test('routes SSH-workspace browsing through the SSH host, fail-closed across a real drop', async ({
  ...
  try {
    ...
    await installBrowserPaneMountCensus(koluxPage)

    // (1) Remote-only origin renders -- the causal egress oracle.
    const loginTab = await createBrowserTab(koluxPage, worktreeId, LOGIN_URL, 'SSH login')
    ...
    const loginProbe = await probeTabWebview(koluxPage, loginTab.id)
    expect(
      loginProbe?.partition,
      'SSH-workspace pages must mount on a derived route partition'
    ).toMatch(ROUTE_PARTITION_RE)

    const routedCensus = await readBrowserPaneMountCensus(koluxPage)
    ...
    const echoTab = await createBrowserTab(koluxPage, worktreeId, ECHO_URL, 'SSH echo')
    ...
    expect(
      (await probeTabWebview(koluxPage, echoTab.id))?.partition,
      'both SSH-workspace tabs must share one route partition'
    ).toBe(loginProbe?.partition)

    const beforeDrop = await readSshState(koluxPage, remote.targetId)
    ...
    await expect
      .poll(
        async () => {
          dropOutcome = await reloadTab(koluxPage, echoTab.id, 30_000)
    ...
    expect(
      await readPageLoadError(koluxPage, echoTab.id),
      'the fenced page must surface a load failure in the product state'
    ).not.toBeNull()
    expect(
      (await probeTabWebview(koluxPage, echoTab.id))?.marker ?? null,
    ...
    const afterDrop = await reconnectSshTarget(koluxPage, remote.targetId)
    expect(
    ...
    await expect
      .poll(
        async () => {
          recoveryOutcome = await reloadTab(koluxPage, echoTab.id, 30_000)
    ...
    const optOutTab = await createBrowserTab(koluxPage, worktreeId, OPT_OUT_URL, 'SSH opt-out')
    await expect
      .poll(async () => (await probeTabWebview(koluxPage, optOutTab.id))?.partition ?? null, {
        timeout: 60_000,
        message: 'the opt-out tab never attached a guest'
      })
      .not.toBeNull()
    const optOutPartition = (await probeTabWebview(koluxPage, optOutTab.id))?.partition
    expect(
      optOutPartition,
      'with routing disabled a new tab must not mount on a route partition'
    ).not.toMatch(ROUTE_PARTITION_RE)

    const optOutCensus = censusFor(await readBrowserPaneMountCensus(koluxPage), optOutTab.id)
    expect(
      optOutCensus.some((entry) => entry.kind === 'gate-preparing'),
      'the opt-out pane must skip the SSH-routing gate entirely'
    ).toBe(false)
    await expect
      .poll(async () => (await reloadTab(koluxPage, optOutTab.id, 30_000)).outcome, {
        timeout: 120_000,
        intervals: [1_000, 2_000],
...
test('holds the mount and offers a working local escape hatch when the SSH host is unavailable', async ({
  ...
  try {
    ...
    await expect
      .poll(async () => (await readSshState(koluxPage, remote.targetId)).status, {
        timeout: 60_000,
    ...
    const strandedPane = koluxPage.locator(`[data-browser-overlay-tab-id="${strandedTab.id}"]`)
    await expect(
    ...
    const strandedCensus = await readBrowserPaneMountCensus(koluxPage)
    ...
    await expect
      .poll(async () => (await probeTabWebview(koluxPage, strandedTab.id))?.marker ?? null, {
        timeout: 120_000,
        intervals: [250, 500, 1_000],
        message: 'the escape hatch never produced a page that could load from this device'
      })
      .toBe(LOCAL_DEVICE_MARKER)
    const escapedPartition = (await probeTabWebview(koluxPage, strandedTab.id))?.partition
    expect(
      escapedPartition,
      'the explicit local-browsing choice must mount off the route partition'
    ).not.toMatch(ROUTE_PARTITION_RE)
    for (const entry of censusFor(await readBrowserPaneMountCensus(koluxPage), strandedTab.id)) {
      if (entry.kind === 'webview') {
        expect(
          entry.partition,
...
test('classifies a real AllowTcpForwarding no refusal and keeps Try anyway routed', async ({
  ...
  try {
    ...
    const remote = await connectDockerSshRelayTarget(koluxPage, target)
    expect(
      (await readSshState(koluxPage, remote.targetId)).status,
    ...
    const blockedPane = koluxPage.locator(`[data-browser-overlay-tab-id="${blockedTab.id}"]`)
    ...
    const blockedCensus = censusFor(await readBrowserPaneMountCensus(koluxPage), blockedTab.id)
    ...
    await expect
      .poll(async () => (await probeTabWebview(koluxPage, blockedTab.id))?.partition ?? null, {
        timeout: 120_000,
        intervals: [250, 500, 1_000],
        message: '"Try anyway" never mounted a guest'
      })
      .not.toBeNull()
    const overrideCensus = censusFor(await readBrowserPaneMountCensus(koluxPage), blockedTab.id)
    ...
    await expect
      .poll(() => readPageLoadError(koluxPage, blockedTab.id), {
        timeout: 180_000,
        intervals: [500, 1_000, 2_000],
    ...
    await testInfo.attach('try-anyway-load-error', {
      body: JSON.stringify(await readPageLoadError(koluxPage, blockedTab.id)),
      contentType: 'application/json'
    })
    expect(
      (await probeTabWebview(koluxPage, blockedTab.id))?.marker ?? null,
