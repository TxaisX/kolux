...
// scoped to this spec's app launches via koluxAppExtraEnv (same pattern as
...
test.use({
  ...
  koluxAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the KOLUX_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.kolux-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(koluxPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(koluxPage, worktreeId)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(koluxPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(koluxPage, lastActiveTab.tabId)).hasManager).toBe(true)
      const visibleState = await readTerminalTabViewState(koluxPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          koluxPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      expect(await countMountedPaneManagers(koluxPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(koluxPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(koluxPage, visibleTab.tabId)).hasManager).toBe(true)
...
test.describe('Terminal hidden worktree retention budget', () => {
  test.use({
    koluxAppExtraEnv: {
      KOLUX_E2E_TERMINAL_PARKING_DELAY_MS: String(PARKING_DELAY_MS),
      ...
      KOLUX_E2E_TERMINAL_RETENTION_LIMIT: '1'
    },
    koluxAppExtraArgs: ['--enable-precise-memory-info']
  })
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(koluxPage)
    const victimWorktreeId = await waitForActiveWorktree(koluxPage)
    await skipUnlessParkingWired(koluxPage)

    const decoyWorktreeId = (await getAllWorktreeIds(koluxPage)).find(
    ...
    await updateTerminalSettings(koluxPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(koluxPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.kolux-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(koluxPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(koluxPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(koluxPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(koluxPage, decoyWorktreeId)
      await expect
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(koluxPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(koluxPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(koluxPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(koluxPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(koluxPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(koluxPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(koluxPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(koluxPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(koluxPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(koluxPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(koluxPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(koluxPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(koluxPage, true)
      await expect
        .poll(() => countMountedPaneManagers(koluxPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(koluxPage)
      testInfo.annotations.push({
      ...
      expect(await countMountedPaneManagers(koluxPage, decoyTabIds)).toBe(decoyTabIds.length)
