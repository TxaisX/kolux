...
test('shows Stop and a spinner during a held reload, then returns to Reload', async ({
  ...
  try {
    ...
    const tab = await koluxPage.evaluate(
      ({ worktreeId, url }) => {
    ...
    const pane = koluxPage.locator(`[data-browser-overlay-tab-id="${tab.id}"]`)
    const reload = pane.getByRole('button', { name: 'Reload' })
    await expect(reload).toBeVisible({ timeout: 20_000 })
    await reload.click()
    await secondRequest
    await expect
      .poll(async () => (await getBrowserTabs(koluxPage, worktreeId)).at(-1)?.url, {
        timeout: 20_000
      })
      .toBe(url)
    ...
    await koluxPage.screenshot({ path: testInfo.outputPath('reload-held.png') })
    reloadReleased = true
