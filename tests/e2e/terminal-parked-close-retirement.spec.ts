...
test('closing a parked terminal tab retires its exact PTY session', async ({ koluxPage }) => {
  ...
  const tab = koluxPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`)
  await expect(tab).toBeVisible()

  const marker = `PARKED_CLOSE_READY_${randomUUID()}`
  const keepAliveScript = `process.stdout.write(${JSON.stringify(`${marker}\n`)}); setInterval(() => {}, 1000)`
  await execInTerminal(koluxPage, ptyId, nodeTerminalCommand(['-e', keepAliveScript]))
  await expect
    .poll(() => getTerminalContent(koluxPage), {
      timeout: 10_000,
      message: 'long-lived terminal child did not print its ready marker'
    })
    .toContain(marker)
  await expect.poll(() => hasPtySession(koluxPage, ptyId), { timeout: 10_000 }).toBe(true)
  ...
  await expect
    .poll(() => hasPtySession(koluxPage, ptyId), {
      timeout: 15_000,
