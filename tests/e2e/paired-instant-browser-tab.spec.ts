...
async function readAddressBarState(
  ...
  return page.evaluate(() => {
    const bars = document.querySelectorAll('[data-kolux-browser-address-bar]')
...
test('keeps an address typed into the staged tab when the paired create is adopted', async ({
  ...
  try {
    ...
    await client.page.locator('[data-kolux-browser-address-bar]').first().waitFor()
    await expect
    ...
    const addressBar = client.page.locator('[data-kolux-browser-address-bar]').first()
...
test('adopts a paired browser tab without rebuilding its chrome', async ({
  ...
  try {
    ...
    const marked = await client.page.evaluate(() => {
      const input = document.querySelector('[data-kolux-browser-address-bar]')
      if (!input) {
        return false
      }
    ...
    expect(
      await client.page.evaluate(() => ({
        bars: document.querySelectorAll('[data-kolux-browser-address-bar]').length,
        marked: document.querySelectorAll('[data-e2e-staged-address-bar="marked"]').length,
        sameNode:
          document.querySelector('[data-kolux-browser-address-bar]') ===
