...
const SETUP_SCRIPT_COMMAND = 'echo kolux-e2e-setup'
...
/** Repo whose shared kolux.yaml carries a real setup script, plus a second worktree. */
function createRepoWithSharedSetupScript(repoPath: string, featureWorktreePath: string): void {
  ...
  writeFileSync(path.join(repoPath, 'README.md'), '# Unreadable kolux.yaml E2E\n')
...
/**
 * Makes the main process report the failure the fix now surfaces: kolux.yaml could
 ...
 */
async function installUnreadableKoluxYamlFault(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ ipcMain }) => {
    type InvokeHandler = (event: unknown, ...args: unknown[]) => unknown
    const faultState = globalThis as typeof globalThis & {
      __koluxE2eKoluxYamlUnreadable?: boolean
    }
    ...
    faultState.__koluxE2eKoluxYamlUnreadable = true
    ipcMain.removeHandler('hooks:check')
    ipcMain.handle('hooks:check', async (event, ...args) => {
      if (faultState.__koluxE2eKoluxYamlUnreadable) {
...
/** kolux.yaml becomes readable again — every later check runs the production handler. */
async function healKoluxYamlRead(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(() => {
    ;(
      globalThis as typeof globalThis & { __koluxE2eKoluxYamlUnreadable?: boolean }
    ).__koluxE2eKoluxYamlUnreadable = false
  })
}
...
test.describe('Setup script prompt', () => {
  ...
  test('recovers from an unreadable kolux.yaml instead of pinning the failed verdict', async ({
    electronApp,
    koluxPage
  }, testInfo) => {
    const repoPath = testInfo.outputPath('unreadable-kolux-yaml-repo')
    const featureWorktreePath = testInfo.outputPath('unreadable-kolux-yaml-feature')
    createRepoWithSharedSetupScript(repoPath, featureWorktreePath)

    await installUnreadableKoluxYamlFault(electronApp)
    ...
    const promptCard = koluxPage.locator('[data-setup-script-prompt-layer]')
    const inspectionError = promptCard.getByText(INSPECTION_ERROR_TEXT)
    await expect(inspectionError).toBeVisible({ timeout: 20_000 })

    // kolux.yaml is readable again; the card is still pinned to the failed verdict.
    await healKoluxYamlRead(electronApp)
    const healthyCheck = await koluxPage.evaluate(
    ...
    await koluxPage.waitForTimeout(RECORDING_DWELL_MS)

    // Activating another worktree in the same repo must re-inspect.
    const featureRow = worktreeRow(koluxPage, featureWorktreeId)
    await expect(featureRow).toBeVisible()
    await worktreeRowSurface(koluxPage, featureWorktreeId).click()
    await expect(featureRow).toHaveAttribute('aria-current', 'page')

    // The repo has a valid kolux.yaml scripts.setup, so no prompt may remain.
