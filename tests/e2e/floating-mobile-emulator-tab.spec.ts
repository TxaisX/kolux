...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(koluxPage)
  await openFloatingPanelIfNeeded(koluxPage)

  const openPanel = koluxPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(koluxPage, testInfo)
