...
test('Linear filter chips keep readable names after the dropdown closes', async ({
  ...
  await expect
    .poll(() => getStoreState<string>(koluxPage, 'activeView'), { timeout: 5_000 })
    .toBe('tasks')
  const filtersButton = koluxPage.getByRole('button', { name: 'Filters', exact: true })
  await expect(filtersButton).toBeVisible()
  await expect(koluxPage.getByText(FIXTURE.issue.title, { exact: true })).toBeVisible()

  await filtersButton.click()
  const popover = koluxPage.locator('[data-slot="popover-content"]')
