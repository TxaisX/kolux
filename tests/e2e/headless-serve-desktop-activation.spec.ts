...
import { test, expect, forwardElectronProcessLogs } from './helpers/kolux-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getKoluxElectronLaunchArgs } from './helpers/electron-launch-args'
...
function createHeadlessLaunchIsolation(userDataDir: string): ElectronHomeIsolation {
  ...
  return createElectronHomeIsolation({
    ...
    launchEnv: {
      ...
      KOLUX_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1'
    },
    extraEnv: {},
    userDataDir
  })
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'kolux-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getKoluxElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.KOLUX_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getKoluxElectronLaunchArgs(mainPath, false), {
      env,
