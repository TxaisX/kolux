...
type SyntheticTuiInjectionWindow = Window & {
  __terminalPtyDataInjection?: {
    inject: (paneKey: string, data: string) => boolean
  }
  __syntheticTuiLoadState?: SyntheticTuiLoadState
}

type SyntheticTuiLoadState = {
  errorMessage?: string
...
export async function startSyntheticTuiInjection({
  frameCount,
  intervalMs,
  page,
  paneKeys
  ...
  await page.evaluate(
    ({ frameCount, intervalMs, paneKeys, panesPerTimerTask }) => {
      ...
      const state: SyntheticTuiLoadState = {
        pendingTimers: [],
        stopped: false
      }
      target.__syntheticTuiLoadState = state
        ...
        for (let paneOffset = 0; paneOffset < paneKeys.length; paneOffset += panesPerTimerTask) {
          ...
          const timer = window.setTimeout(() => {
            ...
            for (const [batchIndex, paneKey] of paneBatch.entries()) {
              ...
              try {
                const injected = injector.inject(paneKey, syntheticTuiFrameSource(paneIndex, frame))
                if (!injected) {
      ...
      function syntheticTuiFrameSource(paneIndex: number, frame: number): string {
        const row = (frame % 18) + 4
        const spinner = ['|', '/', '-', '\\'][frame % 4]
        const body = `${'tui '.repeat(10)}pane=${paneIndex} frame=${frame}`
        return [
          '\x1b[?2026h',
          '\x1b[?25l',
          `\x1b[1;2H\x1b[38;2;255;138;0m${spinner} TUI synthetic agent ${paneIndex}\x1b[0m`,
  ...
  return {
    stop: async () => {
      await page.evaluate(() => {
        const target = window as SyntheticTuiInjectionWindow
        const state = target.__syntheticTuiLoadState
        if (!state) {
          return
        }
        state.stopped = true
        ...
        delete target.__syntheticTuiLoadState
        if (errorMessage) {
