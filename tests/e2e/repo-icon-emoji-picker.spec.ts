...
async function openRepoSettings(page: Page, repoId: string): Promise<void> {
  // Why: the host OS locale (e.g. ko-KR) drives Kolux's default UI language.
...
test.describe('Repository icon emoji picker', () => {
  test('search selects a native emoji and persists it as the repo icon', async ({
    ...
    await attachScreenshot(koluxPage, testInfo, 'repo-icon-emoji-picker-search')
    ...
    await attachScreenshot(koluxPage, testInfo, 'repo-icon-emoji-picker-selected')
  })
})
