...
describe.skipIf(!isMac || !e2eOptIn)('computer-use macOS e2e (TextEdit)', () => {
  ...
  test('get-app-state returns TextEdit state', async () => {
    const result = await runKoluxCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])
  ...
  test('click, type-text, and re-observe show inserted text', async () => {
    const before = parseJsonOutput<{ result: ComputerSnapshotResult }>(
      (await runKoluxCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])).stdout
    )
    ...
    const marker = `kolux computer e2e ${Date.now()}`
    await runKoluxCli(['computer', 'type-text', '--app', 'TextEdit', '--text', marker])
  ...
  test('paste-text and hotkey verify TextEdit text replacement', async () => {
    ...
    const marker = `kolux paste final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('kolux paste first')
  })
  ...
  test('accessibility text actions work when TextEdit is not frontmost', async () => {
    ...
    const marker = `kolux unfocused final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('kolux unfocused first')
  })
