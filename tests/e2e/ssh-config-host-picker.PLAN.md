...
secondary **Add all N to Kolux** action (no re-adopt). Settings → SSH → **Import**
...
| Search, result limit, suppressed aliases, alreadyInKolux  | `ssh-config-host-picker.test.ts`                     |
...
1. **Isolated HOME** — E2E already sets `HOME` to `{userDataDir}/home`. Seed config with:
   ...
   ```
2. **Unique aliases** — prefix Host entries and Kolux labels with
...
| **Expect** | Dialog title **Choose from ~/.ssh/config**; body **No hosts in ~/.ssh/config**; **Add all to Kolux** disabled; **Back** returns to form. |
...
| **Steps**  | Open picker.                                                                                                          |
| **Expect** | Host list `SSH config hosts` shows both aliases; subtitle `user@hostname:port`; button **Add all 2 to Kolux** enabled. |
...
### P5 — Already-in-Kolux badge + disabled row

|            |                                                                                                              |
| ---------- | ------------------------------------------------------------------------------------------------------------ |
| **Setup**  | Config hosts alpha + bravo. Seed Kolux target with `configHost`/`label` matching alpha (via `ssh.addTarget`). |
| **Steps**  | Open picker.                                                                                                 |
| **Expect** | Alpha shows **In Kolux** badge and is not clickable; bravo still selectable; **Add all 1 to Kolux** (not 2).   |

### P6 — Add all N to Kolux imports new hosts only

|            |                                                                                                                                     |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Setup**  | Config with 2 new hosts; no Kolux targets for them.                                                                                  |
| **Steps**  | **Add all 2 to Kolux** → wait for success toast / return to form or list refresh.                                                    |
| **Expect** | Both targets exist (DOM in Settings SSH and/or listTargets); re-open picker shows **All hosts already in Kolux** / both **In Kolux**. |
...
| **Expect** | Deleted config host reappears as a Kolux target; toast sync count ≥ 1. |
...
- **N2** Case-insensitive alias: config `Prod`, existing label `prod` → **In Kolux**.
...
| P5 In Kolux badge / count              | `ssh-config-host-import.spec.ts` |
