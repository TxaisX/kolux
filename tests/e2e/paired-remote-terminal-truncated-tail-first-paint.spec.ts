...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'kolux-paired-truncated-tail-'))
...
test.use({
  koluxAppExtraEnv: {
    KOLUX_E2E_TERMINAL_PARKING_DELAY_MS: String(RETENTION_PARK_DELAY_MS),
...
test('legacy paired hosts retain the lossy hidden-manager budget fallback @headful', async ({
  ...
  test.setTimeout(120_000)
  const worktreeIds = await koluxPage.evaluate(() =>
    window.__store
      ?.getState()
      .allWorktrees()
      .slice(0, 2)
