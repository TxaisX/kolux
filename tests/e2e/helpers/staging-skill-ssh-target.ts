...
export function stagingSkillSshTargetFromEnvironment(): StagingSkillSshTarget | null {
  const host = process.env.KOLUX_E2E_SKILL_SSH_HOST?.trim()
  const username = process.env.KOLUX_E2E_SKILL_SSH_USERNAME?.trim()
  const identityFile = process.env.KOLUX_E2E_SKILL_SSH_IDENTITY_FILE?.trim()
  ...
  const port = Number(process.env.KOLUX_E2E_SKILL_SSH_PORT?.trim() || '22')
