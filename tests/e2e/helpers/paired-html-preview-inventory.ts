 ...
 * *browser* page located by a workspace document and served over `kolux-preview://`, so the two
...
export async function readDocPreviewRenderedText(
  ...
  return page.evaluate(async (targetSelector) => {
    const guest = document.querySelector('webview[src^="kolux-preview://"]') as {
...
export async function readDocPreviewGuestRects(
  ...
  return page.evaluate(() =>
    [...document.querySelectorAll('webview[src^="kolux-preview://"]')].map((node) => {
...
export async function readDocPreviewGuestUrl(page: Page): Promise<string | null> {
  return page.evaluate(
    () => document.querySelector('webview[src^="kolux-preview://"]')?.getAttribute('src') ?? null
  )
}
