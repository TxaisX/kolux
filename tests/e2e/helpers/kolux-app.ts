/**
 * Shared Electron fixture for Kolux E2E tests.
 *
...
type KoluxTestFixtures = {
  ...
  koluxAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running Kolux instance.
  launchEnv: NodeJS.ProcessEnv
}

type KoluxWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad KOLUX_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const KOLUX_E2E_SLOWMO_MS_RAW = process.env.KOLUX_E2E_SLOWMO_MS
const KOLUX_E2E_SLOWMO_MS = ((): number => {
  if (KOLUX_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(KOLUX_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[kolux-e2e] KOLUX_E2E_SLOWMO_MS="${KOLUX_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: KOLUX_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.KOLUX_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.koluxHeadful === true
}
...
// same KOLUX_E2E_FORWARD_APP_LOGS-gated stdout/stderr capture.
export function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.KOLUX_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with Kolux-specific fixtures.
 *
 * `koluxPage` — the main Kolux renderer window.
 *
 ...
 */
export const test = base.extend<KoluxTestFixtures, KoluxWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'kolux-e2e-userdata-'))
    ...
    // Kolux's own agent runtime) set it so Electron behaves as a plain Node
    ...
    const homeIsolation = createElectronHomeIsolation({
      ...
      launchEnv,
      extraEnv: koluxAppExtraEnv,
      userDataDir
    })
    // Why: KOLUX_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with KOLUX_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = KOLUX_E2E_SLOWMO_MS
    // Why: KOLUX_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.KOLUX_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...koluxAppExtraArgs, ...getKoluxElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. KOLUX_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: KOLUX_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: KOLUX_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. KOLUX_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        ...((process.env.KOLUX_E2E_SSH_LOCALHOST === '1' ||
          process.env.KOLUX_E2E_SSH_DOCKER === '1' ||
          process.env.KOLUX_E2E_NESTED_RUNTIME_SSH === '1') &&
        !cleanEnv.KOLUX_RELAY_PATH
          ? { KOLUX_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { KOLUX_E2E_HEADFUL: '1' } : { KOLUX_E2E_HEADLESS: '1' })
      }
    })
  ...
  koluxAppExtraEnv: [{}, { option: true }],
  koluxAppExtraArgs: [[], { option: true }],
  ...
  koluxPage: async ({ sharedPage }, provideFixture) => {
