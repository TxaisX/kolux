 ...
 *
 * Why: the default `koluxPage` fixture creates a fresh `userDataDir` per test
...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedKolux>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'kolux-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedKolux> => {
    runtimeWsPort ??= await reserveRestartRuntimeWsPort()
    const app = await electron.launch({
      args: getKoluxElectronLaunchArgs(mainPath, headful),
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
        KOLUX_E2E_RUNTIME_WS_PORT: String(runtimeWsPort)
      }
    })
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.KOLUX_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to Kolux's workspace root after the visibility rollout.
