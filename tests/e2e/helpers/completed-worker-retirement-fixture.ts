...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'kolux-e2e-retired-worker-'))
...
const ledger = process.env.KOLUX_E2E_CODEX_LIFECYCLE_LEDGER
...
process.stdin.on('data', (chunk) => {
  ...
  if (input.includes('KOLUX_E2E_EXIT_AFTER_DONE')) {
...
export const completedWorkerLaunchEnv = {
  ...
  KOLUX_E2E_CODEX_LIFECYCLE_LEDGER: lifecycleLedgerPath
}
...
export function runBuiltKoluxCli(
  args: string[],
  options: { userDataDir: string; cwd: string }
): unknown {
  const {
    KOLUX_ENVIRONMENT: _environment,
    KOLUX_PAIRING_CODE: _pairingCode,
    KOLUX_USER_DATA_PATH: _userDataPath,
    ...cleanEnv
  } = process.env
  void _environment
  void _pairingCode
  ...
  const output = execFileSync(
    ...
    {
      cwd: options.cwd,
      env: { ...cleanEnv, KOLUX_USER_DATA_PATH: options.userDataDir },
      encoding: 'utf8',
      timeout: 30_000
    }
  )
