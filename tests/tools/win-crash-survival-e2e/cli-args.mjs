...
// installed, packaged Kolux.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Kolux's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Kolux.exe>] [options]
...
Options:
  --exe-path <path>        Installed Kolux.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Kolux). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Kolux.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Kolux.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Kolux.exe>'
    )
