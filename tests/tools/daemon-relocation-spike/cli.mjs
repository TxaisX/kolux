...
// COPIED Kolux.exe (ELECTRON_RUN_AS_NODE=1) can host the terminal daemon and a
...
Required (launch mode):
  --app-dir <path>    Path to a packaged win-unpacked build (contains Kolux.exe)
...
Options:
  --tier <t>          File-set tier to copy: "full" (default), "no-gpu", "minimal"
                        full    = Kolux.exe + icudtl.dat + both snapshot blobs +
                        ...
                        no-gpu  = full minus GPU/render DLLs (libEGL, libGLESv2,
                                  ...
                                  ffmpeg.dll kept
                        minimal = Kolux.exe + icudtl.dat + both snapshots +
