...
> `win-unpacked` build so that a **copied `Kolux.exe`** — run with
...
The daemon is `fork()`ed from the app's own Electron binary (`Kolux.exe`) with
...
round-trips the nonce, the daemon's main module is the copied `Kolux.exe`, and
**no** loaded module resolves under `--app-dir`.

## Tiers (defined as data in `tier-file-set.mjs`)

Every tier includes the irreducible core: `Kolux.exe`, `icudtl.dat`, both V8
...
### `KOLUX_NODE_PTY_NATIVE_DIR`

The reverted #7421 added a `node-pty` patch that reads
`KOLUX_NODE_PTY_NATIVE_DIR` to override the native dir. **The current branch's
...
spike still _sets_ `KOLUX_NODE_PTY_NATIVE_DIR` to the relocated native dir so it
...
will need to **re-add the `KOLUX_NODE_PTY_NATIVE_DIR` patch** from #7421. If it
...
process's **main module** is the copied `Kolux.exe`, not the install-dir one.
...
- Whether `TIER_MINIMAL` (no top-level DLLs) boots `Kolux.exe` as node at all, or
...
ship minimal; if `Kolux.exe` fails to boot without the non-GPU DLLs, fall back to
