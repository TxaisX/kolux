...
// COPIED Kolux.exe (ELECTRON_RUN_AS_NODE=1), launched from outside the install
...
function makeSocketPath() {
  if (process.platform === 'win32') {
    return `\\\\?\\pipe\\kolux-daemon-spike-${randomUUID().slice(0, 12)}`
  }
  return join(process.env.TMPDIR ?? '/tmp', `kolux-daemon-spike-${randomUUID().slice(0, 12)}.sock`)
}
