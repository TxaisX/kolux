...
// watches our exit code). Each scenario mimics a real Kolux usage pattern:
//
      ...
      sub = await watcher.subscribe(
        ...
        (err, events) => {
          if (err) {
            recordError(err)
            errored = true
            // Kolux's createWatcher error path: unsubscribe from inside the
            // error callback.
            if (sub) {
              stats.unsubscribes++
...
async function main() {
  ...
  // Outside the repo worktree: a live Kolux instance may be watching the repo,
  // and this churn must not feed the production watcher.
  const baseDir = path.join(os.tmpdir(), 'kolux-7547-harness', `run-${process.pid}`)
