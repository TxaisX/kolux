...
**Windows only.** Given two Kolux Windows installers (version N and N+1), this
...
- **The daemon is identified by command-line marker, never by exe name.** With
  `ELECTRON_RUN_AS_NODE` the daemon image is `Kolux.exe`; a relocated Phase 1
...
| Flag                                          | Meaning                                                     |
| --------------------------------------------- | ----------------------------------------------------------- |
| `--from <path>` / `--to <path>`               | Local `kolux-windows-setup.exe` for base (N) / update (N+1)  |
...
- **Pre-existing app process → hard refusal.** If a Kolux _app_ process (not a
...
- **Pre-existing install → refusal unless `--allow-existing-install`.** If an
  Kolux install already exists under `%LOCALAPPDATA%\Programs`, the run refuses,
...
(`%LOCALAPPDATA%\Programs\Kolux`). A developer's box already has a real Kolux there,
...
1. **Snapshots** the shared state (`registry-shortcut-backup.mjs`): `reg export`s
   each existing key to `.reg` files, copies the Kolux `*.lnk` shortcuts, and records
...
and the Kolux shortcuts — the state that steers a future update and the user-visible
...
pnpm win-update-e2e \
  ...
  --expect cold-restore --install-dir C:\KoluxE2E
```
...
1. **Preflight** — assert win32; warn if elevated; **refuse** to run if a
   pre-existing Kolux _app_ process (not a daemon) is running that the harness
...
2. **Install N** silently (`<setup.exe> /S`) and locate `Kolux.exe`.
...
| `registry-shortcut-backup.mjs` | Isolated mode: snapshot/restore the shared HKCU keys + Kolux shortcuts         |
...
- The harness assumes the packaged main honors `KOLUX_E2E_USER_DATA_DIR` to
