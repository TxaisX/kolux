// Persistence seed for a fresh packaged Kolux profile: dismisses onboarding and
...
// kolux-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites kolux-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Kolux treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@kolux.test')
