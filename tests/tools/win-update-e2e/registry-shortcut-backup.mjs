...
/**
 * Read-only discovery of the machine's current Kolux install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Kolux' -or ($il -and $il -match '\\\\Programs\\\\kolux')) {`,
    ...
    `  if ($il -and ($sn -eq 'Kolux' -or $il -match 'kolux')) {`,
...
/** The two directories NSIS writes Kolux shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Kolux *.lnk shortcuts under the given dirs (recursive). */
export function discoverKoluxShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectKoluxLnks(dir, found)
  }
  return found
}

function collectKoluxLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectKoluxLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /kolux/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Kolux shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverKoluxShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Kolux shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverKoluxShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Kolux install pointer may be hijacked to the test directory.')
  console.error('!! The next real Kolux update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Kolux shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverKoluxShortcuts(), null, 2))
}
