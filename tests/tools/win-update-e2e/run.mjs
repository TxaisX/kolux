...
//
// Given two Kolux Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `KOLUX-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `kolux-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Kolux.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: a Kolux install existed before this run. It was OVERWRITTEN and the\n' +
