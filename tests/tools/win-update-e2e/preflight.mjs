...
// it created. To avoid ever touching a user's live Kolux, it REFUSES to run when
// a pre-existing Kolux app process (not a daemon) is already running that it did
...
/**
 * Find running Kolux APP processes (main window process), excluding daemons.
 * The daemon runs as Kolux.exe too but always carries the daemon-entry.js marker
 ...
 * test dir (fatal) or is the developer's real Kolux elsewhere (informational).
 */
export function findAppProcesses() {
  const command = [
    `$procs = @(Get-CimInstance Win32_Process -Filter "Name = 'Kolux.exe'" -ErrorAction SilentlyContinue |`,
  ...
  // query must abort the run rather than look like "no Kolux is running".
  const { stdout, stderr, code, error } = runCommandSync(command)
  if (error) {
    throw new Error(`Failed to query Kolux app processes: ${error.message}`)
  }
  if (code !== 0) {
    throw new Error(`Failed to query Kolux app processes (exit ${code}): ${stderr || stdout}`)
  }
  ...
  try {
    ...
    throw new Error(
      `Kolux app process query returned invalid JSON: ${parseError.message}\n` +
        `stdout:\n${trimmed}\nstderr:\n${stderr}`
    )
  }
}
 ...
 * pre-existing Kolux app is running (never kill a user's process) or if a Kolux
 ...
 * in installDir triggers the existing-install refusal. A real Kolux running or
...
export function preflight({ baselinePath, allowExistingInstall = false, installDir = null }) {
  ...
  if (isolated) {
    ...
    if (inTarget.length > 0) {
      ...
      throw new Error(
        `Refusing to run: ${inTarget.length} Kolux app process(es) are running from the ` +
    ...
    if (elsewhere.length > 0) {
      warnings.push(
        `${elsewhere.length} Kolux app process(es) are running from outside the isolated ` +
    ...
    throw new Error(
      `Refusing to run: ${appProcesses.length} Kolux app process(es) are already ` +
  ...
  if (existingInstall && !allowExistingInstall) {
    throw new Error(
      isolated
        ...
        : `Refusing to run: a Kolux install already exists at ${existingInstall}. ` +
            `This run would silently OVERWRITE it with the --from/--to versions and ` +
            `leave the --to version installed — destroying a real Kolux install on a ` +
            `developer machine. Pass --allow-existing-install to proceed anyway ` +
            `(your prior build will NOT be restored), or uninstall Kolux first. Clean ` +
