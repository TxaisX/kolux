// Identify and inspect the Kolux terminal daemon on Windows.
//
// The daemon is forked with ELECTRON_RUN_AS_NODE=1, so on Windows its process
// image is Kolux.exe (the Electron binary running as plain Node) — it CANNOT be
...
/** Default packaged userData root on Windows: %APPDATA%\Kolux. */
export function defaultUserDataDir() {
  const appData =
    process.env.APPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Roaming')
  return path.join(appData, 'Kolux')
}
...
export function findDaemonProcesses(scope = '') {
  ...
  // ELECTRON_RUN_AS_NODE the daemon's image is Kolux.exe today but a relocated
