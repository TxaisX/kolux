...
// Why: under a Git Bash setup runner, Kolux exports KOLUX_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.KOLUX_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.KOLUX_WORKTREE_PATH || cwd, platform, env)
