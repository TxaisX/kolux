...
async function readRendererProbe(page) {
  return await runWithTimeout(
    'renderer probe read',
    () => page.evaluate(() => globalThis.__koluxApphangProbe?.probe ?? null),
    rendererActionTimeoutMs
  )
}
...
async function resetRendererProbe(page) {
  ...
  await runWithTimeout(
    ...
    () =>
      page.evaluate(() => {
        const probe = globalThis.__koluxApphangProbe?.probe
        if (!probe) {
          return
        }
...
async function runActivationCycle(page, target, args) {
  ...
  const marker = `KOLUX_APPHANG_${target.index}_${Date.now()}`
