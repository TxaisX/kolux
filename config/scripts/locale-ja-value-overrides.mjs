...
const JA_AUDIT_OVERRIDES = {
  ...
  "Install the Browser Use skill so agents can operate Kolux's browser.":
    'Browser Use スキルをインストールすると、 Agent が Kolux のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from Kolux — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。Kolux から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch Kolux if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は Kolux を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this Kolux server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート Kolux サーバー] → [この Kolux サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  'Change this later in Settings → Browser.': '後で [設定] → [ブラウザ] から変更できます。',
  'Kolux Cloud sign-in is not configured': 'Kolux Cloud へのサインインが設定されていません',
  ...
  'only when Kolux cannot derive the API base URL from the git remote.':
    'は、Kolux が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when Kolux cannot derive the API URL from the remote.':
    'は、Kolux がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, Kolux refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、Kolux はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. Kolux skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、Kolux は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from Kolux':
    '含まれている {{value1}} {{value0}} 件を Kolux から削除',
  'People in {{orgName}} who can collaborate on Kolux.':
    '{{orgName}} で Kolux を共同利用できるメンバーです。',
  'People in your organization who can collaborate on Kolux.':
    '組織内で Kolux を共同利用できるメンバーです。',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Kolux server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート Kolux サーバー] → [この Kolux サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving Kolux': 'Kolux から離れずに、タスクから Agent を開始',
  'Workspace created from {{value0}}, but Kolux could not fast-forward local {{value1}} because {{value2}}':
    'ワークスペースは {{value0}} から作成されましたが、{{value2}} のため Kolux はローカル {{value1}} を fast-forward できませんでした',
  ...
  'kolux.yaml first, then your local commands.':
    'まず kolux.yaml、次にローカルのコマンドの順です。',
  'Rename Kolux-created branches from the initial agent task.':
    'Kolux が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  'Dim files matched by .gitignore in the file explorer.':
    'ファイルエクスプローラーで、.gitignore に一致するファイルを薄く表示します。',
  'Command line Kolux runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに Kolux が実行するコマンドライン。',
  ...
  'Repository not in Kolux': 'このリポジトリは Kolux に登録されていません',
  ...
  'Clears them from Kolux only. Remote files, worktrees, and branches are left untouched.':
    'Kolux からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the Kolux CLI before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、Kolux CLI をインストールしてください。',
  'Run grok in a terminal on the computer running Kolux and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'Kolux を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "Kolux works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'Kolux はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Optional. Kolux can use your normal Codex login; add accounts only if you want quick switching in Kolux.':
    '任意。Kolux は通常の Codex ログインを使用できます。Kolux 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Kolux can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。Kolux は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Kolux works with your existing provider logins; add accounts only if you want Kolux to help switch between them.':
    '任意。Kolux は既存のプロバイダーのログインと連携します。Kolux に切り替えを任せたい場合にのみ、アカウントを追加してください。',
