...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'kolux-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'kolux-wayland-gpu-userdata-'))
  ...
  try {
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.KOLUX_WAYLAND_GPU_VERBOSE === '1') {
