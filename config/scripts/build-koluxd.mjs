#!/usr/bin/env node
/**
 * Bundle `koluxd` — the Kolux runtime served from plain Node, no Electron.
 *
...
} from '../../src/shared/koluxd-artifacts.ts'

const ROOT = join(import.meta.dirname, '..', '..')
const OUT_DIR = join(ROOT, 'out', 'koluxd')
const ENTRY = join(ROOT, 'src/main/koluxd/main.ts')
// Why beside koluxd.js: the watcher runs in a forked child so a native @parcel/watcher
...
// Why beside koluxd.js: koluxd forks the terminal daemon so PTYs outlive the runtime process,
// and `getDaemonEntryPath()` probes the app root for this exact filename. Without it every
// koluxd restart would SIGKILL every running terminal.
...
const OUT_FILE = join(OUT_DIR, 'koluxd.js')
...
// Why only these: measured, not guessed. `node-pty` is a hard `require.resolve` — koluxd
 ...
 *  directory under `outdir`, and both children must land flat beside koluxd.js — that is where
...
const output = Object.values(result.metafile.outputs).find(
  (o) => o.entryPoint === 'src/main/koluxd/main.ts'
)
...
if (graphErrors.length > 0) {
  console.error(`[build-koluxd] ${graphErrors.join('\n')}`)
  // Why this can exceed the ratchet baseline: the ratchet measures the graph reachable
  // from kolux-runtime + runtime-rpc, but this entry also imports ipc/pty directly to
  // install the PTY controller. Once koluxd ships, it should become a ratchet entry
  ...
  // smoke-loads its entries for exactly this reason, and koluxd cannot join that guard
  ...
  const smoke = spawnSync(process.execPath, [OUT_FILE, '--koluxd-smoke-load-check'], {
    encoding: 'utf8',
    timeout: 60_000
  })
  ...
  if (smoke.error || smoke.signal || smoke.status !== 0) {
    console.error(
      `[build-koluxd] the bundle did not load under plain Node.\n` +
  ...
  // token and a PTY — `smoke:koluxd-terminal` does that end to end, through koluxd.
  ...
  const daemonSmoke = spawnSync(
    ...
    ],
    {
      encoding: 'utf8',
      timeout: 60_000,
      env: { ...process.env, KOLUX_DAEMON_ENTRY_LOAD_CHECK: '1' }
    }
  )
  ...
  if (daemonSmoke.error || daemonSmoke.signal || daemonSmoke.status !== 0) {
    console.error(
      `[build-koluxd] the daemon child did not load under plain Node.\n` +
  ...
  if (watcherFailure) {
    console.error(
      `[build-koluxd] the watcher child did not run under plain Node.\n${watcherFailure}`
    )
    process.exitCode = 1
  }
}

// Why a content hash and not KOLUXD_VERSION alone: the remote install directory is keyed on
...
if (process.exitCode !== 1) {
  ...
  for (const filename of koluxdArtifactFilenames()) {
    const artifactPath = join(OUT_DIR, filename)
    if (!existsSync(artifactPath)) {
      throw new Error(
        `koluxd declares ${filename} in KOLUXD_ARTIFACTS but never emitted it. Add the build ` +
          'step, or drop it from src/shared/koluxd-artifacts.ts.'
      )
    }
    hash.update(readFileSync(artifactPath))
  }
  const fullVersion = `${KOLUXD_VERSION}+${hash.digest('hex').slice(0, 12)}`
  writeFileSync(join(OUT_DIR, KOLUXD_VERSION_FILENAME), fullVersion)
  console.log(
    `[build-koluxd] ok — ${fullVersion}, ${(output.bytes / 1024 / 1024).toFixed(2)} MB, ${Object.keys(output.inputs).length} modules, zero electron and node:sqlite imports.`
  )
}
...
async function smokeLoadWatcherChild() {
  const probeDir = mkdtempSync(join(tmpdir(), 'koluxd-watcher-smoke-'))
