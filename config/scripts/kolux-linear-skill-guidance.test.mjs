...
// Why: kolux-linear and its legacy linear-tickets alias now ship hybrid discovery stubs, so
...
const canonicalGuidePath = join(projectDir, 'skill-guides', 'kolux-linear.md')
const legacyGuidePath = join(projectDir, 'skill-guides', 'linear-tickets.md')
const canonicalStubPath = join(projectDir, 'skills', 'kolux-linear', 'SKILL.md')
...
describe('kolux-linear skill guidance', () => {
  it('keeps canonical and legacy Linear guide bodies from drifting', () => {
    ...
    expect(canonical).toContain('name: kolux-linear')
  ...
  it('documents targeted project discovery in both skill names', () => {
    ...
    for (const skill of [canonical, legacy]) {
      expect(skill).toContain('KOLUX linear project list --query <project-name>')
      expect(skill).toContain('Run only the command for the metadata you need')
    }
  })

  // Why: a bare `kolux` at line start resolves to the GNOME Orca screen reader on Linux and
  // starts speech on the user's machine, so guide examples use the resolved-executable
  // placeholder instead.
  it('keeps Linear guide examples off a bare kolux command name', () => {
    for (const guidePath of [canonicalGuidePath, legacyGuidePath]) {
      ...
      expect(skill, guidePath).toContain(
        '`KOLUX` is a placeholder for the executable you resolved in the stub'
      )
      expect(skill, guidePath).not.toMatch(/^kolux /mu)
      expect(skill, guidePath).not.toMatch(/\$KOLUX(?:_|\b)/u)
    }
  })
...
describe('kolux-linear install stubs', () => {
  const cases = [
    { name: 'kolux-linear', stubPath: canonicalStubPath, guidePath: canonicalGuidePath },
  ...
  for (const { name, stubPath, guidePath } of cases) {
    it(`points ${name} at the version-matched guide and preserves the safe resolver`, () => {
      ...
      expect(stub).toContain(`KOLUX skills get ${name}`)
    ...
    it(`drops the changing command reference from the installable ${name} file`, () => {
      ...
      expect(stub).not.toMatch(/\bkolux linear search\b/iu)
      expect(stub).not.toMatch(/\bkolux linear comment\b/iu)
