...
describe('PR workflow parallelism', () => {
  ...
  it('reuses native preparation after the dependency action gate', () => {
    ...
    expect(packageStep.env.KOLUX_REUSE_PREPARED_NATIVE_RUNTIME).toBe('1')
  })
  ...
  it('keeps verify as the aggregate required check', () => {
    ...
    // KOLUX_BROWSER_EXECUTABLE, so it only guards anything if verify actually reads it.
    expect(verifyStep.env.KOLUXD_BROWSER).toBe('${{ needs.koluxd_browser.result }}')
    expect(verifyStep.run).toContain('"$KOLUXD_BROWSER"')
