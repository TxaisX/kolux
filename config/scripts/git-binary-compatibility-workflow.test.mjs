...
describe('Git binary compatibility PR gate', () => {
  it('runs the real-binary contract at each compatibility boundary', () => {
    ...
    expect(step?.run).toContain('KOLUX_GIT_COMPAT_BINARY="$source/git"')
    expect(step?.run).toContain('alpine/git:edge-2.38.1|2.38.1')
    expect(step?.run).toContain('alpine/git:v2.49.1|2.49.1')
    expect(step?.run).toContain('KOLUX_GIT_COMPAT_IMAGE="$image"')
  ...
  it('restores the baseline Git build before the matrix runs', () => {
    ...
    expect(steps[cacheIndex].with.path).toBe('~/.cache/kolux-git-compat/git-2.25.5')
    expect(steps[matrixIndex].run).toContain('source="$HOME/.cache/kolux-git-compat/git-2.25.5"')
