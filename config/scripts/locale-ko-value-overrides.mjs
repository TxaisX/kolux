...
export const KO_VALUE_OVERRIDES = {
  ...
  'Kolux uses the GitHub CLI (gh) to show pull requests, issues, and checks.':
    'Kolux는 GitHub CLI(gh)를 사용하여 PR, 이슈 및 검사를 표시합니다.',
  ...
  'Review inactive workspaces before deleting their local files and Kolux state.':
    '로컬 파일과 Kolux 상태를 삭제하기 전에 비활성 워크스페이스를 확인하세요.',
  ...
  'Install the Kolux skill so agents know to use the Kolux CLI.':
    '에이전트가 Kolux CLI를 사용하도록 Kolux 스킬을 설치하세요.',
  ...
  "Control Kolux from your phone. Check on agents, review changes, and kick off tasks while you're away from your desk.":
    '휴대폰에서 Kolux를 제어하세요. 자리를 비운 동안 에이전트를 확인하고, 변경 사항을 리뷰하고, 작업을 시작하세요.',
