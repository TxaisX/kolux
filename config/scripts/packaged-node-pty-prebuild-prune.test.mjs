 ...
 * build carries Kolux's job-object exports, so leaving the prebuilt conpty.node
 * beside it means an ABI mismatch or an AV quarantine silently downgrades the
 * app to a binary that cannot own a PTY tree.
 *
 * The siblings must survive: Kolux's patch deletes the `conpty_console_list` and
...
describe('prunePackagedNodePty: the Windows conpty fallback', () => {
  ...
  beforeEach(() => {
    resources = mkdtempSync(join(tmpdir(), 'kolux-prune-'))
  })
  afterEach(() => {
  ...
  it.each(SIBLINGS)('keeps %s, which exists nowhere else in a packaged build', (sibling) => {
    // Kolux's patch removes the conpty_console_list and winpty gyp targets, so a
