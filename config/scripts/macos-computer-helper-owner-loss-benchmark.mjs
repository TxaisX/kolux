...
const INTERNAL_ENV = 'KOLUX_COMPUTER_HELPER_OWNER_BENCH_INTERNAL'
const EXPECTATION_ENV = 'KOLUX_COMPUTER_HELPER_OWNER_BENCH_EXPECTATION'
const HELPER_RECORD_PATH_ENV = 'KOLUX_COMPUTER_HELPER_OWNER_BENCH_HELPER_RECORD_PATH'
const RESULT_PATH_ENV = 'KOLUX_COMPUTER_HELPER_OWNER_BENCH_RESULT_PATH'
...
const helperPath = path.join(helperAppPath, 'Contents', 'MacOS', 'kolux-computer-use-macos')
...
function startSidecar() {
  ...
  const child = fork(sidecarPath, [], {
    ...
    env: {
      ...
      KOLUX_COMPUTER_SIDECAR: '1',
      KOLUX_COMPUTER_MACOS_HELPER_APP_PATH: helperAppPath
    }
  })
...
function runTrial(executable, expectation) {
  ...
  let trialOutput = ''
  try {
    launcherDir = mkdtempSync(path.join(tmpdir(), 'kolux-helper-owner-bench-launcher-'))
    trialTempDir = mkdtempSync(path.join(path.sep, 'tmp', 'kolux-owner-bench-'))
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'kolux-helper-owner-benchmark', main: 'main.cjs' })
    )
    writeFileSync(
