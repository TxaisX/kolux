...
describe('local Linux build target', () => {
  ...
  it('passes an explicit target and matching artifact-name environment', () => {
    ...
    runLocalLinuxBuild({
      arch: 'arm64',
      environment: { PATH: '/bin', KOLUX_LINUX_ARM64_RELEASE: undefined },
      execFile,
      platform: 'linux',
      cwd: '/workspace'
    })
    expect(execFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: '/workspace',
        env: expect.objectContaining({ KOLUX_LINUX_ARM64_RELEASE: '1' }),
        stdio: 'inherit'
      })
    )
    ...
    runLocalLinuxBuild({
      arch: 'x64',
      environment: { PATH: '/bin', KOLUX_LINUX_ARM64_RELEASE: '1' },
      execFile,
      platform: 'linux',
      cwd: '/workspace'
    })
    expect(execFile).toHaveBeenLastCalledWith(
      ...
      expect.objectContaining({
        env: expect.not.objectContaining({ KOLUX_LINUX_ARM64_RELEASE: expect.anything() })
      })
    )
  })
