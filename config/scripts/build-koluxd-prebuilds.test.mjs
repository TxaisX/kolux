...
import {
  ...
  readManifest,
  slotName
} from './build-koluxd-prebuilds.mjs'

const PATCHED_BINDING_GYP =
  "'ldflags': ['-Wl,--no-as-needed,-l:libutil.so.1,-l:libpthread.so.0,--as-needed']"
const PATCHED_PTY_CC = '__asm__(".symver openpty,openpty@" KOLUX_GLIBC_COMPAT_VERSION);'

const dirs = []
const stage = (bindingGyp, ptyCc) => {
  const dir = mkdtempSync(join(tmpdir(), 'koluxd-prebuild-src-'))
  dirs.push(dir)
...
describe('slot naming', () => {
  it('covers every platform koluxd ships to', () => {
...
describe('readManifest', () => {
  it('returns null instead of throwing when no matrix has been built', () => {
    const dir = mkdtempSync(join(tmpdir(), 'koluxd-prebuild-manifest-'))
    dirs.push(dir)
