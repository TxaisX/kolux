...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `kolux ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { KOLUX_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { KOLUX_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'kolux://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, KOLUX_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, KOLUX_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, KOLUX_ENVIRONMENT: 'no-such-environment' }],
    ...
    // KOLUX_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [['environment', 'list', '--json'], { ...isolated, KOLUX_ENVIRONMENT: 'no-such-environment' }],
    [['environment', 'list', '--json'], { ...isolated, KOLUX_PAIRING_CODE: 'not-a-pairing-code' }],
    [['agent-context', '--json'], { ...isolated, KOLUX_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `kolux serve` runs Kolux until Ctrl+C and `kolux open` /
...
//   2. MUTATING — writes outside KOLUX_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside KOLUX_USER_DATA_PATH'])
])
