...
const ITERATIONS = Number.parseInt(process.env.KOLUX_PR_CARRY_BENCH_ITERATIONS ?? '2000', 10)
const WARMUP = Number.parseInt(process.env.KOLUX_PR_CARRY_BENCH_WARMUP ?? '200', 10)

for (const [name, value] of [
  ['KOLUX_PR_CARRY_BENCH_ITERATIONS', ITERATIONS],
  ['KOLUX_PR_CARRY_BENCH_WARMUP', WARMUP]
]) {
...
const EQUIVALENCE_FIXTURES = [
  `noise ${'x'.repeat(400)}https://github.com/acme/kolux/pull/7`,
  `https://github.com/acme/kolux/pull/1${'x'.repeat(600)}`,
  `https://github.com/acme/kolux/pull/1${'x'.repeat(600)}https`,
  `${'x'.repeat(1000)}https`,
  `${'x'.repeat(1000)}http`,
  'https://github.com/acme/kolux/pull/7 trailing words',
  `${'y'.repeat(600)}`,
  '',
  'https://github.com/acme/kolux/pull/7'
]
