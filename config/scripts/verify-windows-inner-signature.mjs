...
$signature = Get-AuthenticodeSignature -FilePath $env:KOLUX_WINDOWS_INNER_EXECUTABLE
...
export function parseExpectedSigners(value = process.env.KOLUX_WINDOWS_EXPECTED_SIGNERS) {
...
export function parseExpectedThumbprints(value = process.env.KOLUX_WINDOWS_EXPECTED_THUMBPRINTS) {
...
export function validateExecutablePath(executablePath) {
  if (typeof executablePath !== 'string' || executablePath.trim() === '') {
    throw new Error('Usage: node config/scripts/verify-windows-inner-signature.mjs <Kolux.exe>')
  }
...
export function getPowerShellSignatureJson(executablePath, spawnSyncImpl = spawnSync) {
  ...
  const result = spawnSyncImpl(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        KOLUX_WINDOWS_INNER_EXECUTABLE: executablePath
      }
    }
  )
