#!/usr/bin/env node
/**
 * Build one node-pty prebuilt for the CURRENT platform/arch/libc and file it in koluxd's
 * prebuilds matrix, so a deployment target needs no C/C++ toolchain.
 *
 * node-pty is the only ABI-sensitive native module koluxd requires. It is also PATCHED in
 ...
 *
 * koluxd pins its own Node runtime, so the ABI dimension is fixed and the matrix varies
 ...
 * merges the resulting `out/koluxd/prebuilds` trees. `--slot=<name>` forces the label so
 * the glibc/musl distinction is recorded from the container rather than detected.
 *
 * Usage:
 *   node config/scripts/build-koluxd-prebuilds.mjs [--slot=linux-x64-musl]
 *   node config/scripts/build-koluxd-prebuilds.mjs --require-slots   # release gate
 */
...
const PREBUILDS_DIR = join(ROOT, 'out', 'koluxd', 'prebuilds')
...
export function assertNodePtyPatchApplied(nodePtyDir) {
  ...
  if (missing.length > 0) {
    throw new Error(
      [
        '[koluxd-prebuilds] refusing to build: config/patches/node-pty@1.1.0.patch is not applied.',
...
function compileNodePty(dir) {
  ...
  if (existsSync(built)) {
    console.log(`[koluxd-prebuilds] reusing existing build at ${built}`)
    return built
  }
  console.log('[koluxd-prebuilds] compiling node-pty from patched source ...')
  ...
  if (result.status !== 0) {
    throw new Error(`[koluxd-prebuilds] node-gyp rebuild failed (status ${result.status})`)
  }
  if (!existsSync(built)) {
    throw new Error(`[koluxd-prebuilds] node-gyp succeeded but ${built} is missing`)
  }
  return built
}
...
function requireSlots() {
  ...
  if (missing.length > 0) {
    console.error(
      `[koluxd-prebuilds] matrix incomplete — missing ${missing.join(', ')}. ` +
        'Hosts on those slots fall back to a source build and need a C/C++ toolchain.'
    )
    process.exitCode = 1
    return
  }
  console.log(`[koluxd-prebuilds] matrix complete — ${MATRIX_SLOTS.length} slots`)
}
...
function build() {
  ...
  console.log(`[koluxd-prebuilds] stored ${slot}/pty.node`)
  ...
  if (process.platform === 'darwin') {
    ...
    if (!existsSync(helperSource)) {
      throw new Error(`[koluxd-prebuilds] spawn-helper missing at ${helperSource}`)
    }
    copyFileSync(helperSource, join(slotDir, 'spawn-helper'))
    console.log(`[koluxd-prebuilds] stored ${slot}/spawn-helper`)
  }
  ...
  console.log(
    `[koluxd-prebuilds] manifest: node-pty ${manifest.version}, ABI ${manifest.nodeAbi}, slots ${manifest.slots.join(', ')}`
  )
}

if (process.argv[1] && process.argv[1].endsWith('build-koluxd-prebuilds.mjs')) {
