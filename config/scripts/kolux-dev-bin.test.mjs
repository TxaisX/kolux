...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'kolux-dev.mjs')

describe('kolux-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['kolux-dev']).toBe('./config/scripts/kolux-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'kolux-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  devCliInvocation: process.env.KOLUX_DEV_CLI_INVOCATION,',
        '  appExecutable: process.env.KOLUX_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        KOLUX_DEV_CLI_ENTRY_PATH: cliEntry,
        KOLUX_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        KOLUX_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
