...
// Why: kolux-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'kolux-cli.md')
const stubPath = join(projectDir, 'skills', 'kolux-cli', 'SKILL.md')
// Why: orchestration and kolux-emulator also ship hybrid stubs now, so their version-sensitive
...
const emulatorSkillPath = join(projectDir, 'skill-guides', 'kolux-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('kolux CLI skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(skill).toContain(
      'For external Chrome/Safari/webviews or Kolux app chrome/settings, use the Computer Use skill/tool only when the task requires OS/window-level control'
    )
    expect(skill).toContain(
      "Use `kolux-cli` for Kolux's embedded pages and a page-automation tool such as Playwright or CDP for external pages"
    )
  })
  ...
  it('keeps independent worktree lineage separate from Git base selection', () => {
    ...
    expect(skill).toContain('`--no-parent` only controls Kolux lineage')
    expect(skill).toContain('omit `--base-branch` so Kolux uses the repo default base')
  ...
  it('documents non-lifecycle full handoffs and custom Codex model fallback', () => {
    ...
    expect(skill).toContain(
      'KOLUX worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
  ...
  // behind `skills get kolux-cli --reference` so they are not charged to every turn, with
  ...
  it('gates the reconstructible command catalogs behind bundled references', () => {
    ...
    expect(skill).toContain('KOLUX skills get kolux-cli --reference references/<file>.md')
    expect(skill).toContain(
      'If the CLI rejects `--reference`, run `KOLUX skills get kolux-cli --full`'
    )
    for (const reference of [
      ...
      expect(readSkill(join(projectDir, 'skill-guides', 'kolux-cli', reference)).trim()).not.toBe(
        ''
      )
    }
    expect(skill).not.toContain('KOLUX automations create')
    expect(skill).not.toContain('KOLUX artifacts share <file>')
    expect(skill).not.toContain('KOLUX goto --url')
  })
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`kolux eval` expressions, or `kolux exec` commands')
...
describe('kolux CLI install stub', () => {
  ...
  it('does not fall through to another executable on a resolution failure', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong Kolux build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('KOLUX automations create')
