...
async function createPrimaryHome(): Promise<string> {
  const primaryHome = await mkdtemp(path.join(os.tmpdir(), 'kolux-codex-tripwire-'))
...
describe('Codex primary-home tripwire', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const primaryHome = await createPrimaryHome()
      const externalRoot = await mkdtemp(path.join(os.tmpdir(), 'kolux-codex-tripwire-external-'))
