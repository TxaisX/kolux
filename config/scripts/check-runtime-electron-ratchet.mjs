#!/usr/bin/env node
/**
 * Ratchet gate for Electron imports reachable from the Kolux runtime.
 *
 ...
 * `kolux-runtime.ts` reaches ~50 modules that import `electron`, and the number
...
const ENTRY_POINTS = [
  path.join(ROOT, 'src', 'main', 'runtime', 'kolux-runtime.ts'),
  path.join(ROOT, 'src', 'main', 'runtime', 'runtime-rpc.ts'),
  // Why koluxd too: it imports ipc/pty directly to install the PTY controller, so its
  ...
  path.join(ROOT, 'src', 'main', 'koluxd', 'main.ts')
]
...
function renderBaseline(files) {
  return [
    '# Modules reachable from the Kolux runtime that import `electron`.',
    ...
    '# (see `pnpm run build:koluxd`). Any entry means the runtime got less portable;',
...
async function main() {
  ...
  if (added.length > 0) {
    console.error(
      `[runtime-electron-ratchet] ${added.length} new module(s) reachable from the Kolux runtime now import electron:
