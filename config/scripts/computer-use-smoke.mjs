...
const cliPath =
  process.env.KOLUX_COMPUTER_SMOKE_CLI_PATH ?? resolve(repoRoot, 'out', 'cli', 'index.js')
...
const preferredApps = (
  requestedApps ??
  process.env.KOLUX_COMPUTER_SMOKE_APPS ??
  ...
  const child = spawnSync(process.execPath, [cliPath, ...cliArgs], {
    ...
    env: {
      ...
      KOLUX_USER_DATA_PATH:
        process.env.KOLUX_COMPUTER_SMOKE_USER_DATA_PATH ?? defaultDevUserDataPath()
    }
  })
  if (child.status !== 0) {
...
function defaultDevUserDataPath() {
  if (process.platform === 'darwin') {
    return resolve(homedir(), 'Library', 'Application Support', 'kolux-dev')
  }
  if (process.platform === 'win32') {
    return resolve(process.env.APPDATA ?? resolve(homedir(), 'AppData', 'Roaming'), 'kolux-dev')
  }
  return resolve(process.env.XDG_CONFIG_HOME ?? resolve(homedir(), '.config'), 'kolux-dev')
}
