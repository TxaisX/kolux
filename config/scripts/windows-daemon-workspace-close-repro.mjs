 ...
 * graceful-then-immediate kill pair emitted when Kolux closes a workspace.
...
const iterations = Number(process.env.KOLUX_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'kolux-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\kolux-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, KOLUX_USER_DATA_PATH: scratch }
    }
  )
