...
const ITERATIONS = Number.parseInt(process.env.KOLUX_CC_SCAN_BENCH_ITERATIONS ?? '150', 10)
const WARMUP = Number.parseInt(process.env.KOLUX_CC_SCAN_BENCH_WARMUP ?? '20', 10)

for (const [name, value] of [
  ['KOLUX_CC_SCAN_BENCH_ITERATIONS', ITERATIONS],
  ['KOLUX_CC_SCAN_BENCH_WARMUP', WARMUP]
]) {
  if (!Number.isInteger(value) || value <= 0) {
...
const dir = mkdtempSync(join(tmpdir(), 'kolux-cc-transcript-bench-'))
try {
  const rows = []
