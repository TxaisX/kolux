...
const operationsGuide = readFileSync('docs/reference/koluxd-operations.md', 'utf8')
const operationsProse = operationsGuide.replace(/\s+/g, ' ')

describe('koluxd operations restart safety', () => {
  ...
  it('fails closed before cgroup-wide maintenance', () => {
    ...
    expect(operationsGuide).not.toContain('sudo -Hu kolux kolux-ide terminal list --json')
    expect(operationsProse).toContain(
    ...
    expect(operationsProse).toContain('Kolux does not yet provide an atomic census-and-stop fence')
  })

  it('does not refer to the unavailable shipping design', () => {
    expect(operationsGuide).not.toContain('docs/design/shipping-koluxd.html')
  })
})
