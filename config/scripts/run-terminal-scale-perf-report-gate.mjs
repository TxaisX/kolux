...
export function parseReportGateArgs(argv, env = process.env) {
  ...
  let reportPath = env.KOLUX_E2E_TERMINAL_PERF_REPORT_PATH || DEFAULT_REPORT_PATH
...
export function runTerminalScalePerfReportGate({
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'kolux-terminal-scale-perf-'))
  ...
  const htmlReportPath = env.KOLUX_E2E_TERMINAL_PERF_HTML_REPORT_PATH || DEFAULT_HTML_REPORT_PATH
