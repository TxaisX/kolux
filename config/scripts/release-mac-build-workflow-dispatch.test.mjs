...
describe('release mac build workflow dispatch', () => {
  it('dispatches the mac workflow and waits for the returned run id', async () => {
    const { fetch, requests } = createGitHubFetch([
      jsonResponse(200, {
        html_url: 'https://github.test/TxaisX/kolux/actions/runs/123',
        workflow_run_id: 123
      }),
      jsonResponse(200, {
        conclusion: 'success',
    ...
    expect(requests[0].path).toBe(
      '/repos/TxaisX/kolux/actions/workflows/release-mac-build.yml/dispatches'
    )
    ...
    expect(requests[1].path).toBe('/repos/TxaisX/kolux/actions/runs/123')
  })
  ...
  it('finds the dispatched run by release tag and parent run id when GitHub returns no body', async () => {
      ...
      jsonResponse(200, {
        workflow_runs: [
          ...
          },
          {
            ...
            display_title: runTitle,
            html_url: 'https://github.test/TxaisX/kolux/actions/runs/124',
            id: 124
          }
        ]
      }),
    ...
    expect(requests[1].path).toBe(
      '/repos/TxaisX/kolux/actions/workflows/release-mac-build.yml/runs'
    )
  ...
  it('fails when the isolated mac workflow does not succeed', async () => {
    const { fetch } = createGitHubFetch([
      jsonResponse(200, {
        html_url: 'https://github.test/TxaisX/kolux/actions/runs/125',
        workflow_run_id: 125
      }),
      jsonResponse(200, {
        conclusion: 'failure',
  ...
  it('reads required workflow settings from the GitHub Actions environment', () => {
    const options = readReleaseMacBuildWorkflowOptions({
      GITHUB_REPOSITORY: 'TxaisX/kolux',
      GITHUB_RUN_ID: '987',
      GITHUB_TOKEN: 'token',
