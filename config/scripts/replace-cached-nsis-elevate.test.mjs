...
beforeEach(() => {
  scratch = mkdtempSync(join(tmpdir(), 'kolux elevate swap '))
})
...
describe('the probe, not the scan, decides whether the swap worked', () => {
  ...
  it('covers a custom bundle the directory scan cannot match', async () => {
    const relative = 'kolux-nsis-mirror/nsis-custom-3.11-0zqp2/elevate.exe'
    const cacheDir = makeCache(relative)
...
// Why no release-cut.yml wiring describe here any more: that workflow was
// deleted at the fork split (only ci.yml and release.yml remain), so the
// step-shape contract it used to guard has no surviving file to read.
