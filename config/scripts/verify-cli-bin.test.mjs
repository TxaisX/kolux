...
/** Builds a temporary Kolux-style project fixture with a compiled CLI entrypoint. */
function makeProjectWithCli(
  content,
  { mode = 0o755, rootPackageType, writeOutPackageJson = true } = {}
) {
  const projectDir = mkdtempSync(path.join(tmpdir(), 'kolux-cli-bin-'))
  ...
  writeFileSync(
    ...
    JSON.stringify({ bin: { kolux: './out/cli/index.js' }, type: rootPackageType }),
    'utf8'
  )
...
describe('verifyPackageCliBin', () => {
  ...
  it('rejects an empty package bin target', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('bin.kolux target is empty')
  })

  it('rejects package bin targets without a Node shebang', () => {
    const { projectDir } = makeProjectWithCli('console.log("kolux")\n')
