...
const ITERATIONS = Number(process.env.KOLUX_SSH_WATCH_BENCH_ITERATIONS ?? '200')
const WARMUP = Number(process.env.KOLUX_SSH_WATCH_BENCH_WARMUP ?? '30')

for (const [name, value] of [
  ['KOLUX_SSH_WATCH_BENCH_ITERATIONS', ITERATIONS],
  ['KOLUX_SSH_WATCH_BENCH_WARMUP', WARMUP]
]) {
...
function makeRoots(count) {
  return Array.from({ length: count }, (_, index) => `/home/dev/worktrees/kolux-${index}`)
}
