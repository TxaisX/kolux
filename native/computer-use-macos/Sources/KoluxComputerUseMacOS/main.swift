...
import Foundation
import ImageIO
import KoluxComputerUseMacOSCore
import ScreenCaptureKit

private let providerName = "kolux-computer-use-macos"
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Kolux Computer Use. Run `kolux computer permissions` or open Settings > Computer Use, grant Accessibility to Kolux Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Kolux Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need Kolux Computer Use toggled off and on again in System Settings.")
    }
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["KOLUX_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Kolux Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Kolux Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Kolux Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable Kolux Computer Use")
        let title = label(titleText, size: 22, weight: .bold)
        let subtitle = label(
            checking
                ? "Checking Accessibility and Screenshots."
                : (ready
                    ? "Kolux can use local apps when you ask."
                    : "Grant permissions so Kolux can use apps when you ask."),
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "kolux-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Kolux Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "kolux-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Kolux Computer Use")
...
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    ...
    if isTrustedKoluxApplication(pid) {
        return true
    }
    guard let parentPid = parentProcessId(pid) else { return false }
    return isTrustedKoluxApplication(parentPid)
}

private func isTrustedKoluxApplication(_ pid: pid_t) -> Bool {
    guard let app = NSRunningApplication(processIdentifier: pid),
    ...
    // Kolux-owned bundle ids; the sidecar peer check must still authorize them.
    return bundleId == "com.txais.kolux" ||
        bundleId.hasPrefix("com.txais.kolux.dev.") ||
        bundleId == "com.github.Electron"
}
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("KOLUX_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
private func runStdio() {
    fputs("Kolux Computer Use provider must be launched by Kolux in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: kolux-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("kolux-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: kolux-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
