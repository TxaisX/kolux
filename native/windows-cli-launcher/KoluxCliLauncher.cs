...
internal static class KoluxCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(KoluxCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Kolux.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Kolux.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Kolux CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable("NODE_OPTIONS", "KOLUX_NODE_OPTIONS");
            MoveEnvironmentVariable("NODE_REPL_EXTERNAL_MODULE", "KOLUX_NODE_REPL_EXTERNAL_MODULE");
            ...
            Environment.SetEnvironmentVariable(
                "KOLUX_CLI_COMMAND",
                requestedCliCommand == "kolux-ide" ? "kolux-ide" : "kolux"
            );
        ...
        {
            Console.Error.WriteLine("Unable to start the Kolux CLI: {0}", error.Message);
            return 1;
        }
    }
